...
public:
	...
	TLAddress(GSM::TypeOfNumber wType, GSM::NumberingPlan wPlan, const char *wDigits)
		: TLElement(), mPlan(wPlan), mDigits(wDigits)
	{
		mType = (wDigits[0] == '+') ? GSM::InternationalNumber : GSM::NationalNumber;
		LOG(DEBUG) << "TLaddrress ctor type=" << mType << " Digits " << wDigits;
	}

	TLAddress(const char *wDigits) : TLElement(), mPlan(GSM::E164Plan), mDigits(wDigits)
	{
		mType = (wDigits[0] == '+') ? GSM::InternationalNumber : GSM::NationalNumber;
		LOG(DEBUG) << "TLaddrress ctor type=" << mType << " Digits " << wDigits;
	}
...
public:
	...
	TLValidityPeriod(unsigned wVPF = 0xFF) : TLElement(), mVPF(wVPF), mExpiration(7 * 24 * 60 * 60 * 1000) {}
...
public:
	...
	TLUserData(unsigned wDCS = 0x100, bool wUDHI = false) : TLElement(), mDCS(wDCS), mUDHI(wUDHI), mLength(0) {}

	/** Initialize from a raw encoded data. */
	TLUserData(unsigned wDCS, const BitVector2 wRawData, unsigned wLength, bool wUDHI = false)
		: TLElement(), mDCS(wDCS), mUDHI(wUDHI), mLength(wLength)
	{
	...
	TLUserData(const char *text, GSM::GSMAlphabet alphabet = GSM::ALPHABET_7BIT, bool wUDHI = false)
		: TLElement(), mDCS(0), mUDHI(wUDHI), mLength(0)
	{
		switch (alphabet) {
...
public:
	...
	TLMessage() : mMMS(true), mSRI(false), mRP(false) {}
	...
	virtual void text(std::ostream &os) const { os << "MTI=" << MTI(); }
...
protected:
	...
	void writeUnused(TLFrame &fm) const
	{
		fm.fill(0, 3, 2);
	} ///< Fill unused bits with 0s
	  //@}
};
...
public:
	TLDeliver(const TLAddress &wOA, const TLUserData &wUD, unsigned wPID = 0)
		: TLMessage(), mOA(wOA), mPID(wPID), mUD(wUD)
	{
	}

	TLDeliver(const TLUserData &wUD) : TLMessage(), mUD(wUD) {}
...
public:
	...
	RPAddress(const char *wDigits) : L3CalledPartyBCDNumber(wDigits) {}

	RPAddress(const GSM::L3CallingPartyBCDNumber &other) : L3CalledPartyBCDNumber(other) {}
};
...
public:
	...
	RPUserData() : L3ProtocolElement(), mTPDU() {}

	RPUserData(const TLFrame &wTPDU) : L3ProtocolElement(), mTPDU(wTPDU) {}

	RPUserData(const TLMessage &TLM) : L3ProtocolElement() { TLM.write(mTPDU); }
...
public:
	RPCause(unsigned wValue) : L3ProtocolElement(), mValue(wValue) {}

	size_t lengthV() const { return 1; }

	void writeV(GSM::L3Frame &dest, size_t &wp) const { dest.writeField(wp, mValue, 8); }

	void parseV(const GSM::L3Frame &src, size_t &rp) { mValue = src.readField(rp, 8); }

	void parseV(const GSM::L3Frame &src, size_t &rp, size_t expectedLength)
	{
		mValue = src.peekField(rp, 8);
		rp += 8 * expectedLength;
	}

	void text(std::ostream &os) const { os << std::hex << "0x" << mValue << std::dec; }
};
...
public:
	...
	enum MessageType { Data = 0x0, Ack = 0x2, Error = 0x4, SMMA = 0x6 };

	RPMessage(unsigned wReference = 0) : mReference(wReference) {}
...
public:
	...
	RPData(unsigned ref, const RPAddress &wOriginator, const TLMessage &TLM)
		: RPMessage(ref), mOriginator(wOriginator), mUserData(TLM)
	{
	}
	...
	size_t l2BodyLength() const { return mOriginator.lengthLV() + mDestination.lengthLV() + mUserData.lengthLV(); }
...
public:
	RPAck(unsigned mReference) : RPMessage(mReference) {}
...
public:
	RPError(const RPCause &wCause, unsigned mReference) : RPMessage(mReference), mCause(wCause) {}
...
public:
	CPCause(unsigned wValue) : L3ProtocolElement(), mValue(wValue) {}
...
public:
	CPUserData() : L3ProtocolElement() {}

	CPUserData(const RPMessage &RPM) : L3ProtocolElement(), mRPDU(RPM.bitsNeeded()) { RPM.write(mRPDU); }
	...
	{
		mRPDU.text(os);
	} // called for operator<<(...,CPUserData) by L3ProtocolElement.
};
...
public:
	...
	enum MessageType { DATA = 0x01, ACK = 0x04, ERROR = 0x10 };

	CPMessage(unsigned wTI) : L3Message(), mTI(wTI) {}
...
public:
	CPAck(unsigned wTI = 7) : CPMessage(wTI) {}
...
public:
	CPError(unsigned wTI = 7) : CPMessage(wTI), mCause(0x7f) {}

	CPError(unsigned wTI, const CPCause &wCause) : CPMessage(wTI), mCause(wCause) {}

	int MTI() const { return ERROR; }
	size_t l2BodyLength() const { return mCause.lengthV(); }
	void writeBody(GSM::L3Frame &dest, size_t &wp) const;
	// pat 5-14-2013: This was an assert, but I believe this can happen because parseSMS does not triage out this
	// type of message, so changed to devassert.
...
public:
	CPData(unsigned wTI = 7) : CPMessage(wTI) {}

	CPData(unsigned wTI, const RPMessage &RPM) : CPMessage(wTI), mData(RPM) {}
