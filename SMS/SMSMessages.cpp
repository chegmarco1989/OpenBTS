...
CPMessage *SMS::parseSMS(const GSM::L3Frame &frame)
{
	...
	// The L3Message::CPMessage is a base class for CPData, CPAck, CPError, one of which is created by the CPFactory
	// above. The below calls L3Message::parse which calls the parseBody from the derived class. For CPAck and
	// CPError, parseBody is null (or worse, assert out - a former bug.) For CPData messages:  calls
	// CPData::parseBody which then calls L3ProtocolElement::parseLV which calls: CPUserData::parseV, which just
	// copies the data into CPUserData::mRPDU; which is an L3Frame::RLFrame
	retVal->parse(frame);
	LOG(DEBUG) << *retVal;
	return retVal;
}
...
TLMessage *SMS::parseTPDU(const TLFrame &TPDU, bool directionUplink)
{
	...
	if (directionUplink) {
		...
		switch ((TLMessage::MessageType)TPDU.MTI()) {
		// 10-2013: Pat added the DELIVER which is the downlink message so we can parse it for reporting
		// purposes.
...
void CPData::parseBody(const L3Frame &src, size_t &rp) { mData.parseLV(src, rp); }

void CPData::writeBody(L3Frame &dest, size_t &wp) const { mData.writeLV(dest, wp); }
...
void CPError::writeBody(L3Frame &dest, size_t &wp) const { mCause.writeV(dest, wp); }
...
void CPUserData::parseV(const L3Frame &src, size_t &rp, size_t expectedLength)
{
	...
	if (actualLength < (int)numBits) {
		...
		LOG(ERR) << "Invalid SMS frame:" << LOGVAR(expectedLength * 8) << " (from L3 header)"
			 << LOGVAR(actualLength);
		L3_READ_ERROR;
	}
...
{
	LOG(DEBUG) << "src=" << src << " (length=" << src.length() << ") rp=" << rp
...
void RPMessage::text(ostream &os) const { os << MTI() << " ref=" << mReference; }
...
void RPError::writeBody(RLFrame &dest, size_t &wp) const { mCause.writeLV(dest, wp); }

void RPError::parseBody(const RLFrame &dest, size_t &wp) { mCause.parseLV(dest, wp); }
...
ostream &SMS::operator<<(ostream &os, TLMessage::MessageType val)
{
	switch (val) {
	case TLMessage::DELIVER:
		os << "SMS-DELIVER/REPORT";
		break;
	case TLMessage::STATUS_REPORT:
		os << "SMS-STATUS-REPORT/COMMAND";
		break;
	case TLMessage::SUBMIT:
		os << "SMS-SUBMIT/REPORT";
		break;
	default:
		os << hex << "0x" << (int)val << dec;
		break;
...
size_t TLValidityPeriod::length() const
{
	...
	case 1:
		return 1; // relative format, 9.2.3.12.1
	case 2:
		return 7; // enhanced format, 9.2.3.12.2
	case 3:
		return 7; // absolute format, 9.2.3.12.3
	default:
		assert(0); // someone forgot to initialize the VPF
	}
}
...
void TLValidityPeriod::parse(const TLFrame &src, size_t &rp)
{
	...
	case 2: {
		...
		if (vp < 144)
			...
			minutes = 24 * 60 * (vp - 166);
		else
			minutes = 7 * 24 * 60 * (vp - 192);
		mExpiration = Timeval();
...
void TLValidityPeriod::write(TLFrame &dest, size_t &wp) const
{
	...
	if (minutes <= 720)
		...
		vp = 143 + (minutes - 720) / 30;
	else if (minutes < 43200)
		vp = 166 + minutes / (24 * 60);
	else
		vp = 192 + minutes / (7 * 24 * 60);
	if (vp > 255)
		vp = 255;
...
std::string TLUserData::decode() const
{
	...
	case 247: {
		...
		if (mUDHI) {
			...
			LOG(DEBUG) << "UDHL(octets)=" << udhl << " UDHL(septets)=" << udh_septets
				   << " pointer(bits)=" << crp << " text_length(septets)=" << text_length;
		}
...
size_t TLSubmit::l2BodyLength() const { return 1 + mDA.length() + 1 + 1 + mVP.length() + mUD.length(); }
...
{
	LOG(DEBUG) << "TLDEliver::l2BodyLength OA " << mOA.length() << " SCTS " << mSCTS.length() << " UD "
		   << mUD.length();
...
void TLTimestamp::text(std::ostream &os) const { mTime.text(os); }
