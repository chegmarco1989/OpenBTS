...
class MOCMachine : public CCBase {
	...
	// |-----------CMServiceRequest------------>|                              | old: CMServiceResponder calls
	// MOCProcedure
...
class AssignTCHMachine : public CCBase {
	enum State { stateStart, stateAssignTimeout };
	...
	// immediately when it expires or it might abort a successful reassignment, so this timer must not be in the
	// L3TimerId list.
	//
...
protected:
	...
	// |            call assignTCHF             |                              | (used only for EA; for VEA we
	// assigned TCH in AccessGrantResponder)
	// |<---------L3AssignmentCommand-----------|                              | assignTCHF, repeated until answered
	// |--------AssignmentComplete(FACCH)------>|                              |
	// DCCHDispatchRR,AssignmentCompleteHandler, calls MOCController or MTCController
...
// This message indicates establishment of both an MM Layer (ie, LocationUpdating has been performed 4.08 4.5.1) and a
// CM Layer connection.
...
#if UNUSED
// MachineStatus ProcedureDetach::machineRunState(int state, const GSM::L3Message* l3msg, const SIP::DialogMessage
// *sipmsg)
//{
	...
	if (isL3TIValid()) {
		...
		if (tran()->clearingGSM()) {
			...
			if (sendCause) {
				...
				// Handset sent disconnect; our reply L3Release does not include a Cause Element.
				// GSM 4.08 9.3.18.1.1
...
// Perform a network initiated clearing 24.008 5.4.  If things look ok send a disconnect and continue waiting for a
// Release message, or if things have gone wrong, send a ReleaseRequest and kill the transaction.  We used to do that
// all the time but some handsets (BLU phone) report "Network Failure" if you dont go through the disconnect procedure.
...
MachineStatus CCBase::closeCall(TermCause cause)
{
	...
	tran()->teCloseDialog(cause); // Make sure; this is redundant because the call will be repeated when the
				      // transaction is killed,
	// We could assert this if we dont call this until after an L3Setup.
	if (isL3TIValid()) {
		unsigned l3ti = getL3TI();
		// We dont have to send a disconnect at all, but if you dont, the phone may report that the call
		// "failed".
		...
		if (ccstate == CCState::Active) {
			if (1) {
				...
				// (pat 10-24-2013) As an option per 24.008 5.4.2: we could send a Release message and
				// start T308
				channel()->l3sendm(GSM::L3Release(l3ti, cause.tcGetCCCause()));
				setGSMState(CCState::ReleaseRequest);
			}
			...
			channel()->l3sendm(GSM::L3ReleaseComplete(
				l3ti, cause.tcGetCCCause())); // This is a CC message that releases this Transaction.
		}
	} else {
	...
	LOG(DEBUG) << "finish";
	// The caller is a state machine.  We cannot remove the transaction yet because the state machine is still using
	// it. The state machine caller should return MachineStatusQuitTran which causes handleMachineStatus() to call
	// teRemove to finish transaction destruction.
	return MachineStatus::QuitTran(cause);
}
...
// TODO: Get rid of this.  The terminator should Send a message to the MMLayer indicating type of termination (operator
// intervention or emergency call) which should be copied out to all the transactions in the MMContext.
...
// TODO (pat) 2-2014: The MS can send DTMF messages before the call is connected; currently we just ignore them, which
// works, but we should probably at least send a reject.
MachineStatus CCBase::defaultMessages(int state, const GSM::L3Message *l3msg)
{
	if (!l3msg) {
		return unexpectedState(state, l3msg);
	}		 // Maybe unhandled dialog message.
	...
	case L3CASE_MM(CMServiceAbort): {
		...
		// 4.08 5.2.1 and 4.5.1.7: If the MS wants to cancel before we get farther it should send a
		// CMServiceAbort.
	...
	case L3CASE_CC(Release): {
		...
		if (dmsg->mFacility.mExtant)
			WATCH(dmsg); // USSD DEBUG!
	...
	case L3CASE_CC(ReleaseComplete): {
		...
		// tran()->teCloseDialog(TermCause::Local(TermCodeNormalDisconnect)); // Redundant, and we dont know
		// what initiated it so this error is not correct
		setGSMState(CCState::NullState); // redundant, we are deleting this transaction.
		...
		if (cause.tcIsEmpty()) {
			cause = TermCause::Local(L3Cause::Normal_Call_Clearing);
		}
		return MachineStatus::QuitTran(cause);
	}
...
MachineStatus MOCMachine::handleSetupMsg(const L3Setup *setup)
{
	...
	string calledNumber;
	{
		if (!setup->haveCalledPartyBCDNumber()) {
			...
			// It is MOC, so we should not be sending an error to any dialogs, but we will fill in a SIP
			// error anyway.
...
MachineStatus MOCMachine::serviceAccept()
{
	...
	if (!isVeryEarly()) {
		if (!channel()->reassignAllocNextTCH()) {
			...
			tran()->teCloseDialog(cause); // TODO: This will become redundant with closeChannel and should
						      // be removed later.
	...
	// We could attach the MMContext to the MMUser at any time but it might start receiving calls or SMS
	// immediately, so we are going to wait until this call kicks off, which may be safer.

	return MachineStatusOK;
}

// This is used both for MOC and emergency calls, which are differentiated by the service type in the CMServiceRequest
// message. (pat) The Blackberry will attempt an MOC even if periodic LUR returned unauthorized!
MachineStatus MOCMachine::machineRunState(int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case L3CASE_SIP(dialogStarted): {
		return MachineStatusOK; // It just means the dialog has not received an answer to the initial INVITE
					// yet.
	}
...
#define ATTEMPT_TO_FIX_ZTE_PHONE 1
		...
		// If you set in-band audio it will play whatever you send it, but it will just not generate its own
		// ring tone in any case.
		// L3ProgressIndicator progressIE(L3ProgressIndicator::ReturnedToISDN);  This one tells it to not use
		// in-band audio, but did not help.  L3ProgressIndicator
		// progressIE(L3ProgressIndicator::InBandAvailable); To make the ZTE work I tried: Progress=Unspecified,
		// NotISDN and Queuing.
	...
	case L3CASE_SIP(dialogActive): {
		...
		if (gConfig.getBool("GSM.Cipher.Encrypt")) {
			...
			if (!encryptionAlgorithm) {
				...
				LOG(DEBUG) << "no ki: NOT sending Ciphering Mode Command on " << *channel()
					   << " for IMSI" << tran()->subscriberIMSI();
			}
		}
	...
	case L3CASE_CC(ConnectAcknowledge): {
		if (getDialog()->isActive()) {
			...
			// Not possible.
			PROCLOG(ERR) << "Connect Acknowledge received in incorrect SIP Dialog state:"
				     << getDialog()->getDialogState();
		}
		return callMachStart(new InCallMachine(tran()));
	}
	...
	int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case stateStart: {
		...
		if (isVeryEarly()) {
			// For very early assignment, we gave the MS a TCH in the initial ImmediateAssignment but we
			// need a mode change.
			...
			// For early (not "very early") assignment, we gave the MS an SDDCH in the initial
			// ImmediateAssignment. Send the TCH assignment now. (pat) We do not support "late assignment"
			// as defined in 4.08 7.3.2.
			channel()->reassignStart();
			// And I quote 4.08 11.1.2: "This timer [T3101] is started when a channel is allocated with an
			// IMMEDIATE ASSIGNMENT message. It is stopped when the MS has correctly seized the channels."
			// If we receive a reassignment failure we will resend the assignment, which often works the
			// second time. The TChReassignment timer controls how long we will keep re-trying. We used to
			// use 3 (T3101-1) seconds, but I dont think this time is related to T3101 and I dont know what
			// the ultimate limit is, maybe nothing.  I am going to up it just use T3101.
	...
	case L3CASE_RR(ChannelModeModifyAcknowledge): {
		...
		if (!modeOK)
			return closeCall(TermCause::Local(L3Cause::Channel_Unacceptable));
	...
	// This arrives on the new FACCH, however, the channel() comes from the Context which is still mapped to the old
	// channel, but reassignmentComplete knows this.
	case L3CASE_RR(AssignmentComplete): {
		...
		// tran()->mSipDialogMessagesBlocked = false;	// Next process will handle the postponed dialog
		// messages.
		return MachineStatusPopMachine;
	}
	...
	case stateAssignTimeout: {
		// This is the case where we received neither AssignmentComplete nor AssignmentFailure - it is loss of
		// radio contact.
		LOG(INFO) << "SIP term info stateAssignTimeout NoUserResponding";
	caseAssignTimeout:
		channel()->reassignFailure();
		// TODO: This is not optimal - we should drop back to the MMLayer to see if it wants to do something
		// else. Determine and pass cause	SVGDBG
		...
		if (getDialog()) {
			getDialog()->dialogCancel(cause);
		} // Should never be NULL, but dont crash.
...
{
	PROCLOG2(DEBUG, state) << LOGVAR(state) << LOGVAR(l3msg) << LOGVAR(sipmsg)
			       << LOGVAR2("imsi", tran()->subscriber());
	switch (state) {
	case stateStart: {
		...
		// pat 2-2014: Attempt to make the buggy ZTE phone by sending an explicit L3Signal IE in the L3Setup
		// message. Update: did not help.
		// L3Signal tone(L3Signal::SignalBusyToneOn);	// Tryed both Ringing and Busy tone, no joy.
		// setupmsg.setSignal(tone);
		PROCLOG(INFO) << "sending L3Setup to call " << LOGVAR2("calling", tran()->calling()) << tran()
			      << LOGVAR(setupmsg);
		channel()->l3sendm(setupmsg);
		setGSMState(CCState::CallPresent);
		timerStart(T303, T303ms, TimerAbortTran); // Time state "Call Present"; start CMServiceRequest recv;
							  // stop CallProceeding recv.
	...
	case L3CASE_CC(CallConfirmed): {
		...
		timerStart(T310, T310ms, TimerAbortTran); // Time state "Call Confirmed"; start CallConfirmed recv; stop
							  // Alert,Connect,Disconnect recv.
		// Change channels.
	...
	case L3CASE_SIP(dialogActive): { // SIP Dialog received SIP ACK to 200 OK.
		...
		// (pat) To doug: The place to move cipher starting is probably InCallMachine::machineRunState case
		// stateStart.
		if (gConfig.getBool("GSM.Cipher.Encrypt")) {
			int encryptionAlgorithm =
				gTMSITable.tmsiTabGetPreferredA5Algorithm(tran()->subscriberIMSI().c_str());
	...
	int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case L3CASE_RR(HandoverComplete): {
		...
		if (dialog == NULL) {
			...
			LOG(ERR) << "handover failure due to failure to create dialog for "
				 << tran(); // Will probably never happen.
	...
	case L3CASE_SIP(dialogActive): {
		...
		// TODO: I moved this to InCallMachine but I dont want to test handover right now so leave this here
		// too; doesnt hurt to call mmAttachByImsi twice.
...
static bool supportRFC2833() { return gConfig.getBool("SIP.DTMF.RFC2833"); }
static bool supportRFC2976()
{
	// Unfortunately the config option was originall misnamed, so test for both.
	return gConfig.getBool("SIP.DTMF.RFC2976") ||
	       (gConfig.defines("SIP.DTMF.RFC2967") && gConfig.getBool("SIP.DTMF.RFC2967"));
}
...
MachineStatus InCallMachine::machineRunState(int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case L3CASE_CC(StartDTMF): {
		...
		if (supportRFC2833()) {
			...
			if (!s)
				PROCLOG(ERR) << "DTMF RFC-28333 failed.";
			mDtmfSuccess |= s;
		}
		if (supportRFC2976()) {
			...
			// If RFC2697 used we will send acknowledgement to MS when SIP OK arrives, otherwise send it
			// now.
			acknowledgeDtmf();
		}
		return MachineStatusOK;
	...
	case L3CASE_SIP(dialogStarted):
		devassert(0);
		return MachineStatus::QuitTran(
			TermCause::Local(L3Cause::Sip_Internal_Error)); // Shouldnt happen, but dont crash.
...
void TestCallMachine::testCallStart(TranEntry *tran)
{
	...
	if (iBufString.find("RESTART") != std::string::npos) {
		...
		Control::TranEntry *tran = Control::TranEntry::newMTC(NULL, msid, GSM::L3CMServiceType::TestCall, "0");
...
void initTestCall(TranEntry *tran) { tran->teSetProcedure(new TestCallMachine(tran)); }

void initMTC(TranEntry *tran) { tran->teSetProcedure(new MTCMachine(tran)); }
