...
public:
	...
	TMSI_t &operator=(uint32_t wVal)
	{
		mValid = true;
		mVal = wVal;
		return *this;
	}
	TMSI_t &operator=(const TMSI_t &other)
	{
		mVal = other.mVal;
		mValid = other.mValid;
		return *this;
...
struct CCState {
	enum CallState {
		...
		// AnsweredPaging = 100,		// Not a GSM Call Control state.  Intermediate state used by
		// OpenBTS between Paging and CallPresent
		MOCInitiated = 1,  // state N1 "Call initiated".
				   // 5.1.2.1.3 specifies state U1 in MS for MOC entered when MS requests "call
				   // establishment". 4.1.2.2.3 specifies state N1 in network received "call
				   // establishment request" but has not responded. Since these are CC states, we
				   // previously assumed that "call establishment" meant an L3Setup message, not CM
				   // Service Request. However, 24.008 11.3 implies that "CallInitiated" state starts
				   // when CM Service Request is sent. The optional authorization procedure intervenes
				   // between receipt of CM Service Request and sending the Accept.
		...
		SMSDelivering =
			101, // MT-SMS set when paging answered; MT-SMS initial TransactionEntry state is NullState.
...
// A message to the CS L3 state machine.  The message may come from a GSM LogicalChannel (FACCH, SDCCH, or SACCH), GPRS,
// or SIP. This is part of the L3 rewrite.
class GenericL3Msg {
public:
	enum GenericL3MsgType { MsgTypeLCH, MsgTypeSIP };
	...
	// GenericL3Msg(GSM::L3Frame *wFrame, L3LogicalChannel *wChan) : ml3Type(MsgTypeLCH),
	// ml3frame(wFrame),ml3ch(dynamic_cast<L3LogicalChannel*>(wChan)),mSipMsg(0) { assert(ml3frame); }
	GenericL3Msg(GSM::L3Frame *wFrame, GSM::L2LogicalChannel *wChan)
		: ml3Type(MsgTypeLCH), ml3frame(wFrame), ml3ch(wChan), mSipMsg(0)
	{
		assert(ml3frame);
	}
	GenericL3Msg(SIP::DialogMessage *wSipMsg, std::string wCallId)
		: ml3Type(MsgTypeSIP), ml3frame(0), ml3ch(0), mSipMsg(wSipMsg), mCallId(wCallId)
	{
		assert(mSipMsg);
	}
	~GenericL3Msg();
