...
class MMUser : public MemCheckMMUser /*: public RefCntBase*/ {
	// This is lock is to add/remove from the Transaction queues, since they are written from a thread in the SIP
	// directory and read from the thread running the LogicalChannel.
...
protected:
	void mmuFree(MMUserMap::iterator *it, TermCause cause /*= TermCauseUnknown*/); // This is the destructor.  It is
										       // not public.  Can only delete
										       // from gMMLayer because we must
...
public:
	...
	void MMContextInit();
	void mmcFree(TermCause cause); // This is the destructor.  It is not public.  Can only delete from gMMLayer
				       // because we must lock the MMUserMap first.
...
public:
	...
	bool mmIsEmpty();
	bool mmCheckNewActivity(); // Check for new activity.  Return true if any found. Also checks for normal channel
				   // release.
...
public:
	...
	// (pat) We could be blocked for several reasons, including paging, waiting for LUR to complete, waiting for
	// channel to change, etc. But if we are paging, reset the paging timer so we keep paging.
	...
	bool mmIsBusy(string &imsi)
	{
		...
		if (!mmu)
			return false;
		if (mmu->mmuMTCq.size())
			return true; // Someone already waiting in the MTC queue.
		if (!mmu->mmuContext)
			return false;
