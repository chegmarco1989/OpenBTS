...
HandoverEntry::HandoverEntry(const TranEntry *tran) : mMyTranID(tran->tranID()), mHandoverOtherBSTransactionID(0)
{
...
void TranEntry::setDialog(SIP::SipDialog *dialog)
{
	mDialog = dialog;
	dialog->setTranId(mID);
}
...
{
	if (SipDialog *dialog = getDialog()) {
		return dialog->RTPPort();
	}
	return 0;
}

TranEntry *TranEntry::newMOSSD(MMContext *wChannel) { return newMO(wChannel, L3CMServiceType::SupplementaryService); }
...
TranEntry *TranEntry::newMOSMS(MMContext *wChannel) { return newMO(wChannel, L3CMServiceType::ShortMessage); }
	...
	SimpleKeyValue &params, L3LogicalChannel *wChannel, unsigned wHandoverOtherBSTransactionID)
{
	...
	// TranEntry(proxy,imsi,wChannel,GSM::L3CMServiceType::HandoverCall,CCState::HandoverInbound);
	...
	if (IMSI)
		result->mSubscriber = FullMobileId(IMSI);
	...
	if (codec)
		result->mCodecs = CodecSet((CodecType)atoi(codec));
	...
	unsigned wHandoverOtherBSTransactionID, SimpleKeyValue &params)
{
	...
	if (refer) {
		...
		const char *inp;
		char *outp, *outbuf = (char *)alloca(strlen(refer) + 1);
...
#if EXTERNAL_TRANSACTION_TABLE
	// Delete the SQL table entry.  (pat) There wont be any for LocationUpdating procedure, or transactions that did
	// not run until they got an IMSI.
	char query[100];
...
void TranEntry::text(ostream &os) const
{
	...
	stateText(os);
	if (!isStuckOrRemoved()) {
		// Nothing else I am willing to risk saying about a removed transaction, for fear of trigging an
		// exception.
		if (channel())
			os << " chan=(" << *channel() << ")";
		else
...
void TranEntry::runQuery(const char *query) const
{
	...
	for (unsigned i = 0; i < mNumSQLTries; i++) {
		if (sqlite3_command(gNewTransactionTable.getDB(), query))
			return;
	}
	LOG(ALERT) << "transaction table access failed after " << mNumSQLTries << "attempts. query:" << query
		   << " error: " << sqlite3_errmsg(gNewTransactionTable.getDB());
}
#endif
	...
	//{
	//	if (mDialog == NULL) { return; }	// TODO: We might want to see the LUR transactions, which also
	// do  not have an mSIP.
	//
	...
	//" 		"VALUES  (%u,%u,     %u,     '%s','%s',      %u,'%s',  '%s',   '%s',    '%s',      '%s',
	//'%s')", 		tranID(),now,now, 		serviceTypeSS.str().c_str(),
	//		subscriber.c_str(),
	//		mL3TI,
	//		mCalled.digits(),
	//		mCalling.digits(),
...
void TranEntry::setChannel(L3LogicalChannel *wChannel)
{
	...
	if (mChannel) {
		sprintf(query, "UPDATE TRANSACTION_TABLE SET CHANGED=%u,CHANNEL='%s' WHERE ID=%u", (unsigned)time(NULL),
			mChannel->descriptiveString(), tranID());
	} else {
...
//			(this->mNextChannel && ((void*)this->mNextChannel == (void*)lch ||
//(void*)this->mNextChannel->getL2Channel()->SACCH() == (void*)lch)));
//}
...
{
	devassert(channel()->chtype() ==
		  FACCHType); // This is the type returned by the TCHFACCHLogicalChannel, even though it is TCH too.
	return channel();
}

unsigned TranEntry::getL3TI() const { return mL3TI; }
...
void TranEntryProtected::setGSMState(CallState wState)
{
	...
	char query[150];
	sprintf(query, "UPDATE TRANSACTION_TABLE SET GSMSTATE='%s',CHANGED=%u WHERE ID=%u", stateString, now, tranID());
	runQuery(query);
#endif
}
...
SIP::SipState TranEntry::echoSipState(SIP::SipState state) const
{
	...
	char query[150];
	sprintf(query, "UPDATE TRANSACTION_TABLE SET SIPSTATE='%s',CHANGED=%u WHERE ID=%u", stateString, now, tranID());
	runQuery(query);
#endif
...
#if EXTERNAL_TRANSACTION_TABLE
	char query[151];
	snprintf(query, 150, "UPDATE TRANSACTION_TABLE SET CALLED='%s' WHERE ID=%u", mCalled.digits(), tranID());
	runQuery(query);
#endif
}
...
#if EXTERNAL_TRANSACTION_TABLE
	char query[151];
	snprintf(query, 150, "UPDATE TRANSACTION_TABLE SET L3TI=%u WHERE ID=%u", mL3TI, tranID());
	runQuery(query);
#endif
}
...
string TranEntry::handoverString(string peer, string cause) const
{
	...
	// if (getGSMState()==CCState::Handover_Outbound) os << " outbound-ref=" <<
	// handover->mOutboundReference.value();
	os << " L3TI=" << mL3TI;
	if (mCalled.digits()[0])
		os << " called=" << mCalled.digits();
	if (mCalling.digits()[0])
		os << " calling=" << mCalling.digits();
	if (cause.size())
		os << " cause=" << cause;
	...
	// 3. Subsequently, if there is a Contact field, all messages bypass the proxies and are sent directly to the
	// Contact.
	// 4. But the proxies might want to see the messages too, so they can add a "required-route" parameter which
	// trumps the "contact" header and specifies that messages are sent there instead.  This is called "Loose
	// Routing."  What a mess. And I quote: "These procedures separate the destination of the request (present in
	// the Request-URI) from
	//			the set of proxies that need to be visited along the way (present in the Route header
	// field)."
	...
	//<< "," << 	sip->RTPSession()->rtp.snd_seq << "," << 	sip->RTPSession()->rtp.last_rtcp_report_snt_r <<
...
void NewTransactionTable::ttInit()
{
	...
	// mIDCounter = random();
	mIDCounter = 100; // pat changed.  0 is reserved.  Start it high enough so it cannot possibly be confused with
			  // an L3TI.
...
bool TranEntry::teIsTalking()
{
	...
	if (this->getGSMState() == CCState::Active) {
		// WATCHINFO(LOGVAR2("GSMState",this->getGSMState())
		// <<LOGVAR2("meas",this->channel()->getL2Channel()->getSACCH()->measurementResults()));
...
bool NewTransactionTable::ttIsTalking(TranEntryId tranid)
{
	bool result = false;
	ScopedLock lock(mttLock, __FILE__, __LINE__);
	if (TranEntry *tran = ttFindById(tranid)) {
		result = tran->teIsTalking();
	}
	return result;
}
...
TranEntry *NewTransactionTable::ttFindByTypeAndOffset(GSM::TypeAndOffset desc)
{
	...
	for (NewTransactionMap::iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		...
		if (thisChan->typeAndOffset() != desc)
			continue;
		return itr->second;
	}
...
TranEntry *NewTransactionTable::ttFindByMobileIDState(const L3MobileIdentity &mobileID, CallState state)
{
	...
	for (NewTransactionMap::iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		if (itr->second->deadOrRemoved())
			continue;
		if (itr->second->getGSMState() != state)
			continue;
...
bool NewTransactionTable::isBusy(const L3MobileIdentity &mobileID)
{
	...
	for (NewTransactionMap::iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		...
		bool speech = service == GSM::L3CMServiceType::EmergencyCall ||
...
TranEntry *NewTransactionTable::ttFindByL3Msg(GSM::L3Message *l3msg, L3LogicalChannel *lch)
{
	...
	for (NewTransactionMap::iterator itr = gNewTransactionTable.mTable.begin();
		...
		if (tran->deadOrRemoved())
			continue;
		if (!tran->isChannelMatch(lch))
			continue;
		GSM::L3CMServiceType service = tran->service();
		switch (pd) {
		case L3CallControlPD:
			return tran; // Only one for now.
			// if (service.isCC() && tran->getL3TI() == dynamic_cast<L3CCMessage*>(l3msg)->TI()) { return
			// tran; }
			continue;
		case L3SMSPD:
			return tran; // Only one for now.
			// if (service.isSMS() && tran->getL3TI() == dynamic_cast<L3CPMessage*>(l3msg)->TI()) { return
			// tran; }
			continue;
		case L3MobilityManagementPD:
		case L3RadioResourcePD:
			// We dont yet have a separate MobilityManagement layer, so MM and RR messages are handled by
			// the primary TranEntry, which is either the LocationUpdateRequest or the in-progress CC
			// TranEntry, which needs RR messages to modify the channel for the voice call.
			if (service.isMM()) {
				return tran;
			}
			if (service.isSMS())
...
TranEntry *NewTransactionTable::ttFindHandoverOther(const L3MobileIdentity &mobileID, unsigned otherBS1TranId)
{
	...
	for (NewTransactionMap::iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		TranEntry *tran = itr->second;
		LOG(DEBUG) << "comparing " << tran
			   << LOGVAR2("HandoverOtherBSTransactionID",
				      (tran->mHandover ? tran->mHandover->mHandoverOtherBSTransactionID : -1));
		if (tran->deadOrRemoved())
			continue;
		if (!tran->mHandover) {
		...
		if (tran->mHandover->mHandoverOtherBSTransactionID != otherBS1TranId) {
			LOG(DEBUG)
			"no match " << tran->mHandover->mHandoverOtherBSTransactionID << "!=" << otherBS1TranId;
			continue;
		}
...
L3LogicalChannel *NewTransactionTable::findChannel(const L3MobileIdentity &mobileID)
{
	...
	for (NewTransactionMap::iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		if (itr->second->deadOrRemoved())
			continue;
		if (!itr->second->subscriber().fmidMatch(mobileID))
			continue;
		L3LogicalChannel *chan = itr->second->channel();
		if (!chan)
			continue;
		if (chan->chtype() == FACCHType)
			return chan;
		if (chan->chtype() == SDCCHType)
			return chan;
...
size_t NewTransactionTable::dump(ostream &os, bool showAll) const
{
	...
	for (NewTransactionMap::const_iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		if ((!showAll) && itr->second->deadOrRemoved())
			continue;
		sz++;
		os << *(itr->second) << endl;
	}
...
TranEntryId NewTransactionTable::findLongestCall()
{
	...
	for (NewTransactionMap::iterator itr = mTable.begin(); itr != mTable.end(); ++itr) {
		...
		if (!(itr->second->channel()))
			continue;
		if (itr->second->getGSMState() != CCState::Active)
			continue;
...
void TranEntry::tePushProcedure(MachineBase *it) { mProcStack.push_back(it); }
...
{
	if (currentProcedure() == wProc) {
		return;
	}
	MachineBase *old = tePopMachine();
	wDeleteCurrent = true; // 9-24-2013: We always delete except when pusing into a procedure and that is handled by
			       // tePushProcedure.
	if (wDeleteCurrent && old) {
		delete old;
	}
...
// handleMachineStatus returns a bool and is the final status-handler called when all state machines have processed the
...
MachineStatus TranEntry::handleRecursion(MachineStatus status)
{
	while (status == MachineStatusPopMachine) { // return to previous procedure on stack
		...
		LOG(DEBUG)
		"popped to " << currentProcedure()->debugName() << " at state " << currentProcedure()->mPopState;
		status = currentProcedure()->machineRunState(currentProcedure()->mPopState);
	}
	return status;
}

// Return TRUE if the status indicates the message or whatever had a message handler, regardless of the success/fail
// result.
bool TranEntry::handleMachineStatus(MachineStatus status)
{
	...
	case MachineStatus::MachineCodeQuitChannel: // aka MachineStatusQuitChannel
		...
		if (channel() && !channel()->isReleased()) {
			// If the caller did not already call this, we dont know what the heck happened, so do a RELEASE
			// instead of HARDRELEASE.
	...
	case MachineStatus::MachineCodeQuitTran: // aka MachineStatusQuitTran
		...
		// In all cases the caller was supposed to already send termination messages toward both layer2 (the
		// handset) and SIP Dialog (the network) however, we will perform additional termination to make sure.
...
// (pat 5-13-2014) Update update: Yes the locks appear to be used - I got the "waiting more than one second" emssage
// from lockAndInvokeFrame;
//   the situation was starting then cancelling a call on the same handset.  This may have been in the midst of a
//   reassignment procedure so messages were arriving simultaneously on both channels.  Looks like the CC Disconnect was
//   sent on the SDCCH right before the reassign procedure, then a dialog cancel came in too.
...
{
	if (this->getGSMState() == CCState::TranDeleted) {
		return false;
	} // (pat) Paranoid check that TranEntry still extant.
	LOG(DEBUG);
	bool result = false;
	RefCntPointer<TranEntry> saver = this;
	{
...
bool TranEntry::lockAndInvokeFrame(const L3Frame *frame, const L3Message *l3msg)
{
	...
	RefCntPointer<TranEntry> saver = this;
	{
		...
		if (MachineBase *proc = currentProcedure()) {
			...
			LOG(INFO) << "Received message for transaction with no state machine. " << this
				  << *frame; // Should never happen.
		}
	}
	return result;
}
...
bool TranEntry::lockAndInvokeTimeout(L3Timer *timer)
{
	...
	// from being deleted until this routine exists.  Without this, the ScopedLock tries to reference the deleted
	// transaction.
	bool result = false;
	RefCntPointer<TranEntry> saver = this;
...
void TranEntry::teCloseDialog(TermCause cause)
{
	...
	// The dialog can also be NULL because the phone will send a DISCONNECT first thing if a previous call did not
	// close correctly.
	SipDialog *dialog = getDialog();
...
void TranEntry::teRemove(TermCause cause)
{
	...
	//	LOG(INFO) << "SIP term info dialogCancel called in teRemove cancel cause: " << cause << " L3Cause: " <<
	// l3Cause << " SIPerror: " << SIPerror; 	dialog->dialogCancel(cause, l3Cause);		// Does
	// nothing if dialog not yet started.
	//}

	// (pat 9-29-2014) Harry was seeing intermittent crashes in the code below.
	// Crash was inside sqlite when calling gConfig.getStr() from cdrServiceStart(), which would appear to be a
	// memory corruption in something immediately preceding, but unknown what it could be. I cannot find any
	// problems, but this is the only new addition, so to be safe, we will completely avoid this code unless
	// specifically enabled by setting Control.CDR.Dirname.
	...
	// If the TranEntry never got into an MMContext, ie, deleted from MMUser::MMFree, this is the first time it has
	// been put under RefCntPointer control - from this point on it will be deleted when the last reference count
	// disappears, which is the pop_front out of the sDeletedTranEntrys below.
	sDeletedTranEntrys.push_back(RefCntPointer<TranEntry>(this));

	if (mContext) {
		mContext->mmDisconnectTran(this);
	} // DANGER: this deletes the transaction as a side effect.
	mContext = NULL;
...
void TranEntry::teCloseCallNow(TermCause cause, bool sendCause)
{
	...
	if (isL3TIValid() && tran()->getGSMState() != CCState::NullState &&
		...
		if (sendCause) {
			channel()->l3sendm(GSM::L3ReleaseComplete(l3ti, cause.tcGetCCCause())); // This is a CC message
												// that releases this
												// Transaction
												// immediately.
		} else {
			channel()->l3sendm(GSM::L3ReleaseComplete(
				l3ti)); // This is a reply that confirms transaction was released.
		}
	}
...
void writePrivateHeaders(SipMessage *msg, const L3LogicalChannel *l3chan)
{
	...
	// TODO: If we add the MSC params to this, especially L3TI, the SIP message will completely encapsulate
	// handover. TA=<timing advance> TE=<TA error> UpRSSI=<uplink RSSI> TxPwr=<MS tx power> DnRSSIdBm=<downlink
	// RSSI> Get the values.
	if (l3chan) {
		char phy_info[200];
...
static string lookupPhoneNumber(string imsi)
{
	...
	if (0 == strncasecmp(number.c_str(), "tel:", 4)) {
	...
	TermCause cause) // If true, make a CMR instead of a CDR.  CMRs have more info.
{
	...
	labelmoc:
		...
		if (SipDialog *dialog = this->getDialog()) {
			...
			if (dialog->dsPeer())
				cdr.cdrPeer = dialog->dsPeer()->mipName;
		}
	...
	case L3CMServiceType::ShortMessage:
		if (this->mMessage.size() == 0 && !makeCMR) {
			goto labelIgnore;
		}
		cdr.cdrType = "MOSMS";
		goto labelmoc;
	...
	case L3CMServiceType::HandoverCall: // The handover code sets the type to MobileOriginatedCall or
					    // MobileTerminatedCall so we should not see this.
		break;
	...
	labelmtc:
		...
		if (SipDialog *dialog2 = this->getDialog()) {
			...
			if (dialog2->dsPeer())
				cdr.cdrPeer = dialog2->dsPeer()->mipName;
		}
...
void CdrService::cdrOpenFile()
{
	...
	if (tm.tm_yday == cdrCurrentDay) {
		// We already tried to open the file.  Either we succeeded or not, but we wont try to open it on every
		// transaction.
		return;
	}
	if (mpf) {
	...
	if (0 == dirname.size()) {
		return;
	} // Disabled if the Control.CDR.Dirname is empty.
...
void *CdrService::cdrServiceLoop(void *arg)
{
	...
	while (!gBTS.btsShutdown()) {
		...
		if (!cdr) {
			continue;
		}		     // paranoid, but maybe happens during shutdown.
		self->cdrOpenFile(); // We may have to open a new file if the date changed.
		if (self->mpf) {
			cdr->cdrWriteEntry(self->mpf);
		}
		delete cdr;
	}
	return NULL;
