...
struct CBMessage {
	...
	bool setMessageCode(unsigned mc)
	{
		mMessageCode = mc;
		mMessageCode_change = true;
		return true;
	}

	UInt_z mUpdateNumber; // Another unique id for message.
	Bool_z mUpdateNumber_change;
	bool setUpdateNumber(unsigned un)
	{
		mUpdateNumber = un;
		mUpdateNumber_change = true;
		return true;
	}
	...
	bool setMessageId(unsigned id)
	{
		mMessageId = id;
		mMessageId_change = true;
		return true;
	}
	...
	// Unfortunately, DCS == 0 is a valid value (german), so I'm not even sure how we should store the language
	// code, but probably need both the DCS and the language code, which would require a CBS database update, which
	// would mean we would have to check for and handle older revs of the database, so I am punting this. So here is
	// some code to interface to the SMSCB database, but it doesnt do anything.
	UInt_z mLanguageCode; // 2 ascii chars encoded as a short word.
	Bool_z mLanguageCode_change;
	bool setLanguageCode(unsigned lc)
	{
		mLanguageCode = lc;
		mLanguageCode_change = true;
		return true;
	}
	bool setLanguage(std::string lc)
	{
		if (lc.size() == 0 || lc == "-") {
	...
	std::string getLanguage()
	{
		...
		char buf[3];
		buf[0] = mLanguageCode & 0x7f;
		buf[1] = (mLanguageCode >> 8) & 0x7f;
		buf[2] = 0;
		return std::string(buf);
	}

	std::string mMessageText; // The message itself.
	bool setMessageText(std::string mt)
	{
		mMessageText = mt;
		return true;
	}
	...
	bool match(CBMessage &other)
	{
		return mGS == other.mGS && mMessageCode == other.mMessageCode && mUpdateNumber == other.mUpdateNumber &&
		       mMessageId == other.mMessageId && mLanguageCode == other.mLanguageCode &&
		       mMessageText == other.mMessageText;
	}
...
int CBSAddMessage(CBMessage &msg, std::string &errMsg);
bool CBSGetMessages(std::vector<CBMessage> &result, std::string text = "");
