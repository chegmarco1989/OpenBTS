...
// The USupServ version 2 command is processUnstructuredSS-Request (3GPP 24.80) and has two arguments:
// (uSupServ-DataCodingScheme, uSupServ-String)
...
// The SS-operation-codes are in from
// http://cpansearch.perl.org/src/REHSACK/Net-Radio-Location-SUPL-Test-0.001/asn1src/MAP-SS-Code.asc Here is a register
// message:    0b7b1c0da10b0201010201 0c 30 03 04 01 91                      7f 01 00 0b	// PD=0xb. 7b MTI (0x3b)
// ored with 0x80. 1c FacilityIEI 0d  length of facility contents == 13 		a1		component type
// tag = Invoke (24.080 3.6.2) 		0b		component length == 12
...
//		04		- octet?
//		01		length of param
//		91		the activateSS argument data, maybe "BarringOfOutgoingCalls SS-Code == 91.  Selected by
//*33.
...
static string ussdEncode(string in)
{
	...
	if (offset == 1) {
		// Special case as per 23.038 6.1.2.3 - If there are exactly 7 bits at the end they are padded with a
		// CR.
		accum |= ('\r' << 1);
	}
	if (offset) {
...
class SupServCodes {
	...
	void addSSCode(unsigned ssCode, string serviceCodeDigits) { ssCodes[ssCode] = serviceCodeDigits; }
	...
	void ssCodesInit()
	{
		...
		// I think HOLD and MultiParty are handled by L3 messages at the MS level, and these MAP-SS-Codes are
		// used only at the MSC level.
		// addSSCode(0x42,??); // HOLD see section 4.5.5.  Ha ha, there is no such section in either 22.030
		// or 22.083.  It is in 2.30.  addSSCode(0x51, ??); // MPTTY see 4.5.5 22.084
...
private:
	...
	unsigned ssParseLinkedID(const unsigned char *data, unsigned datalen, unsigned &datai)
	{
		...
		if (data[datai] != ssLinkedIDTag) {
			return 0;
		}
		datai++;
		unsigned linkIDLen = data[datai++];
		if (linkIDLen != 1) {
			throw SSParseExcept(format("unexpected link ID length: %d", linkIDLen));
		}
	...
	{
		if (data[datai++] != ssInvokeIDTag) {
			throw SSParseExcept("missing required Invoke ID Tag");
		}
		unsigned invokeIDLen = data[datai++];
		if (invokeIDLen != 1) {
			throw SSParseExcept(format("unexpected invoke ID length: %u", invokeIDLen));
		}
	...
	{
		if (data[datai++] != ssOperationCodeTag) {
			throw SSParseExcept("no operation code tag");
		}
		unsigned operationCodeLen = data[datai++];
		if (operationCodeLen != 1) {
			throw SSParseExcept(
				format("unexpected operation code length: %u at %u", operationCodeLen, datai));
		}
	...
	string ssParseParameter(const unsigned char *data, unsigned datalen, unsigned &datai)
	{
		...
		if (tag != ssOctetParam) {
			throw SSParseExcept(
				format("Unexpected parameter type %u, expected %u at %u", tag, ssOctetParam, datai));
		}
		unsigned paramLen = data[datai++];
		if (paramLen > datalen - datai) {
			throw SSParseExcept(
				format("parameter len (%u) exceeds parameter list len (%u)", paramLen, datalen));
		}
		// Finally, a parameter:
	...
	void ssParseParameterList(const unsigned char *data, unsigned datalen, unsigned &datai)
	{
		...
		if (tag != ssSequenceTag && tag != ssSetTag) {
			throw SSParseExcept(format("unexpected parameter tag: %u", tag));
		}
...
MachineStatus MOSSDMachine::machineRunState(int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case L3CASE_SIP(dialogBye): {
		...
		if (umsg == NULL) {
			LOG(ERR) << "USSD client error: could not convert DialogMessage to DialogUssdMessage "
				 << sipmsg;
			return MachineStatus::QuitTran(TermCause::Local(L3Cause::USSD_Error));
		}
