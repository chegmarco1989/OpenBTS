...
// (pat) For l3rewrite the TMSI will be assigned by the VLR, which will be either a new SubscriberRegistry program or
...
#define MAX_VALID_TMSI \
	0x3fffffff // 30 bits.  We dont make TMSIs this high, but the invalid TMSIs start one higher than this.
...
// We keep valid TMSIs that we are not sure the MS received, and also TMSIs that may have authorized in the past and
// subsequently become unauthorized. For unauthorized phones, we used to go ahead and create valid TMSIs anyway, but I
// dont want to do that now that TMSIs are genuine random numbers and somewhat expensive to allocate, plus there could
// be many of them and I dont want them using up TMSI address space, because eventually we may receive TMSIs from
// neighbors as well. Another alternative was to have a separate TMSI->IMSI mapping table, but I dont think that is any
// better. If you do not specify a primary key, sqlite assigns a new key one higher than the highest existing in the
// table. So I will do is let it go ahead and assign TMSIs 1,2,3,4 for the case where the TMSI is not used, Fine, so we
// will use values up to 64K as valid TMSIs, and values above that for invalid TMSIs. This seems to do a good job and
// sqlite does not seem to increase the database size much when you assign primary keys that are not tightly clustered.
...
static string shortHeader = "IMSI TMSI IMEI AUTH CREATED ACCESSED TMSI_ASSIGNED";
static string longHeader =
	shortHeader + " PTMSI_ASSIGNED AUTH_EXPIRY REJECT_CODE ASSOCIATED_URI ASSERTED_IDENTITY WELCOME_SENT";
//"IMSI TMSI IMEI AUTH CREATED ACCESSED TMSI_ASSIGNED PTMSI_ASSIGNED AUTH_EXPIRY REJECT_CODE ASSOCIATED_URI
// ASSERTED_IDENTITY WELCOME_SENT";
static string longerHeader = longHeader + " A5_SUPPORT POWER_CLASS RRLP_STATUS OLD_TMSI OLD_MCC OLD_MNC OLD_LAC";
//"IMSI TMSI IMEI AUTH CREATED ACCESSED TMSI_ASSIGNED PTMSI_ASSIGNED AUTH_EXPIRY REJECT_CODE ASSOCIATED_URI
// ASSERTED_IDENTITY WELCOME_SENT A5_SUPPORT POWER_CLASS RRLP_STATUS OLD_TMSI OLD_MCC OLD_MNC OLD_LAC";
static const char *create =
	...
	// "APP_FLAGS INTEGER DEFAULT 0, "	// Application-specific flags  (pat) No one is using this so I took it
	// out.
	...
	"OLD_LAC INTEGER, "     // previous network LAC
			    ...
			    // We need cached authorization for the case where we are using open registration and TMSIs
			    // are being sent, to verify the TMSI to avoid TMSI collisions, however, this should never
			    // have been the in the BTS
			    // - needs to be in sipauthserve if anywhere, so we always call the Registrar to get fresh
			    // challenges For UMTS this is a requirement, so there is no point putting in a special case
			    // for 2G GSM to cache a single RAND here. Cached authentication could also be used in the
			    // case where the Registrar is unreachable; but in that case we should fall back to using
			    // IMSIs and fail open.  We dont handle that well at the moment.
	...
	"WELCOME_SENT INTEGER DEFAULT 0," // 0 == welcome message not sent yet; 1 == sent by us; 2 == sent by someone
					  // else.
	...
	// 3. if a previously authorized MS becomes unauthorized we want to keep the entry here to reserve the TMSI
	// permanently.
	...
	"TMSI_ASSIGNED INTEGER DEFAULT 0," // Set when the TMSI has been successfully assigned to the MS, ie, the MS
					   // knows it.
	"PTMSI_ASSIGNED INTEGER DEFAULT 0" // Set when the P-TMSI has been successfully assigned to the MS by the SGSN,
					   // ie, the MS knows it.
...
static int tmsi2table(uint32_t tmsi) { return (int)tmsi; }
static uint32_t table2tmsi(int storedValue) { return storedValue; }
static bool tmsiIsValid(int rawTmsi) { return rawTmsi != 0 && rawTmsi <= MAX_VALID_TMSI; }

bool configTmsiTestMode() { return gConfig.getNum("Control.LUR.TestMode"); }

bool configSendTmsis() { return gConfig.getBool("Control.LUR.SendTMSIs"); }
...
// (pat) If the TMSI_TABLE version does not match expected, drop the TMSI_TABLE before returning, and the caller will
// recreate it.
...
void TMSITable::tmsiTabCleanup()
{
	...
	if (maxage == 0 || maxage >= 99999) {
		return;
	} // punt if disabled or ridiculous.
	...
	if (changes) {
		LOG(INFO) << "Deleted " << changes
			  << " expired entries from TMSITable with age < Control.TMSITable.Maxage=" << maxage
			  << " hours";
	}
}

void TMSITable::tmsiTabClearAuthCache() { runQuery("UPDATE TMSI_TABLE SET AUTH_EXPIRY=0"); }
...
void TMSITable::tmsiTabInit()
{
	...
	if (1) {
		// Not that it matters, since this is done only once, but because TMSI is a primary key this query is
		// probably optimized.
		...
		if (qHighestTmsi.sqlSuccess()) {
			...
			if (highest_tmsi > sHighestFakeTmsi) {
				sHighestFakeTmsi = highest_tmsi;
			}
		} else {
		...
		while (qinit.sqlResultSize()) {
			...
			if (rawtmsi > (int)sHighestFakeTmsi) {
				sHighestFakeTmsi = rawtmsi;
			}
			if (!qinit.sqlStep())
				break;
		}
...
int TMSITable::tmsiTabOpen(const char *wPath)
{
	...
	if (!sqlite_command(mTmsiDB, TmsiTableDefinition::create)) {
		LOG(EMERG) << "Cannot create TMSI table in file:" << mTablePath
			   << " using:" << TmsiTableDefinition::create;
		exit(1);
		// return 1;
	}
	...
	if (!sqlite_command(mTmsiDB, enableWAL)) {
		LOG(EMERG) << "Cannot enable WAL mode on database at " << wPath
			   << ", error message: " << sqlite3_errmsg(mTmsiDB);
	}
...
struct TSqlString : public string {
	char buf[100];
	void appendf(const char *fmt, int val)
	{
		snprintf(buf, 100, fmt, val);
		append(buf);
	}
	void appendf(const char *fmt, const char *val)
	{
		snprintf(buf, 100, fmt, val);
		append(buf);
	}
	void appendf(const char *fmt, string val)
	{
		snprintf(buf, 100, fmt, val.c_str());
		append(buf);
	}
	void appendf(const char *fmt, const char *a, string b)
	{
		snprintf(buf, 100, fmt, a, b.c_str());
		append(buf);
	}
	void appendf(const char *fmt, const char *a, int b)
	{
		snprintf(buf, 100, fmt, a, b);
		append(buf);
	}
...
{
	if (store->imei_changed) {
		addc("IMEI", store->imei);
		store->imei_changed = false;
	}
	if (store->auth_changed) {
		addc("AUTH", store->auth);
		store->auth_changed = false;
	}
	if (store->authExpiry_changed) {
		addc("AUTH_EXPIRY", store->authExpiry);
		store->authExpiry_changed = false;
	}
	if (store->rejectCode_changed) {
		addc("REJECT_CODE", store->rejectCode);
		store->rejectCode_changed = false;
	}
	if (store->assigned_changed) {
		addc("TMSI_ASSIGNED", store->assigned);
		store->assigned_changed = false;
	}
	if (store->a5support_changed) {
		addc("A5_SUPPORT", store->a5support);
		store->a5support_changed = false;
	}
	if (store->powerClass_changed) {
		addc("POWER_CLASS", store->powerClass);
		store->powerClass_changed = false;
	}
	if (store->kc_changed) {
		addc("kc", store->kc);
		store->kc_changed = false;
	}
	if (store->associatedUri_changed) {
		addc("ASSOCIATED_URI", store->associatedUri);
		store->associatedUri_changed = false;
	}
	if (store->assertedIdentity_changed) {
		addc("ASSERTED_IDENTITY", store->assertedIdentity);
		store->assertedIdentity_changed = false;
	}
	if (store->welcomeSent_changed) {
		addc("WELCOME_SENT", store->welcomeSent);
		store->welcomeSent_changed = false;
	}
}
...
unsigned TMSITable::allocateTmsi()
{
	...
	// Since the source code is public, there is no point using a random number generator unless we use a truly
	// random seed. The result may be used as a P-TMSI so it must be less than 30 bits.
...
{
	LOG(INFO) << "update entry for" << LOGVAR(imsi) << LOGVAR(store->auth_changed) << LOGVAR(store->auth)
		  << LOGVAR(store->assigned_changed) << LOGVAR(store->assigned);
...
// This routine checks to see if the record for this IMSI already exists; the caller actually knows this and could
					  ...
					  const GSM::L3LocationAreaIdentity *lai, uint32_t oldTmsi)
{
	...
	if (sendTmsis) {
		// We are handling several cases here.  If it is a new record we are allocating a new tmsi for the first
		// time, or if it is an existing record that was created without an assigned tmsi, ie, it has a fake
		// tmsi, then we are updating it to a real tmsi. We never go backwards, ie, once we allocate a real
...
bool TMSITable::tmsiTabGetStore(string imsi, TmsiTableStore *store) const
{
	...
	LOG(DEBUG) << LOGVAR2("auth", store->auth) << LOGVAR2("authExpiry", store->authExpiry)
		   << LOGVAR2("assigned", store->assigned) << LOGVAR2("rejectCode", store->rejectCode)
		   << LOGVAR2("welcomSent", store->welcomeSent);
	return true;
}
...
unsigned TMSITable::tmsiTabGetTMSI(const string imsi, bool onlyIfKnown) const
{
	...
	if (!q3.sqlSuccess()) {
		...
		LOG(DEBUG) << "found" << LOGVAR(imsi) << LOGVAR(onlyIfKnown) << LOGVAR(q3.getResultInt(0))
			   << LOGVAR(q3.getResultInt(1));
		int rawtmsi = q3.getResultInt(0); // the returned tmsi.
		int tmsiAssigned =
			q3.getResultInt(1); // true if the tmsi has been sent to the handset in a tmsi assignment.
		if (onlyIfKnown && tmsiAssigned == 0) {
			return 0;
		}
...
vector<vector<string>> TMSITable::tmsiTabView(int verbosity, bool rawFlag, unsigned maxrows) const
{
	...
	string header1 = verbosity >= 2
				 ? TmsiTableDefinition::longerHeader
	...
	while (query.sqlResultSize() && (nrows++ < maxrows)) {
		...
		for (col = 0; col < vh1.size(); col++) {
			...
			if (header == "TMSI") {
				...
				// All other column types.  If they are integer they are converted to "" if NULL else
				// decimal.
...
{
	if (A5Bits & GSM::EncryptionAlgorithm::Bit5_3)
		return 3;
	// if (A5Bits & GSM::EncryptionAlgorithm::Bit5_2) return 2;	// not supported
	if (A5Bits & GSM::EncryptionAlgorithm::Bit5_1)
		return 1;
	return 0;
}
...
#if UNUSED
// Note that we overwrite the P-Associated-URI and P-Asserted-Identity, even if the incoming are empty.  If we dont know
// them, we dont know them.
