...
// There is a major fauxpax in C++ that all virtual classes bypass the class hierarchy and are finalized only in the
// most derived class. I dont see any implementation reason for this, so it is just speced this way arbitrarily.
// Therefore if a virtual method in a virtual base class is over-ridden in an intermediate class in a hierarchy
// involving multiple paths to a final derived class, there must also be a unique method defined in the final derived
// class. This is true if any class in the path is virtual, so the the virtual class basically contaminates the
// hierarchy and makes virtual methods nearly worthless in that hierarchy.  It is a pretty big oops. The consequence for
// us in our attempt to create state machines using protected methods to implement the states is that it creates a
// problem with shared data.  The shared data wants to be in a virtual class common to all the classes implementing
// state machines. However, that same virtual class cannot be in the path of the virtual methods implementing the
// states. Therefore the shared data must be in a separate class so that the main path to virtual methods is not
// contaminated by 'virtual' anywhere. The alternatives are either to put the data in a separately allocated class and
// use pointers, or put virtual accessor methods in the LUBase class and the data itself in the most derived class.
// Another problem to beware is if the final derived class defines a pure virtual method, that seems to over-ride the
// definition of the same method in the intermediate classes, which may be a bug.
...
struct RegistrationResult {
	...
	RegistrationResult()
		: mRegistrationStatus(RegistrationUninitialized), mRejectCause(L3RejectCause::Zero), mSipCode(0)
	{
	}
	void regSetSuccess() { mRegistrationStatus = RegistrationSuccess; }
	void regSetError() { mRegistrationStatus = RegistrationError; }
	void regSetFail(unsigned sipCode, MMRejectCause cause)
	{
		mRegistrationStatus = RegistrationFail;
		mSipCode = sipCode;
		mRejectCause = cause;
	}
	void regSetChallenge(string wRand)
	{
		mRegistrationStatus = RegistrationChallenge;
		mRand = wRand;
	}
...
enum TmsiStatus {
	...
	tmsiAuthenticated, // MS sent a TMSI that was found in the TMSI table (started as tmsiProvisional) and
			   // authenticated ok.
...
// We save up everything until we have authenticated the MS and then put it in the TMSI table only if MS authenticates
// ok. Previously it was included as virtual by every Procedure, but now it lives in the TransactionEntry.
class MMSharedData {
...
public:
	...
	uint32_t mOldTmsi; // The tmsi sent in LUR, which is irrelevant if the LAI is not ours, but saved only for
			   // reporting purposes.
	// string mRAND;
	...
	void setTmsi(uint32_t tmsi, TmsiStatus status)
	{
		mAssignedTmsi = tmsi;
		mTmsiStatus = status;
	}
	...
	MMSharedData() : mAssignedTmsi(0), mTmsiStatus(tmsiNone), mOldTmsi(0), mQueryType(GSM::NoIDType) {}
};
...
class LUAuthentication : public LUBase /*, public virtual LUSharedData*/ {
	enum States { stateStart, stateRegister2Response };
