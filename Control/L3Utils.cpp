...
void L3TimerList::timerStop(L3TimerId tid) { mtlTimers[tid].tStop(); }
...
bool L3TimerList::timerExpired(L3TimerId tid) { return mtlTimers[tid].tIsExpired(); }
...
void L3TimerList::text(std::ostream &os) const
{
	for (int tid = 0; tid < cNumTimers; tid++) {
		...
		if (timer->tIsExpired()) {
			os << LOGVAR2(timer->tName(), "expired");
		}
		if (timer->tRemaining()) {
			os << LOGVAR2(timer->tName(), timer->tRemaining());
		}
	}
}
