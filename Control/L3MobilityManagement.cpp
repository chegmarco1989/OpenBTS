...
void NewPagingResponseHandler(const L3PagingResponse *resp, MMContext *mmchan)
{
	...
	if (!gMMLayer.mmPageReceived(mmchan, mobileId)) {
		...
		LOG(WARNING) << "Paging Reponse with no Mobility Management record (probably timed out) for "
			     << mobileId;
	...
	const FullMobileId &msid, L3LogicalChannel *DCCH)
{
	...
	// (pat) TODO: We should store the authorization state of the welcome message that was sent so that when there
	// is an authorization state change (ie, from unauthorized to authorized) we can send a new welcome message. But
	// this should all be moved into sipauthserve anyway.
	...
	if (stmp == "PLMN") {
		...
		// If it is a periodic updating we assume a welcome message was sent by us but we lost the tmsi database
		// somehow.
		...
		// If it is periodic updating, then we assume that we have seen the MS previously but our TMSI_TABLE
		// database was lost.
	...
	if (!gConfig.defines(messageName) || !gConfig.defines(shortCodeName))
		return;
	string message = gConfig.getStr(messageName);
	string shortCode = gConfig.getStr(shortCodeName);
	if (!message.length() || !shortCode.length())
		return;
	...
	mmsd->store.setWelcomeSent(
		1); // welcome message sent by us.  TODO: We should not set this until we have confirmation of delivery.
	return;
}
	...
	case stateStart: {
		...
		if (mMobileID.type() == TMSIType) {
			...
			if (imsi.size()) {
				...
				// just use the authorization value saved in the TMSITable from the most recent LUR on
				// this BTS. Note that the handset may have been subsequently unauthorized on a
				// different BTS and we would not know about it.
				// TODO We could check the date of our cached authorization and force a re-authorization
				// if it is expired.
				tran()->setSubscriberImsi(imsi, false);
...
{
	if (!tran()->mMMData) {
		tran()->mMMData = new MMSharedData;
	}
	return tran()->mMMData;
}

// TODO: Reject cause should be determined in a more central location, probably sipauthserve.
// We may want different reject codes based on the IMSI or MSISDN of the MS, or on the CM service being requested (CC,
// SMS, USSD, GPRS), although this code here is used only by LUR.
...
bool LUBase::openRegistration() const
{
	...
	if (openRegistrationPat.size() && gConfig.isValidValue("Control.LUR.OpenRegistration", openRegistrationPat)) {
		...
		LOG(DEBUG) << "Checking Open Registration Open Registration" << LOGVAR(imsi) << LOGVAR(allow)
			   << LOGVAR2("pattern", openRegistrationPat);
		if (allow) {
			string rejectPat(gConfig.getStr("Control.LUR.OpenRegistration.Reject"));
			if (rejectPat.size() &&
...
//		- accept unconditionally without registration.  For CC we must query the IMSI.  For others we dont even
// have to do that.
...
//		- REGISTER, OpenRegistration backup: On REGISTER failure, accept, optionally with cached nonce/sres
// authentication.
...
MachineStatus LUStart::stateRecvIdentityResponse(const GSM::L3IdentityResponse *resp)
{
	...
	if (idtype == IMSIType) {
		...
		{
			// Check for perfidy on the part of the MS.  We're checking that it did not send two different
			// IMSIs in the same attempt, something that will probably never happen.
		...
		// and if so just reject immediately, but this doesn't happen often and I'm just going to let it proceed
		// again with the whole registration process.
		LOG(DEBUG) << LOGVAR(imsi) << LOGVAR(ludata()->mSecondAttempt)
			   << LOGVAR(ludata()->mPrevRegisterAttemptImsi) << LOGVAR(ludata()->getTmsiStatus());
		...
		return MachineStatusOK; // Just ignore it.  T3270 is still running.  Maybe it will return what we asked
					// for later.
	}
	...
	if (ludata()->mQueryType == idtype) { // success
		...
		// The MS goofed.
		LOG(WARNING) << "MS Identity Response for " << ludata()->mQueryType << " returned " << idtype
			     << " instead";
...
MachineStatus LUStart::machineRunState(int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case stateHaveIds: {
		...
		// OpenBTS version 3 generated a TMSI here for every new phone we see, even if we don't actually assign
		// it.
		...
		// or preferably, the Registrar would return a string of challenge/response pairs so we can keep using
		// them.
		...
		if (!ludata()->mSecondAttempt && gTMSITable->tmsiTabGetStore(getImsi(), store)) {
			...
			if (authExpiry && time(NULL) <= authExpiry) {
				if (!store->isAuthorized()) {
					...
					// need to inform sipauthserve of the whereabouts of the handset so it can
					// update the database used by asterisk for MTC.
	...
	int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case L3CASE_MM(AuthenticationResponse): {
...
#endif
		{
			...
			return machPush(
				new L3RegisterMachine(tran(), SIPDTRegister, SRESstr, &ludata()->mRegistrationResult),
	...
	case stateRegister2Response: {
		...
		case RegistrationFail: // In which case the mSipCode tells why.
			if (ludata()->mSecondAttempt == 0 && ludata()->getTmsiStatus() == tmsiProvisional) {
				...
				// Registration by TMSI failed.  Try again using an IMSI.  To do that we will start
		...
		{
			// Query for classmark?
			// (pat) We need to do this if the IMEI changed also, because a new handset may have different
			// capabilities. Instead of checking IMEI, just always query the classmark and dont worry about
			// checking whether we already have a valid classmark or not.
		...
		if (gConfig.getBool("GSM.Cipher.Encrypt")) {
			...
			if (!encryptionAlgorithm) {
				LOG(DEBUG) << "A5/3 and A5/1 not supported: NOT sending Ciphering Mode Command on "
					   << *channel() << " for " << getImsiName();
			} else if (channel()->getL2Channel()->decryptUplink_maybe(getImsi(), encryptionAlgorithm)) {
				LOG(DEBUG) << "sending Ciphering Mode Command on " << *channel() << " for "
					   << getImsiName();
				...
				LOG(DEBUG) << "no ki: NOT sending Ciphering Mode Command on " << *channel() << " for "
					   << getImsiName();
			}
		}
		return callMachStart(new LUFinish(tran()));
	...
	case L3CASE_RR(CipheringModeComplete): {
		// The fact the message arrived means success.  We hope.  Even if that were not true, we should proceed
		// anyway.
		return callMachStart(new LUFinish(tran()));
	}
	...
	case L3CASE_MM(TMSIReallocationComplete): {
		...
		if (!ludata()->mExpectingTmsiReallocationComplete) {
			...
			if (!newTmsi) {
				...
				// it wont be used again after initial authentication.
...
MachineStatus LUFinish::stateSendLUResponse()
{
	...
	if (ludata()->store.getWelcomeSent() == 0) {
		...
		if (stmp == "PLMN") {
			// We only send the registration message if it is an imsi attach.
			// If it is a normal updating we assume a welcome message was sent by a different BTS, or
			// possibly earlier by us and the TMSI_TABLE database was lost. If it is a periodic updating we
			// assume a welcome message was sent by us but we lost the tmsi database somehow.
			if (!ludata()->isImsiAttach()) {
				ludata()->store.setWelcomeSent(2); // welcome message sent by someone else
			}
		} else if (stmp == "NORMAL") {
			// We send the registration message the first time this BTS sees this MS.
			// If it is periodic updating, then we assume that we have seen the MS previously but our
			// TMSI_TABLE database was lost.
			if (!ludata()->isInitialAttach()) {
				ludata()->store.setWelcomeSent(2); // welcome message sent by us previously.
			}
		} else {
			// This is the stmp == 'FIRST' option.
			// We send the message if the WELCOME_SENT field is 0, regardless of the status reported by the
			// MS.
		}
	}
	...
	if (authorization != AuthUnauthorized) {
		...
		// We update the TMSI_ASSIGNED status in the TMSI_TABLE to reflect the phone's opinion, which could
		// differ from the BTS.
		switch (ludata()->getTmsiStatus()) {
		case tmsiFailed:
			// Getting here means we succeeded on the second attempt: TMSI failed but IMSI passed, ie, it
			// was a TMSI collision. Fall through...
		case tmsiNone: {
		...
		case tmsiNotAssigned: {
			...
			// We may need to assign a new TMSI if the MS has just become registered and formerly had a fake
			// tmsi, or if the SendTMSIs option is on, so we call tmsiTabCreateOrUpdate instead of
			// tmsiTabUpdate.
	...
	// Some MS (BLU phone) do not seem to be happy about starting an SMS on the same channel immediately after the
	// LUR, so we have to hang up the channel and re-page the MS to do the next procedure, which sucks.
	...
	if (ludata()->needsTmsiAssignment() && sendTMSIs && newTmsi) {
		...
		// (pat) This used to be 1 second but the BLU phone, for one, does not send the TMSI Reallocation
		// complete fast enough.
		timerStart(TMisc1, 5000, stateLUAcceptTimeout);
		L3MobileIdentity mid(newTmsi);
		// (pat 10-2013) I tried sending the welcome message on the same channel after sending the location
...
MachineStatus LUFinish::statePostAccept()
{
	...
	LOG(DEBUG) << LOGVAR(ludata()->isImsiAttach()) << LOGVAR(ludata()->getTmsiStatus())
	...
	int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case stateStart:
		...
		// even though it is defined in the RR timers, I think it is only applicable to a CC "L3Disconnect"
		// because the reason cited for the delay is to allow time for additional messages, but there would not
		// be any for a low level delay. There is a T3111 timer on channel release code in GSML1FEC, and this is
		// redundant. Another thought: maybe the channel close prejudicially closes the LAP2Dm communication,
		// and this is to give LAP2Dm a chance to get the message through.
...
L3RegisterMachine::L3RegisterMachine(TranEntry *wTran, SIP::DialogType wMethod,
	...
	: LUBase(wTran), mSRES(wSRES), mRResult(wRResult)
{
...
// RFC-3261 sec 10.2: "A REGISTER request does not establish a dialog."  A dialog normally has both inbound and outbound
...
string RegistrationResult::text()
{
	...
	if (mRegistrationStatus == RegistrationFail) {
		result += format(",SipCode=%u,RejectCause=%u", mSipCode, mRejectCause);
	}
	return result;
}

// (pat) 5-2014.  We dont currently save the detach information in OpenBTS.  I dont want to set the TMSI table AUTH or
// AUTH_EXPIRY to 0, because we may need those for a later authorization if backhaul is cut or central authorization
// entity needs to be refreshed from our database.  I dont want to add a new TMSI table field at this time, since it
// outdates all existing TMSI tables. The best way would would be to make AUTH a bit field and add bits. However, we
// dont currently use that information;  we could send an immediate final response code to an INVITE or MESSAGE for a
// handset that did an Imsi-Detach if the beacon AttachDetach flag is still set, however, that will not work when we
// support multiple BTS per LAC - the final response should be sent from the central authority, not the individual BTS.
// So I am going to do nothing else at this time.
// (pat) NOTE:  Kazoo may send a dialog failure to the imsi detach, which will try to go to the transaction, and is
// currently ignored.
