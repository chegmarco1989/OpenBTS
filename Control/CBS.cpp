...
static sqlite3 *CBSConnectDatabase(bool whine)
{
	...
	if (rc) {
		if (whine)
			LOG(EMERG) << "Cannot open Cell Broadcast Service database on path " << path << ": "
				   << sqlite3_errmsg(sCBSDB);
	...
	if (!sqlite3_command(sCBSDB, createSMSCBTable)) {
		if (whine)
			LOG(EMERG) << "Cannot create Cell Broadcast Service table";
		return NULL;
	}
	// Set high-concurrency WAL mode.
	if (!sqlite3_command(sCBSDB, enableWAL)) {
		if (whine)
			LOG(EMERG) << "Cannot enable WAL mode on database at " << path
				   << ", error message: " << sqlite3_errmsg(sCBSDB);
	}
	return sCBSDB;
}
...
static const char *crackRowNames =
	"GS,MESSAGE_CODE,UPDATE_NUMBER,MSGID,LANGUAGE_CODE,MESSAGE,SEND_COUNT,SEND_TIME,ROWID";
...
// Return false on error;  return true on success, which means we accessed the table - the result size is the number of
// entries.
...
int CBSClearMessages() { return cbsRunQuery("DELETE FROM SMSCB WHERE 1"); }
			 ...
			 vector<string> *both)
{
	...
	if (vals) {
		vals->push_back(format("%u", uval));
	}
	if (both) {
		both->push_back(format("%s=%u", col, uval));
	}
}
...
static void update1field(const char *col, string sval, vector<string> *cols, vector<string> *vals, vector<string> *both)
{
	...
	if (vals) {
		vals->push_back(format("'%s'", sval));
	}
	if (both) {
		both->push_back(format("%s='%s'", col, sval));
	}
}
				...
				bool all)
{
	...
	if (all || msg.mMessageCode_change) {
		update1field("MESSAGE_CODE", msg.mMessageCode, cols, vals, both);
	}
	if (all || msg.mUpdateNumber_change) {
		update1field("UPDATE_NUMBER", msg.mUpdateNumber, cols, vals, both);
	}
	if (all || msg.mMessageId_change) {
		update1field("MSGID", msg.mMessageId, cols, vals, both);
	}
	if (all || msg.mLanguageCode_change) {
		update1field("LANGUAGE_CODE", msg.mLanguageCode, cols, vals, both);
	}
	// if (all || msg.mLanguage_change) { update1field("LANGUAGE_CODE",msg.getLanguageCode(),cols,vals,both); }
	if (all || msg.mMessageText.size()) {
		update1field("MESSAGE", msg.mMessageText, cols, vals, both);
	}
	// ROWID is a synthetic field.
	if (msg.mRowId_change) {
		update1field("ROWID", msg.mRowId, cols, vals, both);
	}
}
...
int CBSAddMessage(CBMessage &msg, string &errorMsg)
{
	...
	// like this: INSERT OR REPLACE INTO SMSCB (GS,MESSAGE_CODE,UPDATE_NUMBER,MSGID,LANGUAGE_CODE,MESSAGE) VALUES
	// (0,0,0,0,0,'whatever') Cannot use REPLACE: REPLACE only works if INSERT would generate a constraint conflict,
	// ie, duplicate UNIQ field.
...
static void CBSSendMessage(CBMessage &msg, GSM::CBCHLogicalChannel *CBCH)
{
	...
	LOG(INFO) << "sending message ID=" << msg.mMessageId << " code=" << msg.mMessageCode << " in " << numPages
		  << " pages: " << msg.mMessageText;
	...
	for (unsigned page = 0; page < numPages; page++) {
		...
		if (false && msg.mLanguageCode) {
			...
			while (dp < 82 && mp < messageLen) {
				...
				// (pat) Setting the high byte to 0 is just wrong - the user would want to put the
				// 16-bit characters in the database, that is the point of using UCS2.
				thisPage[dp++] = 0;
			...
			int buf = 0;
			int shift = 0;
			// The spec (above) says to put this language stuff in, but it doesn't work on my samsung galaxy
			// y.  (dbrown) encode7(languageCode >> 8, shift, dp, buf, thisPage); encode7(languageCode &
			// 0xFF, shift, dp, buf, thisPage); encode7('\r', shift, dp, buf, thisPage);
...
{
	ostringstream os;
	os << LOGVARM(mGS) << LOGVARM(mMessageCode) << LOGVARM(mUpdateNumber) << LOGVARM(mMessageId)
	   << LOGVAR(mMessageText);
	return os.str();
}

void CBMessage::cbtext(std::ostream &os) { os << cbtext(); }
...
void *SMSCBSender(void *)
{
	...
	bool whine = true;
	while (!CBSConnectDatabase(whine)) {
		sleep(2);
		whine = false;
	}
