...
public:
	TranEntryId tranID() const { return mMyTranID; }
...
// All the DCCHDispatch code is only involved in the initial message starting an L3 Procedure - after that the handler
// functions poll. The TransactionEntry was not involved in message delivery except for the responses to
// ImmediateAssignment, paging, and handover. The LocationUpdating procedure did not use a TransactionEntry at all. The
// TI [Transaction Identifiers] were simply allocated round robin, which is incorrect but at least reduces the risk of
// accidently using an active one with an MS.
...
// I repeat: If we have a link to a phone, we have one or more TranEntrys, which is the defining characteristic of
// TranEntry. In GSM the L3LogicalChannel is identified by association with an L2LogicalChannel (SDCCH or TCH.) In UMTS
// the L3LogicalChannel is identified by a UNRTI identity. (fyi, in UMTS many UE share the physical radio channel and
// the URNTI is used to tell them apart; the URNTI is assigned by the BTS to the UE the very first thing, before the UE
// identity is known.) A TranEntry is always created for each L3 procedure, which is a series of L3 messages with the
// phone, including LUR [Location Update Request.] In 3GPP parlance an "L3 Transaction" involves a CC or SMS transaction
// with an associated TI [Transaction Identifier]". Our TranEntry may or may not represent an "L3 Transaction" in this
// sense, for example, the LUR case is not an "L3 Transaction", and we also allocate TranEntrys for the MO case as soon
// as we have a channel, which is before the TI is allocated. A TranEntry is not allocated a TI until it is started on
// the MS, so queued MT transactions do not yet have a TI. There may be multiple TranEntrys per L3LogicalChannel.
// Specifically, the spec allows four simultaneous active L3 Transactions on the phone:
// two CS [Circuit Switched, ie voice call] Transactions (the active one and the "on-hold" one) plus one SMS transaction
// in each direction. Additional MT CS or SMS transactions are queued in the MM Context until they can be started. The
// TranEntry may optionally be associated with a SipDialog, which is created only if/when the SIP Dialog is created.
// Specifically, BTS originated SMS transactions never have a SIP Dialog, and the LUR only uses a SipDialog briefly for
...
// For MO transactions, including LUR, the MS identity is not initially known and the TranEntry is identified only by
// the channel. (This is true for GSM, UMTS, and even GPRS.) Rarely, the MS may identify itself immediately by IMSI, but
// usually we must run an identification and/or authorization process. At some point we will learn the MS identity
// (IMSI), at which time we can attach it to its IMSI-based MM Context, and may initiate transactions that are queued in
// the MM Context.  (Except for MO-SOS calls, which may never have an IMSI identity, which means they may never link to
// an MM Context.) The MM Context is responsible for deciding what channel to use for a transaction, specifically, if
// there is already a channel to an MS then a queued MT transaction will try to start on that channel, otherwise page
// the MS. When a CS connection ends, if there are queued SMS transactions we are supposed to move the MS from TCH to
// SDCCH. When a radio-link is lost, the MM Context must be notified so it can clean up the SIP side of all queued MT
// transactions. NOTE: 3GPP 04.11 2.2 indicates that SMS messages use the following channels:
...
// There are weird cases related to TMSIs, because in the MO case the TMSI->IMSI mapping, even if saved in the TMSI
// table or received from the VLR, is not known authoritatively until we have authenticated the phone.  While we are
// doing any MM procedure with an MS identified by TMSI, any MT transactions queued in the MM Context for the IMSI
// associated with the TMSI must be blocked until the TMSI is resolved one way or the other. TMSI collisions are
// possible.  For example, multiple MS could answer a page by TMSI; we could process them serially or conceivably we
// could run authorization procedures on both of them simultaneously. An MS that is initially identified by TMSI and
// subsequently fails authentication is supposed to stop using that TMSI, allowing other MS in the cell with the same
// TMSI to try answering the page. Reasons for TMSI collisions include reboot of the TMSI allocation authority (the VLR
// or SubscriberRegistry), or because phones roaming in have established TMSIs from a previously visited VLR with the
// same PLMN+LAC. The MS remembers its assigned TMSI *forever* in the SIM card until it is reassigned (or over-written
// by running out of memory slots to remember TMSIs as a result of visiting too many different PLMNs), so even in a
// perfect world it would still be impossible to avoid TMSI collisions.
...
// The same MS may have both a SACCH and either a FACCH or DCCH; these messages are steered to the same
// TransactionEntries. Note that there does not appear to be anything preventing an MS from allocating multiple
// TCH+FACCH or SDCCH, but we dont worry about that.  I believe the current code would even work as long as the MS used
// the same LogicalChannels for all the messages associated with a particular Procedure, but we dont currently bother to
// check the IMSI to try to match up possible multiple logical channels. Multiple TranEntrys on the same MS are
// differentiated by the TI [Transaction Identifier], which was formerly just ignored.

// Destruction:  Formerly TransactionEntrys were kept around a long time partly because they were also the parent of SIP
// session. Now when the radio-link to the MS is released, all TransactionEntries can be released immediately. The MM
// Context must also be deleted immediately - it is not designed to be a permanent TMSI-IMSI mapping repository. SIP
// Dialog destruction is handled separately in the SIP directory by timer. There are timers on the underlying
// L2LogicalChannel that prevent channel reuse which prevents stray messages from reaching L3 after a channel release.
...
protected:
	// (pat) After l3rewrite sip-side and radio-side contention is handled by funneling all messages into a single
	// queue. I am keeping a lock for safety anyway in case contention is added later, for example, by handling
	// timers in a different thread. We lock the TranEntry while we are running a state machine procedure. Cant use
	// the old lock without either breaking the existing code or rewriting all the accessor functions, so here is a
	// new lock for the l3rewrite.
...
public:
	CodecSet getCodecs() { return mCodecs; }

public:
	void setCodecs(CodecSet wCodecs) { mCodecs = wCodecs; }
...
protected:
	void teSetContext(MMContext *wContext) { mContext = wContext; }
...
public:
	...
	**/
	...
	bool isL3TIValid() const { return getL3TI() != TranEntry::cL3TIInvalid; }
...
private:
	...
	/** Removal status. */
private:
	void teRemove(TermCause cause); // Keep private.  Dont call from Procedures.  Call teCloseCall or teCancel or
					// chanClose instead.
public:
	void teCancel(TermCause cause)
	{
		teRemove(cause);
	} // Used from MM.  Cancel dialog and delete transaction, do not notify MM layer.  Used within MMLayer.
...
public:
	...
	*/
	...
	bool ttSetDialog(TranEntryId tid, SIP::SipDialog *dialog); // Unlike TranEntry::setDialog(), this can be used
								   // from other directories, other threads.
