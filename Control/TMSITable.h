...
class TmsiTableStore {
	...
	Bool_z store_valid; // Set when we have either updated the store or confirmed the imsi does not exist in the
			    // TMSI_TABLE.
	string imei;
	Bool_z imei_changed;
	string kc;
	...
	Bool_z associatedUri_changed;
	string assertedIdentity;
	Bool_z assertedIdentity_changed;
	Int_z assigned;
	Bool_z assigned_changed;
	Int_z a5support;
	Bool_z a5support_changed;
	Int_z powerClass;
	Bool_z powerClass_changed;
	// non-zero auth means authorized or accepted via openregistration or network failure.
	Enum_z<Authorization> auth;
	Bool_z auth_changed;

	Int_z authExpiry;
	Bool_z authExpiry_changed;
	Int_z rejectCode;
	Bool_z rejectCode_changed;
	// We dont have a welcomeSent_valid because if the IMSI is not in the TMSI_TABLE welcomeSent defaults to 0 which
	// is correct.
	Int_z welcomeSent;
	Bool_z welcomeSent_changed;

public:
	Authorization getAuth()
	{
		devassert(store_valid);
		return auth;
	}
	bool isAuthorized()
	{
		return getAuth() != AuthUnauthorized;
	} // authorized or passed open registration or network failure and FailOpen
	int getAuthExpiry()
	{
		devassert(store_valid);
		return authExpiry;
	...
	{
		auth = AuthUnauthorized;
		auth_changed = true;
		rejectCode = (int)val;
		rejectCode_changed = true;
	}
	...
	void setKc(string wKc)
	{
		kc = wKc;
		kc_changed = true;
	}
	void setAssociatedUri(string wAssociatedUri)
	{
		associatedUri = wAssociatedUri;
		associatedUri_changed = true;
	}
	void setAssertedIdentity(string wAssertedIdentity)
	{
		assertedIdentity = wAssertedIdentity;
		assertedIdentity_changed = true;
	}
	void setWelcomeSent(int val)
	{
		if (welcomeSent != val) {
			welcomeSent = val;
			welcomeSent_changed = true;
		}
	}
	void setClassmark(int wa5support, int wpowerClass)
	{
		a5support = wa5support;
		a5support_changed = true;
		powerClass = wpowerClass;
		powerClass_changed = true;
	}
...
public:
	...
	*/
	// uint32_t tmsiTabAssign(const string IMSI, const GSM::L3LocationAreaIdentity * lur, uint32_t oldTmsi,
