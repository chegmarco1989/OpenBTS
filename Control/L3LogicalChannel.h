...
class L3LogicalChannel {
	...
	// L3 state machines in case the TI [Transaction Identifier] in the message does not match this
	// TransactionEntry.
...
private:
	...
	enum ChannelState {
		...
		// chReleased,			// Channel released by Layer3 by sending RELEASE or HARDRELEASE to
		// layer 2.
		// These two states could be combined since they are functionally the same, but nice for debugging to
		// tell what happened. No longer used:
		// chReassignPending,		// This is an SDCCH with pending reassignment to TCH.
		// chReassignComplete, // This is an SDCCH after successful reassignment to TCH, ie, we are going to
		// release it momentarily.  chReassignFailure	// This channel is the reassignment target; needs to be
		// released after a reassignment failure.
	} volatile mChState;
...
public:
	...
	std::string chanGetImsi(bool verbose) const; // If the IMSI is known, return it, else string("") or if verbose,
						     // something to display in error messages and CLI.
