...
// Why cant we just use CC or RR Cause?  Because some of the reasons do not exist in any other single list, for example,
// NoAnswerToPage. An established SipDialog is ended by a SIP BYE, and an MO [Mobile Originated] SipDialog is canceled
// early using a SIP CANCEL, so this is used only for the case of an INVITE response where the ACK message has not been
// sent, or as a non-invite message (eg, SMS MESSAGE) error response.  As such, there are only a few codes that the SIP
// side cares about.  The vast majority of plain old errors, for example, loss of contact with the MS or reassignFailure
// will just map to the same SIP code so we use TermCauseUnknown, however, all such cases are distinguished from
// TermCauseNoAnswerToPage in that we know the MS is on the current system.

// pat added 6-2014, so every client that closes a transaction is forced to explicitly specify all needed cancellation
// information.
class TermCause {
...
public:
	...
	int tcGetSipCodeAndReason(string &sipreason); // Returns the nearest SIP code, and the sip reason phrase derived
						      // from the real cause.
