...
{
	ostringstream ss;
	// const char *requestType=mNpeType==npeCC ? "CC" : mNpeType==npeSMS ? "SMS" : mNpeType==npeGPRS ? "GPRS" :
	// "undefined";  ss << "NewPagingEntry(" <<LOGVAR(requestType) <<LOGVAR(mImsi) <<LOGVAR(mTmsi)
	// <<LOGHEX(mGprsClient) <<LOGVAR(mImmAssign) <<LOGVAR(mDrxBegin) <<")";
	ss << "NewPagingEntry(" << LOGVAR(mInitialChanType) << LOGVAR(mImsi) << LOGVAR(mTmsi) << LOGHEX(mGprsClient)
	   << LOGVAR(mImmAssign) << LOGVAR(mDrxBegin) << ")";
	return ss.str();
}
