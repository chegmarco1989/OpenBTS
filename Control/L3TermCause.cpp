...
// Config option Control.Termination.CauseToSIP.<causename> specifies the SIP code, and optionally, the SIP reason
// phrase, to be sent to the SIP peer when this L3 Cause occurs. Config option Control.Termination.SIPToCause.<SIP-code>
// specifies the name of the layer3 cause to be used for a specific SIP code. The Layer3 cause is passed to the handset
// to indicate the message to display, and also may be saved in the CDR. To make this easier for the user we will use
// the L3 cause as the SIP reason phrase, so the reason phrase can be placed in the config option to control it.
...
static int cause2SipCode(AnyCause acause, string &reason)
{
	...
	if (reason.empty()) {
		reason = string(L3Cause::AnyCause2Str(acause));
	}
	if (sip) {
		return sip;
	}
	...
	switch (acause.value) {
	// These are CC Causes:
	case L3Cause::Unknown_L3_Cause:
		sip = 486;
		break; // Busy
	case L3Cause::Unassigned_Number:
		sip = 404;
		break; // Not Found
	case L3Cause::No_Route_To_Destination:
		sip = 404;
		break; // Not found
	case L3Cause::Channel_Unacceptable:
		sip = 503;
		break; // Service Unavailable.
	case L3Cause::Operator_Determined_Barring:
		sip = 503;
		break; // Service Unavailable.
	case L3Cause::Normal_Call_Clearing:
		sip = 200;
		break; // OK
	case L3Cause::User_Busy:
		sip = 486;
		break; // Busy Here
	case L3Cause::No_User_Responding:
		sip = 408;
		break; // Request Timeout
	case L3Cause::User_Alerting_No_Answer:
		sip = 480;
		break; // Temporarily Unavailable.
	// RFC3261 says use 403 instead of 603 unless we know with certainty there is no second choice like forwarding
	// or voicemail.
	case L3Cause::Call_Rejected:
		sip = 403;
		break; // Forbidden.
	case L3Cause::Number_Changed:
		sip = 410;
		break; // Gone
	// Preemption,
	// Non_Selected_User_Clearing,
	// Destination_Out_Of_Order,
	case L3Cause::Invalid_Number_Format:
		sip = 484;
		break; // Address Incomplete
	case L3Cause::Facility_Rejected:
		sip = 503;
		break; // Service Unavailable.
	// Response_To_STATUS_ENQUIRY,
	case L3Cause::Normal_Unspecified:
		sip = 200;
		break; // OK
	case L3Cause::No_Channel_Available:
		sip = 503;
		break; // Service Unavailable.
	...
	case L3Cause::Resources_Unavailable:
		sip = 503;
		break;
...
string TermCause::tcGetSipReasonHeader()
{
	...
	default:
		q850cause =
			cccause; // All other CC-Causes that we use have the same values as corresponding Q.850 causes.
		break;
	}
...
//		RR:RadioInterfaceFailure=1, RR:UplinkQuality=2, RR:UplinkStrength=3, RR:DownlinkQuality=4,
// RR:DownlinkStrength=5, RR:Distance=6, 		BTS:O&MIntervention=7,
...
// 		MAP Absent-Subscriber error is used for NoPagingResponse, IMSI Detached, Restricted Area for CS
// services;
...
//		Congestion
//	MM Layer failures.  If these happen at the MTC end, we should send code upward.  In all cases A gets
// L3Cause::NoUserResponding?
//		AuthenticationReject (at the 4.08 level this is a message no specific code, but we can use the
// Control.LUR.UnprovisionedRejectCause
//		Many other failures possible at higher level: IMSI unknown, IMSI invalid, no number configured, use MM
// or  MAP reject cause.
//
...
//
// 		SipCodeKey(ConfigurationKeyMap &map,const char *name, int defaultValue, const char *units, const char
// *range, const char *help = "")
// 		{
...
//		The Event Location can be local handset, local BTS (radio problems), network, remote BTS or remote
// handset. 		The Locus can be CC (messages from handset), MM, RR, CN (core network)
//		Note: The Layer3 CC Cause includes messages from all loci.

// Termination before connection:
//	Event:AlertingNoAnswer
// 		MTC rings, no answer; MOC gets L3Cause::UserAlertingNoAnswer; SIP 408 (Request Timeout) (See
// L3StateMachine.cpp)  Lynx3.1.3 CDR:No-answer
//	Event:CallRejected
// 		MTC declines (aborts while ringing) MOC gets L3Cause::CallRejected; SIP 403: Forbidden (per RFC3261 sec
// 15) Lynx3.1.4 CDR:B-party-abort
//	Event:Cancel
//		MOC cancel (hangs up while ringing) MTC gets L3Cause::NormalCallClearing?;  MOC sends SIP CANCEL, MTC
// sends 487 Request Terminated
//			Lynx3.1.5: CDR:A-party-abort.     could argue "NormalCallClearing" is reserved for termination
// after  connect.
//			Pat says: how do we represent this in the CDR?  This is A-party normal hangup with 0 seconds
// connect  time. 	Event:IMSIDetach 		MOC IMSIDetach during ringing; identical to MOC Cancel
// Lynx3.1.9:CDR:A-party-abort
...
//	Event:NoPagingResponse
//		MTC NoPagingResponse (off or outside coverage area); L3Cause::DestinationOutOfOrder;
// Lynx3.1.6&7:CDR:No-page-response
//			either SIP 408 (Request Timeout) or 504 (Server Timeout)  404 (Not Found) doesnt seem right
// because  the server 				has definitive info that user does not exist, but in our case a new page
// may  succeed. 	Event:UserBusy
// 		MTC busy (has existing call, and call-hold/call-wait not available)  MOC gets SIP 486 (not 600)
// Lynx3.1.8:CDR:B-party-busy
//	Event::AuthorizationFailure Reason:MMRejectCause from 24.008 10.5.3.6
...
//	Event::Disconnect (Disconnect, Release or ChannelRelease message)  Reason:L3Cause passed by handset in message.
// 		MOC (A-party) disconnect; L3Cause::NormalCallClearing; SIP BYE, no sip code, Reason:none Lynx3.1.1
// CDR:A-party-normal-disconnect 		MTC (B-party) disconnect; L3Cause::NormalCallClearing; SIP BYE, no sip
// code Reason:none Lynx3.1.2 CDR:B-party-normal-disconnect
//	Event::OperatorIntervention
// 		Operator Intervention.	L3Cause::Preemption SIP 480 (Temporarily Unavailable) or 503 (Service
// Unavailable)
//	Event::Preemption
// 		Preemption by Emergency Call. L3Cause::Preemption  SIP 480 (Temporarily Unavailable) or 503 (Service
// Unavailable)
//	Event::RadioLoss  Reason:BSS Cause GSM*.08-3.2.2.5
//		MOC or MTC connection lost (simulated by MS turned off, which is not really correct since it generates a
// detach); RRcause?  SIP BYE?
//			Lynx 3.1.11&3.1.12:CDR:Termination-error if MOC or CDR:B-party-abort if MTC lost.  That's just
// dopey. 			We can use L3cause::NormalUnspecified, which we do not use for anything else.
//		Peer gets SIP BYE,

// Return a call-control cause, formerly aka L3Cause, from GSM 4.08 10.5.4.11.
// Call-control causes are meant for communication to the handset, and in the case where we are sending a CC cause to
// the handset, the cause will already always be a CC cause. But we are also sending them to the outside world in SIP,
// and maybe we wills tore them in the CDRs as well, so if it is not a call-control cause, return the nearest match.
L3Cause::CCCause TermCause::tcGetCCCause()
{
	...
	case L3Cause::LocusCustom:
		...
		case L3Cause::No_Paging_Response:
		case L3Cause::IMSI_Detached: // This is a MAP error code, which would have to map to l3cause
		...
		case L3Cause::No_Transaction_Expected: // Used when we dont expect termination to have any transactions
						       // to close out.
...
static int sipCode2AnyCauseFromConfig(int sipcode)
{
	...
	if (gConfig.defines(configOptionName)) {
		...
		if (cause) {
			...
			LOG(ERR) << "Config option '" << configOptionName << "' specifies unrecognized cause name:'"
				 << causeName << "'";
		}
	}
	return 0;
		     ...
		     bool alerted) // True if we ever received sip code 180.
{
	...
	if (!acause) {
		...
		case 503: // SIP Service Unavailable.  Asterisk sends this often.
			...
			break;
		case 0:
			// This is most likely an early MTD Mobile Terminated Disconnect caused by CANCEL before ACK
			// received.
			acause = L3Cause::User_Busy;
			break;
		default:
...
int parseReasonHeaderQ850(const char *reasonHeader)
{
	...
	if (!causep || (commap && causep > commap))
		return 0;
	causep += strlen("cause");
	int result;
	if (1 != sscanf(causep, " = %d", &result))
		return 0;
	return result;
}
