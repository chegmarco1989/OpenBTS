...
#define L3CASE_DIALOG_STATE(sipDialogState) \
	L3CASE_RAW(16, sipDialogState) // SIP messages use pd == 16, which is unused by GSM.
...
// 3. MM CONNECTION ACTIVE The MM sublayer has a RR connection to a mobile station. One or more MM connections are
// active, or no 	MM connection is active but an RRLP procedure is ongoing.
...
// 6. TMSI REALLOCATION INITIATED The TMSI reallocation procedure has been started by the network. The timer T3250 is
// running.
// 7. CIPHERING MODE INITIATED The cipher mode setting procedure has been requested to the RR sublayer.
// 8a. WAIT FOR MOBILE ORIGINATED MM CONNECTION A CM SERVICE REQUEST message is received and processed, and the MM
// sublayer 	awaits the "opening message" of the MM connection.
// 9. WAIT FOR REESTABLISHMENT The RR connection to a mobile station with one or more active MM connection has been
// lost.
...
// The spec assumes that the MM entity and CM entity are separate, but we run all procedures on top of a
// TransactionEntry object, so we can combine some MM states.
enum MMState {
	MMStateUndefined,
	...
	MMBlocked, // blocked for LUR, Identification or Authentication in progress, either on this IMSI (blocked for
		   // sure) or on this TMSI (speculatively blocked.)  The outcome of the procedure may determine whether
...
public:
	RemovedTransaction(unsigned wTransactionID = 0) : ControlLayerException(wTransactionID)
	{
