...
void L3LogicalChannel::L3LogicalChannelReset()
{
	...
	chanFreeContext(
		TermCause::Local(L3Cause::No_Transaction_Expected)); // If we do cancel any dialogs, it is in error.
	LOG(DEBUG);
...
L3LogicalChannel::~L3LogicalChannel() { chanFreeContext(TermCause::Local(L3Cause::No_Transaction_Expected)); }

// This virtual method should never be called; it is over-ridden by the sub-class for all channel types that matter.
const char *L3LogicalChannel::descriptiveString() const { return "undefined"; }
...
const L2LogicalChannel *L3LogicalChannel::getL2Channel() const { return dynamic_cast<const L2LogicalChannel *>(this); }
...
void L3LogicalChannel::l3sendm(const GSM::L3Message &msg, const GSM::Primitive &prim /*=GSM::DATA*/, SAPI_t SAPI /*=0*/)
{
	...
	WATCHINFO("l3sendm" << LOGVAR(prim) << LOGVAR(SAPI) << LOGVARP(msg) << " "
			    << this); // 'this' is the descriptive string of the channel.
...
void L3LogicalChannel::l3sendf(const GSM::L3Frame &frame)
{
	...
	// 3-14-2014 pat: Changed the LOG levels, formerly we sent the frame to INFO and the message to DEBUG, but the
	// frame is raw, so I reversed it.
...
void L3LogicalChannel::l3sendp(const GSM::Primitive &prim, SAPI_t SAPI)
{
	...
	WATCHINFO("l3sendp" << LOGVAR(prim) << LOGVAR(SAPI) << " "
			    << this); // 'this' is the descriptive string of the channel.
...
L3Frame *L3LogicalChannel::waitForEstablishOrHandover()
{
	while (true) {
		...
		if (req == NULL)
			continue;
		if (req->primitive() == L3_ESTABLISH_INDICATION)
			return req;
		if (req->primitive() == HANDOVER_ACCESS)
			return req;
...
void L3LogicalChannel::reassignStart()
{
	...
	mNextChan->chanFreeContext(
		TermCause::Local(L3Cause::No_Transaction_Expected)); // This is supposed to be a no-op.
	...
	// Note we do not want to do a HARDRELEASE if this fails, because that bypasses the very timer we are supposed
	// to be using.
...
// Release of nextChan also occurs if a channel drops out of the main service loop and the nextChan state is still
// ReassignTarget
void L3LogicalChannel::reassignFailure()
{
	...
	if (mNextChan) {
		...
		mNextChan->chanSetState(
			L3LogicalChannel::chRequestRelease); // This will probably block the chan for 30 seconds.
		...
		// The service loops check dcch->chanRunning(), so they will not do anything that might try to use the
		// Context we are freeing..
	...
	// The MS is supposed to release the channel, which will send a RELEASE primitive up to Layer3 to close the
	// channel.
}
...
{
	{
		...
		if (mNextChan->mChState != chEstablished) {
			// The nextChan was supposed to get an ESTABLISH primitive then the L3AssignComplete command in
			// order to get here. There could be a logic error or the MS may have dropped the channel at
			// this inopportune moment, so it is not necessarily an error.
			LOG(NOTICE) << "Next channel in unexpected state, dropping channel" << LOGVARM(mNextChan);
			chanSetState(chRequestRelease);
			return;
		}
		mChContext->mmSetChannel(mNextChan);
		LOG(INFO) << "successful channel reassignment" << LOGVAR2("from-channel", this)
			  << LOGVAR2("to-channel", mNextChan);
		mNextChan = NULL;
	} // release lock
	...
	// chanSetState(L3LogicalChannel::chReassignComplete);	// Redundant with sending the HARDRELEASE, this will
	// cause the service loop to exit.
}
...
{
	LOG(DEBUG) << this;
	// Just in case this gets called with a next channel, release it too, although it would eventually time out on
	// its own.
	if (mNextChan) {
...
std::ostream &L3LogicalChannel::chanText(ostream &os) const
{
	...
	if (mNextChan) {
		os << LOGVAR2("nextchan", mNextChan->descriptiveString());
	}
	return os;
}
...
ostream &operator<<(ostream &os, const L3LogicalChannel &chan)
{
	chan.chanText(os);
	return os;
}
...
bool L3LogicalChannel::isTCHF() const { return chtype() == GSM::FACCHType; }

bool L3LogicalChannel::isSDCCH() const { return chtype() == GSM::SDCCHType; }
