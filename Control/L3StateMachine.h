...
// For SDCCH, the ch just gets reused after mT3109 expiry, and L3 is not notified - we just hope it is no longer in use
// after 30s.
...
struct MachineStatus {
	...
	enum MachineStatusCode {
		MachineCodeOK, // continue the procedure, meaning return to L3 message handler and wait for the next
			       // message.
		MachineCodePopMachine, // return to previous procedure on stack
		MachineCodeQuitTran, // Pop all machines from stack and remove the transaction.  This is the normal exit
				     // from a completed procedure.
...
public:
	// Args are the L3Procedure we are calling and the state in the current procedure to which we will return when
	// nextProcedure is popped.
...
// CS [Circuit Switched] L3 messages are specified in 3GPP 4.08, as opposed to PS [Packet Switched] L3 messages handled
// by the SGSN. This is part of the L3 rewrite.
