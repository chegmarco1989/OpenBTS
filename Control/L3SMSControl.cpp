...
void SMSCommon::l3sendSms(const L3Message &msg) { channel()->l3sendm(msg, GSM::L3_DATA, getSmsSap()); }
...
MachineStatus MOSMSMachine::machineRunState(int state, const GSM::L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case stateIdentResult: {
		if (!mIdentifyResult) {
			// const L3CMServiceReject reject =
			// L3CMServiceReject(L3RejectCause::Invalid_Mandatory_Information);
			// (pat 6-2014) I think this is wrong, based on comment below, so changing it to the main
			// channel: l3sendSms(L3CMServiceReject(L3RejectCause::Invalid_Mandatory_Information),SAPI0);
	...
	case L3CASE_SMS(DATA): {
		...
		// (pat) The SMS message has already been through L3Message:parseL3, which called SMS::parseSMS(source),
		// which manufactured a CPMessage::CPData and called L3Message::parse() which called CPData::parseBody
		// which called L3Message::parseLV, which called CPUserData::parseV to leave the result in
		// L3Message::CPMessage::CPData::mData. As the mathemetician said after filling 3 blackboards with
		// formulas: It is obvious!
		...
		// (pat) Update: If we cant parse far enough to get the ref we send a CPError that does not need the
		// ref.
#if 0
			unsigned ref;
			bool success = false;
		...
		// If we cant parse the message, we send RP-Error immeidately, otherwise we wait for the dialog to
...
// The CM Service Request to start a new MO-SMS during an existing one may arrive before the final ACK of the previous
// MO-SMS, as per GSM 4.11 5.4 Therefore there are two MO-SMS possible: MMContext::TE_MOSMS1 is the old one and
// TE_MOSMS2 is the new one.
void startMOSMS(const GSM::L3MMMessage *l3msg, MMContext *mmchan)
{
	...
	if (!prevMOSMS.self()) {
		...
		// Check for perfidy on the part of the MS: it cannot start a new MO-SMS unless the previous is nearly
		// finished.
		// SmsState smsState = getCurrentSMSState();
		MachineBase *base = prevMOSMS->currentProcedure();
		bool badbunny = false;
		if (base) { // This may happen if the MS is a bad bunny and sends two CM Sevice Requests effectively
			    // simultaneously.
			MOSMSMachine *smssm = dynamic_cast<typeof(smssm)>(base);
		...
		if (badbunny) {
			LOG(ERR) << "Received new MO-SMS before previous MO-SMS completed"
				 << LOGVAR2("MOSMS", prevMOSMS.self()) << l3msg;
		...
		if (prevMOSMS2.self()) {
			LOG(ERR) << "Received third simultaneous MO-SMS, which is illegal:"
				 << LOGVAR2("MO-SMS1", prevMOSMS.self()) << LOGVAR2("MO-SMS2", prevMOSMS2.self());
			// Now what?  We could kill the oldest one or reject the new one.
			// Kill the oldest one, on the assumption that this indicates a bug in our code and that SMS is
			// hung.
...
bool MTSMSMachine::createRPData(RPData &rp_data)
{
	...
	if (strncmp(contentType, "text/plain", 10) == 0) {
		...
		try { // I suspect this is here to catch the above FIXED crash when string is zero length
			...
			LOG(WARNING) << "SMS parsing failed (above L3)";
			// Cause 95, "semantically incorrect message".
			// LCH->l2sendf(CPData(L3TI,RPError(95,this->mRpduRef)),3); if you ever use this, it should call
			// l3sendSms
			return false;
		} catch (GSM::L3ReadError) {
			LOG(WARNING) << "SMS parsing failed (in L3)";
	...
	int state, const L3Frame *frame, const L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	case stateStart: {
		...
		if (getDialog() && getDialog()->isFinished()) {
			...
			return MachineStatus::QuitTran(
				TermCause::Local(L3Cause::SMS_Timeout)); // could be a sip internal error?
		}
	...
	// SABM was started by us and handset simultaneously, so we just dont bother with making ESTABLISH_CONFIRM
	// separate.
	...
	case L3CASE_SMS(DATA): {
		...
		// Ack in SIP domain.
		if (!getDialog()) {
			LOG(DEBUG) << "No dialog found for MTSMS; could be welcome message, CLI SMS, or Dialog "
				      "pre-destroyed error";
		} else if (success) {
...
xxxxxxxx This version is not used
		...
		// We do this immediately upon reception of a SIP message to error check it before queueing it for
		// delivery to an MS. Return result or NULL on failure.  SIP should return error 400 "Bad Request" in
		// this case.
		RPData *
	parseSMS(const char *callingPartyDigits, const char *message, const char *contentType)
{
	// TODO: Read MIME Type from smqueue!!
