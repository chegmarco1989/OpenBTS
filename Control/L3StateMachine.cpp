...
void MachineBase::timerStart(L3TimerId tid, unsigned val, int nextState) { tran()->timerStart(tid, val, nextState); }

void MachineBase::timerStop(L3TimerId tid) { tran()->timerStop(tid); }

void MachineBase::timerStopAll() { tran()->timerStopAll(); }

void MachineBase::machineErrorMessage(
...
MachineStatus MachineBase::machPush(MachineBase *wCalledProcedure, // The L3Procedure we are calling.
...
MachineStatus MachineBase::closeChannel(RRCause rrcause, Primitive prim, TermCause upstreamCause)
{
	...
	// TODO: Make sure the routines that handle incoming dialog messages check for channel already in a released
	// state. This is not quite right: The ReleaseRequest state is for sending a CC Release, not an RR
	// ChannelRelease.
...
ControlLayerException MachineBase::procHandleL3Msg(GSM::L3Message *l3msg, L3LogicalChannel *lch)
{
	...
	IF you use this again : procInvoke was replaced by calls to handleMachineStatus inside lockAnd... methods of
					TranEntry procInvoke(state, l3msg, NULL);
}
#endif
...
MachineStatus MachineBase::callMachStart(MachineBase *wProc, unsigned startState)
{
	...
	return tran()->handleRecursion(wProc->machineRunState1(startState)); // Unless the StateMachine starts with a
									     // message, the start state must be state 0
									     // in every Machine.
}

L3LogicalChannel *MachineBase::channel() const { return tran()->channel(); }
...
bool MachineBase::isL3TIValid() const { return getL3TI() != TranEntry::cL3TIInvalid; }
...
// 3GPP 4.08 4.5.1.1 says "Upon receiving a CM service request" the network may start any common MM or RR procedure
// including classmark interrogation, identification, authentication and cypering.
...
MachineStatus MachineBase::machineRunState(
...
MachineStatus handlePrimitive(const L3Frame *frame, L3LogicalChannel *lch)
{
	...
	default:
		...
		//	lch->chanRelease(RELEASE); 		// Kill off all the transactions associated with this
		// channel. 	return MachineStatusQuitChannel;
		//
		// case HARDRELEASE:
		//	if (frame->getSAPI() == 0) { lch->chanRelease(HARDRELEASE); } 		// Release the channel.
		//	return MachineStatusQuitChannel;
		// default:
		//	LOG(ERR) <<lch<<"unhandled primitive: " << frame->primitive();	// This is horrible.  But
		// lets warn instead of crashing.
		//	// Fall through
...
MachineStatus MachineBase::machineRunState1(
	int state, const L3Frame *frame, const L3Message *l3msg, const SIP::DialogMessage *sipmsg)
{
	...
	if (frame) {
		...
		if (!frame->isData()) {
			return handlePrimitive(frame, channel());
		}
	}
...
MachineStatus MachineBase::dispatchFrame(const L3Frame *frame, const L3Message *l3msg)
{
	...
	if (frame) {
		if (frame->isData()) {
			// Note: the frame MTI must be ANDed with 0xbf.  See 4.08 10.4.  I moved that into class
			// L3Frame.
...
MachineStatus MachineBase::dispatchTimeout(L3Timer *timer)
{
	...
	if (nextState >= 0) {
		...
		// (pat) TODO: We should not use one error fits all here; the error should be set up when the timer was
		// established.
		TermCause cause = TermCause::Local(L3Cause::No_User_Responding); // SVG 5/20/14 changed this from
										 // InterworkingUnspecified to
										 // NoUserResponding
		...
		case L3CMServiceType::MobileTerminatedCall: {
			LOG(INFO) << "SIP term info dispatchTimeout call teCloseCallNow  servicetype: "
				  << tran()->servicetype(); // SVGDBG
		...
		return closeChannel(
			L3RRCause::Timer_Expired, L3_RELEASE_REQUEST, TermCause::Local(L3Cause::No_User_Responding));
	} else {
		assert(0);
...
MachineStatus MachineBase::dispatchSipDialogMsg(const SIP::DialogMessage *dialogmsg)
{
	return machineRunState1(
		L3CASE_DIALOG_STATE(dialogmsg->dialogState()), (L3Frame *)NULL, (L3Message *)NULL, dialogmsg);
}
...
static void csl3HandleLCHMsg(GSM::L3Message *l3msg, L3LogicalChannel *lch)
{
	...
	// I think previously nothing.   But we are not going to try to aggregate messages from multiple channels in the
	// same MS together. We assume that all the messages for a single Machine arrive on a single channel+SACCH pair.
...
static bool checkPrimitive(Primitive prim, L3LogicalChannel *lch, int sapi)
{
	...
	case MDL_ERROR_INDICATION: ///< channel error
		...
		lch->chanRelease(L3_RELEASE_REQUEST,
			TermCause::Local(
				L3Cause::Layer2_Error)); // Kill off all the transactions associated with this channel.
	...
	case L3_RELEASE_INDICATION: ///< normal channel release
		// if (lch->mChState == L3LogicalChannel::chReassignPending || lch->mChState ==
		// L3LogicalChannel::chReassignComplete) {
		...
		if (sapi == 0)
			lch->chanRelease(L3_RELEASE_REQUEST, TermCause::Local(L3Cause::Normal_Call_Clearing));
		return false;
	default:
		LOG(ERR) << lch << "unhandled primitive: " << prim; // oops!  But lets warn instead of crashing.
		// Something horrible happened.
		lch->chanRelease(L3_RELEASE_REQUEST, TermCause::Local(L3Cause::L3_Internal_Error)); // Kill off all the
												    // transactions
												    // associated with
												    // this channel.
		// lch->freeContext();
...
static void csl3HandleFrame(const GSM::L3Frame *frame, L3LogicalChannel *lch)
{
	...
	if (!checkPrimitive(frame->primitive(), lch, frame->getSAPI())) {
		return;
	}
...
#endif
	if (!found) {
		LOG(DEBUG) << "sip message code=" << sipmsg->mSIPStatusCode << " no matching transaction found.";
	}
}
#endif
...
static unsigned newUpdateCallTraffic(TranEntry *transaction, GSM::TCHFACCHLogicalChannel *TCH)
{
	...
	LOG(DEBUG);
	if (transaction->getGSMState() != CCState::Active) {
		return false;
	}
	unsigned activity = 0;
	...
	unsigned numFlushed = 0;
	{
		...
		if (numFlushed) {
			LOG(DEBUG) << TCH << " ulFrame flushed " << numFlushed << " in " << testTimeStart.elapsed()
				   << " msecs";
		}
	}
...
static bool checkemMessages(L3LogicalChannel *dcch, int delay)
{
	...
	// be safe and handle the message with the generic L3 handler instead of dispatching it directly to this
	// transaction.
	...
	if (set->mmcTerminationRequested) {
		...
		dcch->chanClose(L3RRCause::Preemptive_Release, L3_RELEASE_REQUEST,
			TermCause::Local(L3Cause::Operator_Intervention));
		return true;
	}
	...
	// Sadly we cannot process the sip messages in a separate global L3 thread because the TranEntry/procedure may
	// be locked for up to 30 seconds when sending to LAPDm, which would then stall that L3 thread.
...
// Handle TCH traffic during a call.  This is called from the dedicated thread created for a combination I full rate TCH
// in GSMConfig. It is called from DCCHDispatch. Prior to L3rewrite TCH traffic and messages were handled by
// callManagementLoop(), which called pollInCall(), updateGSMSignalling()
static void l3CallTrafficLoop(L3LogicalChannel *dcch)
{
	...
	while (dcch->chanRunning() && !gBTS.btsShutdown()) {
		if (tch->radioFailure()) {
			...
			dcch->chanRelease(L3_HARDRELEASE_REQUEST,
				TermCause::Local(L3Cause::Radio_Interface_Failure)); // Kill off all the
										     // transactions
										     // associated with this
										     // channel.
			// tran->getDialog()->dialogCancel(TermCause::TermCodeUnknown, GSM::L3Cause::Unknown_L3_Cause);
			// // was forceSIPClearing  tran->teRemove();
			return;
		}
		...
		LOG(DEBUG) << dcch << " main loop" << LOGVAR(nextDelay)
			   << " found tran:" << (tran != NULL); // warning: tran may be NULL.
		if (tran != NULL) {
			if (tran->deadOrRemoved()) {
				...
				LOG(NOTICE)
					<< "attempting to use a defunct Transaction" << LOGVAR(dcch) << LOGVAR(*tran);
			...
			// The termination request comes from the CLI or from RadioResource to make room for an SOS
			// call.
			L3Cause::AnyCause termcause = tran->terminationRequested();
			if (termcause.value != 0) {
				LOG(DEBUG) << dcch << " terminationRequested";
				tran->terminateHook(); // Gives the L3Procedure state machine a chance to do something
						       // first.
				// GSM 4.08 3.4.13.4.1: Use RR Cause PreemptiveRelease if terminated for a higher
				// priority, ie, emergency, call
				dcch->chanClose(
					L3RRCause::Preemptive_Release, L3_RELEASE_REQUEST, TermCause::Local(termcause));
			...
			if (tran->getGSMState() == CCState::HandoverOutbound) {
				if (outboundHandoverTransfer(tran, dcch)) {
					...
					dcch->chanRelease(
						L3_HARDRELEASE_REQUEST, TermCause::Local(L3Cause::Handover_Outbound));
					return; // We wont be back.
				}
			}
		...
		// so the in-call SMS handler hangs for several seconds (in the best case.)  The rxFrame below also
		// hangs. In order not to wait too long to service of these two things, I am adding a super-hack to
		// alternate servicing them. I think we need to add another thread to run SAPI3.
		if (++alternate % 2) {
			if (checkemMessages(dcch, nextDelay)) {
				gResetWatchdog();
				nextDelay = 0;
				continue;
			}
		...
		if (tran != NULL) {
			...
			if (unsigned theseBytes = newUpdateCallTraffic(tran, tch)) {
				...
				// if (now != lasttime) { LOG(DEBUG) "call traffic" <<LOGVAR(bytes) <<LOGVAR(dcch)
				// <<LOGVAR(tran); }
...
// TODO: When a channel is first opened we should save the CMServiceRequest or LocationUpdateRequest or PagingResponse
// and initiate an identification and optional authorization procedure on the channel itself.  That is true for both GSM
// and UMTS. Once we have an IMSI, we can create TranEntrys for each procedure that is MO, and also move each each MTC
// or MT-SMS TranEntry onto this channel. If there was a race between MTC and MOC it needs to be resolved at the same
// time, although that may turn out to be a no-op, ie, resolved by whether the MS sent CMServiceRequest or
// PagingResponse first.  If the former, the MT calls need to turn into call-waiting things. Note MS could send multiple
// simultaneous MO CMServiceRequests - one for CS and one for SMS. FOR NOW: Just have a primary transaction on the
// channel, which is known from the GSMState aka CallState.
static void L3SDCCHLoop(L3LogicalChannel *dcch)
{
	...
	while (dcch->chanRunning() && !gBTS.btsShutdown()) {
		if (dcch->radioFailure()) { // Checks expiry of T3109, set at 30s.
			...
			// gNewTransactionTable.ttLostChannel(dcch);
			// (pat) 5-2014: Changed to RELEASE from HARDRELEASE - even though we can no longer hear the
			// handset, it might still hear us so we have to deactivate SACCH and wait T3109.
			dcch->chanRelease(L3_RELEASE_REQUEST,
				TermCause::Local(L3Cause::Radio_Interface_Failure)); // Kill off all the
										     // transactions
										     // associated with this
										     // channel.
		...
		// in handling SIP messages or timers, and could be much longer since none of those are precision
		// timers.
		if (checkemMessages(dcch, 100)) {
			gResetWatchdog();
			continue;
		}
	}
...
void L3DCCHLoop(L3LogicalChannel *dcch, L3Frame *frame)
{
	...
	case L3LogicalChannel::chRequestRelease:
		// The RELEASE primitive will block up to 30 seconds, so we NEVER EVER send it from anywhere but right
		// here. To release the channel, set the channel state to chReleaseRequest and let it come here to
		// release the channel.
		// FIXME: Actually, LAPDm blocks in this forever until it gets the next ESTABLISH, so this is where the
		// serviceloop really waits.
		dcch->l3sendp(
			L3_RELEASE_REQUEST); // WARNING!  This must be the only place in L3 that sends this primitive.
		break;
...
void CSL3StateMachine::csl3ServiceLoop()
{
	if (IS_LOG_LEVEL(DEBUG)) {
		...
		if (gNewTransactionTable.dump(os, true)) {
			LOG(DEBUG) << os.str();
		}
	}
	try {
...
{
	while (1) {
		gCSL3StateMachine.csl3ServiceLoop();
	}
	return NULL;
}
