...
//	assignTCHF
//		on success (have TCH), if MTCq, start it, else (they hung up) if SMSq run switchToSDDCH, else run
// closeChannel. 	switchToSDCCH
...
// (pat) WARNING: If this routine returns true it has performed the gMMLock.unlock() corresponding to a lock() in the
// caller. Setting the lock in one function and releasing it in another sucks and should be fixed.
...
bool MMContext::mmCheckNewActivity()
{
	...
	// If there is a mobility management procedure in progress then we wont start anything else until it is
	// finished. We shouldnt need to lock gMMLock yet because the MMContext cannot be deleted while in the thread
	// that called us, and mmcServiceRequests is a thread safe queue.
	if (!mmInMobilityManagement()) {
		...
		if (mmcMMU) {
			if (mmcMMU->mmuServiceMTQueues()) {
				return true;
			} // If it returns true it unlocked the lock, gack.
		}
		gMMLock.unlock();
	}
...
//	// The purpose of this is to choose the channel type, so it doesnt really matter what the servicetype is as long
// as it is one that can use SDCCH. 	return L3CMServiceType::ShortMessage;
//}
	...
	TermCause cause) // Some callers deleted it from the MMUsers more efficiently than looking it up again.
{
...

	{
		...
		while (TranEntry *tran = mmuMTCq.pop_frontr()) {
			tran->teCancel(cause);
		}
		while (TranEntry *tran = mmuMTSMSq.pop_frontr()) {
			tran->teCancel(cause);
		}
...
bool MMContext::mmCheckSipMsgs()
{
	...
	// (pat) Update 9-19-2014: Formerly, when this global lock was in place we got the "blocked more than one second
	// at..." messages during the stress test.  Ticket #1905 reports a crash that looks like using a transaction
	// here while it is being deleted.  Since I have separated layer2 from layer3 with InterthreadQueues since the
	// last test, I am re-enabling this global lock to attempt to fix the crash. (pat) 10-10-2014 update: Holding
	// the global lock here appears to cause system lockups as reported in St Pierre.  Not sure why. So here is the
	// revised plan: The gMMLock will be used only temporarily to protect access to the list, then we will drop the
	// global lock and attempt to invoke lockAndInvokeSipMsgs, which will be modified to check that the transaction
	// has not been deleted in the interim period - the mL3RewriteLock in the transaction prevents collisions at
	// that transaction level. ScopedLock lock(gMMLock,__FILE__,__LINE__);
...
bool MMContext::mmCheckTimers()
{
	// Update: We cannot hold the global lock while invoking state machines because they can block.
	// As an interim measure, just dont lock this and hope for the best.
	// ScopedLock lock(gMMLock,__FILE__,__LINE__);	// I think this is unnecessary; the channel cannot change
	// when this is called, but be safe.
	// Check MM timers.
	TranEntryVector tranlist;
	mmGetTranList(tranlist);
	...
	//	// The old SDCCH channel will be released when the l2recv finishes and the channel notices that its
	// state has changed.
	//	// mPrevChan->l3sendp(GSM::HARDRELEASE);  Dont do this.  It can block.  Set the chReassignComplete flag
	// and let that thread do it. 	mPrevChan->chanSetState(L3LogicalChannel::chReassignComplete);
	//
...
// 			// So if no transaction matched this specific TI, we send the message to the primary MO-SMS
// transaction and hope for the best. 			if (tran == NULL) { tran = mmcTE[TE_MOSMS1]; }
// 			break;
// 		}
...
// 				// We have to notify the Procedure when complete, however when rmsimsieassignComplete
// returns
// 				// we have replaced the MMContext on this channel with the one from the old channel,
// 				// and 'this' has been deleted.  So we pass the tran that needs to be notified to
// reassignComplete
...
// 			// run the MM procedure first to identify the channel, then send a message to the CS procedure
// to
// start it. 			tran = mmcTE[TE_MM] ? mmcTE[TE_MM] : mmcTE[TE_CS1] ? mmcTE[TE_CS1] : mmcTE[TE_MOSMS1] ?
// mmcTE[TE_MOSMS1] : mmcTE[TE_MTSMS]; 			break; 		default: 			LOG(ERR) <<
// "unrecognized L3 frame:"<<LOGVAR(pd)<<LOGVAR(ti); 			return NULL;	// hopeless.
// 	}
// 	return NULL;
// }
#endif
...
RefCntPointer<TranEntry> MMContext::findTran(const L3Frame *frame, const L3Message *l3msg) const
{
	...
	if (frame && !frame->isData()) {
		...
		return mmcTE[TE_MTSMS].self(); // We dont need to keep this locked because the tran is used only
					       // internally, so can use self and return TranEntry*
	}
	if (frame && frame->isData()) {
		pd = frame->PD();
		...
		pd = l3msg->PD();
		ti = l3msg->TI(); // Only call control, SMS and SS frames have a useful ti; returns nonsense for other
				  // PDs.
	} else {
	...
	case L3SMSPD: {
		...
		// For MO-SMS the TI in the transaction is not set until the first CP-DATA message arrives.
		// So if no transaction matched this specific TI, we send the message to the primary MO-SMS transaction
		// and hope for the best.
		if (TranEntry *te = mmcTE[TE_MOSMS1].self()) {
			return te;
		}
		break;
	}
	...
	case L3MobilityManagementPD:
		// TODO: This is a hack.  We should split the Procedures into MM and CS parts, and
		// run the MM procedure first to identify the channel, then send a message to the CS procedure to start
		// it.
		// return mmcTE[TE_MM] ? mmcTE[TE_MM] : mmcTE[TE_CS1] ? mmcTE[TE_CS1] : mmcTE[TE_MOSMS1] ?
		// mmcTE[TE_MOSMS1] : mmcTE[TE_MTSMS];
		for (unsigned txi = TE_MM; txi < TE_num; txi++) {
			if (TranEntry *te = mmcTE[txi].self()) {
				return te;
			}
		}
		break;
	case L3NonCallSSPD: {
		...
		for (ActiveTranIndex ati = TE_CS1; ati <= TE_CSHold; ati = (ActiveTranIndex)(ati + 1)) {
			...
			if (cs && cs->matchL3TI(ti, true)) {
				WATCHINFO("Found SS message matching CC transaction"
					  << LOGVAR2("message ti", ti) << LOGVAR2("transaction ti", cs->getL3TI()));
				return cs;
			}
		}
		...
		if (te) {
			...
			WATCHINFO("Sending SS message to SS machine" << LOGVAR2("message ti", ti)
								     << LOGVAR2("transaction ti", te->getL3TI()));
		} else {
	...
	LOG(INFO) << "No transaction found to handle frame with" << LOGVAR2("PD", pd) << LOGVAR2("TI", ti)
		  << LOGVAR2("MTI", frame->MTI());
...
bool MMContext::mmDispatchL3Msg(const L3Message *l3msg, L3LogicalChannel *chan)
{
	...
	//		if (cs && (l3msg->MTI() == L3CCMessage::Setup || l3TISigBits(cs->getL3TI()) ==
	// l3TISigBits(l3msg->TI()))) { 			tran = cs;
	//		}
	//		break;
	//	}
	//	case L3SMSPD: {
	...
	//				break;
	//			}
	//			// For MO-SMS the TI in the transaction is not set until the first CP-DATA message
	// arrives.
	//			// So if no transaction matched this specific TI, we send the message to the primary
	// MO-SMS  transaction and hope for the best. 			if (tran == NULL) { tran = mmcTE[TE_MOSMS1]; }
	//		}
	//		break;
	//	}
	//	case L3RadioResourcePD:
	...
	//		// TODO: This is a hack.  We should split the Procedures into MM and CS parts, and
	//		// run the MM procedure first to identify the channel, then send a message to the CS procedure
	// to  start it. 		tran = mmcTE[TE_MM] ? mmcTE[TE_MM] : mmcTE[TE_CS1] ? mmcTE[TE_CS1] :
	// mmcTE[TE_MOSMS1]
	// ?
	// mmcTE[TE_MOSMS1] : mmcTE[TE_MTSMS]; 		break; 	default: 		LOG(ERR) << "unrecognized
	// L3"<<LOGVAR(pd); 		return NULL;	// hopeless.
	//}
...
void MMContext::mmcPageReceived() const
{
	...
	if (!tran1.isNULL()) {
		// The MS sent a page response when there is an active voice call?  Either that or we are totally goofed
		// up.
...
TODO:
	...
	if (chan) {
		LOG(INFO) << "using existing channel " << chan->descriptiveString();
	} else {
...
{
	// (pat) The BLU phone seems to have a bug that a new MTC beginning too soon after a previous MTC with the same
	// TI seems to hang the phone, even though we definitely went through the CC release procedure whose specific
...
void MMContext::mmcText(std::ostream &os) const
{
	...
	if (mmcTE[TE_MM] != NULL) {
		os << LOGVAR2("MM", *mmcTE[TE_MM]);
	}
	if (mmcTE[TE_CS1] != NULL) {
		os << LOGVAR2("CS", *mmcTE[TE_CS1]);
	}
	if (mmcTE[TE_MOSMS1] != NULL) {
		os << LOGVAR2("MO-SMS", *mmcTE[TE_MOSMS1]);
	}
	if (mmcTE[TE_MOSMS2] != NULL) {
		os << LOGVAR2("MO-SMS2", *mmcTE[TE_MOSMS2]);
	}
	if (mmcTE[TE_MTSMS] != NULL) {
		os << LOGVAR2("MT-SMS", *mmcTE[TE_MTSMS]);
	}
	if (mmcTE[TE_SS] != NULL) {
		os << LOGVAR2("SS", *mmcTE[TE_SS]);
	}
	if (mmcTE[TE_TCall] != NULL) {
		os << LOGVAR2("TCall", *mmcTE[TE_TCall]);
	}
	os << ")";
}
...
void MMContext::getTranIds(TranEntryList &tranlist) const
{
	...
	for (unsigned ati = TE_first; ati < TE_num; ati++) {
		if (mmcTE[ati] != NULL) {
			tranlist.push_back(mmcTE[ati]->tranID());
		}
	}
	if (mmcMMU) {
		// TODO
...
// After this, the RefCntPointer in mmcTE takes over the job of deleting the transaction when the last pointer to it
// disappears.
void MMContext::mmConnectTran(ActiveTranIndex ati, TranEntry *tran)
{
	...
	devassert(gMMLock.lockcnt()); // Caller locked it.
	// When a primary transaction is deleted we may promote the secondary transaction, so keep trying to make sure
	// we delete them all:
	...
	for (unsigned ati = TE_first; ati < TE_num; ati++) {
		if (!oldmmc->mmcTE[ati].isNULL()) {
			if (mmcTE[ati].isNULL()) {
				...
				// I don't think this is possible for double paging responses (see comments at mmcLink
				// and NewPagingResponseHandler) because we call mmcLink immediately when the second
				// page is received, so the new MMC is empty. I'm not sure about other cases; the logic
				// is too complicated. We will keep the new (more recent) transaction and the old
				// transaction on the old MMC will be dropped when that channel is closed.
				LOG(ERR) << "Handset has changed channels and has transactions running on the both "
					    "channels.  "
					 << LOGVAR2("old channel", oldmmc->tsChannel())
					 << LOGVAR2("new channel", tsChannel())
				...
				// which the caller should do immediately.  We could call teCancel here but teCancel is
				// tricky and I would like to reduce the number of calls to it.  This probably doesnt
				// happen anyway. If this does happen, we could be more clever, like a voice transaction
				// could move to the secondary slot, etc.
			}
		}
	}
}
...
void MMContext::mmcLink(MMUser *mmu)
{
	...
	// We send two immediate assignments, and the MS may respond to both!  First it does an L1 LAPDm negotiation on
	// A and sends a Paging Response there, which connects its MMU to the MMContext for A, then it does an L2 LAPDm
	// negotiation on B and sends a second Paging Response there, so we get here with this == channel B but with the
	// MMU attached to channel A.  So we must disconnect the existing channel and move the MMU to the new channel.
	// We have to move the transactions from the old MMContext to the new; for example if there was only one MTC
	// transaction and it has already been moved from the MMU to the MMC, then the MMU is empty of transactions
	// which will release channel B immmediately in mmCheckNewActivity.
...
void MMContext::mmcFree(TermCause cause)
{
	...
	for (unsigned i = 0; i < TE_num; i++) {
		...
		for (unsigned tries = 0; tries < 3; tries++) {
			if (mmcTE[i] != NULL) {
				mmcTE[i]->teCancel(cause);
			} // Removes the transaction from mmcTE via mmDisconnectTran
		}
...
void MMLayer::mmFreeContext(MMContext *mmc, TermCause cause)
{
	...
	ScopedLock lock(gMMLock, __FILE__, __LINE__);
	LOG(DEBUG) << mmc;
	if (--mmc->mmcChannelUseCnt > 0)
		return;

	MMUser *mmu = mmc->mmcMMU;
	mmc->mmcUnlink(); // resets mmcMMU

	// This channel was closed normally, or because of channel loss (for example, reassign failure) or an internal
	// error. In the latter cases there could be running SipDialogs on the channel and we have to tell them
	// something. The SipCode 408 allows the peer to retry immediately.
	...
	// 480 indicates some temporary form of resource unavailability or congestion but resource is accessible and can
	// be checked 503 indicates the service is unavailable but does not imply for how long
	...
	if (mmu) {
		...
		if (mmu->mmuIsEmpty()) {
			mmu->mmuFree(NULL, TermCause::Local(L3Cause::No_Transaction_Expected)); // TermCause is not used
...
MMUser *MMLayer::mmFindByImsi(
	string imsi, // Do not change this to a reference.  We need a copy of the string
	...
	bool create)
{
	...
	if (create) {
		// Use insert so we only traverse the map tree once.  If element does not exist, a pair with
		// MMUser*==NULL is inserted. This wonderful insert method returns a pair<MMUserMap::iterator,bool> but
		// we only want the iterator.
...
MMUser *MMLayer::mmFindByTmsi(uint32_t tmsi)
{
	...
	for (MMUserMap::iterator it = MMUsers.begin(); it != MMUsers.end(); ++it) {
		...
		if (mmutmsi.valid() && mmutmsi.value() == tmsi) {
			result = mmu;
			break;
		}
	}
...

	{
		...
		for (MMUserMap::iterator it = MMUsers.begin(); it != MMUsers.end();) {
			...
			if (mmu->mmuPageTimer.passed()) {
				...
				// because that implies we know that the user is at the BTS, but if it did not answer
				// the page, we do not. Paul at Null Team recommended 504.
				// FIXME URGENTLY:  Dont do an mmFree within the gMMLock, although we need to make sure
...
void MMLayer::printMMUsers(std::ostream &os, bool onlyUnattached)
{
	...
	for (MMUserMap::iterator it = MMUsers.begin(); it != MMUsers.end(); ++it) {
		MMUser *mmu = it->second;
		if (onlyUnattached && mmu->mmuIsAttached()) {
			continue;
		}
		mmu->mmuText(os);
		os << endl;
