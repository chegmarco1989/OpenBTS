...
void CodecSet::text(std::ostream &os) const
{
	...
	if (mCodecs & UMTS_AMR)
		os << " UMTS_AMR";
	if (mCodecs & UMTS_AMR2)
		os << " UMTS_AMR2";
	if (mCodecs & TDMA_EFR)
		os << " TDMA_EFR";
	if (mCodecs & PDC_EFR)
		os << " PDC_EFR";
	if (mCodecs & AMR_FR_WB)
		os << " AMR_FR_WB";
	if (mCodecs & UMTS_AMR_WB)
		os << " UMTS_AMR_WB";
	if (mCodecs & OHR_AMR)
		os << " OHR_AMR";
	if (mCodecs & OFR_AMR_WB)
		os << " OFR_AMR";
	if (mCodecs & OHR_AMR_WB)
		os << " OFR_AMR_WB";
	if (mCodecs & PCMULAW)
		os << " PCMULAW";
	if (mCodecs & PCMALAW)
...
const char *CCState::callStateString(CallState state)
{
	...
	case MOCInitiated:
		return "MOC-initiated";
	case MOCProceeding:
		return "MOC-proceeding";
	case MOCDelivered:
		return "MOC-delivered";
	case MTCConfirmed:
		return "MTC-confirmed";
	case CallReceived:
...
std::ostream &operator<<(std::ostream &os, const TMSI_t &tmsi)
{
	if (tmsi.valid()) {
		char buf[10];
		snprintf(buf, sizeof(buf), "0x%x", tmsi.value());
		os << buf;
	} else {
		os << "(no tmsi)";
	}
...
{
	if (mImsi.length()) {
		return string("IMSI") + mImsi;
	}
	if (mTmsi.valid()) {
		return format("TMSI%u", mTmsi.value());
	}
	if (mImei.length()) {
		return string("IMEI") + mImei;
	}
	return string("");
}
...
{
	os << LOGVAR2("Imsi", msid.mImsi.length() ? msid.mImsi : "\"\"") << LOGVAR2("Tmsi", msid.mTmsi)
