...
void origTest()
{
	...
	BitVector mC =
		"000000000000111100000000000001110000011100001101000011000000000000000111000011110000100100001010000010"
		"100000101000001010000010100000010000000000000000000000000000000000000000000000001100001111000000000000"
		"000000000000000000000000000000000000000010010000101000001010000010100000101000001010000001000000000000"
		"000000000000110000111100000000000001110000101000001100000001000000000000";
	SoftVector mCS(mC);
	...
	const char *label, ViterbiBase *coder, unsigned frameSize, unsigned iRate, unsigned order, bool debug)
{
	...
	for (float badness = 0.0; badness <= 1.0; badness += 0.1)
		for (int perr = 0; perr <= 50; perr += 10) {
			...
			// cout << label << ", " << "% ambiguous bits in : % bad bits out = " << perr << " : " << pbbo
			// <<LOGVAR(bec)<<LOGVAR(bitErrors)<<endl;
			cout << label << LOGVAR(badness) << LOGVAR2("bad bits in", errsIn) << " " << perr << "%"
			     << LOGVAR2("bad_bits_out", errsOut) << " " << pbbo << "%" << LOGVAR(bec)
			     << LOGVAR(bitErrors) << endl;
		}
}
