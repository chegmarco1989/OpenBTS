...
// This (together with L3RejectCause, which is referenced by Locus below) encompasses the complete list of reasons a
// transaction can be terminated. See the TermCause class for conversion of these causes into other cause types
// (CC-cause, SIP error code, or Q.850 code.)
struct L3Cause {
	...
	// Note that these are almost the same as ITU Recommendation Q.850 codes, but not quite - the Preemption value
	// is different.
	enum CCCause {
	...
	// We dont put RR causes here because they are somewhat orthogonal to the other causes here and add no new
	// information.
	enum Locus {
	...
	enum BSSCause {
		...
		Channel_Assignment_Failure =
			0xa | LocusBSS,		      // (called: Radio Interface Failure, reversion to old channel)
		...
		Emergency_Preemption = 0x29 | LocusBSS, // Emergency Preemption as opposed to Operator_Intervention
							// (Defined as just Preemption in 48.008, but we must
							// distinguish from CCCause::Preemption.)
	...
	enum CustomCause {
		// We use No_Paging_Response because there is no precise L3Cause for this (closest is
		// DestinationOutOfOrder)
		No_Paging_Response = 1 | LocusCustom,
		IMSI_Detached, // This is a MAP error code.  If we did not have this we would have to use
			       // DestinationOutOfOrder.
		Handover_Outbound,
