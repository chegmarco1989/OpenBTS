...
#include <GSM/GSMCommon.h> // For Z100Timer - I really dont want to include the other stuff here.
...
public:
	...
	L3SmMsgActivatePdpContextRequest mPendingPdpr; // the request upon which we are pending, since the pending has
						       // to be cleared by the Rab setup response handler
...
PdpContext::PdpContext(GmmInfo *wgmm, mg_con_t *wmgp, int nsapi, int llcsapi)
	: mpcGmm(wgmm), mNSapi(nsapi), mLlcSapi(llcsapi),
	  // mNSapi(pdpr.mNSapi),
