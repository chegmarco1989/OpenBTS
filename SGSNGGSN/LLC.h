...
struct LLCFormat {
	enum type { Invalid, I, S, U, UI, ISack, SSack };
...
class LlcParity : public Parity32 {
	static const uint32_t sFCSGenerator = (1 << 24) + (1 << 23) + (1 << 21) + (1 << 20) + (1 << 19) + (1 << 17) +
					      (1 << 16) + (1 << 15) + (1 << 13) + (1 << 8) + (1 << 7) + (1 << 5) +
					      (1 << 4) + (1 << 2) + 1;
...
struct LlcDefs {
	...
	enum U_S_Commands { SCMD_RR = 0, SCMD_ACK = 1, SCMD_RNR = 2, SCMD_SACK = 3 };
};
...
struct LlcEntityUserData : public LlcEntity {
	...
	unsigned mN201U;
	LlcEntityUserData(unsigned wLlcSapi, SgsnInfo *si) : LlcEntity(si), mLlcSapi(wLlcSapi)
	{
...
Sndcp::Sndcp(unsigned wNSapi, unsigned wLlcSapi, LlcEntityUserData *wlle)
	: mSNS(sUmSNS), mNSapi(wNSapi), mLlcSapi(wLlcSapi), mlle(wlle)
//,mPdp(0)
{
...
struct LlcEngine {
	...
	LlcEngine(SgsnInfo *si)
		: mLleGmm(si), mLleUserData3(3, si), mLleUserData5(5, si), mLleUserData9(9, si), mLleUserData11(11, si)
	{
	...
	LlcEntity *getLlcEntity(unsigned sapi)
	{
		...
		case LlcSapi::GPRSMM:
			return &mLleGmm;
		case LlcSapi::UserData3:
			return &mLleUserData3;
		case LlcSapi::UserData5:
			return &mLleUserData5;
		case LlcSapi::UserData9:
			return &mLleUserData9;
		case LlcSapi::UserData11:
			return &mLleUserData11;
		default:
			return 0;
		}
	}
