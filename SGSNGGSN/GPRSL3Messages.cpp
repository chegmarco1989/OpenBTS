...
{
	if (frame.size() < 2) {
		os << "L3 Message size too small =" << frame.size();
		return;
	}
	try {
		// Standard L3 header:
...
void GmmMobileIdentityIE::text(std::ostream &os) const
{
	...
	switch (mTypeOfId) {
	case 1:
		os << LOGVAR2("IMSI", getAsBcd());
		break;
	case 2:
		os << LOGVAR2("IMEI", getAsBcd());
		break;
	case 3:
		os << LOGVAR2("IMEISV", getAsBcd());
		break;
	case 4:
		os << LOGHEX2("(P)TMSI", mTmsi);
		break;
	case 5:
		os << LOGVAR2("TMGI", ByteVectorTemp((char *)mIdData, mLen).hexstr());
		break;
	case 6:
		os << LOGVAR2("type6", ByteVectorTemp((char *)mIdData, mLen).hexstr());
		break;
	case 7:
		os << LOGVAR2("type7", ByteVectorTemp((char *)mIdData, mLen).hexstr());
		break;
	}
}
...
void SmQoS::setPeakThroughput(unsigned bytepSec) // In Bytes/sec
{
	for (unsigned code = 1; code < sPeakThroughputTableSize; code++) {
		if (bytepSec <= sPeakThroughputTable[code]) {
			setPeakThroughputCode(code);
		}
	}
...
void SmQoS::setMeanThroughput(unsigned bytepHour) // KBytes/sec
{
	for (unsigned code = 1; code < sMeanThroughputTableSize; code++) {
		if (bytepHour <= sMeanThroughputTable[code]) {
			setMeanThroughputCode(code);
		}
	}
...
void SmQoS::setMaxBitRate(unsigned kbitps, bool uplink)
{
	...
	if (kbitps == 0) {
		...
		code = 0x40 + ((kbitps - 64) / 8);
	} else if (kbitps <= 8640) {
		code = 0x80 + ((kbitps - 576) / 64);
	} else {
...
int SmQoS::getMaxBitRate(bool uplink)
{
	...
	case 0:
		return code;
	case 0x40:
		return 64 + 8 * (code & 0x3f);
	default:
		return 576 + 64 * (code & 0x3f);
	}
}
...
const char *AccessTechnologyType2Name(AccessTechnologyType type)
{
	switch (type) {
		...
		CASENAME(GSM_T810)
	default:
		return "unknown AccessTechnologyType";
	}
}
						 ...
						 size_t end)		   // end of capabilities list
{
	...
	if (bv.readField(rp, 1)) {
		mCaps[A5Bits] = bv.readField(rp, 7);
	} else if (prev) {
		mCaps[A5Bits] = prev->mCaps[A5Bits];
	}
	...
	if (rp >= end)
		return;
	if (bv.readField(rp, 1)) {
		mCaps[EightPSKPowerCapability] = bv.readField(rp, 2);
	}
	if (rp >= end)
		return;
...
AccessCapabilities::CapType AccessCapabilities::mPrintList[] = {
	GPRSMultislotClass, GPRSExtendedDynamicAllocationCapability, GERANFeaturePackage1};
...
void AccessCapabilities::text2(std::ostream &os, bool verbose) const
{
	...
	if (!verbose) { // TODO: how to switch to get the full list?
		...
		for (unsigned j = 0; j < sizeof(mPrintList) / sizeof(CapType); j++) {
			...
			if (mCaps[cap] != -1) {
				os << LOGVAR2(CapName((CapType)cap), mCaps[cap]);
			}
		}
	} else {
		// Full list:
		for (int i = 0; i < CapsMax; i++) {
			if (mCaps[i] != -1) {
				os << LOGVAR2(CapName((CapType)i), mCaps[i]);
			}
		}
	}
}
...
void MsRaCapability::parseMsRaCapability()
{
	...
	for (int numTechs = 0; numTechs < sMsRaCapMaxTypes; numTechs++) {
		try {
			...
			if (rp + 15 >= sizeBits()) {
				break;
			} // Not enough room left for anything useful.
			if (readField(rp, 1) == 0) {
				break;
			}		    // End of list marker.
...
void L3GmmMsgServiceReject::gmmWriteBody(ByteVector &msg) { msg.appendByte(mGmmCause); }
...
void L3GmmMsgDetachRequest::textBody(std::ostream &os) const
{
	...
	if (mGmmCausePresent) {
		os << LOGVAR(mGmmCause) << "=" << GmmCause::name(mGmmCause);
	}
	if (mMobileIdPresent) {
...
void L3GmmMsgIdentityResponse::gmmParseBody(L3GmmFrame &src, size_t &rp) { mMobileId.parseLV(src, rp); }

void L3GmmMsgIdentityResponse::textBody(std::ostream &os) const { os << LOGVAR2("mobileId", mMobileId.str()); }
...
{
	os << LOGVAR(mTearDownIndicator) << LOGVAR(mCause) << "=" << SmCause::name(mCause) << " PCO=" << mPco.str();
}
...
void L3GmmMsgGmmStatus::textBody(std::ostream &os) const { os << LOGVAR(mCause) << "=" << GmmCause::name(mCause); }

void L3SmMsgSmStatus::textBody(std::ostream &os) const { os << LOGVAR(mCause) << "=" << SmCause::name(mCause); }
