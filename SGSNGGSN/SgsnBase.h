...
#define GGSNLOG1(level, who, stuff) \
	if (IS_LOG_LEVEL(level) || sgsnDebug()) \
		_LOG(level) << who << stuff;
// Normal log plus put it in the ggsn.log file.
#define GGSNLOG2(level, who, stuff) \
	GGSNLOG1(level, who, stuff); \
	if (sgsnDebug()) \
		MGLOG(who << stuff);
