		...
		while (pc < end) {
				...
				if (pc[0] == 1) { // command = configure-request
					...
					for (op = &pc[4]; op < pc + ipcp_len; op += op[1]) {
						...
						case 0x81:	 // primary dns.
							...
							if (op[1] < 6) {
							bad_ipcp_opt_len:
								MGERROR("SGSN: Invalid PCO IPCP Config Option Length: "
									"opt=0x%x len=%d\n",
									op[0], op[1]);
								goto next_protocol;
							}
						...
						case 2:
							...
							goto bad_ipcp;
						case 0x82:
							what = "primary NBNS [NetBios Name Service]";
							goto bad_ipcp;
						case 0x84:
							what = "secondary NBNS [NetBios Name Service]";
							goto bad_ipcp;
						default:
						bad_ipcp:
							// It would be nice to send an SMS message that the phone is set
							// up improperly.
			...
			unsupported_protocol:
				if (ipcplen) {
					...
					// MGWARN("SGSN: warning: ignoring PDP Context activation sub-protocol 0x%x %s;
					// MS may require reconfiguration.\n",proto,protname);
				}
			}
		next_protocol:
			pc += ipcplen;
