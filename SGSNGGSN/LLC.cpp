		...
		if (1) {
			for (int n = 0; n < totlen;) {
				...
				if (xidlen <= 4) {
					...
					// The only xid item with length > 4 is the L3 params, just hope we dont get
					// those.
...
void LlcFrameU::llcProcess(LlcEntity *lle)
{
	...
	if (cmd == UCMD_XID) {
		...
		LLCWARN("LLC XID frame received" << LOGVAR2("size", xids.size())
						 << LOGVAR2("llcsapi", lle->getLlcSapi()));
		handleXid(lle, xids);
	} else {
		const char *cmdname = "?";
		switch (cmd) {
		...
		LLCWARN("LLC U frame ignored" << LOGVAR(cmdname) << LOGVAR2("PF", getUPF()) << LOGVAR2("M", getUM())
					      << LOGVAR2("llcsapi", lle->getLlcSapi()));
	}
}
...
LlcEntityGmm *LlcEngine::getLlcGmm() { return dynamic_cast<LlcEntityGmm *>(getLlcEntity(LlcSapi::GPRSMM)); }
...
void LlcEntityUserData::lleUplinkData(ByteVector &payload)
{
	...
	if (sndcp == 0) {
		if (!mSI->isRegistered()) {
			...
			LLCINFO("received packet to detached MS on nsapi="
				<< nsapi << " Sending Implicitly_Detached message " << mSI);
...
int LlcFrameDump::headerLength()
{
	...
	case LLCFormat::ISack:
		return 5 + 1 + (mK + 1 + 7) / 8;
	case LLCFormat::SSack:
		return 3; // Minimum length is probably 4, not 3.
	case LLCFormat::Invalid:
		return -1;
	default:
		assert(0);
	}
...
void LlcFrameDump::llcParseDump()
{
	...
	case LLCFormat::ISack:
		...
		if ((int)size() < headerLength()) {
			mFormat = LLCFormat::Invalid;
			return;
		}
		// Fall through
	case LLCFormat::I:
...
{
	os << "format=" << LLCFormat::name(mFormat) << LOGVAR2("SAPI", getSapi()) << LOGVAR2("LlcPD", getLlcPD())
	   << LOGVAR2("CR", getCR());
	switch (mFormat) {
	case LLCFormat::I:
	case LLCFormat::ISack:
...
uint32_t Parity32::computeCrc(ByteVector &bv) { return computeCrc(bv.begin(), bv.size()); }
