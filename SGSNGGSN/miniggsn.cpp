...
static char *packettoa(char *result, unsigned char *packet, int len)
{
	...
	if (verbose && iph->protocol == IPPROTO_TCP) {
		...
		sprintf(result, "proto=%s %d byte packet seq=%u ack=%u id=%u frag=%u from %s:%d to %s:%d",
			...
			ip_ntoa(iph->saddr, nbuf1), ntohs(tcph->source), ip_ntoa(iph->daddr, nbuf2), ntohs(tcph->dest));
		...
		sprintf(result, "proto=%s %d byte packet id=%u frag=%u from %s:%d to %s:%d",
			ip_proto_name(iph->protocol), len, iph->id, iph->frag_off, ip_ntoa(iph->saddr, nbuf1),
			ntohs(udph->source), ip_ntoa(iph->daddr, nbuf2), ntohs(udph->dest));
	} else {
...
bool miniggsn_init()
{
	...
	if (ggConfig.mgMaxConnections > 254) {
		MGERROR("%s specifies too many connections (%d) specifed, using 254", SQL_PDP_MAX_COUNT,
	...
	if ((ntohl(mgIpBasenl) & 0xff) == 0) {
		MGERROR("miniggsn: %s address should not end in .0 but proceeding anyway: %s", SQL_IP_BASE,
			ip_base_str);
	}
