...
TransceiverManager::TransceiverManager(int numARFCNs, const char *wTRXAddress, int wBasePort)
	: mHaveClock(false), mClockSocket(wBasePort + 100)
{
...
unsigned TransceiverManager::C0() const { return mARFCNs.at(0)->ARFCN(); }
...
void ::ARFCNManager::start() { mRxThread.start((void *(*)(void *))ReceiveLoopAdapter, this); }
...
void ::ARFCNManager::receiveBurst(const RxBurst &inBurst)
{
	...
	if (proc == NULL) {
		LOG(DEBUG) << "ARFNManager::receiveBurst time " << inBurst.time() << " in unconfigured TDMA position T"
			   << TN << " FN=" << FN << ".";
		return;
	}
