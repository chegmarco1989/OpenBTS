...
#define bmFR_MODE_RX_COUNTING_32BIT \
	(1 << 2) // Rx is counting with a 32 bit counter
...
#define bmFR_RX_SYNC \
	(1 << bitnoFR_RX_SYNC) // 1 If this is a master "sync now" and send sync to slave.
			       //  If this is a slave "sync now" (testing purpose only)
			       //  Sync is allmost the same as reset (clear all counters and buffers)
			       //  except that the io outputs and settings don't get reset (otherwise it couldn't send
			       //  the sync to the slave)
			       // 0 Normal operation

#define bmFR_RX_SYNC_MASTER \
	(1 << bitnoFR_RX_SYNC_MASTER) // 1 This is a rx sync master, output sync_rx on rx_a_io[15]
				      // 0 This is not a rx sync master
#define bmFR_RX_SYNC_SLAVE \
	(1 << bitnoFR_RX_SYNC_SLAVE) // 1 This is a rx sync slave, follow sync_rx on
				     // rx_a_io[bitnoFR_RX_SYNC_INPUT_IOPIN]  0 This is not an rx sync slave.

// Caution The master settings will output values on the io lines.
// They inheritely enable these lines as output. If you have a daughtercard which uses these lines also as output then
// you will burn your rad1 and daughtercard.  If you set the slave bits then your rad1 won't do anything if you don't
// connect a master.
// Rx Master/slave control register
//
// The way this is supposed to be used is connecting a (short) 16pin flatcable from an rx daughterboard in RXA master
// io_rx[8..15] to slave io_rx[8..15] on RXA of slave rad1 This can be done with basic_rx boards or dbsrx boards
...
// You cannot use a 16pin flatcable if you are using FLEX400 or FLEX2400 daughterboards, since these use a lot of the io
// pins.  You can still link them but you must use only a 2pin or 1pin cable  You can also use a 2-wire link. put a 2pin
// header on io[15],gnd of the master RXA daughterboard and connect it to io15,gnd of the slave RXA db.  You can use a
// cable like the ones found with the leds on the mainbord of a PC.  Make sure you don't twist the cable, otherwise you
// connect the sync output to ground.  To be save you could also just use a single wire from master io[15] to slave
// io[15], but this is not optimal for signal integrity.
