/* TransceiverRAD1/Transceiver.cpp */
/*-
...
Transceiver::Transceiver(int wBasePort, const char *TRXAddress, int wSamplesPerSymbol, GSM::Time wTransmitLatency,
	RadioInterface *wRadioInterface, unsigned int wNumARFCNs, unsigned int wOversamplingRate, bool wLoadTest)
	: mClockSocket(wBasePort, TRXAddress, wBasePort + 100)
{
	...
	for (unsigned int j = 0; j < wNumARFCNs; j++) {
		...
		if (wNumARFCNs > 1)
			mDemodServiceLoopThread[j] = new Thread(32768);
	...
	if (mMultipleARFCN) {
		...
		for (unsigned int j = 0; j < mNumARFCNs; j++) {
			...
			frequencyShift(mFrequencyShifter[j], mFrequencyShifter[j],
				2.0 * M_PI * (beaconFreq + j * 400e3) / (1625.0e3 / 6.0 * mOversamplingRate));
		}
	...
	for (unsigned int cn = 0; cn < mNumARFCNs; cn++) {
		for (int tn = 0; tn < 8; tn++) {
			signalVector *modBurst =
				modulateBurst(gDummyBurst, *gsmPulse, 8 + (tn % 4 == 0), mSamplesPerSymbol);
			...
			if (cn == 0)
				...
				scaleVector(*modBurst, 0.0);
			if (mMultipleARFCN) {
				signalVector *interpVec =
					polyphaseResampleVector(*modBurst, mOversamplingRate, 1, interpolationFilter);
...
radioVector *Transceiver::fixRadioVector(BitVector &burst, int RSSI, GSM::Time &wTime, int ARFCN)
{
	...
	if (mMultipleARFCN) {
		signalVector *interpVec =
			polyphaseResampleVector(*((signalVector *)newVec), mOversamplingRate, 1, interpolationFilter);
...
{
	SoftVector *burst = demodulateBurst(wVector, *gsmPulse, mSamplesPerSymbol, 1.0, 0.0);
...
void Transceiver::pushRadioVector(GSM::Time &nowTime)
{
	...
	while (radioVector *staleBurst = mTransmitPriorityQueue.getStaleBurst(nowTime)) {
		...
		LOG(NOTICE) << "dumping STALE burst in TRX->USRP interface cn=" << staleBurst->ARFCN() << " at "
			    << staleBurst->time();
	...
	while (radioVector *next = (radioVector *)mTransmitPriorityQueue.getCurrentBurst(nowTime)) {
		int CN = next->ARFCN();
		if (CN >= (int)mNumARFCNs) {
			LOG(ERR) << "attempt to send burst on illegal ARFCN. C" << CN << "T" << TN << " FN "
				 << nowTime.FN();
			delete next;
			continue;
		}
	...
	for (unsigned int CN = 0; CN < mNumARFCNs; CN++) {
		...
		if (IGPRS == mChanType[CN][TN]) {
			LOG(DEBUG) << (sendVec ? "adding" : "setting") << " GPRS filler burst on C" << CN << "T" << TN
				   << " FN " << nowTime.FN();
		}
		if (!sendVec) {
			sendVec = tmpVec;
...
CorrType Transceiver::expectedCorrType(GSM::Time currTime, int arfcn)
{
	...
	if (mHandoverActive[arfcn][burstTN])
		return RACH;
...
void Transceiver::pullRadioVector()
{
	...
	// LOG(DEBUG) << "receiveFIFO: read radio vector " << rxBurst << " at time: " << rxBurst->time() << ", new size:
	// " << mReceiveFIFO->size();
	...
	for (unsigned int i = 0; i < mNumARFCNs; i++) {
		...
		if (mMultipleARFCN) {
			...
			signalVector *rcvVec =
				polyphaseResampleVector(*ARFCNVec, 1, mOversamplingRate, decimationFilter);
			delete ARFCNVec;
	...
	if (strcmp(command, "POWEROFF") == 0) {
		...
		if (!mTxFreq || !mRxFreq)
			...
			if (!mOn) {
				...
				mRFIFOServiceLoopThread->start(
					(void *(*)(void *))RFIFOServiceLoopAdapter, (void *)this);
				...
				for (unsigned int i = 0; i < mNumARFCNs; i++) {
					...
					cs->trx = this;
					cs->CN = i;
					mTransmitPriorityQueueServiceLoopThread[i]->start(
						(void *(*)(void *))TransmitPriorityQueueServiceLoopAdapter, (void *)cs);
					Demodulator *demod = new Demodulator(i, this, mStartTime);
					mDemodulators[i] = demod;
					if (mNumARFCNs > 1)
						mDemodServiceLoopThread[i]->start(
							(void *(*)(void *))DemodServiceLoopAdapter, (void *)demod);
				}

				// mRFIFOServiceLoopThread->start((void * (*)(void*))RFIFOServiceLoopAdapter,(void*)
				// this);  mFIFOServiceLoopThread->start((void *
				// (*)(void*))FIFOServiceLoopAdapter,(void*) this);
		...
		if (mOn) {
			sprintf(response, "RSP NOISELEV 0 %d", (int)mDemodulators[0]->mNoiseFloorRSSI);
		} else {
		...
		sscanf(buffer, "%3s %s %s", cmdcheck, command,
			param); // (pat) this was formerly &param which was incorrect.
...
void *RFIFOServiceLoopAdapter(Transceiver *transceiver)
{
	...
	while (1) {
		transceiver->driveReceiveFIFO();
		if (!isMulti)
			transceiver->mDemodulators[0]->driveDemod(true);
		// transceiver->driveTransmitFIFO();
		pthread_testcancel();
	}
	return NULL;
...
Demodulator::Demodulator(int wCN, Transceiver *wTRX, GSM::Time wStartTime)
{
...
void Demodulator::driveDemod(bool wSingleARFCN)
{
	...
	if (rxBurst) {
		...
		DEMOD_DEBUG << "burst parameters: "
			    << " CN: " << mCN << " time: " << burstTime << " RSSI: " << RSSI << " TOA: " << TOA
			    << " bits: " << *rxBurst;
...
SoftVector *Demodulator::demodRadioVector(radioVector *rxBurst, GSM::Time &wTime, int &RSSI, int &timingOffset)
{
	...
	if (corrType == TSC) {
		...
		{
			if (channelResponse[timeslot])
				delete channelResponse[timeslot];
			if (DFEForward[timeslot])
				delete DFEForward[timeslot];
			if (DFEFeedback[timeslot])
				delete DFEFeedback[timeslot];
		...
		success = analyzeTrafficBurst(*vectorBurst, mTSC, 3.0, mSamplesPerSymbol, &amplitude, &TOA,
			mMaxExpectedDelay, estimateChannel, &channelResp, &chanOffset);
		...
		if (success) {
			...
			SNRestimate[timeslot] = amplitude.norm2() / (mEnergyThreshold * mEnergyThreshold +
									    1.0); // this is not highly accurate
			if (estimateChannel) {
				...
				DEMOD_DEBUG << "SNR: " << SNRestimate[timeslot]
					    << ", DFE forward: " << *DFEForward[timeslot]
					    << ", DFE backward: " << *DFEFeedback[timeslot];
			}
		} else {
			double framesElapsed = rxBurst->time() - prevFalseDetectionTime;
			DEMOD_DEBUG << "wTime: " << rxBurst->time() << ", pTime: " << prevFalseDetectionTime
				    << ", fElapsed: " << framesElapsed;
		...
		success = detectRACHBurst(*vectorBurst,
			...
			mSamplesPerSymbol, &amplitude, &TOA);
		if (success) {
	...
	if ((rxBurst) && (success)) {
		if ((corrType == RACH) || (!needDFE)) {
			burst = demodulateBurst(*vectorBurst, *gsmPulse, mSamplesPerSymbol, amplitude, TOA);
		} else { // TSC
			scaleVector(*vectorBurst, complex(1.0, 0.0) / amplitude);
			burst = equalizeBurst(*vectorBurst, TOA - chanRespOffset[timeslot], mSamplesPerSymbol,
