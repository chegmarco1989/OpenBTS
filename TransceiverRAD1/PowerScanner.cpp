/* TransceiverRAD1/PowerScanner.cpp */
/*-
...
int scanDirection(RAD1Device *rad, TIMESTAMP &timestamp,  // Physical RAD values
	...
	SpectrumMap::LinkDirection linkDir, SpectrumMap &spectrumMap);
...
int main(int argc, char *argv[])
{
	try {
		...
		scanDirection(rad, timestamp, band, startARFCN, stopARFCN, &GSM::uplinkFreqKHz,
			SpectrumMap::LinkDirection::Up, spectrumMap);
		cout << endl << "Scanning Downlink" << endl;
		scanDirection(rad, timestamp, band, startARFCN, stopARFCN, &GSM::downlinkFreqKHz,
			SpectrumMap::LinkDirection::Down, spectrumMap);
	...
	FrequencyConverter arfcnToFreqKHz, SpectrumMap::LinkDirection linkDir, SpectrumMap &spectrumMap)
{
	...
	for (int ARFCN = startARFCN; ARFCN <= stopARFCN; ARFCN++) {
			...
			if (rd) {
				for (int i = 0; i < rd; i++) {
					...
					*wordPtr = usrp_to_host_u32(*wordPtr);
					sum += (readBuf[2 * i + 1] * readBuf[2 * i + 1] +
						readBuf[2 * i] * readBuf[2 * i]);
					num += 1.0;
				}
			}
			timestamp += rd;
...
ConfigurationKeyMap getAllConfigurationKeys()
{
	...
	tmp = new ConfigurationKey("PowerScanner.dBmOffset", "0", "dBm", ConfigurationKey::FACTORY,
	...
	tmp = new ConfigurationKey("PowerScanner.DBPath", "/var/run/PowerScannerResults.db", "",
		ConfigurationKey::FACTORY, ConfigurationKey::FILEPATH, "", true,
		"Path to the scanning results database.");
	map[tmp->getName()] = *tmp;
	delete tmp;

	tmp = new ConfigurationKey("PowerScanner.IntegrationTime", "250", "milliseconds", ConfigurationKey::FACTORY,
		ConfigurationKey::VALRANGE, "50:5000", false, "Power detection integration time in milliseconds.");
	map[tmp->getName()] = *tmp;
	delete tmp;

	tmp = new ConfigurationKey("PowerScanner.RxGain", "97", "dB", ConfigurationKey::FACTORY,
