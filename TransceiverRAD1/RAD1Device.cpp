/* TransceiverRAD1/RAD1Device.cpp */
/*-
...
bool RAD1Device::compute_regs(double freq, unsigned *R, unsigned *control, unsigned *N, double *actual_freq)
{
	if (freq < 1.2e9) {
		DIV2 = 1;
		freq_mult = 2;
	...
	*control = (P << 22) | (PD << 20) | (CP2 << 17) | (CP1 << 14) | (PL << 12) | (MTLD << 11) | (CPG << 10) |
		   (CP3S << 9) | (PDP << 8) | (MUXOUT << 5) | (CR << 4) | (PC << 2);
...
bool RAD1Device::tx_setFreq(double freq, double *actual_freq)
{
	...
	writeLock.lock();
	m_uTx->writeSpi(0, SPI_ENABLE_TX_A, SPI_FMT_MSB | SPI_FMT_HDR_0, write_it((R & ~0x3) | 1, result), 3);
	m_uTx->writeSpi(0, SPI_ENABLE_TX_A, SPI_FMT_MSB | SPI_FMT_HDR_0, write_it((control & ~0x3) | 0, result), 3);
	usleep(10000);
	m_uTx->writeSpi(0, SPI_ENABLE_TX_A, SPI_FMT_MSB | SPI_FMT_HDR_0, write_it((N & ~0x3) | 2, result), 3);
	writeLock.unlock();

	if (m_uTx->readIO() & PLL_LOCK_DETECT)
		return true;
	if (m_uTx->readIO() & PLL_LOCK_DETECT)
		return true;
	return false;
...
bool RAD1Device::rx_setFreq(double freq, double *actual_freq)
{
	...
	unsigned char result[3];
	writeLock.lock();
	m_uRx->writeSpi(0, SPI_ENABLE_RX_A, SPI_FMT_MSB | SPI_FMT_HDR_0, write_it((R & ~0x3) | 1, result), 3);
	m_uRx->writeSpi(0, SPI_ENABLE_RX_A, SPI_FMT_MSB | SPI_FMT_HDR_0, write_it((control & ~0x3) | 0, result), 3);
	usleep(10000);
	m_uRx->writeSpi(0, SPI_ENABLE_RX_A, SPI_FMT_MSB | SPI_FMT_HDR_0, write_it((N & ~0x3) | 2, result), 3);
	writeLock.unlock();

	usleep(100000);
	if (m_uRx->readIO() & PLL_LOCK_DETECT)
		return true;
	if (m_uRx->readIO() & PLL_LOCK_DETECT)
		return true;
...
double RAD1Device::setRxGain(double dB)
{
	...
	m_uRx->writeAuxDac(0, (int)ceil((1.2 + 0.02 - (dB / rfMax)) * 4096.0 / 3.3));
...
int RAD1Device::readSamples(short *buf, int len, bool *overrun, TIMESTAMP timestamp, bool *underrun, unsigned *RSSI)
{
#ifndef SWLOOPBACK
	if (!m_uRx)
		return 0;
	...
	// The packet timestamp is used as an index into the data buffer so packets are arranged in order of increasing
	// timestamp.
	while (1) {
		...
		for (int pktNum = 0; pktNum < (readLen / 512); pktNum++) {
			...
			if (chan == 0x01f) {
				...
				if ((word2 >> 16) == ((0x01 << 8) | 0x02)) {
					...
					if ((timestampOffset == 0) ||
						fabs((float)newTimestampOffset - (float)timestampOffset) /
								(float)timestampOffset <
							0.1)
						timestampOffset = newTimestampOffset;
					else { // new offset is more than 10% from old one, then its bogus, so ignore it
					       // and keep going
						LOG(ERR) << "Ignoring bad update of timestamp offset: "
							 << newTimestampOffset << ", keeping offset at "
							 << timestampOffset;
					}
			...
			if (chan != 0) {
				LOG(DEBUG)
					<< "chan: " << chan << ", timestamp: " << pktTimestamp << ", sz:" << payloadSz;
				continue;
			}
			if ((word0 >> 28) & 0x04) {
				if (underrun)
			...
			if (cursorStart * 2 + payloadSz / 2 > currDataSize) {
				...
				// (pat) This is trickey. For a cicular copy using memcpy(a,b,c); memcpy(d,e,f); it is
				// required that e==b+c, but it does because tmpBuf is (uint32_t*)
				memcpy(data + cursorStart * 2, tmpBuf + 2,
					(currDataSize - cursorStart * 2) * sizeof(short));
				memcpy(data, tmpBuf + 2 + (currDataSize / 2 - cursorStart),
					payloadSz - (currDataSize - cursorStart * 2) * sizeof(short));
			} else {
			...
			// LOG(DEBUG) << "timeStart: " << timeStart << ", timeEnd: " << timeEnd << ", pktTimestamp: " <<
			// pktTimestamp;
		}
	}
...
#else
	if (loopbackBufferSize < 2)
		return 0;
	int numSamples = 0;
	struct timeval currTime;
	gettimeofday(&currTime, NULL);
	double timeElapsed =
		(currTime.tv_sec - lastReadTime.tv_sec) * 1.0e6 + (currTime.tv_usec - lastReadTime.tv_usec);
	...
	memcpy(loopbackBuffer, loopbackBuffer + 2 * numSamplesToRead, sizeof(short) * loopbackBufferSize);
...
int RAD1Device::writeSamples(short *buf, int len, bool *underrun, unsigned long long timestamp, bool isControl)
{
	writeLock.lock();
...
bool RAD1Device::setTxFreq(double wFreq, double wAdjFreq)
{
	...
	m_uRx->writeAuxDac(2, freq_cal << 4);
	if (!tx_setFreq(wFreq + 1 * LO_OFFSET, &actFreq))
		return false;
...
bool RAD1Device::setRxFreq(double wFreq, double wAdjFreq)
{
	...
	freq_cal = (unsigned int)round(wAdjFreq); // 128+round(wAdjFreq/wFreq*5.4e6);
	m_uRx->writeAuxDac(2, freq_cal << 4);
	if (!rx_setFreq(wFreq - 2 * LO_OFFSET, &actFreq))
		return false;
