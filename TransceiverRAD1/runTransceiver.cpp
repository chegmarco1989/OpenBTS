/* TransceiverRAD1/runTransceiver.cpp */
/*-
...
// (pat) We cannot currently make this a command line option because the ConfigurationTable must be statically inited
...
int main(int argc, char *argv[])
{
	...
	int numARFCN = 1;
	// pat 3-2014: If no args, get params from config file.  Why do we even bother making this particular parameter
	// a transceiver argument when the rest of the parameters come from the config file?
	if (argc > 1) {
		numARFCN = atoi(argv[1]);
	} else {
		if (gConfig.defines("GSM.Radio.ARFCNs")) {
			numARFCN = gConfig.getNum("GSM.Radio.ARFCNs");
		}
	}
	...
	if (deviceID > 0) {
		deviceID--;
	} // Config TRX.RadioNumber is 1 based, actual board number is 0 based.
	...
	Transceiver *trx = new Transceiver(gConfig.getNum("TRX.Port"), gConfig.getStr("TRX.IP").c_str(), SAMPSPERSYM,
		GSM::Time(2, 0), radio, numARFCN, mOversamplingRate, false);
...
ConfigurationKeyMap getConfigurationKeys()
{
	...
	tmp = new ConfigurationKey("TRX.RadioNumber", "0", "", ConfigurationKey::FACTORY, ConfigurationKey::VALRANGE,
		"0:9", // Not likely to have 10 radios on the same computer.  Not likely to have >1
		true,
		"If non-0, use multiple radios on the same cpu, numbered 1-9.  Must change TRX.Port also.  Provide a "
		"separate config file for each OpenBTS+Radio combination using the environment variable or --config "
		"command line option.");
	map[tmp->getName()] = *tmp;
	delete (tmp);
	...
	// (pat 4-2014) Added this after having problems with "radio dropouts" and the solution was to change this to
	// larger number (5).
	{
		ConfigurationKey tmp("TRX.LatencyBumpUp", "1", "time measured in GSM timeslots",
			ConfigurationKey::DEVELOPER, ConfigurationKey::VALRANGE, "1:10", false,
			"Hardware-specific latency adjustment for transceiver clock.  This affects the speed of upward "
			"latency adjustments.  ");
		map[tmp.getName()] = tmp;
	}
