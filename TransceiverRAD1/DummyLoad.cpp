...
void DummyLoad::updateTime(void)
{
	...
	double timeElapsed = (currTime.tv_sec - startTime.tv_sec) * 1.0e6 + (currTime.tv_usec - startTime.tv_usec);
...
bool DummyLoad::stop() { return true; }

// NOTE: Assumes sequential reads
int DummyLoad::readSamples(short *buf, int len, bool *overrun, TIMESTAMP timestamp, bool *wUnderrun, unsigned *RSSI)
{
	updateTime();
	underrunLock.lock();
	*wUnderrun = underrun;
	...
	if (currstamp + len < timestamp) {
		...
		if (amount < dummyBurstSz - dummyBurstCursor) {
			...
			memcpy(buf, dummyBurst + dummyBurstCursor * 2,
				sizeof(short) * 2 * (dummyBurstSz - dummyBurstCursor));
			int retVal = dummyBurstSz - dummyBurstCursor;
			dummyBurstCursor = 0;
			return retVal;
		}
...
int DummyLoad::writeSamples(short *buf, int len, bool *wUnderrun, unsigned long long timestamp, bool isControl)
{
	updateTime();
	underrunLock.lock();
...
bool DummyLoad::updateAlignment(TIMESTAMP timestamp) { return true; }
