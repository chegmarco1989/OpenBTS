...
RadioInterface::RadioInterface(RadioDevice *wRadio, int wReceiveOffset, int wRadioOversampling,
	int wTransceiverOversampling, bool wLoadTest, unsigned int wNumARFCNs, GSM::Time wStartTime)
...
void RadioInterface::pushBuffer(void)
{
	...
	if ((int)sendCursor < 2 * INCHUNK * samplesPerSymbol)
		return;

	// send resampleVector
	// (pat) returned samplesWritten is always (INCHUNK*1)/2/sizeof(short);
	int samplesWritten = mRadio->writeSamples(sendBuffer, INCHUNK * samplesPerSymbol, &underrun, writeTimestamp);
	...
	if ((int)sendCursor > 2 * samplesWritten)
		memcpy(sendBuffer, sendBuffer + samplesWritten * 2,
			sizeof(short) * 2 * (sendCursor / 2 - samplesWritten));
...
void RadioInterface::pullBuffer(void)
{
	...
	int samplesRead =
		mRadio->readSamples(shortVector, OUTCHUNK * samplesPerSymbol, &overrun, readTimestamp, &localUnderrun);
	...
	while (samplesRead < OUTCHUNK * samplesPerSymbol) {
		...
		samplesRead += mRadio->readSamples(shortVector + 2 * samplesRead,
			OUTCHUNK * samplesPerSymbol - samplesRead, &overrun, readTimestamp, &localUnderrun);
		underrun |= localUnderrun;
...
bool RadioInterface::setVCTCXO(unsigned int tuneVoltage) { return mRadio->setVCTCXO(tuneVoltage); }

bool RadioInterface::tuneTx(double freq, double adjFreq) { return mRadio->setTxFreq(freq, adjFreq); }

bool RadioInterface::tuneRx(double freq, double adjFreq) { return mRadio->setRxFreq(freq, adjFreq); }
...
void RadioInterface::start()
{
	...
	if (loadTest) {
		...
		for (int j = 0; j < numARFCN; j++) {
			...
			frequencyShift(frequencyShifter, frequencyShifter,
...
void RadioInterface::driveReceiveRadio()
{
	...
	while (rcvSz > (symbolsPerSlot + (tN % 4 == 0)) * samplesPerSymbol) {
		...
		// LOG(DEBUG) << "receiveFIFO: wrote radio vector at time: " << mClock.get() << ", new size: " <<
		// mReceiveFIFO.size() ;
