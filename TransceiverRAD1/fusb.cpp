...
static void free_lut(libusb_transfer *lut)
{
...
static void generic_callback(struct libusb_transfer *lut)
{

	// Fish out devhandle from endpoint
	fusb_devhandle *dev_handle = (fusb_devhandle *)((fusb_ephandle *)lut->user_data)->get_fusb_devhandle();
...
static libusb_transfer *alloc_lut(fusb_ephandle *self, int buffer_length, int endpoint, bool input_p,
	unsigned char *write_buffer, fusb_devhandle *dh)
{
...
fusb_devhandle::fusb_devhandle(libusb_device_handle *udh, libusb_context *ctx)
	: d_ctx(ctx), d_teardown(false), d_udh(udh)
{
	// that's it
}

fusb_devhandle::~fusb_devhandle() { d_teardown = true; }

fusb_ephandle *fusb_devhandle::make_ephandle(int endpoint, bool input_p, int block_size, int nblocks)
{
	return new fusb_ephandle(this, endpoint, input_p, block_size, nblocks);
}
...
void fusb_devhandle::pending_add(libusb_transfer *lut) { d_pending_rqsts.push_back(lut); }

/*
 * Pull from the pending list
 */

libusb_transfer *fusb_devhandle::pending_get()
{
...
bool fusb_devhandle::pending_remove(libusb_transfer *lut)
{
...
bool fusb_devhandle::_submit_lut(libusb_transfer *lut)
{
...
bool fusb_devhandle::_reap(bool ok_to_block_p)
{
	int ret;
	struct timeval tv;

	// Save pending size
	size_t pnd_size = d_pending_rqsts.size();
...
fusb_ephandle::fusb_ephandle(fusb_devhandle *dh, int endpoint, bool input_p, int block_size, int nblocks)
	: d_devhandle(dh), d_write_work_in_progress(0), d_write_buffer(0), d_read_work_in_progress(0), d_read_buffer(0),
	  d_read_buffer_end(0), d_endpoint(endpoint), d_input_p(input_p), d_block_size(block_size), d_nblocks(nblocks),
	  d_started(false)
{
	...
	for (int i = 0; i < d_nblocks; i++)
		d_free_list.push_back(
			alloc_lut(this, d_block_size, d_endpoint, d_input_p, d_write_buffer, d_devhandle));
}
...
int fusb_ephandle::write(const void *buffer, int nbytes)
{
...
struct libusb_transfer *fusb_ephandle::get_write_work_in_progress()
{
	if (d_write_work_in_progress)
...
void fusb_ephandle::reap_complete_writes()
{
...
int fusb_ephandle::read(void *buffer, int nbytes)
{
	if (!d_started) // doesn't matter here, but keeps semantics constant
		return -1;
...
void fusb_ephandle::free_list_add(libusb_transfer *lut)
{
...
libusb_transfer *fusb_ephandle::free_list_get()
{
...
void fusb_ephandle::completed_list_add(libusb_transfer *lut)
{
	assert((lut->user_data) == this);
	d_completed_list.push_back(lut);
}

libusb_transfer *fusb_ephandle::completed_list_get()
{
...
bool fusb_ephandle::submit_lut(libusb_transfer *lut)
{
