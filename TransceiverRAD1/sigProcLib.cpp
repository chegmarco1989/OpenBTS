/* TransceiverRAD1/sigProcLib.cpp */
/*-
...
void sigProcLibDestroy(void)
{
	...
	for (int i = 0; i < 8; i++) {
		if (gMidambles[i] != NULL) {
			if (gMidambles[i]->sequence)
				delete gMidambles[i]->sequence;
			if (gMidambles[i]->sequenceReversedConjugated)
				delete gMidambles[i]->sequenceReversedConjugated;
			delete gMidambles[i];
			gMidambles[i] = NULL;
		}
	}
	if (gRACHSequence) {
		if (gRACHSequence->sequence)
			delete gRACHSequence->sequence;
		if (gRACHSequence->sequenceReversedConjugated)
			delete gRACHSequence->sequenceReversedConjugated;
...
void initSincTables(void)
{
	for (int i = 0; i < SINCTABLESIZE; i++) {
		...
		for (int j = 0; j < SINCWAVEFORMSIZE; j++)
			*sincBurstItr++ =
				(complex)sinc(M_PI_F * ((float)j - (float)(SINCWAVEFORMSIZE / 2) - fracOffset));
...
signalVector *generateGSMPulse(int symbolLength, int samplesPerSymbol)
{
...
signalVector *frequencyShift(signalVector *y, signalVector *x, double freq, float startPhase, float *finalPhase)
{
...
signalVector *correlate(signalVector *a, signalVector *b, signalVector *c, ConvType spanType, bool bReversedConjugated,
	unsigned startIx, unsigned len)
{
...
signalVector *gaussianNoise(int length, float variance, complex mean)
{
...
bool generateMidamble(signalVector &gsmPulse, int samplesPerSymbol, int TSC)
{
	...
	if (gMidambles[TSC]) {
		if (gMidambles[TSC]->sequence != NULL)
			delete gMidambles[TSC]->sequence;
		if (gMidambles[TSC]->sequenceReversedConjugated != NULL)
			delete gMidambles[TSC]->sequenceReversedConjugated;
	}
	...
	*(emptyPulse.begin()) = 1.0;
	...
	// (pat) fwiw the uses of gTrainingSequence formerly did clones here.  I added the call to alias() to prevent
	// that.
	signalVector *middleMidamble =
		modulateBurst(gTrainingSequence[TSC].alias().segment(5, 16), emptyPulse, 0, samplesPerSymbol);
	signalVector *midamble = modulateBurst(gTrainingSequence[TSC].alias(), gsmPulse, 0, samplesPerSymbol);

	if (midamble == NULL)
		return false;
	if (middleMidamble == NULL)
		return false;
...
bool generateRACHSequence(signalVector &gsmPulse, int samplesPerSymbol)
{
	...
	if (gRACHSequence) {
		if (gRACHSequence->sequence != NULL)
			delete gRACHSequence->sequence;
		if (gRACHSequence->sequenceReversedConjugated != NULL)
			delete gRACHSequence->sequenceReversedConjugated;
	}

	signalVector *RACHSeq = modulateBurst(gRACHSynchSequence, gsmPulse, 0, samplesPerSymbol);
...
bool detectRACHBurst(signalVector &rxBurst, float detectThreshold, int samplesPerSymbol, complex *amplitude, float *TOA)
{
...
bool analyzeTrafficBurst(signalVector &rxBurst, unsigned TSC, float detectThreshold, int samplesPerSymbol,
	complex *amplitude, float *TOA, unsigned maxTOA, bool requestChannel, signalVector **channelResponse,
	float *channelResponseOffset)
{
	...
	if ((int)maxTOA < 3 * samplesPerSymbol)
		maxTOA = 3 * samplesPerSymbol;
	unsigned spanTOA = maxTOA;
	if ((int)spanTOA < 5 * samplesPerSymbol)
	...
	unsigned expectedTOAPeak =
		(unsigned)round(gMidambles[TSC]->TOA + (gMidambles[TSC]->sequenceReversedConjugated->size() - 1) / 2);
	...
	correlate(&burstSegment, gMidambles[TSC]->sequenceReversedConjugated, &correlatedBurst, CUSTOM, true,
	...
	*TOA = (*TOA) - (maxTOA);
	...
	LOG(DEBUG) << "TCH peakAmpl=" << amplitude->abs() << " RMS=" << RMS << " peakToMean=" << peakToMean
		   << " TOA=" << *TOA;
	...
	if (requestChannel && (peakToMean > detectThreshold)) {
		...
		int maxI = -1;
		for (int i = 0; i < 7; i++) {
			if (TOAoffset + (i - 5) * samplesPerSymbol + channelVector.size() > correlatedBurst.size())
				continue;
			if (TOAoffset + (i - 5) * samplesPerSymbol < 0)
				continue;
			correlatedBurst.segmentCopyTo(channelVector, (int)floor(TOAoffset + (i - 5) * samplesPerSymbol),
				channelVector.size());
		...
		*channelResponse = new signalVector(channelVector.size());
		correlatedBurst.segmentCopyTo(**channelResponse, (int)floor(TOAoffset + (maxI - 5) * samplesPerSymbol),
...
signalVector *createLPF(float cutoffFreq, int filterLen, float gainDC)
{
...

{
	...
	while (retItr < retVec->end()) {
		...
		if (tLow > wVector.size() - 1)
			break;
		if (tHigh > wVector.size())
			break;
	...
	signalVector **feedbackFilter)
{
	...
	*G0ptr = 1.0 / sqrtf(SNRestimate);
	...
	// float d;
	for (int i = 0; i < Nf; i++) {
		float d = G0.begin()->norm2() + G1.begin()->norm2();
	...
	*feedForwardFilter = new signalVector(Nf);
	...
	for (int i = 0; i < Nf; i++) {
		...
		float d = G0.begin()->norm2() + G1.begin()->norm2(); /* FIXME: not sure */
		*w = w_i / d;
		w++;
	}
...
SoftVector *equalizeBurst(signalVector &rxBurst, float TOA, int samplesPerSymbol,
