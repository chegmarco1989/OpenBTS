...
rnrad1Tx::rnrad1Tx(int whichBoard, unsigned int wInterpRate, const std::string fpgaFilename = "",
	...
	: rnrad1Core(whichBoard, RAD1_TX_INTERFACE, RAD1_TX_ALTINTERFACE, fpgaFilename, firmwareFilename, false)
{
	...
	result &= write9862(
		REG_TX_IF, TX_IF_USE_CLKOUT1 | TX_IF_I_FIRST | TX_IF_INV_TX_SYNC | TX_IF_2S_COMP | TX_IF_INTERLEAVED);
	...
	mEndptHandle = mDevHandle->make_ephandle(RAD1_TX_ENDPOINT, false, blockSize, numBlocks);
	...
	mTxModulatorShadow = (TX_MODULATOR_DISABLE_NCO | TX_MODULATOR_COARSE_MODULATION_NONE);
	setTxFreq(0);
}
...
bool rnrad1Tx::setSampleRateDivisor(unsigned int div) { return writeFpgaReg(FR_TX_SAMPLE_RATE_DIV, div - 1); }
...
bool rnrad1Tx::writeOE(int value, int mask) { return writeFpgaReg(FR_OE_0, (mask << 16) | (value & 0xffff)); }

bool rnrad1Tx::writeIO(int value, int mask) { return writeFpgaReg(FR_IO_0, (mask << 16) | (value & 0xffff)); }
...
bool rnrad1Tx::writeRefClk(int value) { return writeFpgaReg(FR_TX_A_REFCLK, value); }

bool rnrad1Tx::writeAuxDac(int dac, int value) { return rnrad1Core::writeAuxDac(dac, value); }

bool rnrad1Tx::readAuxAdc(int adc, int *value) { return rnrad1Core::readAuxAdc(true, adc, value); }
...
rnrad1Tx *rnrad1Tx::make(int which_board, unsigned int wInterpRate, const std::string fpgaFilename = "",
	const std::string firmwareFilename = "")
{
	try {
		rnrad1Tx *u = new rnrad1Tx(which_board, wInterpRate, fpgaFilename, firmwareFilename);
		return u;
	} catch (...) {
		return NULL;
	}
