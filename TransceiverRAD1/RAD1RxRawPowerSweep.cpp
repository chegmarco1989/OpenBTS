/* TransceiverRAD1/RAD1RxRawPowerSweep.cpp */
/*-
...
int main(int argc, char **argv)
{
	...
	if (argc > 2)
		startFreqkHz = (double)atoi(argv[2]);
	if (argc > 3)
		endFreqkHz = (double)atoi(argv[3]);
	...
	for (double freqkHz = startFreqkHz; freqkHz <= endFreqkHz; freqkHz += 200) {
		double sum = 0.0;
		unsigned long num = 0;
		if (!usrp->setRxFreq(freqkHz * 1.0e3, 108))
			printf("RX failed!");
		for (int j = 0; j < 250; j++) {
			short readBuf[512 * 2];
