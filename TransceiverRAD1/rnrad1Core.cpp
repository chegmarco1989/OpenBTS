/* TransceiverRAD1/rnrad1Core.cpp */
/*-
...
int usbMsg(struct libusb_device_handle *udh, int request, int value, int index, unsigned char *bytes, int len)
{
	int requesttype = (request & 0x80) ? VRT_VENDOR_IN : VRT_VENDOR_OUT;

	int ret = libusb_control_transfer(udh, requesttype, request, value, index, bytes, len, 1000);
...
struct libusb_device_handle *rad1OpenInterface(libusb_device *dev, int interface, int altinterface)
{
	...
	if ((ret = libusb_set_interface_alt_setting(udh, interface, altinterface)) < 0) {
...
static const int hashSlotAddr[2] = {RAD1_HASH_SLOT_0_ADDR, RAD1_HASH_SLOT_1_ADDR};

bool rad1GetHash(libusb_device_handle *udh, int which, unsigned char hash[RAD1_HASH_SIZE])
{
	which &= 1;

	// we use the Cypress firmware upload command to fetch it.
	int r = libusb_control_transfer(
		udh, 0xc0, 0xa0, hashSlotAddr[which], 0, (unsigned char *)hash, RAD1_HASH_SIZE, 1000);
...
bool rad1SetHash(libusb_device_handle *udh, int which, const unsigned char hash[RAD1_HASH_SIZE])
{
	which &= 1;

	// we use the Cypress firmware down load command to jam it in.
	int r = libusb_control_transfer(
		udh, 0x40, 0xa0, hashSlotAddr[which], 0, (unsigned char *)hash, RAD1_HASH_SIZE, 1000);
...
static bool writeRam(libusb_device_handle *udh, unsigned char *buf, int startAddr, size_t len)
{
	int addr;
	int n;
	int a;
	...
	for (addr = startAddr; addr < startAddr + (int)len; addr += quanta) {
		...
		a = libusb_control_transfer(
			udh, 0x40, 0xA0, addr, 0, (unsigned char *)(buf + (addr - startAddr)), n, 1000);
...
bool rad1LoadFirmware(libusb_device_handle *udh, const char *filename, unsigned char hash[RAD1_HASH_SIZE])
{
	...
	while (!feof(f)) {
		...
		if (type == 0) {
			...
			if (((a + checksum) & 0xff) != 0x00) {
				LOG(ERR) << "Checksum failed: got " << (unsigned int)-a << " expected "
...
bool rad1LoadFpga(libusb_device_handle *udh, const char *filename, unsigned char hash[RAD1_HASH_SIZE])
{
	bool ok = true;
	...
	for (int i = 0; i < FR_USER_0; i++) {
		...
		usbMsg(udh, VRQ_SPI_WRITE, 0x00 | (i & 0x7f),
			((SPI_ENABLE_FPGA & 0xff) << 8) | ((SPI_FMT_MSB | SPI_FMT_HDR_1) & 0xff), buf, 4);
...
rad1LoadStatus rad1LoadFirmwareNth(int nth, const char *filename, bool force, libusb_context *ctx)
{
	...
	if (access(filename, R_OK) != 0) {
		...
		s = ULS_ERROR;
	else if (!force && rad1GetHash(udh, FIRMWARE_HASH_SLOT, rad1_hash) &&
...
bool rad1_load_standard_bits(
	int nth, bool force, const std::string fpga_filename, const std::string firmware_filename, libusb_context *ctx)
{
	rad1LoadStatus s;
	const char *filename;
	...
	if (access(filename, R_OK) != 0) {
		...
		s = ULS_ERROR;
	else if (!force && rad1GetHash(udh, FPGA_HASH_SLOT, rad1_hash) &&
		 memcmp(file_hash, rad1_hash, RAD1_HASH_SIZE) == 0) {
		s = ULS_ALREADY_LOADED;
		// printf("hash: %s %s\n",file_hash,rad1_hash);
...
rnrad1Core::rnrad1Core(int which_board, int interface, int altInterface, const std::string fpga_filename = "",
	const std::string firmware_filename = "", bool skipLoad = false)
{
	mudh = 0;
	mctx = 0;
	...
	if (!skipLoad) {
		if (!rad1_load_standard_bits(which_board, false, fpga_filename, firmware_filename, mctx)) {
...
int rnrad1Core::sendRqst(int request, bool flag) { return usbMsg(mudh, request, flag, 0, 0, 0); }

int rnrad1Core::checkUnderrun(unsigned char *status)
{
	return usbMsg(mudh, VRQ_GET_STATUS, 0, GS_TX_UNDERRUN, status, 1);
}

int rnrad1Core::checkOverrun(unsigned char *status)
{
	return usbMsg(mudh, VRQ_GET_STATUS, 0, GS_RX_OVERRUN, status, 1);
}
...
bool rnrad1Core::writeI2c(int i2c_addr, unsigned char *buf, int len)
{
	...
	return usbMsg(mudh, VRQ_I2C_WRITE, i2c_addr, 0, buf, len) == len;
}
...
bool rnrad1Core::readI2c(int i2c_addr, unsigned char *buf, int len)
{
	...
	if (usbMsg(mudh, VRQ_I2C_READ, i2c_addr, 0, (unsigned char *)buf, len) != len)
		return false;
...
{
	return writeFpgaReg(FR_DC_OFFSET_CL_EN, (mFpgaShadows[FR_DC_OFFSET_CL_EN] & ~mask) | (bits & mask));
}
...
bool rnrad1Core::readAuxAdc(bool isTx, int which_adc, int *value)
{
	*value = 0;
	...
	unsigned char aux_adc_control = AUX_ADC_CTRL_REFSEL_A    // on chip reference
...
std::string rnrad1Core::readEeprom(int i2c_addr, int eeprom_offset, int len)
{
	...
	unsigned char cmd[1];
	cmd[0] = eeprom_offset;
	if (!writeI2c(i2c_addr, cmd, 1))
		return "";

	int orig_len = len;
	while (len > 0) {
		int n = std::min(len, MAX_EP0_PKTSIZE);
		if (!readI2c(i2c_addr, p, n)) {
			return "";
		}
		len -= n;
		p += n;
...
bool rnrad1Core::writeFpgaReg(int regno, int value) //< 7-bit regno, 32-bit value
{
	...
	return writeSpi(0x00 | (regno & 0x7f), SPI_ENABLE_FPGA, SPI_FMT_MSB | SPI_FMT_HDR_1, buf, 4);
}
...
bool rnrad1Core::readFpgaReg(int regno, int *value) //< 7-bit regno, 32-bit value
{
	*value = 0;
	...
	bool ok = readSpi(0x80 | (regno & 0x7f), SPI_ENABLE_FPGA, SPI_FMT_MSB | SPI_FMT_HDR_1, buf, 4);
...
bool rnrad1Core::write9862(int regno, unsigned char value)
{
	...
	return writeSpi(
		0x00 | (regno & 0x3f), SPI_ENABLE_CODEC_A, SPI_FMT_MSB | SPI_FMT_HDR_1, (unsigned char *)buf, 1);
}

bool rnrad1Core::read9862(int regno, unsigned char *value) const
{

	return readSpi(0x80 | (regno & 0x3f), SPI_ENABLE_CODEC_A, SPI_FMT_MSB | SPI_FMT_HDR_1, value, 1);
}
...
{
	return (usbMsg(mudh, VRQ_SPI_WRITE, optional_header, ((enables & 0xff) << 8) | (format & 0xff), buf, len) ==
		len);
}
...
bool rnrad1Core::readSpi(int optional_header, int enables, int format, unsigned char *buf, int len) const
{
	...
	if (usbMsg(mudh, VRQ_SPI_READ, optional_header, ((enables & 0xff) << 8) | (format & 0xff), (unsigned char *)buf,
		    len) != len) {
		return false;
	}
...
bool rnrad1Core::start() { return true; }
