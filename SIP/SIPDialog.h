...
class SipMTInviteServerTransactionLayer : public virtual SipInviteServerTransactionLayerBase {
	...
	// state machine to send the 200 OK response, which makes it look more similar to the non-INVITE server
	// transaction (figure 8.) After an ACK is received, TimerI is how long we will soak up additional ACKs, but who
	// cares?  We will soak them up as long as the dialog is extant.
	SipTimer mTimerH;
	...
	{
		if (!dsPeer()->ipIsReliableTransport()) {
			mTimerH.setOnce(64 * T1);
		}
	}

protected:
	void mtWriteLowSide(SipMessage *sipmsg)
	{ // Outgoing message.
...
class SipDialog : public MemCheckSipDialog,
		  public virtual SipDialogBase,
		  public virtual SipMOInviteClientTransactionLayer,
		  public virtual SipMTInviteServerTransactionLayer {

private:
	// We only send one state change message to L3 for each change of DialogState.  This is the previous one we
	// sent. There is no current dialog state - it is derived from SIPBase::mState
...
public:
	void dialogPushState(SipState newState, int code, char timer = 0);
	// Possibly send a message to L3 if the SIP state has changed.
private:
	void dialogChangeState(SipMessage *dmsg);
...
public:
	...
	{
		DialogState::msgState st = getDialogState();
		return st == DialogState::dialogFail || st == DialogState::dialogBye;
	}
	...
	void sendError(int code, const char *errorString)
	{
		LOG(ALERT) << "unimplemented" << code << errorString;
	} // TODO;
	...
	virtual ~SipDialog();
	void sipStartTimers()
	{ // We dont do it this way any more.
	}
	void sipStopTimers()
	{ // We dont do it this way any more.
	}
	bool dgWriteHighSide(SipMessage *msg)
	{
		...
		case SIPDTMOUssd:
			...
			return true;
		default:
			return false; // This indicates a serious error on the part of the SIP peer.
		}
	}
