...
struct SipParseError : public std::exception {
	...
	virtual const char *what() const throw() { return "SipParseError"; }
};
...
{
	while (endpos > 0 && strchr(trimchars, startp[endpos - 1])) {
		endpos--;
	}
	return endpos;
}
...
// Note that if you trimleft and trimright a string of spaces, the two indicies would cross, so be careful.  See
// trimboth.
size_t trimleftn(const char *startp, size_t startpos /*=0*/, const char *trimchars /*=" \t\r\n"*/)
{
	while (startp[startpos] && strchr(trimchars, startp[startpos])) {
		startpos++;
	}
	return startpos;
}
...
string commaListPopFront(string *cl) // Modify cl and return the result.
{
	...
	if (cl->empty()) {
		return result;
	} // This is probably an error on the part of the caller.
...
public:
	SipChar()
	{
		...
		for (uchar ch = 'a'; ch <= 'z'; ch++) {
			charClassData[ch] = ccIsToken;
		}
		for (uchar ch = 'A'; ch <= 'Z'; ch++) {
			charClassData[ch] = ccIsToken;
		}
		for (uchar ch = '0'; ch <= '9'; ch++) {
			charClassData[ch] = ccIsToken;
		}
		for (uchar *cp = (uchar *)token; *cp; cp++) {
			charClassData[*cp] |= ccIsToken;
		}
	}
...
SipParamList::iterator SipParamList::paramFindIt(const char *name)
{
	for (SipParamList::iterator it = this->begin(); it != this->end(); it++) {
		if (strceql(it->mName.c_str(), name)) {
			return it;
		}
	}
	return this->end();
...
struct SipParseLine {
	...
	{
		const char *bp = pp;
		if ((pp = strpbrk(bp, sepchars)) == NULL) {
			pp = bp + strlen(bp);
		}
	...
	string scanQuotedString()
	{
		...
		while (*pp) {
			if (*pp == '"') {
				pp++;
				return string(result, rp - result);
			}
			if (*pp == '\\') {
				pp++;
			}
	...
	string scanToken()
	{
		...
		while (SipChar::isToken(*pp)) {
			pp++;
		}
		return string(bp, pp - bp);
	}

	string scanTokenOrQuotedString() { return (*pp == '"') ? scanQuotedString() : scanToken(); }
	...
	// '='. pp may point to space, scan past that first. Leave pp pointing at the char immediately after the param,
	// which may be a space or separator.
...
bool crackUri(string header, string *before, string *uri, string *tail)
{
	...
	if (before)
		*before = header.substr(0, trimrightn(header.c_str(), nUriBegin));
	if (uri)
		*uri = header.substr(nUriBegin + 1, nUriEnd - nUriBegin - 1);
	if (tail)
		*tail = header.substr(nUriEnd + 1);
...
void parseAuthenticate(string stuff, SipParamList &params)
{
	...
	try {
		...
		do {
			...
			// and this would reject it.  This has not been a problem because we dont use that field, but
			// lets fix it anyway. formerly: if (! parser.scanGenericParam(param)) break;
			if (parser.scanGenericParam(param)) {
				params.push_back(param);
			}
		} while (parser.scanChar(','));
...
void SipVia::viaParse(string vialine)
{
	...
	try {
		...
		//	if (strcaseeql(param.mName.c_str(),"branch")) {		// not sure if this is case
		// insensitive, but be safe. 		mViaBranch = param.mValue; 		break;	// We can break;
		// we dont care about any other parameters.
		//	}
		//}
		while (parser.scanChar(';')) {
			SipParam param;
...
class SipParseMessage : SipParseLine {
	...
	{
		LOG(ERR) << "SIP Parse error at line " << linecnt << " " << msg << ". Parsing:'" << pp << "'"
			 << " SIP message=" << start;
	}
	...
	void spError(const char *msg)
	{
		if (eolp) {
			*eolp = '\r';
		} // Make the buffer printable again.
		logError(msg);
		throw SipParseError();
	}
	...
	void scanSipVersion()
	{
		if (!(strncaseeql(pp, "SIP", 3))) {
			spError("Expecting 'SIP'");
		}
		pp += 3; // skip over 'SIP'
		if (*pp++ != '/') {
			spError("Invalid SIP-version");
		}
		...
		if (strncmp(version.c_str(), "2.0", 3)) {
			LOG(NOTICE) << "unexpected SIP version=" << version;
		}
	}
...
public:
	SipMessage *sipParse()
	{
		...
		while (nextLine()) {
			...
			if (name.empty() || !scanChar(':')) {
				spError("Line without header");
			}
			...
			if (strceql(name, "to") || strceql(name, "t")) {
				...
				sipmsg->msmMaxForwards =
					trimboth(string(pp)); // We already trimmed left, but its ok to do it again.
				// No need to treat this specially here, even though authenticate info does not follow
				// normal SIP parsing rules.
...
SipMessage *sipParseBuffer(const char *buffer)
{
	...
	// This is OK because no thread contention for the buffer is possible because all callers pass a unique string
	// for parsing.
...
string CallTerminationCause::getQ850CallTermText(int l3Cause)
{
	...
	case GSM::L3Cause::Unknown_L3_Cause:
		return "Unknown Cause 0";
	case GSM::L3Cause::Unassigned_Number:
		return "Unallocated Number"; // 1
	case GSM::L3Cause::No_Route_To_Destination:
		return "No Route to Destination";
	case GSM::L3Cause::Channel_Unacceptable:
		return "Channel Unacceptable";
	case GSM::L3Cause::Operator_Determined_Barring:
		return "Preemption";
	case GSM::L3Cause::Normal_Call_Clearing:
		return "Normal Call Clearing"; // 16
	case GSM::L3Cause::User_Busy:
		return "User Busy"; // 17
	case GSM::L3Cause::No_User_Responding:
		return "No User Responding"; // 18
	case GSM::L3Cause::User_Alerting_No_Answer:
		return "No Answer from User (User alerted)";
	case GSM::L3Cause::Call_Rejected:
		return "Call Rejected"; // 21
	case GSM::L3Cause::Number_Changed:
		return "Number Changed";
	case GSM::L3Cause::Preemption:
		return "Exchange Routing Error";
	case GSM::L3Cause::Non_Selected_User_Clearing:
		return "Non-selected User Clearing";
	case GSM::L3Cause::Destination_Out_Of_Order:
		return "Destination Out of Order";
	case GSM::L3Cause::Invalid_Number_Format:
		return "Invalid Number Format (Address incomplete)";
	case GSM::L3Cause::Facility_Rejected:
		return "Facility Rejected";
	case GSM::L3Cause::Response_To_STATUS_ENQUIRY:
		return "Response to STATUS ENQUIRY";
	case GSM::L3Cause::Normal_Unspecified:
		return "Normal, Unspecified";
	case GSM::L3Cause::No_Channel_Available:
		return "No Circuit/Channel Available";
	case GSM::L3Cause::Network_Out_Of_Order:
		return "Network Out of Order";
	case GSM::L3Cause::Temporary_Failure:
		return "Temporary Failure";
	case GSM::L3Cause::Switching_Equipment_Congestion:
		return "Switching Equipment Congestion";
	case GSM::L3Cause::Access_Information_Discarded:
		return "Access Information Discarded";
	case GSM::L3Cause::Requested_Channel_Not_Available:
		return "Requested Circuit/Channel N/A";
	case GSM::L3Cause::Resources_Unavailable:
		return "Resource Unavailable, Unspecified";
	case GSM::L3Cause::Quality_Of_Service_Unavailable:
		return "Quality of Service Not Available";
	case GSM::L3Cause::Requested_Facility_Not_Subscribed:
		return "Requested Facility Not Subscribed";
	case GSM::L3Cause::Incoming_Calls_Barred_Within_CUG:
		return "Outgoing Calls Barred Within CUG";
	case GSM::L3Cause::Bearer_Capability_Not_Authorized:
		return "Incoming Calls Barred Within CUG";
	case GSM::L3Cause::Bearer_Capability_Not_Presently_Available:
		return "Bearer Capability Not Available";
	case GSM::L3Cause::Service_Or_Option_Not_Available:
		return "Service or Option N/A, unspecified";
	case GSM::L3Cause::Bearer_Service_Not_Implemented:
		return "Bearer Capability Not Implemented";
	case GSM::L3Cause::ACM_GE_Max:
		return "ACM greater or equal to ACM max";
	case GSM::L3Cause::Requested_Facility_Not_Implemented:
		return "Requested Facility Not Implemented";
	case GSM::L3Cause::Only_Restricted_Digital_Information_Bearer_Capability_Is_Available:
		return "Only Restricted Digital Bearer Cap supported";
	case GSM::L3Cause::Service_Or_Option_Not_Implemented:
		return "Service or Option Not Implemented, Unspecified";
	case GSM::L3Cause::Invalid_Transaction_Identifier_Value:
		return "Invalid Call Reference Value";
	case GSM::L3Cause::User_Not_Member_Of_CUG:
		return "User Not Member of CUG";
	case GSM::L3Cause::Incompatible_Destination:
		return "Incompatible Destination";
	case GSM::L3Cause::Invalid_Transit_Network_Selection:
	...
	case GSM::L3Cause::Invalid_Mandatory_Information:
		return "Mandatory Information Element is Missing";
	case GSM::L3Cause::Message_Type_Not_Implemented:
		return "Message Type Non-existent / Not Implemented";
	case GSM::L3Cause::Messagetype_Not_Compatible_With_Protocol_State:
		return "Message Incompatible With Call State or Message Type";
	case GSM::L3Cause::IE_Not_Implemented:
		return "IE/Parameter Non-existent or Not Implemented";
	case GSM::L3Cause::Conditional_IE_Error:
		return "Invalid Information Element Contents";
	case GSM::L3Cause::Message_Not_Compatible_With_Protocol_State:
		return "Message Not Compatible With Call State";
	case GSM::L3Cause::Recovery_On_Timer_Expiry:
		return "Recovery on Timer Expiry";
	case GSM::L3Cause::Protocol_Error_Unspecified:
		return "Message With Unrecognized Parameter, Discarded";
	case GSM::L3Cause::Interworking_Unspecified:
