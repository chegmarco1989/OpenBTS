...
// different kinds SIP Transactions, each of which has a Client (initiating) and Server (receiving) side, for 8 types
// total. 		a. INVITE, 		b. non-INVITE and outside of any dialog. 		c. non-INVITE
// within a dialog. 		d. REGISTER, which are sufficiently different to be a whole type by themselves. I
// started by translating the state machines from RFC3261 sec 17 directly into code, and intended to use them for all
// types of SIP Transactions, but that just did not work well. The INVITE and non-INVITE types are too different, and
// additionally, the RFC3261 state machine for INVITE only goes part-way, then dumps control onto the Transaction User.
...
// is clearer if the transaction layer classes are a base class of the client class (either dialog or TU [Transaction
// User]) rather than being passed to a separate Transaction Layer machine and back. So this is how it ended up: The
// INVITE (a) SIP Transaction code has been moved to SipMTInviteServerTransactionLayer and
// SipMTInviteServerTransactionLayer, which are base classes of the Dialog, and makes passing the messages through the
// TransactionLayer much clearer. The MESSAGE (b) is handled the same way because it was easier to connect to the
// Control layer with a Dialog. It would be better to have a base class which is the connection layer to the Control
// directory, but I have to stop cleaning up somewhere. The (c) Server side is handled by simple sip message handlers in
// the Dialog class. So this class is used only for Client-side (c) and (d) and could be simplified.
...
protected:
	...
	TranEntryId mstTranId;  // The associed L3 Transaction, if any.  TODO: Now this could use a RefCntPointer to the
				// transaction.
...
protected:
	...
	virtual bool TLWriteHighSideV(SipMessage *msg) = 0; // TL processes an incoming message from the outside world,
							    // returns true if should go to TU.
...
protected:
	// The idiotic C++ constructor paradigm obfuscates construction so badly in this case that we are not going to
	// use it. A SipTransaction is created locked both to make sure the periodic service routine does not process it
	// before it is completely constructed and to avoid the problem of an incoming message being routed to the
	// transaction during its initialization.
...
public:
	...
	void TLWriteHighSide(SipMessage *msg)
	{ // SIP Interface sends incoming messages here.
		LOG(DEBUG);
...
protected:
	bool stIsInvite()
	{
		return mstOutRequest.isINVITE();
	}	     // We ended up not using this class for INVITE, but some code still here.
	...
	bool TLWriteHighSideV(SipMessage *msg); // TL processes an incoming message from the outside world, returns true
						// if should go to TU.
...
struct SipRegisterTU : public SipClientTrLayer {
	...
	void TUWriteHighSideV(SipMessage *sipmsg);
	// SipRegisterTU(const FullMobileId &msid, const string &rand, const string &sres, L3LogicalChannel *chan);
	// // msid is imsi and/or tmsi
...
struct SipMOByeTU : public SipClientTrLayer {
	string stGetMethodNameV()
	{
		static const string cByeStr("BYE");
		return cByeStr;
	}
	void TUWriteHighSideV(SipMessage *sipmsg);
...
struct SipMOCancelTU : public SipClientTrLayer {
	string stGetMethodNameV()
	{
		static const string cCancelStr("CANCEL");
		return cCancelStr;
	}
	void TUWriteHighSideV(SipMessage *sipmsg);
	// TUTimeoutV not needed; on timeout we set dialog state to SSFail.
...
struct SipDtmfTU : public SipClientTrLayer {
	string stGetMethodNameV()
	{
		static const string infostr("INFO");
		return infostr;
	}
