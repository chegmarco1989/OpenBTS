...
// Dialog: "A peer-to-peer SIP relationship between two user agents that persists for some time".  Established by
// INVITE. Session: "A collection of participants and the media streams between them"  Established by INVITE, using SDP.
// The idea is that a dialog may inolve multiple transactions to do in-session modifications like call-hold or transfer.
// RFS-3261 sec 5: The SIP layer protocols are:  1. syntax and encoding; 2. Transport; 3. Transaction; 4. Transaction
// User (TU) Transport Layer: The Transport Layer is responsible for selecting TCP/UDP/other and handling via requests,
// which alter where the responses would go.  sec 18.2.1: TransportLayer is supposed to filter messages and silently
// throw away invalid ones. TransportLayer is supposed to be able to auto-switch between TCP & UDP, but that may not
// apply to us because our messages are always tiny - voice data packets. TransportLayer is supposed to process the Via
// field to provide redirection. Transaction Layer: A Transaction = a request along with all responses to that request.
// TransactionLayer matches responses to requests (SIPInterface.cpp) handles retransmissions and timeouts (SIPEngine)
// TransactionLayer is supposed to one of the simple state machines in sec 17.  State machine type is chosen by message
// type. Transaction User: Sends a request and expects a response.  It can send a CANCEL, which the TransactionLayer
// state machine must deal with.
...
string SipDialogMap::makeTagKey(string callid, string localTag)
{
	...
	// The local tag is sufficient to disambiguate the two dialogs with the same callid for direct bts-to-bts
	// communication. Note that whether the local tag is in the "from" or "to" depends on the message direction.
...
void SipDialogMap::dmAddLocalTag(SipDialog *dialog)
{
	...
	if (mDialogMap.getNoBlock(oldkey, fnd, true)) { // Removes from the map.
		...
		// I am adding dialog instead of fnd in case of some bug where the old did not exist, we ignore it and
		// keep going.
...
// For incoming [server] invites, we add the call dialog initially without the local tag, then later add the localtag to
// the key when we get the ACK.
...
void SipTUMap::tuMapAdd(SipTransaction *tup)
{
	...
	if (SipTransaction *existingTU = mTUMap.readNoBlock(key)) {
		LOG(ERR) << "Warning: adding second SipTransaction with branch:" << tup->stBranch() << LOGVAR(key)
			 << LOGVAR(existingTU) << LOGVAR(tup);
	}
	// Deletes the old.
...
void SipTUMap::tuMapPeriodicService()
{
	...
	if (cnt)
		LOG(DEBUG) << "finished, number deleted=" << cnt;
}
...
bool SipTUMap::tuMapDispatch(SipMessage *msg)
{
	...
	if (!tup) {
		return false;
	} // Eventually this will be an error, but not everything is transitioned to use TU yet.
...
void MySipInterface::purgeDeadDialogs()
{
	...
	for (sit = mDeadDialogs.begin(); sit != mDeadDialogs.end();) {
		...
		LOG(DEBUG) << "purgeDeadDialogs" << LOGVAR2("deletable", dialog->dgIsDeletable()) << *dialog
			   << LOGVAR2("ttIsDeletable", gNewTransactionTable.ttIsDialogReleased(dialog->mTranId));
...
void printDialogs(ostream &os) { gSipInterface.printDialogs(os); }
...
bool MySipInterface::checkTU(SipMessage *msg)
{
	if (msg->msmCode == 0) {
		return false;
	} // This is a request and only replies go directly to TUs.
...
void MySipInterface::newDriveIncoming(char *content)
{
	...
	try {
		...
		if (*methodname) {
			LOG(DEBUG) << "non-initiating SIP method " << methodname;
		}
...
void MySipInterface::handleInvite(SipMessage *msg, bool isINVITE)
{
	...
	//		LOG(DEBUG) << "after";
	//		//if (TranEntry* transactionOC =
	// gNewTransactionTable.ttFindBySIPCallId(callerMobileID,outboundCallIdNum))
	...
	//			//if (asprintf(&newCallIdNum,"%s_TC",callIDNum)) {}	// The useless 'if' shuts up
	// gcc.
	...
	if (existing.self()) {
		...
		if (existing->sameInviteOrMessage(msg)) {
			...
			// We want to keep paging this TransactionEntry.  We cant just send a message through the Dialog
			// because there is not yet any L3Procedure started on the TransactionEntry.

			TranEntry *transaction1 = existing->findTranEntry();
			if (!transaction1) {
				LOG(DEBUG) << "repeated INVITE/MESSAGE with no transaction record";
			}
			// LOG(INFO) << "pre-existing transaction record: " << transaction1;	// uggh.  This could
			// crash if tran freed here.

			// And if no channel is established yet, page again.  The check for that is handled over in the
			// Control directory. (pat) We could be blocked for several reasons, including paging, waiting
			// for LUR to complete, waiting for channel to change, etc.  But if we are paging, reset the
			// paging timer so we keep paging.
			...
			// This message is not in this dialog, so dont use this: existing->MODSendERROR(msg, 488, "Not
			// Acceptable Here", false);
	...
	// For MO, callid is decided by us, and set by calling SIPEngine::user() from the imsi, called by the TranEntry
	// constructor. In both cases mCallIdHeader is set from saveInviteOrMessage. The inbound/outbound callid
	// distinction is for the special case where the same BTS is both. newMT above calls SIPEngine(...,imsi from
	// mobileID) calls sipSetUser(imsi) which sets the mCallId for an MO transaction. Then this SIPUser invocation
	// overwrites mCallId with the callid for an MT transaction.
	// FullMobileId msid;
	// msid.mImsi = toIMSIDigits;

	// Doesnt matter if functions succeed or fail below.  We return true from this function to indicate the SIP
	...
	if (isINVITE) { // as opposed to MESSAGE
		...
		if (smsContentType == "") {
			LOG(NOTICE) << "MT-SMS incoming MESSAGE method with no content type (or memory error) for "
				    << msid.mImsi;
...
bool MySipInterface::newCheckInvite(SipMessage *msg)
{
	...
	if (msg->isINVITE()) {
		...
		if (existing.self()) {
			...
			if (msg->smGetCode() ? msg->smGetBranch() == existing->mInviteViaBranch
					     : msg->isACK() || msg->isCANCEL()) {
				WATCH("SIP Sending message to existing dialog"
					<< LOGVAR2("state", existing->getSipState()));
				LOG(DEBUG) << "Sending message to existing dialog" << LOGVAR(msg->smGetCode())
					   << LOGVAR(msg->smGetBranch()) << LOGVAR(existing->mInviteViaBranch)
					   << LOGVAR(msg->smGetMethodName()) << existing->dialogText();
				if (!existing->dgWriteHighSide(msg)) {
					LOG(ERR) << "Confusing SIP message not handled, to dialog:"
						 << existing->dialogText(false) << " message was:" << msg;
				}
...
static void periodicServiceLoop(MySipInterface *si)
{
	while (!gBTS.btsShutdown()) {
		...
		if (howlong < 250) {
			howlong = 250;
		} // Dont eat all the cpu cycles if someone accidently sets this too low.
		msleep(howlong);
	}
}
...
void SipInterface::siInit()
{
	...
	// You MUST call srandom first or the numbers returned by random are deterministic, which will result in
	// collisions between "random" numbers on different BTS that are using identical random number sequences.
	struct timeval now;
...
void SIPInterfaceStart() { gSipInterface.msiInit(); }
