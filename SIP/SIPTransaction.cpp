...
// Note: We dont need to worry about the branch param because we have only one dialog, and one each of the possible
// methods.
bool SipClientTrLayer::stMatchesMessageV(SipMessage * /*msg*/)
{
	// We are not going to do it this way.
	// return (msg->smViaBranch() == mstOutRequest.smViaBranch() && msg->smCSeqMethod() ==
	// mstOutRequest.smCSeqMethod());
	return false;
}
...
//	3. method matches, exceptfor ACK, in which case the method of the requested that created the transaction is
// INVITE.
	...
	case stProceeding:
		...
		default:
			...
			if (stIsInvite()) {
				...
				if (code >= 300) {
					mstDialog->MOCSendACK();
				} else {
					stDestroyV();
					return true;
				} // The TU is responssible for sending the ACK, after connection setup.
			}
			if (stIsReliableTransport()) {
				stDestroyV();
				return true;
			} else {
				mTimerDK.set(32 * 1000);
			}
			return true;
		}
	case stCompleted:
...
void SipClientTrLayer::TLWriteLowSideV(SipMessage *request)
{
	...
	if (!mstOutRequest.msmReqMethod.empty()) {
		assert(request->isACK());
	}
...
bool SipClientTrLayer::TLPeriodicServiceV()
{
	...
	ScopedLock lock(mstLock);
	if (mstState == stInitializing) {
		return false;
	}
	if (mstState == stTerminated) {
		return true;
	}
	...
	if (mTimerDK.expired()) {
		stDestroyV();
		return true;
	} // The transaction completed, a message was sent to layer3, so we just exit.
	if (mTimerBF.expired()) {
	...
	if (mstState == stCallingOrTrying && mTimerAE.expired()) {
		stWrite(&mstOutRequest);
		mTimerAE.setDouble();
	}
	return false;
}
...
// Note: The registrar may return messages, which we must ignore for the Unregister case since there is no transaction
// to receive them.
void SipRegisterTU::TUWriteHighSideV(SipMessage *sipmsg)
{
	...
	if (code == 0) {
		...
		// (pat) We can no longer stash Kc directly in the TMSI table because the entry is not created until the
		// MS is validated.
		if (stKind == KindRegister) {
		...
		case 100:
			return; // we dont care.
		case 200:
			LOG(ERR) << whatami << "received invalid code:" << code
				 << " in message:" << sipmsg; // Code in the range 201-299 is allegedly impossible.
...
void SipMOByeTU::TUWriteHighSideV(SipMessage *sipmsg)
{
	...
	if (sipmsg->msmCode >= 200) {
		stSetDialogState(Cleared, sipmsg->msmCode, 'K');
	}
}
...
{
	if (sipmsg->msmCode >= 200) {
		stSetDialogState(Canceled, sipmsg->msmCode, 'K');
	}
}

// void SipMOCancelTU::TUTimeoutV() { stSetDialogState(Canceled,0); }

SipMOCancelTU::SipMOCancelTU(SipDialog *wDialog, string wReasonHeader)
{						    // : SipClientTrLayer(wDialog->dsPeer(), make_branch(),wDialog
