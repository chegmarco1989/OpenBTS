...
public:
	...
	typename BaseType::iterator begin()
	{
		devassert(mMapLock.lockcnt());
		return mList.begin();
	}
	typename BaseType::iterator end()
	{
		devassert(mMapLock.lockcnt());
		return mList.end();
	}
	...
	ValueType pop_frontr(ValueType val)
	{ // Like pop_front, but return the value.
		ScopedLock(this->mMapLock);
		ValueType result = mList.front();
		mList.pop_front(val);
		return result;
	}
	ValueType pop_backr(ValueType val)
	{ // Like pop_back, but return the value.
		ScopedLock(this->mMapLock);
...
struct IPAddressSpec {
	...
	string mipName; // Original address, which may be an IP address in dotted notation or possibly a domain name,
			// and optional port.
...
class SipTimer {
	...
	// int mNextState;		// Payload.  Used as Procedure state to be invoked on timeout.  -1 means abort
	// the procedure.
...
public:
	...
	string text() const
	{
		return mActive ? format("(active=%d remaining=%ld limit=%ld", mActive, mEndTime.remaining(), mLimitTime)
			       : string("(active=0)");
	}
};
