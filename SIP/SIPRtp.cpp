...
void SipRtp::rtpStop()
{
	if (mSession) {
		...
		mSession = NULL; // Prevent rxFrame and txFrame from using it, which is overkill because we dont call
				 // this until the state is not Active.
...
void SipRtp::rtpText(std::ostringstream &os) const
{
	...
	os << LOGVAR(mRTPPort);
	os << LOGVARM(mCodec);
	// warning: The unbelievably stupid << sends the mDTMS char verbatim, and it is 0, which prematurely terminates
	// the string.
	unsigned dtmf = mDTMF;
...
void SipRtp::initRTP1(const char *d_ip_addr, unsigned d_port, unsigned dialogId)
{
	...
	// I'm setting it down such that we will resynchronize whenever we get more than one frame off, where one frame
	// = 160. Update: That worked great as long as there was no jump, but there seems to be a bug in the RTP library
	// that the time jump limit must exceed the buffered amount or it constantly tries to resync. Update: When the
	// timestamp jump occurs there is a very noticeable silence presumably while the jitter buffer is reloaded, so
	// instead of setting this low, we will leave it high (it must be set to something to handle the negative
	// timestamp jump after handover) and go back to setting rxTime from the actual time.
	...
	if (speechBuffer == 0) {
		...
		// is some other drop out, the lost time appears to be added to the total delay through the de-jitter
		// algorithm. At least some phones seem to turn the TCH off completely during an in-call SMS. So lets
		// turn it off.  This code was copied from rtp_session_init(). (pat update) The RTP library goes south
		// if you set the jitter buffer size over about 300 msecs, so heck with it - just use the adaptive
		// algorithm.  It is mostly fixed now anyway.
		{
			// int packets = speechBuffer/20 + 10;		// Number of 20 msec packets needed for
			// buffering, plus some slop.
			JBParameters jbp;
			...
			// The max_packets must be large enough to cover the speech buffer size plus a lot of slop, not
			// sure how much.
	...
	if (code == -1) {
		if (rfc2833) {
			LOG(CRIT) << "RTP session does not support selected DTMF method RFC-2833" << vsdbText();
		} else {
			LOG(CRIT) << "RTP session does not support telephone events" << vsdbText();
		}
	}
}
...
static int get_rtp_tev_type(char dtmf)
{
	...
	case '0':
		return TEV_DTMF_0;
	case '*':
		return TEV_DTMF_STAR;
	case '#':
		return TEV_DTMF_POUND;
	case 'a':
	case 'A':
		return TEV_DTMF_A;
	case 'B':
...
bool SipRtp::txDtmf()
{
	...
	if (!m) {
		// (pat) Failed because, and I quote: "the rtp session cannot support telephony event (because the rtp
		// profile it is bound to does not include a telephony event payload type)."
		LOG(DEBUG) << "fail";
		return false;
	}
	// (pat) Max RTP event duration is 8 seconds, so if we exceed that turn it off.  Back it off a little (5
	// packets, 100ms) because we also need to send the three end packets and it is not clear to me if the
	// DTMFDuration should be incremented or not during the end packets, but we are incrementing. (8 * 50 * 160) -
	// (5 * 160) = 63200.
	if (!mDTMFEnding && mDTMFDuration >= 63200) {
		mDTMFEnding = 1;
	}
	// volume 10 for some magic reason, arg 3 is true to send an end packet.
	// (pat) The magic reason is that the spec says DTMF tones below a certain dB should be ignored by the receiver,
	// which is dumber than snot.
	int code =
		rtp_session_add_telephone_event(mSession, m, get_rtp_tev_type(mDTMF), !!mDTMFEnding, 10, mDTMFDuration);
	int bytes = rtp_session_sendm_with_ts(mSession, m, mDTMFStartTime);
	LOG(DEBUG) << LOGVAR(start) << LOGVAR(mDTMF) << LOGVAR(mDTMFEnding) << LOGVAR(mDTMFDuration) << LOGVAR(code)
		   << LOGVAR(bytes);
...
void SipRtp::txFrame(AudioFrame *frame, unsigned numFlushed)
{
	...
	// (pat) The result is the number of bytes sent over the network, which includes nbytes data + 12 bytes of RTP
	// header.
	...
	if (mDTMF) {
		...
		/*****
			...
			int code =
		rtp_session_add_telephone_event(mSession,m,get_rtp_tev_type(mDTMF),false,10,mDTMFDuration); int bytes =
		rtp_session_sendm_with_ts(mSession,m,mDTMFStartTime); mDTMFDuration += 160;
			//LOG (DEBUG) << "DTMF RFC-2833 sending " << mDTMF << " " << mDTMFDuration <<vsdbText();
			if (code || bytes <=0)
		***/
		if (!txDtmf()) {
...
AudioFrame *SipRtp::rxFrame()
{
	...
	// I added this code controlled by rtpUseRealTime so we could handle the real elapsed time ourselves and not
	// block. This code below suffers no such instability, so we are using it.  Someday if we switch RTP libraries,
	...
	if (rtpUseRealTime) {
		...
		if (mRxRealTime == 0) {
			...
			// We will snag the next frame with a number equal or higher than this.  If there are none
			// available, we get NULL. When there is a discontinuity of missing frames, we get a bunch of
			// NULL frames during the discontinuity, then things pick up normally again.
			mRxTime += 160;
		}
	}
	...
	if (more) {
		LOG(WARNING) << "Incoming RTP lagging behind clock." << vsdbText();
	}
	return result;
}
