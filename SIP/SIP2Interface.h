...
// The TransactionUsers are the layer that sends/receives original messages and has some behavior related to our
// application. RFC3261 17.1.3: When a response is received we are supposed find the client transaction using the branch
// in the top via. Unfortunately, as of 6/2013 our SR is non-compliant.  It adds an extra via with a branch of "1".
// Gotta love that. An alternate way to find the transaction is by the CSeq.  So why is there a branch at all?  For
// reasons that do not apply to us, namely 1.  The branch each proxy to have its own private ids
...
// would want to view those as multiple identical requests and respond to all but the first with a repeated request
// error.
class SipTUMap {
	...
	string tuMakeKey(string callid, string method, int seqnum)
	{
		if (method == "ACK") {
			method = cInviteStr;
		} // Irrelevant, since we dont use TUs for ACK.
...
class SipInterface {
	...
	unsigned
		mInfoCSeqNum; // This is for INFO messages outside a dialog.  Inside a dialog they must use dsNextCSeq()
...
public:
	...
	// each TCH might have a SIPEngine in the TransactionEntry used for voice traffic, and each SDDCH/FACCH might
	// have a SIPEngine used for registration.  Each LogicalChannel was driven by a separate thread to make the
	// polling scheme work. Now for GSM there can be at least two TransactionEntries per MS or UE (for
	// call-hold/waiting), and in UMTS the maximum number of UEs is dependent on the spreading factor used for voice
	// calls, maybe 128. Update: There is really no limit on the number of simultaneous SIPEngines and
	// TransactionEntries, because we start a new one for each inbound SMS or voice call; if there are too many they
	// will get destroyed in the upcoming connection-management layer.
	...
	*/
	MySipInterface()
	// (pat) Dont do this! There is a constructor race between SIPInterface and ConfigurationTable needed by
	// gConfig. :mSIPSocket(gConfig.getNum("SIP.Local.Port"))
	{
	}
