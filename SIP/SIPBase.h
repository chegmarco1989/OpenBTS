...
enum SipState {
	...
	Ringing,     // (pat) MOC received Ringing, notably not used for MTC sent Ringing, which is probably a bug of no
		     // import.
	...
	Cleared, // (pat) MTD sent OK to BYE, or MTD internal error loss of FIFO, or MOSMS received OK, or MTSMS sent
		 // OK.
	...
	MOSMSSubmit, // (pat) SMS message submitted, "MESSAGE" method.  Set but never used.  Message success indicated
		     // by Cleared.
...
public:
	...
	void setSipState(SipState wState)
	{
		mState = wState;
		mStateAge.now();
	}
...
public:
	...
	string sipRemoteDisplayname() const
	{
		return mRemoteHeader.mDisplayName.empty() ? sipRemoteUsername() : dequote(mRemoteHeader.mDisplayName);
	}
...
public:
	...
	void dsSetLocalHeader(const SipPreposition *header)
	{
		LOG(DEBUG);
		mLocalHeader = *header;
	}
	void dsSetRemoteHeader(const SipPreposition *header)
	{
		LOG(DEBUG);
		mRemoteHeader = *header;
	}
...
class SipBase : public MemCheckSipBase,
		public RefCntBase,
		public SipEngine,
		public DialogStateVars,
		public SipBaseProtected {
...
public:
	...
	string localSipUri() { return format("sip:%s@%s", sipLocalUsername(), localIPAndPort()); }
	...
	string preferredIdentity(string localUsername) { return format("<sip:%s@%s>", localUsername, localIP()); }
	...
	//@{
	// The SDP we received from the peer, in the MTC invite or the MOC 200 OK.  We need it for the IP and port
	// number. Note that in the MTC case the peer info is the offer, while in the MOC case it is the final answer.
	// string mSdpPeerInfo;
	...
	int getLastResponseCode(string &reason)
	{ // Return is the reason phrase from an error response.
		if (mLastResponse) {
			reason = mLastResponse->smGetReason();
			return mLastResponse->smGetCode();
		} else
			return 0;
	}
	string getLastResponseReasonHeader()
	{ // Return is the reason phrase from an error response.
...
public:
	...
	// However, sipWrite is called from L3MobilityManagement.cpp for the register message in the pre-transaction
	// code
...
public:
	...
	// Example: SipBase::addCallTerminationReasonDlg(CallTerminationCause(CallTerminationCause::eTermSIP/eQ850, 100,
	// "This is an error"));
...
struct SipCallbacks {
	...
	static void setcallback_ttAddMessage(ttAddMessage_functype callback) { ttAddMessage_callback = callback; }
