...
SipDialog *SipDialogBase::dgGetDialog() { return dynamic_cast<SipDialog *>(this); }
...
SipState SipDialogBase::MODSendCANCEL(TermCause cause)
{
	...
	// cancelTU->mstOutRequest.addCallTerminationReasonSM(CallTerminationCause::eQ850, cause.tcGetCCCause(), ""); //
	// MODSendCANCEL
	cancelTU->sctStart();
	return getSipState();
}
...
void SipDialogBase::MTCInitRTP() { initRTP(); }

void SipDialogBase::MOCInitRTP() { initRTP(); }
...
void SipInviteServerTransactionLayerBase::SipMTCancel(SipMessage *sipmsg)
{
	...
	if (dsPeer()->ipIsReliableTransport()) {
		// It no longer matters whether we use Canceled or MTDCanceling state, and we should get rid of some
		// states.
...
void SipMOInviteClientTransactionLayer::MOWriteHighSide(SipMessage *sipmsg)
{
	...
	if (code == 0) { // It is a SIP Request.  Switch based on the method.
		if (sipmsg->isCANCEL()) {
			...
			// A BYE should not be sent by the peer until dialog established, and we are supposed to send
			// 405 error. but instead we are going to quietly terminate the dialog anyway.
			SipMTBye(sipmsg);
		} else {
		...
		saveMOResponse(sipmsg);
		LOG(DEBUG) << "saveResponse" << sipmsg;
		if (dgIsInvite()) {   // It is INVITE
			stopTimers(); // Yes we stop the timers for every possible case.  TimerBF will be restarted when
				      // handleInviteResponse sends the reply.
...
//		one set of rules (See Section 17).  For 2xx responses, the ACK is generated by the UAC core. (section
// 13)"
...
{
	ostringstream os;
	os << LOGVAR2("Timers: AE", mTimerAE) << LOGVAR2("BF", mTimerBF) << LOGVAR2("K", mTimerK)
	   << LOGVAR2("D", mTimerD);
	return os.str();
}
...
void SipMTInviteServerTransactionLayer::MTSMSSendTrying() { MTCSendTrying(); }
...
void SipMTInviteServerTransactionLayer::MTWriteHighSide(SipMessage *sipmsg)
{ // Incoming message from SIPInterface.
	LOG(DEBUG);
	...
	if (sipmsg->smGetCode() == 0) {
		if (sipmsg->isINVITE()) {
			...
			if (mtLastResponse.smIsEmpty()) {
				if (!gConfig.getBool("SIP.RFC3428.NoTrying")) {
					MTSMSSendTrying();
				} // Otherwise just ignore the duplicate MESSAGE.
			} else {
				sipWrite(&mtLastResponse);
			}
		} else {
...
bool SipMTInviteServerTransactionLayer::mtPeriodicService()
{
	if (mTimerG.expired()) { // Resend timer.
		if (getSipState() == SSFail || getSipState() == Active) {
			...
			// Not sure what to do - I think we will let the CANCEL take precedence, so stop sending this
			// response.
			mTimerG.stop();
		}
		...
		return true; // Stop the dialog now.  It will be deleted by the periodic service loop after the
			     // associated L3 transaction ends.
	} else if (sipIsFinished()) {
		// If one of the kill timers is active, wait for it to expire, otherwise kill now.
				       ...
				       const FullMobileId &msid, const char *SRES)
{
	...
	// We formerly allocated a new Dialog for each REGISTER message so the IMSI was stashed in the dialog, and
	// localSipUri() worked.
	...
	// string fromUriString = makeUriWithTag(username,dsPeer()->mipName,make_tag());	// The port, if any, is
	// already in mipName.
	...
	if (wMethod == SIPDTRegister) {
		...
		if (SRES && strlen(SRES)) {
			if (realm.length() > 0) {
				...
				// strings are only use for sipauthserve. Also remove the extraneous leading comma.
...
void SipDialog::MODSendBYE(TermCause cause)
{
	...
	// byeTU->mstOutRequest.addCallTerminationReasonSM(CallTerminationCause::eQ850, cause.tcGetCCCause(), ""); //
	// MODSendBYE
	byeTU->sctStart();
}
...
SipDialog *SipDialog::newSipDialogMT(DialogType dtype, SipMessage *req)
{
	...
	// 2-2014: RFC 3267 8.2.6.2 says the UAS (sip server) MUST add a "to" tag to a response, and MAY add a "to" tag
	// to a provisional (100) response. The reason is in case the request is forked, the client could distinguish
	// responses from multiple servers, a case that would not happen for us.
...
SipDialog *SipDialog::newSipDialogRegister1() // caller imsi
{
	LOG(DEBUG);
	SipDialog *dialog =
		new SipDialog(SIPDTRegister, gConfig.getStr("SIP.Proxy.Registration"), "SIP.Proxy.Registration");
...
//
// In general, URI encoding (RFC2396) is kind of complicated - for example, we need a-priori knowledge if ";" in the
// string is marking a URI parameter, in which case it needs to be left alone, or if it is part of the URI component, in
// which case it must be escaped. RFC3267 talks about URBNF the URI string may contain: This handles the USSD special
// case.  A USSD request can contain only digits, letters * and #, and of these the only special one is "#" turns into
// "%23".
					 ...
					 L3LogicalChannel *chan)
{
	...
	// this is not a "global" tel number, and therefore by definition it must be a "local" tel number, which
	// requires a context, but we learn from RFC 4967 that we can stick in anything we want for the context, since
	// it is meaningless when used for a USSD string.  This spec is not exactly an "A-team" effort.
...
SipDialog *SipDialog::newSipDialogHandover(TranEntry *tran, string sipReferStr)
{
	...
	// TODO: If any other intervening messages were sent by BTS1 between the REFER and now the CSeqNum will not be
	// correct.
	...
	dialog->mCodec = tran->getCodecs(); // TODO: We need to renegotiate this, or set it from SDP.  There is no point
					    // even setting this here.
...
{
	if (this->mTranId == 0) {
		// No attached transaction.  Can happen if we jumped the gun (the dialog is created before the
		// transaction and there could be a race with an incoming message) or if we responded with an early
		// error to a dialog and never created a transaction for it, for example, 486 Busy Here.
		return NULL;
	}
...
static bool isPhoneNumber(string thing)
{
	...
	if (0 == strncasecmp(thing.c_str(), "IMSI", 4)) {
		return false;
	}	    // It is an IMSI, not a phone number.
...
static string removeUriFluff(string thing)
{
	if (unsigned first = thing.find('<') != string::npos) { // Remove the angle brackets.
		...
		thing = thing.substr(
			0, thing.find_last_of('>')); // chop off trailing angle bracket, and anything following.
	}
...
TranEntry *SipDialog::createMTTransaction(SipMessage *invite)
{
	...
	if (0 == strcasecmp(callerIdSource.c_str(), "auto")) {
		...
		// We can do this automatically because we know whether the displayname, etc, are imsi because they are
		// preceded by "IMSI". btw, the P-Asserted-Identity should by an IMSI; the phone number is supposed to
		// be in the P-Associated-URI.
...
void SipDialog::dialogCancel(TermCause cause)
{
	...
	case SIPDTMOC:
		...
		caseActive:
			...
			break;
		case SSNullState: // (pat) MTC initial state - nothing sent yet. MOC not used because sends INVITE on
				  // construction.
		...
		case HandoverInbound:
			if (mDialogType == SIPDTMOC) {
				...
				// We are the INVITE recipient server and have not received the ACK yet, so we must send
				// an error response. Way back in version 3 this was used for MTC also. RFC3261 (SIP) is
				// internally inconsistent describing the error codes - the 4xxx and 5xx generic
				// descriptions are contradicted by specific error code descriptions.
				// This is from Paul Chitescu at Null Team:
				// "A 504 Server Timeout seems the most adequate response [to MS not responding to
				// page.] 408 is reserved for SIP protocol timeouts (no answer to SIP message) 504
				// indicates some other timeout beyond SIP (interworking) 480 indicates some temporary
				// form of resource unavailability or congestion but resource is accessible and can be
				// checked" 486 "Busy Here" implies that we found the MS but it really is busy. 503
				// indicates the service is unavailable but does not imply for how long
				...
				if (cause.tcGetCCCause() == L3Cause::Normal_Call_Clearing) {
					// (pat 7-2014) The handset MTC hung up normally but the SIP Dialog here is not
					// in the Active state. This weird case occurs if the handset sends a disconnect
					// before it sends a connect, which can happen on some if the user answers and
					// hangs up immediately. We need to send a 200 OK and then BYE instead of
					// sending an early error. We dont care about codecs because we are clearing
					// immediately so just regurgitate the codecs from the INVITE.
...
#if PREVIOUS_CODE
				// (pat) Keeping this old code here for a while to show what SIP responses were returned
				// by the version 4 code. Now SIP responses are formulated from the TermCause by
				// tcGetSipCodeAndReason()
				int sipcode = 486;
				...
				case CancelCauseUnknown: // 0 Loss of contact with MS or an error.
					if (l3Cause == L3Cause::SwitchingEquipmentCongestion) {
						sipcode = 503;
						reason = "Normal circuit congestion";
					} else if (l3Cause == L3Cause::NoUserResponding) {
						sipcode = 408;
						reason = "No user responding";
				...
				case CancelCauseCongestion: // This reason is never used MS is here but no channel avail
							    // or other congestion.
					sipcode = 503;
					reason = "Normal circuit congestion";
					break;

				case CancelCauseNoAnswerToPage: // Not used   We dont have any clue if the MS is in this
								// area or not.
				...
				case CancelCauseOperatorIntervention:
					sipcode = 487;
					reason = "Request Terminated Operator Intervention";
					break;
				}
		...
		case Cleared: // (pat) MTD sent OK to BYE, or MTD internal error loss of FIFO, or MOSMS received OK, or
			      // MTSMS sent OK.
		case SSFail:
	...
	LOG(INFO) << "SIP term info end dialogCancel, cancel cause: " << cause << " mDialogType: " << mDialogType
		  << LOGVAR(state) << LOGVAR(cause);
	// << " bTerminationAdded: " << bTerminationAdded;

	// (pat) The TermCause has the complete termination reason.  The cause was sent in the error response, CANCEL
	// message, or BYE message; we dont need to save it in the dialog. Save termination reason
	...
	//			addCallTerminationReasonDlg(CallTerminationCause::eQ850, cause.tcGetCCCause(), ""); //
	// dialCancel
	//		}
	//}
...
void SipEngine::dialogQueueMessage(DialogMessage *dmsg)
{
	...
	// We dont enqueue on the GSM LogicalChannel because that may change from, eg, SDCCH to FACCH before this
...
bool SipDialog::permittedTransition(DialogState::msgState oldState, DialogState::msgState newState)
{
	...
	if (newState == oldState) {
		...
		if (newState == DialogState::dialogProceeding || newState == DialogState::dialogRinging) {
			return true;
		}
	}
	return false;
	...
	char timer)
{
	...
	LOG(DEBUG) << LOGVAR(oldSipState) << LOGVAR(newSipState) << LOGVAR(getSipState()) << LOGVAR(mPrevDialogState)
		   << LOGVAR(oldDialogState) << LOGVAR(nextDialogState) << dialogText();
...
DialogState::msgState SipDialog::getDialogState() const
{
	...
	case Ringing: // (pat) MOC received Ringing, notably not used for MTC sent Ringing, which is probably a bug of
		      // no import.
	...
	case Canceled:     // (pat) received OK to CANCEL.
	case Cleared: // (pat) MTD sent OK to BYE, or MTD internal error loss of FIFO, or MOSMS received OK, or MTSMS
		      // sent OK.
	...
	otherwise:
		if (code == 0) { // It is a SIP Request.  Switch based on the method.
			if (msg->isBYE()) {
				...
				// This is an error since we have already passed the ACK stage, but lets cancel the
				// dialog anyway.
				SipMTCancel(msg);
			} else {
				// Not expecting any others.  Must send 405 error.
...
{
	if (dmsg) {
		os << "DialogMessage(" << LOGVAR2("MsgState", DialogState::msgStateString(dmsg->mMsgState))
		   << LOGVAR2("StatusCode", dmsg->mSipStatusCode) << ")";
	} else {
