...
public:
	...
	string uriAddress() const
	{ // without sip: or the params or headers.
		if (size() < start)
			return string("");
	...
	void uriSet(string fullUri)
	{
		if (fullUri.size() == 0) {
			clear();
			return;
		} // Dont let fullUri[0] will throw an exception.
		if (fullUri[0] == '<') {
	...
	// void uriSetName(string username,string host, unsigned port=0) {		// The host could also contain a
	// port spec; we dont care. 	assign(makeUri(username,host,port));
	//}
	SipUri() {}
...
public:
	...
	SipPreposition(string wDisplayName, string wUri, string wTag = "")
		: mDisplayName(wDisplayName), mUri(wUri), mTag(wTag)
	{
		rebuild();
	}
...
public:
	enum termGroup { eNone = 0, eTermSIP, eQ850 };
	...
	{
		// LOG(INFO) << "SIP term info CallTerminationCause begin ctor, group: " << group << " cause: "<< cause
		// << " text: " << desc;
		...
		// LOG(INFO) << "SIP term info CallTerminationCause end ctor, group: " << mReasonGroup << " cause: "<<
		// miCause << " text: " << msCauseDesc;
	}
	...
	CallTerminationCause(const CallTerminationCause &Source)
	{
		...
		// LOG(INFO) << "SIP term info CallTerminationCause copy ctor, group: " << mReasonGroup << " cause: "<<
		// miCause << " text: " << msCauseDesc;  LOG(INFO) << "SIP term info text in copy ctor from
		// getTextforEntry: " << this->getTextforEntry();
	}
...
private:
	/*
	SIP: The cause parameter contains a SIP status code.  See section 21 in RFC 3261 Q.850: The cause parameter
	contains an ITU-T Q.850 cause value in decimal representation.  */
...
SipTermList
Example:
	SipBase::addCallTerminationReasonDlg(CallTerminationCause(CallTerminationCause::eTermSIP/eQ850, 100, "This is an
error")); SipMessage::addCallTerminationReasonSM(CallTerminationCause(CallTerminationCause::eTermSIP/eQ850, 100, "This
is an error")); CallTerminationCause sc(CallTerminationCause::eQ850, (int) GSM::L3Cause::NormalCallClearing, "");
...
public:
	...
	void add(CallTerminationCause *pEntry)
	{
		if (((int)pEntry->getTermGroup() != 0) && (pEntry->getTermCause() != 0)) {
			// LOG(INFO) << "SIP term info adding entry to call termination list: " <<
			// pEntry->getTextforEntry();  // SVGDBG  if (lTermList.size() > 1) {
			// Added more that one entry log this so we can make sure this is correct
			//	LOG(INFO) << "SIP term info adding entry to call termination reason, table size: " <<
			// lTermList.size();  // SVGDBG
			//}
			...
			LOG(INFO) << "SIP term info tried to add termination reason with invalid data, term group: "
				  << pEntry->getTermGroup() << " term cause: " << pEntry->getTermCause()
				  << " desc: " << pEntry->getCauseDescription();
		}
		logList(); // SVGDBG
	}
	...
	~SipTermList()
	{
		...
		for (vector<CallTerminationCause *>::iterator it = lTermList.begin(); it != lTermList.end(); it++) {
			// LOG(INFO) << "SIP term info SipTermList dtor remove entry: " << (*it)->getTextforEntry(); //
			// SVGDBG
			delete (*it);
		}
	}
