...
void ResponseTable::addResponse(unsigned code, const char *name) { mMap[code] = string(name); }

string ResponseTable::get(unsigned code) { return gResponseTable[code]; }
...
bool IPAddressSpec::ipSet(string addrSpec, const char *provenance)
{
	...
	if (!resolveAddress(&mipSockAddr, addrSpec.c_str())) {
		...
		if (!resolveAddress(&mipSockAddr, addrSpec.c_str(), 5060)) {
			LOG(CRIT) << "cannot resolve IP address for " << addrSpec << " from "
				  << provenance; // << sbText();
			return false;
		}
	}
...
string make_tag() { return make_tag1("OBTS"); }
...
string makeMD5(string input)
{
	...
	if (buffer != NULL && strlen(buffer) == 33)
		buffer[32] = 0;
...
{
	if (0 == strncasecmp(in, "sip:", 4)) {
		return in + 4;
	} else if (0 == strncasecmp(in, "tel:", 4)) {
		return in + 4;
	} else if (0 == strncasecmp(in, "sips:", 5)) {
		return in + 5;
	}
	return in;
}
...
{
	if (0 == strncasecmp(in.c_str(), "sip:", 4)) {
		return in.substr(4);
	} else if (0 == strncasecmp(in.c_str(), "tel:", 4)) {
		return in.substr(4);
	} else if (0 == strncasecmp(in.c_str(), "sips:", 5)) {
		return in.substr(5);
	}
	return in;
}
