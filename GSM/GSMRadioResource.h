...
public:
	ChannelRequestRecord(unsigned wRA, const GSM::Time &wFrame, float wRSSI, float wTimingError)
		: mRA(wRA), mFrame(wFrame), mRSSI(wRSSI), mTimingError(wTimingError)
	{
	}
...
enum FakeRachType { FakeRachTCH, FakeRachSDCCH, FakeRachLUR, FakeRachGPRS, FakeRachAnswerToPaging };
