...
void L3Frame::text(std::ostream &os) const
{
	...
	if (isData()) {
		...
		os << LOGVAR2("PD", PD()) << LOGHEX2("MTI", this->MTI()) << "(" << mtiname << ")"
		   << LOGVAR2("TI", TI());
	}
	os << " raw=(";
	this->hex(os);
...
L2Frame::L2Frame(const BitVector &bits) : BitVector(23 * 8), mPrimitive(L2_DATA)
{
	idleFill();
...
L2Frame::L2Frame(const L2Header &header, const BitVector &l3, bool noran) : BitVector(23 * 8), mPrimitive(L2_DATA)
{
	idleFill();
	...
	if (gConfig.getBool("GSM.Cipher.ScrambleFiller") && !noran)
		randomizeFiller(header);
}

L2Frame::L2Frame(const L2Header &header) : BitVector(23 * 8), mPrimitive(L2_DATA)
{
	idleFill();
	header.write(*this);
...
L2Control::ControlFormat L2Frame::controlFormat() const
{
	...
	if (bit(8 + 7) == 0)
		return L2Control::IFormat;
	if (bit(8 + 6) == 0)
		return L2Control::SFormat;
...
L2Control::FrameType L2Frame::UFrameType() const
{
	...
	switch (uBits) {
	case 0x07:
		return L2Control::SABMFrame;
	case 0x03:
		return L2Control::DMFrame; // (disconnect mode)
	case 0x00:
		return L2Control::UIFrame; // (unnumbered information)
	case 0x08:
		return L2Control::DISCFrame; // (disconnect)
	case 0x0c:
		return L2Control::UAFrame; // (unnumbered acknowledge)
	default:
		return L2Control::BogusFrame;
	}
}
...
TxBurst::TxBurst(const RxBurst &rx) : BitVector((const BitVector &)rx), mTime(rx.time()) {}

// We put this in the .cpp file to avoid a circular dependency.
RxBurst::RxBurst(const TxBurst &source, float wTimingError, int wRSSI)
	: SoftVector((const BitVector &)source), mTime(source.time()), mTimingError(wTimingError), mRSSI(wRSSI)
{
}
...
ostream &GSM::operator<<(ostream &os, L2Control::FrameType cmd)
{
	...
	case L2Control::RRFrame:
		os << "RR";
		break;
	case L2Control::RNRFrame:
		os << "RNR";
		break;
	case L2Control::REJFrame:
		os << "REJ";
		break;
	case L2Control::IFrame:
		os << "I";
		break;
	default:
		os << "?" << (int)cmd << "?";
...
ostream &GSM::operator<<(ostream &os, Primitive prim)
{
	...
	**/
	// old:os << "?" << (int)prim << "?";
	default:
		os << "(unrecognized primitive:" << (int)prim << ")";
	}
	return os;
}
...
void L3Frame::f3init() { mTimestamp = timef(); }

L3Frame::L3Frame(const L3Message &msg, Primitive wPrimitive, SAPI_t wSapi)
	: BitVector(msg.bitsNeeded()), mPrimitive(wPrimitive), mSapi(wSapi), mL2Length(msg.l2Length())
{
	f3init();
	msg.write(*this);
}

L3Frame::L3Frame(SAPI_t sapi, const char *hexString) : mPrimitive(L3_UNIT_DATA), mSapi(sapi)
{
	f3init();
...
unsigned L3Frame::MTI() const
{
	...
	case L3MobilityManagementPD:
		// (pat) 5-2013: For these protocols only, mask out the unused bits of the raw MTI from the L3 Frame.
		// See 3GPP 4.08 10.4
		return mti & 0xbf;
	default:
		return mti;
...
AudioFrameRtp::AudioFrameRtp(AMRMode wMode)
	: ByteVector((headerSizeBits(wMode) + GSM::gAMRKd[wMode] + 7) / 8), mMode(wMode)
{
	setAppendP(0);
	...
	if (wMode == TCH_FS) {
		...
		appendField(wMode, RtpHeaderSize()); // The CMR field is allegedly not used by anyone yet, but lets set
						     // it to the AMR mode we are using.
