...
L2LAPDm::L2LAPDm(unsigned wC, SAPI_t wSAPI)
	: mRunning(false), mC(wC), mR(1 - wC), mSAPI(wSAPI), mMaster(NULL), mState(LAPDStateUnused), mT200(T200ms),
	  mIdleFrame(/*DATA*/)
{
	// sanity checks
	assert(mC < 2);
	...
	// releaseLink(true,L3_RELEASE_INDICATION);	// (pat) This sends release in both directions, which is
	// undesirable at initialization.
	clearCounters();
	...
	if (gConfig.getBool("GSM.Cipher.ScrambleFiller"))
		mIdleFrame.randomizeFiller(8 * 4);
}
...
void L2LAPDm::writeL1(const L2Frame &frame)
{
	...
	if (!mL2Downstream)
		return; // only possible during initialization, and maybe not even then.
	// It is tempting not to lock this, but if we don't,
	// the ::open operation can result in contention in L1.
	ScopedLock lock(mL1Lock);
	// mL2Downstream->sapWriteHighSide(frame);	// (pat) This blocks in L1Encoder::transmit until the time has
	// passed.
...
void L2LAPDm::writeL1Ack(const L2Frame &frame)
{
	...
	// (pat) Lock is held when called from sendMultiFrameData and sendUFrameDISC, but not sendUFrameSABM.  Why?  I'm
	// fixing it. GSM 04.06 5.4.4.2
...
void L2LAPDm::waitForAck()
{
	...
	while (true) {
		if (mState == LinkReleased)
			break;
		if ((mState == ContentionResolution) && (mVS == mVA))
			break;
		if ((mState == LinkEstablished) && (mVS == mVA))
			break;
...
// Also note that the handset may continue to send up measurement reports for some time after the main channel is
// closed. I dont see where we make the SACCH non active; I think we just wait for valid messages to stop coming and it
// times out via T3109.
void L2LAPDm::releaseLink(bool notifyL3, Primitive releaseType)
{
	...
	// This means that the RELEASE/HARDRELEASE does not close the dowstream channel if arriving from an SMS
	// connection, which is dictated in GSM 4.06 5.4 We no longer send anything to L1.  L2LogicalChannel lurs on the
	// L3 connection and handles it. (pat) After sending the DM we are not supposed to close the channel until we
	// allow some time for a reply; that is now handled by L2LogicalChannel::T3111
	if (notifyL3) {
...
void L2LAPDm::normalRelease()
{
	...
	ScopedLock lock(mLock);
	if (mState == LinkReleased || mState == AwaitingRelease)
		return;
	if (mState == LinkEstablished)
		waitForAck(); // (pat) Does not wait if nothing has been sent.
	// (pat) The handset may ack with a DISC, so check again:
	if (mState == LinkReleased || mState == AwaitingRelease)
		return; // Cant be AwaitingRelease, but check anyway.
	LOG(DEBUG) << this;
	clearCounters();
	mEstablishmentInProgress = false;
...
void L2LAPDm::l2dlWriteHighSide(const L3Frame &frame)
{
	...
	case L3_ESTABLISH_REQUEST: // (pat) Establish SABM mode.
		...
		// See note in GSM 04.06 5.4.1.1.  (pat) And I quote:  "For SAPI 0 the data link is always established
		// by the MS."
		devassert(mSAPI != 0 || mC == 0); // (pat) Could be on SAPI 0 only in test mode.
		if (mState == LinkEstablished) {
			// We are supposed to send ESTABLISH_CONFIRM, but since L3 does not care which side starts the
			// establish process or distinguish between ESTABLISH_CONFIRM and ESTABLISH_INDICATION.
		...
		// mLock.unlock();  (pat 5-2014) I am changing to hold lock while calling sendUFrameSABM, since it
		// modifies mSentFrame.
		sendUFrameSABM();
		mLock.unlock();
		break;
...
void L2LAPDm::lapServiceLoop()
{
	...
	while (mRunning) {
		...
		// since if you set it on sacch it will keep anything from ever going through since sap0 is always in
		// idle (LinkReleased) state. if (mMaster) {
		...
		if (!mT200.active()) {
			if (mState == LinkReleased)
				...
				timeout = T200(); // currently 3.6sec  (pat) Looks like .9 secs on TCH,SDCCH and 4*.9 on
						  // SACCH.  Why?
...
void L2LAPDm::T200Expiration()
{
	...
	case AwaitingEstablish: // (pat) SABM sent.  Should only happen on SAPI 3 since MS establishes link SAPI 0
		// (pat) 4.06 5.5.7: L2 is supposed to query L3 whether to release link or attempt re-establishment, but
		// we just release. It either does nothing ("local end release") or starts a normal release procedure
		// which would send a DISC.
		if (mRC > N200())
			abnormalRelease(false); // (pat) do not send DM.  If anything, we would send DISC.
		else
...
void L2LAPDm::receiveUFrame(const L2Frame &frame)
{
	...
	case L2Control::SABMFrame:
		receiveUFrameSABM(frame);
		break;
	case L2Control::DMFrame:
		receiveUFrameDM(frame);
		break;
	case L2Control::UIFrame:
		receiveUFrameUI(frame);
		break;
	case L2Control::DISCFrame:
		receiveUFrameDISC(frame);
		break;
	case L2Control::UAFrame:
		receiveUFrameUA(frame);
		break;
	default:
...
void L2LAPDm::receiveUFrameSABM(const L2Frame &frame)
{
	...
	case LinkReleased:
		...
		if (frame.L()) {
			...
			// GSM 04.06 5.4.1.4.
			if (mSAPI == 0)
				mState = ContentionResolution; // only SAP 0 is permitted to enter Contention Resolution
	...
	case LinkEstablished:
		...
		if (frame.L()) {
			// (pat) Means SABM includes a command, which would attempt to re-establish contention
			// resolution.
			LOG(ALERT) << "FRAME.L";
...
void L2LAPDm::receiveUFrameUA(const L2Frame &frame)
{
	...
	OBJLOG(INFO) << frame;
	if (!frame.PF()) {
		// (pat) TODO: This looks wrong.  GSM 4.06 5.4.1.2 quote: 'A UA response with the F bit set to "0" shall
		// be ignored.'
		unexpectedMessage(4);
		return;
	}
	...
	case AwaitingEstablish:
		...
		mState = LinkEstablished;
		mAckSignal.signal();
		// We used to use L3_ESTABLISH_INDICATION instead of L3_ESTABLISH_CONFIRM since there is no reason to
	...
	case LinkEstablished: // Pat added: This is probably just a duplicate SABM establishment acknowledgement.
		...
		break;
	case ContentionResolution:
		// (pat 3-13-2014) It is a bug to get a UA in ContentionResolution, but the Blackberry does this
		// sometimes if you do two testcalls back to back, so dont abort the link because of it... (pat
		// 4-7-2014) Update: The problem occurs when you do an endcall followed by a testcall in less than 30
		// seconds. (pat 6-2014) and that may be because we did not deactivate SACCH properly at the end of the
		// call?
		LOG(NOTICE) << "Ignoring unexpected LAPDm UA frame in ContentionResolution mode, which may be a bug in "
			       "the MS.";
		return;
	default:
		unexpectedMessage(5);
...
void L2LAPDm::receiveSFrame(const L2Frame &frame)
{
	...
	case L2Control::RRFrame:
		receiveSFrameRR(frame);
		break;
	case L2Control::REJFrame:
		receiveSFrameREJ(frame);
		break;
	default:
		unexpectedMessage(6);
		return;
...
void L2LAPDm::receiveSFrameRR(const L2Frame &frame) // RR == Receive Ready.
{
	...
	case LinkEstablished:
		...
		// PF is the Poll/Final bit 3.5.1.  true means sender is polling receiver; receiver responds with
		// PF=true.
#if ORIGINAL
...
void L2LAPDm::sendSFrameRR(bool FBit)
{
	...
	OBJLOG(INFO) << "F=" << FBit << LOGVAR(header)
		     << LOGVAR(address); // " state=" << mState << " VS=" << mVS << " VR=" << mVR;
...
void L2LAPDm::sendUFrameUI(const L3Frame &l3)
{
	...
	devassert(mC == 1); // C/R is always 1 for commands except for testing where we could make a second dummy LAPDm
			    // to emulate a handset.
	L2Address address(mC, mSAPI);
	// (pat 6-2014) The U format is used for both control frames (SABM, DISC, DM, UA) and for unacknowledged
	// information transfer. The P/F bit is used for the former, but not for the latter. GSM 4.06 5.2.1: "For
	// unacknowledged information transfer with normal L2 header, the P/F bit is not used and shall be set to "0".
	// Also GSM 4.06 5.3.2 says P is set to 0.
	// This formerly set P to 1, which was wrong, and doug corrected it below for one message type, but it is wrong
	// for all.
	 ...
	 *	l2f.CR(true);	// (pat) It already is 1; it is always 1 for commands from the BTS.  That is the mC
	 *above in L2Address above. l2f.PF(false);
	 * }
	 */
	writeL1NoAck(l2f);
}
...
bool L2LAPDm::sendMultiframeData(const L3Frame &l3)
{
	...
	if (mState == LinkReleased) {
		...
		abnormalRelease(false); // (pat) Do not send DM, just send (probably a repeat) RELEASE to L3 to tell it
	...
	while (bitsRemaining > 0) {
		...
		OBJLOG(DEBUG) << " sendIndex=" << sendIndex << " thisChunkSize=" << thisChunkSize
...
{
	os << " LAPDm(" << myid << '(' << ((void *)this) << ')' << LOGVARM(mSAPI) << LOGVARM(mState);
	if (mState != LAPDStateUnused) {
		os << LOGVARM(mC) << LOGVARM(mR) << LOGVARM(mVS) << LOGVARM(mVA) << LOGVARM(mVR) << LOGVARM(mRC)
		   << LOGVARM(mEstablishmentInProgress) << LOGVARM(mL1In.size()); //<<LOGVARM(mL3Out.size());
		if (mMaster) {
