...
enum EncryptionType { ENCRYPT_NO, ENCRYPT_MAYBE, ENCRYPT_YES };
...
public:
	...
	*/
	...
	// virtual string debugId() const { static string id; return id.size() ? id : (id=format("%s %s
	// ",typeid(this).name(),descriptiveString())); }
...
struct HandoverRecord {
	...
	HandoverRecord(float rssi, float te, double timestamp)
		: mValid(true), mhrRSSI(rssi), mhrTimingError(te), mhrTimestamp(timestamp)
	{
	}
};
...
protected:
	...
	// that sends a LAPDm DISC and waits for a response, or by "local end release" which means drop the channel
	// immediately. T3109 is either: 3.4.13.1.1 the time between deactivation of the SACCH and when the channel is
	// considered recyclable, in which
	...
	*/
	L1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
		: mUpstream(NULL),
		  ...
		  // mT3111(T3111ms),
		  mT3103(gConfig.GSM.Timer.T3103), mDecActive(false),
		  // mRunning(false),
		  // mFER(0.0F),
		  mCN(wCN), mTN(wTN), mMapping(wMapping), mParent(wParent), mEncrypted(ENCRYPT_NO),
...
public:
	...
	*/
	...
	HandoverRecord &handoverPending(bool flag, unsigned handoverRef)
	{
		...
		if (flag) {
			mT3103.set(gConfig.GSM.Timer.T3103);
		} else {
			mT3103.reset();
		}
...
public:
	...
	// conditions, whereas the old way of just using a timer that was reset only required one good frame every timer
	// period, which really only detected total channel loss, not a marginal channel.  This var is only used on
	// SACCH, so we dont bother modifying it in countStolenFrame, since stolen frames are only on TCH/FACCH. Note:
	// Spec says frames are always sent on SACCH in both directions as long as channel is up, which is not
	// necessarily required on the associated SDCCH or TCH/FACCH. Note: Unlike the counts in DecoderStats we are
	// counting message periods, which are 4-frame groups, not individual frames.
...
public:
	...
	*/
	L1FEC() : mEncoder(NULL), mDecoder(NULL), mGprsReserved(0), mGPRSFEC(0) {}
	...
	*/
	...
	{
		assert(mDecoder);
		return mDecoder->mapping();
	}

	virtual const char *descriptiveString() const
	{
		assert(mEncoder);
		return mEncoder->descriptiveString();
	}
	// virtual string debugId() const { static string id; return id.size() ? id : (id=format("%s %s
	// ",typeid(this).name(),descriptiveString())); }
	...
	void setGPRS(bool reserved, GPRS::PDCHL1FEC *pch)
	{
		mGprsReserved = reserved;
		mGPRSFEC = pch;
	}
...
public:
	RACHL1Decoder(const TDMAMapping &wMapping, L1FEC *wParent, unsigned wTN)
		: L1Decoder(0, wTN, wMapping, wParent), mParity(0x06f, 6, 8), mU(18), mD(mU.head(8))
	{
	}
...
public:
	...
	// string debugId() const { static string id; return id.size() ? id : (id=format("SharedL1Encoder %s
	// ",descriptiveString())); }
};
...
public:
	SharedL1Decoder();
	virtual const char *descriptiveString() const = 0;
	// string debugId() const { static string id; return id.size() ? id : (id=format("SharedL1Decoder %s
	// ",descriptiveString())); }
...
class XCCHL1Decoder : virtual public SharedL1Decoder, public L1Decoder {

protected:
public:
	XCCHL1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent);
...
public:
	SDCCHL1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
...
public:			       // dont know why we bother
	unsigned mReportCount; // Number of measurement reports received so far (not including the initial RACH); used
			       // for averaging RSSI and others.
	volatile float mRSSI;  ///< most recent RSSI, dB wrt full scale (Received Signal Strength derived from our
			       ///< measurement of the received burst).
	volatile float
		mTimingError; ///< Timing error history in symbols (derived from our measurement of the received burst).
...
public:
	...
	float getRSSP() const { return mRSSI + gConfig.GSM.MS.Power.Max - mActualMSPower; }
	...
	// Includes the initial rach burst, which is significant because that one provided only TA, did not have any
	// useful power data.
	...
	*/
	...
	virtual const char *descriptiveString() const = 0;
	// string debugId() const { static string id; return id.size() ? id : (id=format("MSPhysReportInfo %s
	// ",descriptiveString())); }
};
...
public:
	SACCHL1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, SACCHL1FEC *wParent)
		: XCCHL1Decoder(wCN, wTN, wMapping, (L1FEC *)wParent), mSACCHParent(wParent)
	{
...
class XCCHL1Encoder : virtual public SharedL1Encoder, public L1Encoder {
public:
	XCCHL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent);
...
public:
	...
	// (pat) Irritating and pointless but harmless double-initialization of Parity and BitVector2s.  Stupid
	// language. (pat) Assume TCH_FS until someone changes the mode to something else.
	TCHFRL1Encoder() : mViterbi(0), mTCHParity(0, 0, 0) { setAmrMode(TCH_FS); }
	// string debugId() const { static string id; return id.size() ? id : (id=format("TCHFRL1Encoder %s
	// ",descriptiveString())); }
};

/** L1 encoder used for full rate TCH and FACCH -- mostry from GSM 05.03 3.1 and 4.2 */
class TCHFACCHL1Encoder : public XCCHL1Encoder, public TCHFRL1Encoder {
...
public:
	TCHFACCHL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent);
...
protected:
	...
	*/
	...
	// string debugId() const { static string id; return id.size() ? id : (id=format("TCHFACCHL1Encoder %s
	// ",descriptiveString())); }
};
...
public:
	...
	*/
	...
	// string debugId() const { static string id; return id.size() ? id : (id=format("TCHFRL1Decoder %s
	// ",descriptiveString())); }
};
...
// fyi: Oh, the TCHFACCHL1Decoder is connected to the XCCHL1Decoder, and the XCCHL1Decoder is connected to
// the...L1Decoder, and the L1Decoder is connected to the...SAPMux, and the SAPMux is connected to the...LogicalChannel,
...
// and the L3FrameFIFO is connected to...LogicalChannel::recv, and thence to whatever procedure in the L3 layers that
// wants to receive these messages, eg MOCController, MTCController, although the L3rewrite will substitute all those
// with the L3 state machine.
class TCHFACCHL1Decoder : public XCCHL1Decoder, public TCHFRL1Decoder {
protected:
	SoftVector mE[8];		       ///< deinterleaving history, 8 blocks instead of 4
	SoftVector mI[8];		       ///< deinterleaving history, 8 blocks instead of 4
	AudioFrameFIFO mSpeechQ;	       ///< output queue for speech frames
	unsigned stealBitsU[8], stealBitsL[8]; // (pat 1-16-2014) These are single bits; the upper and lower stealing
					       // bits found in incoming bursts.

public:
	TCHFACCHL1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent);
	...
	*/
	...
	// Note also that we use SharedL1Decoder::decode() for control frames, but TCHFRL1Decoder::decodeTCH() for TCH
	// frames.
	...
	*/
	...
	const char *descriptiveString() const { return L1Decoder::descriptiveString(); }
	// string debugId() const { static string id; return id.size() ? id : (id=format("TCHFACCHL1Decoder %s
	// ",descriptiveString())); }
...
public:
	GeneratorL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
...
public:
	...
	// string debugId() const { static string id; return id.size() ? id : (id=format("FCCHL1Encoder %s
	// ",descriptiveString())); }
...
public:
	NDCCHL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
...
protected:
	...
	// string debugId() const { static string id; return id.size() ? id : (id=format("NDCCHL1Encoder %s
	// ",descriptiveString())); }
};
...
public:
	BCCHL1Encoder(L1FEC *wParent, unsigned wTN) : NDCCHL1Encoder(0, wTN, gBCCHMapping, wParent) {}
	const char *descriptiveString() const { return "BCCH"; }
	// string debugId() const { static string id; return id.size() ? id : (id=format("BCCHL1Encoder %s
	// ",descriptiveString())); }
...
protected:
	// (pat 5-2014) bool encActive() const { return true; }	// (pat) WRONG!  The SACCH is deactivated before
	...
	// string debugId() const { static string id; return id.size() ? id : (id=format("SACCHL1Encoder %s
	// ",descriptiveString())); }
};
...
public:
	CCCHL1Encoder(const TDMAMapping &wMapping, L1FEC *wParent, unsigned wTN)
		: XCCHL1Encoder(0, wTN, wMapping, wParent)
	{
	}
	// string debugId() const { static string id; return id.size() ? id : (id=format("CCCHL1Encoder %s
	// ",descriptiveString())); }
};
...
public:
	...
	virtual void sendFrame(const L2Frame &);
	// string debugId() const { static string id; return id.size() ? id : (id=format("CBCHL1Encoder %s
	// ",descriptiveString())); }
};
...
public:
	SDCCHL1FEC(unsigned wCN, unsigned wTN, const MappingPair &wMapping) : L1FEC()
	{
		mEncoder = new SDCCHL1Encoder(wCN, wTN, wMapping.downlink(), this);
		mDecoder = new SDCCHL1Decoder(wCN, wTN, wMapping.uplink(), this);
	}
	// string debugId() const { static string id; return id.size() ? id : (id=format("SDCCHL1FEC %s
	// ",descriptiveString())); }
};
...
public:
	CBCHL1FEC(int wCN, int wTN, const MappingPair &wMapping) : L1FEC()
	{
		mEncoder = new CBCHL1Encoder(wCN, wTN, wMapping.downlink(), this);
	}
	// string debugId() const { static string id; return id.size() ? id : (id=format("CBCHL1FEC %s
	// ",descriptiveString())); }
};
...
public:
	TCHFACCHL1FEC(unsigned wCN, unsigned wTN, const MappingPair &wMapping) : L1FEC()
	{
	...
	*/
	...
	{
		assert(mTCHDecoder);
		return mTCHDecoder->recvTCH();
	}

	unsigned queueSize() const
	{
		assert(mTCHDecoder);
		return mTCHDecoder->queueSize();
	}

	// string debugId() const { static string id; return id.size() ? id : (id=format("TCHFACCHL1FEC %s
	// ",descriptiveString())); }
};
...
public:
	SACCHL1FEC(unsigned wCN, unsigned wTN, const MappingPair &wMapping) : L1FEC()
	{
	...
	virtual const char *descriptiveString() const { return mSACCHEncoder->descriptiveString(); }
...
public:
	LoopbackL1FEC(unsigned wCN, unsigned wTN) : L1FEC()
	{
...
public:
	CCCHL1FEC(const TDMAMapping &wMapping, unsigned wTN) : L1FEC()
	{
...
public:
	FCCHL1FEC(unsigned wTN = 0) : NDCCHL1FEC() { mEncoder = new FCCHL1Encoder(this, wTN); }
};

class RACHL1FEC : public NDCCHL1FEC {

public:
	RACHL1FEC(const TDMAMapping &wMapping, unsigned wTN) : NDCCHL1FEC()
	{
...
public:
	SCHL1FEC(unsigned wTN = 0) : NDCCHL1FEC() { mEncoder = new SCHL1Encoder(this, wTN); }
};

class BCCHL1FEC : public NDCCHL1FEC {

public:
	BCCHL1FEC(unsigned wTN = 0) : NDCCHL1FEC() { mEncoder = new BCCHL1Encoder(this, wTN); }
};
