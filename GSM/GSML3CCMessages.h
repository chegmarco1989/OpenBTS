...
public:
	...
	L3CCMessage(unsigned wTI) : L3Message(), mTI(wTI) {}
...
public:
	...
	// The SS Version is variable length, which is nonsense, because it is one byte which can take the values 0
	// or 1.  Woo hoo!
...
public:
	L3Release(unsigned wTI = 7) : L3CCMessage(wTI), mHaveCause(false) {}

	L3Release(unsigned wTI, const CCCause wCause) : L3CCMessage(wTI), mHaveCause(true), mCause(wCause) {}
...
public:
	L3CCStatus(unsigned wTI = 7) : L3CCMessage(wTI) {}

	L3CCStatus(unsigned wTI, const CCCause wCause, const L3CallState &wCallState)
		: L3CCMessage(wTI), mCause(wCause), mCallState(wCallState)
	{
	}
...
public:
	L3ReleaseComplete(unsigned wTI = 7) : L3CCMessage(wTI), mHaveCause(false) {}

	L3ReleaseComplete(unsigned wTI, const CCCause wCause) : L3CCMessage(wTI), mHaveCause(true), mCause(wCause) {}
...
public:
	L3Setup(unsigned wTI = 7) : L3CCMessage(wTI) {}

	L3Setup(unsigned wTI, const L3CalledPartyBCDNumber &wCalledPartyBCDNumber)
		: L3CCMessage(wTI), mHaveCalledPartyBCDNumber(true), mCalledPartyBCDNumber(wCalledPartyBCDNumber)
	{
	}
	...
	L3Setup(unsigned wTI, const L3CallingPartyBCDNumber &wCallingPartyBCDNumber)
		: L3CCMessage(wTI), mHaveCallingPartyBCDNumber(true), mCallingPartyBCDNumber(wCallingPartyBCDNumber)
	{
	}

	void setSignal(L3Signal &whatever)
	{
		mHaveSignal = true;
		mSignal = whatever;
	}
	...
	{
		assert(mHaveCalledPartyBCDNumber);
		return mCalledPartyBCDNumber;
	}
	//@}
...
public:
	L3Alerting(unsigned wTI = 7) : L3CCMessage(wTI)
	//,mHaveProgress(false)
	{
	}

	L3Alerting(unsigned wTI, const L3ProgressIndicator &wProgress)
		: L3CCMessage(wTI), mHaveProgress(true), mProgress(wProgress)
	{
	}
...
public:
	L3Connect(unsigned wTI = 7) : L3CCMessage(wTI), mHaveProgress(false) {}
...
public:
	L3ConnectAcknowledge(unsigned wTI = 7) : L3CCMessage(wTI) {}
...
public:
	L3CallConfirmed(unsigned wTI = 7) : L3CCMessage(wTI)
	// mHaveCause(false),
...
public:
	L3StartDTMF(unsigned wTI = 7) : L3CCMessage(wTI) {}
...
public:
	L3StartDTMFAcknowledge(unsigned wTI, const L3KeypadFacility &wKey) : L3CCMessage(wTI), mKey(wKey) {}
...
public:
	...
	L3StartDTMFReject(unsigned wTI, const CCCause wCause) : L3CCMessage(wTI), mCause(wCause) {}
...
public:
	L3StopDTMF(unsigned wTI = 7) : L3CCMessage(wTI) {}
...
public:
	L3StopDTMFAcknowledge(unsigned wTI) : L3CCMessage(wTI) {}
...
public:
	L3Progress(unsigned wTI, const L3ProgressIndicator &wProgress) : L3CCMessage(wTI), mProgress(wProgress) {}

	L3Progress(unsigned wTI) : L3CCMessage(wTI) {}
...
public:
	L3Hold(unsigned wTI = 7) : L3CCMessage(wTI) {}
...
public:
	/** Reject with default cause 0x3f, "service or option not available". */
	L3HoldReject(unsigned wTI, const CCCause wCause) : L3CCMessage(wTI), mCause(wCause) {}
