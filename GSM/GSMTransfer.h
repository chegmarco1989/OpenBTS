...
enum Primitive {
	...
	L3_DATA_CONFIRM,      ///< sent Lapdm->L2 on successfull acknowledged mode delivery, but currently discarded en
			      ///< route.
	...
	L3_ESTABLISH_REQUEST, // Sent from L3 to L2 and forwarded to LAPDm, only on SAP3 for SMS,
			      // since handset always establishes SAPBM mode on host chan SAP0, and we dont use SACCH
			      // SAP0.
	...
	L3_ESTABLISH_CONFIRM,    // Sent from LAPDm to L3 on completion of L3_ESTABLISH_REQUEST.
				 ...
				 // to handle the case where SABM was sent by BTS and handset simultaneously, so we dont
				 // bother.
	...
	L3_RELEASE_CONFIRM, // Sent from LAPDm when link release is confirmed, but currently discarded en route because
			    // no one cares.
	L3_HARDRELEASE_REQUEST, // Sent from L3 to L2 when in cases where we know with certainty that the channel is
				// unused, which are: channel change, after handover, or if channel must be returned to
				// the free channel pool before being used. Message forwarded to LAPDm to tell it to
	...
	PH_CONNECT,      // sent from L1 to LAPDm via L2 when first good burst is detected on a channel.  Formerly used
			 // ESTABLISH.
	 ...
	 *	// (pat) The ESTABLISH primitive is used on both the L1<->L2 and L2<->L3 interfaces, only on XCCH
	 *channels,
	 *	// which are SDCCH, TCHFACCH and SACCH; all use L2LAPDm and XCCHEncoder/XCCHDecoder.
	 *	// The use of ESTABLISH on these two layer interfaces is not coupled, and the ESTABLISH primitive does
	 *not directly penetrate through L2LAPDm.
	 *	// We could use different primitive names for these two purposes (as per above comment).
	 *	// On L1->L2LAPDm the ESTABLISH is sent on the first good frame, and does nothing but (possibly
	 *redundant?) re-init of L2LAPDm variables.
	 *	// On L2->L1 there is code in L1 to open the channel when an ESTABLISH primitive is seen, but I dont
	 *believe this code is used.
	 *	// So ESTABLISH is only important for L2LAPDm<->L3, where the ESTABLISH primitive starts SABM [reliable
	 *transport mode].
	 *	// On SAP0 the MS always establishes SABM so in uplink the layer3 DCCHDispatch sits around and waits for
	 *it.
	 *	// On downlink, only for MT-SMS, the BTS must initiate SABM, which it does by sending an ESTABLISH to
	 *the high side of L2LAPDm. ESTABLISH,		///< L2<->L3 SABM establishment, or L1->L2 notification of first
	 *good frame.
	 *	// (pat) The RELEASE primitive is sent on L2<->L3 both ways.  On SAPI=0 the RELEASE primitive is only
	 *sent
	 ...
	 *	// (pat) In GSM 4.06 (LAPDm) the HARDRELEASE corresponds to LAPDm MDL-RELEASE (defined in 4.1.1.10
	 *described in 5.4.4.4)
...
public:
	...
	TxBurst(const Time &wTime = Time(0)) : BitVector(gSlotLen), mTime(wTime)
	{
	...
	TxBurst(const BitVector &wSig, const Time &wTime = Time(0)) : BitVector(wSig), mTime(wTime)
	{
	...
	bool operator>(const TxBurst &other) const { return mTime > other.mTime; }
...
public:
	...
	RxBurst(float *wData, const Time &wTime, float wTimingError, int wRSSI)
		: SoftVector(wData, gSlotLen), mTime(wTime), mTimingError(wTimingError), mRSSI(wRSSI)
	{
	}
	...
	// (pat) Actually, these are probably returning clones, not aliases, due to the conversion from Vector via
	// copy-constructor.
...
public:
	L2Address(unsigned wCR = 0, unsigned wSAPI = 0, unsigned wLPD = 0) : mSAPI(wSAPI), mCR(wCR), mLPD(wLPD)
	{
		assert(wSAPI < 4);
	}
...
public:
	...
	L2Header(const L2Address &wAddress, const L2Control &wControl)
		: mFormat(FmtB4), mAddress(wAddress), mControl(wControl)
	{
	}

	/** Pseudolength case, used on non-dedicated control channels. */
	L2Header(const L2Length &wLength) : mFormat(FmtBbis), mLength(wLength) {}
...
public:
...
#else
	// (pat) The default value is never used explicitly, but this is the default constructor for an unspecified
	// L2Frame constructor in descendent classes.
	...
	*/
	...
	bool DCCHIdle() const { return peekField(0, 32) == 0x0103012B; }
...
*/
class L3Frame : public BitVector { // (pat) This is in Layer3, common to UMTS and GSM and someday should move to some
				   // other directory.
...
private:
	...
	size_t mL2Length;		  ///< length, or L2 pseudo-length, as appropriate
			  // (pat) FIXME: Apparently l2length is sometimes in bits and sometimes in bytes?  (Just look
			  // at the constructors.)
	double mTimestamp; // When created.
	void f3init();

public:
	explicit L3Frame(const L3Frame &other)
		: BitVector(other), mPrimitive(other.mPrimitive), mSapi(other.mSapi), mL2Length(other.mL2Length)
	{
		f3init();
	}

	// Dont do this.  A Primitive can be converted to a size_t, so it creates ambiguities in pre-existing code.
	// explicit L3Frame(size_t bitsNeeded)
	// :BitVector(bitsNeeded),mPrimitive(DATA),mSapi(SAPI0),mL2Length(bitsNeeded/8) { f3init(); }

	explicit L3Frame(Primitive wPrimitive)
		: BitVector((size_t)0), mPrimitive(wPrimitive), mSapi(SAPI0), mL2Length(0)
	{
		f3init();
	}
	explicit L3Frame(SAPI_t wSapi, Primitive wPrimitive)
		: BitVector((size_t)0), mPrimitive(wPrimitive), mSapi(wSapi), mL2Length(0)
	{
		f3init();
	}
	...
	// (pat 11-2013) The old BitVector automatically cloned because the BitVector is declared const; now we must be
	// explicit.
	...
	// (pat) This was previously used only to concatenate BitVectors.  With lots of unneeded conversions.  Oops.  So
	// I removed it.
	...
	explicit L3Frame(SAPI_t wSapi, const BitVector &f1, const BitVector &f2) // (pat) added to replace above.
		: BitVector(f1, f2), mPrimitive(L3_DATA), mSapi(wSapi), mL2Length((f1.size() + f2.size()) / 8)
	{
		f3init();
	}
	...
	// (pat 11-2013) The old BitVector automatically cloned because the BitVector is declared const; now we must be
	// explicit.
	explicit L3Frame(SAPI_t wSapi, const L2Frame &source) : mPrimitive(L3_DATA), mSapi(wSapi), mL2Length(source.L())
	{
		f3init();
		clone(source.L3Part());
	}
	...
	// (pat) Note: This previously caused unanticipated auto-conversion from L3Message to L3Frame throughout the
	// code base.
	...
	// pat removed 9-8-2014 because it is unused.  If you put it back in, add an explicit SAPI argument to make sure
	// it is distguished from the other constructors.
	...
	bool operator>(const L3Frame &other) const
	{
		...
		if ((int)this->mSapi < (int)other.mSapi) {
			return false;
		} // SAP 0 trumps SAP 3.
		if ((int)this->mSapi > (int)other.mSapi) {
			return true;
		}					    // SAP 3 is not as good as SAP 0.
...
public:
	...
	// Load the generic data from a ByteVector (aka AudioFrame) into this object and set the AMRMode so the RTP data
	// can be decoded.
