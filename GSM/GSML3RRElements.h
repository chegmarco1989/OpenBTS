...
private:
	...
	// From GSM 5.08 section 9 table 1 we learn the value is in SACCH blocks (which are 480ms each) in the range
	// 4-64, ie 15 steps of 4 SACCH blocks each, which translated is about 2 second granularity.
...
public:
	...
	L3CellSelectionParameters() : L3ProtocolElement()
	{
		// Values dictated by the current implementation are hard-coded.
...
public:
	...
	// BS_AG_BLKS_RES is the number of CCCH NOT used for paging, so the MS does not have to bother listening to them
	// for paging.
	...
	// We are going to use the values 2..9 here, and translate it to 0..7 when writing it to the Control Channel
	// Description IE.
...
public:
	...
	L3FrequencyList(const std::vector<unsigned> &wARFCNs) : L3ProtocolElement(), mARFCNs(wARFCNs) {}
...
public:
	L3CellChannelDescription() : L3FrequencyList() {}
...
public:
	...
	L3NeighborCellsDescription(const std::vector<unsigned> &neighbors) : L3FrequencyList(neighbors) {}
...
public:
	...
	L3NCCPermitted() : L3ProtocolElement()
	{
...
// T3126 is how long the MS listens to CCCH after a RACH, and is dependent on parameters broadcast in
// L3RACHControlParameters. 4.08/24.008 says value is T+2S but no longer than 5secs, where T and S are defined in
// 44.018 3.3.1.2: T is the "Tx Integer" broadcast on BCCH (we default to 14), S is defined by table 3.3.1.1.2.1: See
// RACHSpreadSlots and RACHWaitSParam
...
public:
	...
	L3RACHControlParameters() : L3ProtocolElement()
	{
...
// If the message is sent on a paging channel, it applies to ALL MS using that paging channel, not just the addressed
// MS. GSM 4.08 3.3.2.1.1: The page mode type "Paging Reorganization" causes the MS to listen to all CCCH and BCCH
// messages until it sees a new Page Mode.
...
public:
	...
	L3PageMode(unsigned wPageMode = 0) : L3ProtocolElement(), mPageMode(wPageMode) {}
...
public:
	L3DedicatedModeOrTBF(bool forTBF, bool wDownlink)
		: L3ProtocolElement(), mDownlink(wDownlink), mTMA(0), mDMOrTBF(forTBF)
	{
	}
...
public:
	...
	L3ChannelDescription(TypeAndOffset wTypeAndOffset, unsigned wTN, unsigned wTSC, unsigned wARFCN)
		: L3ProtocolElement(), mTypeAndOffset(wTypeAndOffset), mTN(wTN), mTSC(wTSC), mHFlag(0), mARFCN(wARFCN),
		  mMAIO(0), mHSN(0)
	{
	}

	/** Blank initializer */
	L3ChannelDescription() : mTypeAndOffset(TDMA_MISC), mTN(0), mTSC(0), mHFlag(0), mARFCN(0), mMAIO(0), mHSN(0) {}
...
public:
	L3ChannelDescription2(TypeAndOffset wTypeAndOffset, unsigned wTN, unsigned wTSC, unsigned wARFCN)
...
public:
	...
	L3RequestReference(unsigned wRA, const GSM::Time &when)
		: mRA(wRA), mT1p(when.T1() % 32), mT2(when.T2()), mT3(when.T3())
	{
	}
...
public:
	L3TimingAdvance(unsigned wTimingAdvance = 0) : L3ProtocolElement(), mTimingAdvance(wTimingAdvance) {}
...
public:
	...
	L3RRCauseElement(RRCause wValue = L3RRCause::Normal_Event) : L3ProtocolElement() { mCauseValue = wValue; }
...
public:
	L3PowerCommand(unsigned wCommand = 0) : L3ProtocolElement(), mCommand(wCommand) {}
...
public:
	L3ChannelMode(Mode wMode = SignallingOnly) : L3ProtocolElement(), mMode(wMode) {}
...
public:
	...
	L3WaitIndication(unsigned seconds) : L3ProtocolElement(), mValue(seconds) {}
...
public:
	...
	L3APDUID(unsigned protocolIdentifier = 0) : L3ProtocolElement(), mProtocolIdentifier(protocolIdentifier) {}
...
public:
	...
	L3APDUFlags(unsigned cr = 0, unsigned firstSegment = 0, unsigned lastSegment = 0)
		: L3ProtocolElement(), mCR(cr), mFirstSegment(firstSegment), mLastSegment(lastSegment)
	{
	}
...
public:
	L3MeasurementResults()
		: L3ProtocolElement(), mBA_USED(false), mDTX_USED(false), mMEAS_VALID(true), // true means invalid.
		  mRXLEV_FULL_SERVING_CELL(0), mRXLEV_SUB_SERVING_CELL(0), mRXQUAL_FULL_SERVING_CELL(0),
		  mRXQUAL_SUB_SERVING_CELL(0), mNO_NCELL(0)
	{
	}
	...
	unsigned RXLEV_NCELL_RAW(unsigned i) const
	{
		assert(i < mNO_NCELL);
		return mRXLEV_NCELL[i];
	}
	unsigned RXLEV_NCELLs(unsigned *) const;
	unsigned BCCH_FREQ_NCELL(unsigned i) const
	{
		assert(i < mNO_NCELL);
		return mBCCH_FREQ_NCELL[i];
	}
	unsigned BCCH_FREQ_NCELLs(unsigned *) const;
	unsigned BSIC_NCELL(unsigned i) const
	{
		assert(i < mNO_NCELL);
		return mBSIC_NCELL[i];
	}
	...
	/**@ Converted accessors. */
	//@{
	int RXLEV_FULL_SERVING_CELL_dBm() const { return decodeLevToDBm(mRXLEV_FULL_SERVING_CELL); }
	int RXLEV_SUB_SERVING_CELL_dBm() const { return decodeLevToDBm(mRXLEV_SUB_SERVING_CELL); }
	float RXQUAL_FULL_SERVING_CELL_BER() const { return decodeQualToBER(mRXQUAL_FULL_SERVING_CELL); }
	float RXQUAL_SUB_SERVING_CELL_BER() const { return decodeQualToBER(mRXQUAL_SUB_SERVING_CELL); }
	int RXLEV_NCELL_dBm(unsigned i) const
	{
		assert(i < mNO_NCELL);
		return decodeLevToDBm(mRXLEV_NCELL[i]);
	}
	//@}
	//@}
};
...
public:
	L3CellDescription(unsigned wARFCN, unsigned wNCC, unsigned wBCC)
		: L3ProtocolElement(), mARFCN(wARFCN), mNCC(wNCC), mBCC(wBCC)
	{
	}
...
public:
	L3HandoverReference(unsigned wValue) : L3ProtocolElement(), mValue(wValue) {}
...
public:
	...
	L3CipheringModeSetting(bool wCiphering, int wAlgorithm) : mCiphering(wCiphering), mAlgorithm(wAlgorithm)
	{
...
public:
	L3CipheringModeResponse(bool wIncludeIMEISV) : mIncludeIMEISV(wIncludeIMEISV) {}
...
public:
	L3SynchronizationIndication(bool wNCI, bool wROT, int wSI = 0)
		: L3ProtocolElement(), mNCI(wNCI), mROT(wROT), mSI(wSI & 3)
	{
	}
...
public:
	L3SI13RestOctets()
		: L3RestOctets(), mRAC(gConfig.getNum("GPRS.RAC")), // (pat) aka Routing Area Code.
		  ...
		  // Update: Lets set it back to see if the MS keeps sending measurement reports when it is
		  // non-responsive. Update: If the MS is requested to make measurement reports it reduces the multislot
		  // capability.  Measurements described 45.008
	...
	{
		return 1 + 3 + 4 + 1 + 1 + 8 + 1 + 3 + 2 + mCellOptions.lengthBits() +
		       mPowerControlParameters.lengthBits();
	}
