...
L3CCMessage *GSM::L3CCFactory(L3CCMessage::MessageType MTI)
{
	switch (MTI) {
	case L3CCMessage::Connect:
		return new L3Connect();
	case L3CCMessage::Alerting:
		return new L3Alerting();
	case L3CCMessage::Setup:
		return new L3Setup();
	case L3CCMessage::EmergencySetup:
		return new L3EmergencySetup();
	case L3CCMessage::Disconnect:
		return new L3Disconnect();
	case L3CCMessage::CallProceeding:
		return new L3CallProceeding();
	case L3CCMessage::Release:
		return new L3Release();
	case L3CCMessage::ReleaseComplete:
		return new L3ReleaseComplete();
	case L3CCMessage::ConnectAcknowledge:
		return new L3ConnectAcknowledge();
	case L3CCMessage::CCStatus:
		return new L3CCStatus();
	case L3CCMessage::CallConfirmed:
		return new L3CallConfirmed();
	case L3CCMessage::StartDTMF:
		return new L3StartDTMF();
	case L3CCMessage::StopDTMF:
		return new L3StopDTMF();
	case L3CCMessage::Hold:
		return new L3Hold();
	default: {
...
size_t L3CCCommonIEs::ccCommonLength() const
{
	size_t result = 0;
	if (mHaveFacility)
		result += mFacility.lengthV();
	if (mHaveSSVersion)
		result += mSSVersion.lengthV();
	return result;
}
...
void L3CCCommonIEs::ccCommonParse(const L3Frame &src, size_t &rp)
{
	while (src.size() > rp + 8) {
		...
		switch (thisIEI) {
		case 0x1c:
			mHaveFacility = mFacility.parseTLV(0x1c, src, rp);
			continue;
		case 0x7f:
			mHaveSSVersion = mSSVersion.parseTLV(0x7f, src, rp);
			continue;
		default:
			return;
		}
...
void L3CCCommonIEs::ccCommonWrite(L3Frame &dest, size_t &wp) const
{
	...
	if (mHaveSSVersion)
		mSSVersion.writeTLV(0x7f, dest, wp);
}
...
{
	if (mHaveProgress)
		mProgress.writeTLV(0x1E, dest, wp);
...
size_t L3CallProceeding::l2BodyLength() const
{
	...
	if (mBearerCapability.mPresent)
		sum += mBearerCapability.lengthTLV();
	return sum;
}
...
{
	if (mBearerCapability.mPresent)
		mBearerCapability.writeTLV(0x04, dest, wp);
	if (mHaveProgress)
		mProgress.writeTLV(0x1E, dest, wp);
}
...
void L3Setup::writeBody(L3Frame &dest, size_t &wp) const
{
	...
	if (mHaveCallingPartyBCDNumber)
		mCallingPartyBCDNumber.writeTLV(0x5C, dest, wp);
	if (mHaveCalledPartyBCDNumber)
		mCalledPartyBCDNumber.writeTLV(0x5E, dest, wp);
	if (mSupportedCodecs.mPresent)
		mSupportedCodecs.writeTLV(0x40, dest, wp);
	if (mHaveSignal)
		mSignal.writeTV(0x34, dest, wp);
	ccCommonWrite(dest, wp);
}
...
void L3Setup::parseBody(const L3Frame &src, size_t &rp)
{
	...
	while (rp < src.size()) {
		...
		LOG(DEBUG) << "L3Setup" << LOGHEX(iei) << LOGVAR2("len", src.peekField(rp, 8)) << LOGVAR(rp)
			   << LOGVAR2("remaining", src.size() - rp);
		if ((iei & 0xf0) == 0xd0) {
			continue;
		} // Ignore repeat indicators.
		if ((iei & 0xf0) == 0x80) {
			continue;
		} // Ignore priority level.
		switch (iei) {
		...
		case 0x5c: // Calling part BCD number.
			// You may encounter this error if the code is accidentally converting between L3Frame and
			// L3Message.
		...
		case 0x2e: // (pat) Emergency category 24.008 10.5.4.33.  Not really applicable to us - it will be
			   // "manually initiated".
...
size_t L3Setup::l2BodyLength() const
{
	int len = 0;
	if (mBearerCapability.mPresent)
		len += mBearerCapability.lengthTLV();
	if (mHaveCalledPartyBCDNumber)
		len += mCalledPartyBCDNumber.lengthTLV();
	if (mHaveCallingPartyBCDNumber)
		len += mCallingPartyBCDNumber.lengthTLV();
	if (mHaveSignal)
		len += mSignal.lengthTV();
	len += ccCommonLength();
	return len;
}
...
void L3Disconnect::writeBody(L3Frame &dest, size_t &wp) const { mCause.writeLV(dest, wp); }

void L3Disconnect::parseBody(const L3Frame &src, size_t &rp) { mCause.parseLV(src, rp); }
...
size_t L3Progress::l2BodyLength() const { return mProgress.lengthLV(); }

void L3Progress::writeBody(L3Frame &dest, size_t &wp) const { mProgress.writeLV(dest, wp); }
