...
{
	// (pat 3-2014) Because the changemark was always inited to 0, if you kill OpenBTS, change something that
	// affects the beacon, and restart, the handsets do not know the beacon has changed.  We need the changemark to
	// be persistent across restarts. As a sneaky dirty way to do this, I am putting it in the config database.
	...
	if (changed) {
		if (gConfig.set("GSM.Radio.Band", band)) {
			LOG(NOTICE) << "crisis averted: automatically adjusted GSM.Radio.Band from " << changed
				    << " to " << band << " so GSM.Radio.C0 is in range";
		} else {
...
ChanType *getChan(vector<ChanType *> &chanList, bool forGprs)
{
	...
	if (gConfig.defines(configRandomize)) {
		if (forGprs) {
			...
			if (once) {
				LOG(ALERT) << "Config parameter '" << configRandomize
					   << "' is incompatible with GPRS, removed";
				once = false;
			}
	...
	for (unsigned i = 0; i < sz; i++, pos = (pos + 1) % sz) {
		...
		if (!chan->inUseByGPRS() && chan->recyclable())
			return chan;
	}
	return NULL;
}
...
template <class ChanType>
size_t chanAvailable(const vector<ChanType *> &chanList)
{
	size_t count = 0;
	for (unsigned i = 0; i < chanList.size(); i++) {
		if (chanList[i]->inUseByGPRS()) {
			continue;
		}
...
unsigned GSMConfig::SDCCHActive() const { return countActive(mSDCCHPool); }

unsigned GSMConfig::TCHActive() const { return countActive(mTCHPool); }

unsigned GSMConfig::TCHTotal() const { return countAvailable(mTCHPool); }
...
void GSMConfig::createBeacon(ARFCNManager *radio) { new BeaconC5(radio); }
