...
{
	// MCC
	if (mMCC[0] != other.mMCC[0])
		return false;
	if (mMCC[1] != other.mMCC[1])
		return false;
	if (mMCC[2] != other.mMCC[2])
		return false;
	// MNC
	if (mMNC[0] != other.mMNC[0])
		return false;
	if (mMNC[1] != other.mMNC[1])
		return false;
	if (mMNC[2] != other.mMNC[2])
		return false;
	// LAC
	if (mLAC != other.mLAC)
		return false;
...
void L3CellIdentity::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mID, 16); }

void L3CellIdentity::text(ostream &os) const { os << mID; }
...
bool L3MobileIdentity::fmidMatch(const Control::FullMobileId *mobileId) const
{
	switch (this->type()) {
	case GSM::IMSIType:
		return 0 == strcmp(mobileId->mImsi.c_str(), this->digits());
	case GSM::IMEIType:
		return 0 == strcmp(mobileId->mImei.c_str(), this->digits());
	case GSM::TMSIType:
		return mobileId->mTmsi.valid() && mobileId->mTmsi.value() == this->TMSI();
	default:
		return false; // something wrong, but certainly no match
	}
}
...
void L3CipheringKeySequenceNumber::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mCIValue, 4); }

void L3CipheringKeySequenceNumber::text(ostream &os) const { os << mCIValue; }
