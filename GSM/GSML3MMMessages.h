...
/** GSM 0.408 9.2.18. */
// Sent in response to TMSIReallocationCommand, which we dont currently use, or to LocationUpdatingAccept with a new
// TMSI.
...
public:
	L3LocationUpdatingAccept(const L3LocationAreaIdentity &wLAI, bool wFollowOnProceed = false)
		: L3MMMessage(), mLAI(wLAI), mFollowOnProceed(wFollowOnProceed), mHaveMobileIdentity(false)
	{
	}
...
public:
	...
	L3LocationUpdatingReject(const MMRejectCause cause) : L3MMMessage(), mRejectCause(cause) {}
...
public:
	...
	L3CMServiceReject(const MMRejectCause wCause) : L3MMMessage(), mCause(wCause) {}
...
public:
	L3CMServiceRequest() : L3MMMessage(), mServiceType() {}
...
public:
	L3CMReestablishmentRequest() : L3MMMessage(), mHaveLAI(false) {}
...
public:
	...
	*/
	L3MMInformation(const L3NetworkName &wShortName, const L3TimeZoneAndTime &wTime = L3TimeZoneAndTime())
		: L3MMMessage(), mShortName(wShortName), mTime(wTime)
	{
...
public:
	L3IdentityRequest(MobileIDType wType) : L3MMMessage(), mType(wType) {}
...
public:
	L3AuthenticationRequest(const L3CipheringKeySequenceNumber &wCipheringKeySequenceNumber, const L3RAND &wRAND)
		: mCipheringKeySequenceNumber(wCipheringKeySequenceNumber), mRAND(wRAND)
	{
	}
