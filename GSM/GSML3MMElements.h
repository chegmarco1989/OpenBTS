...
// (pat) This is used to identify the type of TransactionEntry, and so has been extended by Range with non-standard
// codes to identify all possible types of TransactionEntry.
...
public:
	L3CMServiceType(TypeCode wType = UndefinedType) : L3ProtocolElement(), mType(wType) {}
	...
	bool isCC() const
	{
		return mType == MobileOriginatedCall || mType == EmergencyCall || mType == MobileTerminatedCall;
	}
	...
	bool operator==(const L3CMServiceType &other) const { return mType == other.mType; }
...
public:
	...
	L3TimeZoneAndTime(const Timeval &wTime = Timeval(), TimeType type = LOCAL_TIME)
		: L3ProtocolElement(), mTime(wTime), mType(type)
	{
	}
