...
void L3RRMessage::text(ostream &os) const { os << "RR " << (MessageType)MTI() << " "; }

L3RRMessage *GSM::L3RRFactory(L3RRMessage::MessageType MTI)
{
	switch (MTI) {
	case L3RRMessage::ChannelRelease:
		return new L3ChannelRelease();
	case L3RRMessage::AssignmentComplete:
		return new L3AssignmentComplete();
	case L3RRMessage::AssignmentFailure:
		return new L3AssignmentFailure();
	case L3RRMessage::RRStatus:
		return new L3RRStatus();
	case L3RRMessage::PagingResponse:
		return new L3PagingResponse();
	case L3RRMessage::ChannelModeModifyAcknowledge:
		return new L3ChannelModeModifyAcknowledge();
	case L3RRMessage::ClassmarkChange:
		return new L3ClassmarkChange();
	case L3RRMessage::ClassmarkEnquiry:
		return new L3ClassmarkEnquiry();
	case L3RRMessage::MeasurementReport:
		return new L3MeasurementReport();
	case L3RRMessage::ApplicationInformation:
		return new L3ApplicationInformation();
	case L3RRMessage::HandoverComplete:
		return new L3HandoverComplete();
	case L3RRMessage::HandoverFailure:
		return new L3HandoverFailure();
	case L3RRMessage::CipheringModeComplete:
		return new L3CipheringModeComplete();
	case L3RRMessage::HandoverCommand:
		return new L3HandoverCommand();
	// Partial support just to get along with some phones.
	case L3RRMessage::GPRSSuspensionRequest:
		return new L3GPRSSuspensionRequest();
	default:
...
void L3PagingRequestType1::writeBody(L3Frame &dest, size_t &wp) const
{
	...
	if (sz > 1)
		mMobileIDs[1].writeTLV(0x17, dest, wp);
}
...
size_t L3PagingResponse::l2BodyLength() const { return 1 + mClassmark.lengthLV() + mMobileID.lengthLV(); }
...
size_t L3SystemInformationType4::l2BodyLength() const
{
	...
	if (mHaveCBCH)
		len += mCBCHChannelDescription.lengthTV();
	return len;
}

size_t L3SystemInformationType4::restOctetsLength() const { return mType4RestOctets.lengthV(); }
...
void L3ChannelRelease::writeBody(L3Frame &dest, size_t &wp) const { mRRCause.writeV(dest, wp); }
...
void L3AssignmentComplete::parseBody(const L3Frame &src, size_t &rp) { mCause.parseV(src, rp); }
...
void L3AssignmentFailure::parseBody(const L3Frame &src, size_t &rp) { mCause.parseV(src, rp); }
...
void L3RRStatus::parseBody(const L3Frame &src, size_t &rp) { mCause.parseV(src, rp); }
...
void L3MeasurementReport::parseBody(const L3Frame &frame, size_t &rp) { mResults.parseV(frame, rp); }
...
size_t L3ApplicationInformation::l2BodyLength() const { return 1 + mData.lengthLV(); }
...
size_t L3ClassmarkChange::l2BodyLength() const
{
	...
	if (mHaveAdditionalClassmark)
		sum += mAdditionalClassmark.lengthTLV();
	return sum;
}
...
{
	size_t sum = mCellDescription.lengthV() + mChannelDescriptionAfter.lengthV() + mHandoverReference.lengthV() +
		     mPowerCommandAccessType.lengthV() + mSynchronizationIndication.lengthV();
	return sum;
}
...
void L3HandoverComplete::parseBody(const L3Frame &frame, size_t &rp) { mCause.parseV(frame, rp); }
...
void L3HandoverFailure::parseBody(const L3Frame &frame, size_t &rp) { mCause.parseV(frame, rp); }
...
void L3CipheringModeComplete::text(ostream &os) const { L3RRMessage::text(os); }

void L3PhysicalInformation::writeBody(L3Frame &frame, size_t &wp) const { mTA.writeV(frame, wp); }
