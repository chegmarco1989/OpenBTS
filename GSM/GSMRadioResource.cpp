...
ChannelType decodeChannelNeeded(unsigned RA)
{
	...
	if (RA5 == 0x05)
		return TCHFType; // emergency call
	...
	if (RA5 == 0x04)
		return TCHFType; // any channel or any TCH.
	if (RA4 == 0x01)
		return SDCCHType; // SDCCH
	if (RA4 == 0x02)
		return TCHFType; // TCH/F
	if (RA4 == 0x03)
		return TCHFType; // TCH/F
	if ((RA & 0xf8) == 0x78 && RA != 0x7f)
		return PSingleBlock1PhaseType;
	if ((RA & 0xf8) == 0x70)
		return PSingleBlock2PhaseType;

	int NECI = gConfig.getNum("GSM.CellSelection.NECI");
	if (NECI == 0) {
		if (RA5 == 0x07)
			return SDCCHType; // MOC or SDCCH procedures
		if (RA5 == 0x00)
			return SDCCHType; // location updating
	} else {
		if (gConfig.getBool("Control.VEA")) {
			// Very Early Assignment
			if (RA5 == 0x07)
				return TCHFType; // MOC for TCH/F
			if (RA4 == 0x04)
				return TCHFType; // MOC, TCH/H sufficient
		} else {
			// Early Assignment
			if (RA5 == 0x07)
				return SDCCHType; // MOC for TCH/F
			if (RA4 == 0x04)
				return SDCCHType; // MOC, TCH/H sufficient
		}
		if (RA4 == 0x00)
			return SDCCHType; // location updating
		if (RA4 == 0x01)
			return SDCCHType; // other procedures on SDCCH
	}
...
static unsigned createFakeRachRA(FakeRachType rachtype)
{
	...
	default:
		devassert(0);
	case FakeRachTCH:
		return 0xe0 | (0x1f & random()); // top 3 bits are 0x7.
	case FakeRachSDCCH:
		return 0x10 | (0x0f & random()); // top 4 bits are 0001
	case FakeRachLUR:
		return 0x00 | (0x0f & random()); // top 4 bits are 0000
	case FakeRachGPRS:
		return 0x70 | (0x7 & random()); // top 5 bits are 01110.
	case FakeRachAnswerToPaging:
		return 0x80 | (0x1f & random()); // top 3 bits are 100.
	}
}

FakeRachType fakeRachTypeTranslate(const char *name)
{
	if (strcasestr(name, "tch"))
		return FakeRachTCH;
	if (strcasestr(name, "sdcch"))
		return FakeRachSDCCH;
	if (strcasestr(name, "lur"))
		return FakeRachLUR;
	if (strcasestr(name, "gprs"))
		return FakeRachGPRS;
	if (strcasestr(name, "pag"))
		return FakeRachAnswerToPaging;
...
std::ostream &operator<<(std::ostream &os, const RachInfo &rach)
{
	...
	os << LOGVAR(chtype) << LOGVAR2("lur", requestingLUR(RA)) << LOGHEX(RA) << LOGVAR2("TE", rach.initialTA())
	   << LOGVAR2("RSSI", rach.RSSI()) << LOGVAR2("when", rach.mWhen) << LOGVAR(rach.mTN);
	return os;
}
