...
L3ControlChannelDescription::L3ControlChannelDescription() : L3ProtocolElement()
{
	...
	mBS_PA_MFRMS = gConfig.getNum(
		"GSM.CCCH.BS_PA_MFRMS"); // Number of paging multiframes.  Default is 2, the minimum allowed value.
	// Configurable values.
...
unsigned countBeaconTimeslots(int ccch_conf)
{
	...
	default:
		return 1;
	case 2:
		return 2; // Timeslots C0 and C2 with 9 CCCH apiece.
	case 4:
		return 3; // Timeslots C0, C2, C4 with 9 CCCH apiece.
	case 6:
		return 4; // Timeslots C0, C2, C4, C6 with 9 CCCH apiece.
	}
}
...
void L3FrequencyList::writeV(L3Frame &dest, size_t &wp) const
{
	...
	if (numBits < delta) {
		LOG(ALERT) << "L3FrequencyList cannot encode full ARFCN set, base=" << baseARFCN << " delta=" << delta;
	}
	for (unsigned i = 0; i < numBits; i++) {
		unsigned thisARFCN = baseARFCN + 1 + i;
		if (contains(thisARFCN))
...
void L3NCCPermitted::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mPermitted, 8); }

void L3NCCPermitted::text(ostream &os) const { os << hex << "0x" << mPermitted << dec; }
...
void L3PageMode::text(ostream &os) const { os << mPageMode; }
...
void L3TimingAdvance::text(ostream &os) const { os << mTimingAdvance; }

void L3RRCauseElement::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mCauseValue, 8); }

void L3RRCauseElement::parseV(const L3Frame &src, size_t &rp) { mCauseValue = (RRCause)src.readField(rp, 8); }

void L3RRCauseElement::text(ostream &os) const { os << "0x" << hex << mCauseValue << dec; }
...
void L3PowerCommand::text(ostream &os) const { os << mCommand; }

void L3ChannelMode::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mMode, 8); }

void L3ChannelMode::parseV(const L3Frame &src, size_t &rp) { mMode = (Mode)src.readField(rp, 8); }
...
void L3MultiRateConfiguration::text(std::ostream &os) const
{
	char name[20];
	switch (mAmrCodecSet) {
	case codec_set_AMR_FR:
		strcpy(name, "AMR_FR");
		break;
	case codec_set_AMR_HR:
		strcpy(name, "AMR_HR");
		break;
	case codec_set_UMTS_AMR:
		strcpy(name, "UMTS_AMR");
		break;
	default:
		sprintf(name, "0x%x", mAmrCodecSet);
		break;
	}
...
{
	switch (mode) {
	case L3ChannelMode::SignallingOnly:
		os << "signalling";
		break;
	case L3ChannelMode::SpeechV1:
		os << "speech1";
		break;
	case L3ChannelMode::SpeechV2:
		os << "speech2";
		break;
	case L3ChannelMode::SpeechV3:
		os << "speech3";
		break;
	default:
		os << "?" << (int)mode << "?";
...
void L3ChannelMode::text(ostream &os) const { os << mMode; }
...
void L3APDUID::text(ostream &os) const { os << mProtocolIdentifier; }
...
void L3APDUFlags::text(ostream &os) const { os << mCR << "," << mFirstSegment << "," << mLastSegment; }

// APDU Data

L3APDUData::~L3APDUData() {}

L3APDUData::L3APDUData() : L3ProtocolElement() {}

L3APDUData::L3APDUData(BitVector2 data) : L3ProtocolElement(), mData(data) {}
...
L3SI3RestOctets::L3SI3RestOctets()
	: L3RestOctets(), mHaveSI3RestOctets(false), mHaveSelectionParameters(false), mCBQ(0), mCELL_RESELECT_OFFSET(0),
	  mTEMPORARY_OFFSET(0), mPENALTY_TIME(0), mRA_COLOUR(0), mHaveGPRS(false)
{
	...
	// (pat 2-2014) We dont advertise GPRS service in the beacon until the BTS Macro state indicates that the GSM
	// registration period is complete.
...
size_t L3SI3RestOctets::lengthV() const
{
	...
	if (mHaveGPRS)
		sumBits += 1    // H for GPRS Indicator
...
void L3HandoverReference::writeV(L3Frame &frame, size_t &wp) const { frame.writeField(wp, mValue, 8); }

size_t L3CipheringModeSetting::lengthV() const { return 0; }
...
size_t L3CipheringModeResponse::lengthV() const { return 0; }
...
void L3CipheringModeResponse::text(ostream &os) const { os << "includeIMEISV=" << mIncludeIMEISV; }

void L3HandoverReference::text(ostream &os) const { os << "value=" << mValue; }
