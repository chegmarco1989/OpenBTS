...
NeighborHistory &ChannelHistory::getNeighborData(unsigned arfcn, unsigned BSIC)
{
	...
	if ((it = mNeighborData.find(key)) == mNeighborData.end()) {
		// Create a new entry.  You would think this could be easier, but not if you want to avoid the extra
		// unnecessary construction. C++11 adds emplace but we cant use that yet.
		...
		// std::pair<NeighborMap::iterator,bool> blah = mNeighborData.insert(
		// NeighborMap::value_type(key,newone) );
...
void NeighborHistory::nhAddPoint(NCellPoint &npt, FrameNum now)
{
	...
	while (nhList.size()) {
		...
		if (FNDelta(now, bk.ncpFrame) > maxage) {
			nhList.pop_back();
			continue;
		}
		break;
...
void ChannelHistory::chAddPoint(SCellPoint &spt, FrameNum now)
{
	...
	int maxlen = gConfig.GSM.Handover.History.Max;
	while ((int)mSCellData.size() > maxlen) {
		mSCellData.pop_back();
	}
	// Throw away points that are too old.
	int maxage = (1 + maxlen) * 2 * 52; // Each report requires 2 * 52-multiframes, 480ms.
	while (mSCellData.size()) {
		...
		if (FNDelta(now, bk.scpFrame) > maxage) {
			mSCellData.pop_back();
			continue;
		}
		break;
...
Control::BestNeighbor ChannelHistory::neighborFindBest(Control::NeighborPenalty penalty)
{
	...
	for (NeighborMap::iterator it = mNeighborData.begin(); it != mNeighborData.end(); it++) {
		...
		LOG(DEBUG) << LOGVAR(nh.nhBSIC) << LOGVAR(nh.nhARFCN) << LOGVAR(nh.nhConsecutiveCount)
			   << LOGVAR(nh.nhTimestamp);
		...
		if (!best.mValid || thisRxlev > best.mRxlev) {
			...
			LOG(DEBUG) << "found:" << LOGVAR(bestkey) << LOGVAR(best.mARFCN) << LOGVAR(best.mBSIC)
				   << LOGVAR(thisRxlev) << LOGVAR(best.mRxlev);
		}
	}
	return best;
}
...
void ChannelHistory::neighborAddMeasurement(FrameNum when, unsigned arfcn, unsigned BSIC, int rxlevdb)
{
	...
	LOG(DEBUG) << LOGVAR(arfcn) << LOGVAR(BSIC) << LOGVAR(nh.nhTimestamp) << LOGVAR(this->mReportTimestamp)
		   << LOGVAR(isConsecutive);
	if (isConsecutive) {
