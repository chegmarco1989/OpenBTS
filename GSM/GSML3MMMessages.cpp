...
L3MMMessage *GSM::L3MMFactory(L3MMMessage::MessageType MTI)
{
	switch (MTI) {
	case L3MMMessage::LocationUpdatingRequest:
		return new L3LocationUpdatingRequest;
	case L3MMMessage::IMSIDetachIndication:
		return new L3IMSIDetachIndication;
	case L3MMMessage::CMServiceAbort:
		return new L3CMServiceAbort;
	case L3MMMessage::CMServiceRequest:
		return new L3CMServiceRequest;
	...
	case L3MMMessage::MMStatus:
		return new L3MMStatus;
	case L3MMMessage::IdentityResponse:
		return new L3IdentityResponse;
	case L3MMMessage::AuthenticationResponse:
		return new L3AuthenticationResponse;
	case L3MMMessage::TMSIReallocationComplete:
		return new L3TMSIReallocationComplete;
	default:
...
void L3MMMessage::text(ostream &os) const { os << "MM " << (MessageType)MTI() << " "; }
...
size_t L3LocationUpdatingAccept::l2BodyLength() const
{
	...
	if (mHaveMobileIdentity)
		result += mMobileIdentity.lengthTLV();
	if (mFollowOnProceed)
		result += 1;
	return result;
}
...
void L3LocationUpdatingAccept::writeBody(L3Frame &dest, size_t &wp) const
{
	mLAI.writeV(dest, wp);
	if (mHaveMobileIdentity)
		mMobileIdentity.writeTLV(0x17, dest, wp);
	if (mFollowOnProceed)
		dest.writeField(wp, 0xa1, 8); // IEI is A1
}
...
void L3LocationUpdatingAccept::text(ostream &os) const
{
	...
	os << "LAI=(" << mLAI << ")";
	if (mHaveMobileIdentity)
		os << "ID=(" << mMobileIdentity << ")";
}

void L3LocationUpdatingReject::writeBody(L3Frame &dest, size_t &wp) const { mRejectCause.writeV(dest, wp); }
...
void L3TMSIReallocationComplete::text(ostream &os) const { L3MMMessage::text(os); }
...
void L3CMServiceReject::writeBody(L3Frame &dest, size_t &wp) const { mCause.writeV(dest, wp); }
...
void L3MMStatus::parseBody(const L3Frame &src, size_t &rp) { mRejectCause.parseV(src, rp); }
...
{
	if (mShortName.lengthV() > 1)
		mShortName.writeTLV(0x45, dest, wp);
