...
// (pat) This class is a GSM-specific dedicated logical channel, meaning that at any given time the channel is connected
// to just one MS. GPRS and UMTS do not use dedicated logical channels, rather they use shared resources in layer 2, and
// in fact they dont even really have a logical channel type entity in layer 2. Therefore I split this class into L2 and
// L3 portions, where the L3LogicalChannel is common with UMTS and managed in the Control directory. This
// L2LogicalChannel is rarely referenced outside this directory; primarily just to send L3 level handover information to
// a Peer.
...
public:
	...
	*/
	void l2sendp(const GSM::Primitive &prim, SAPI_t SAPI = SAPI0) { l2sendf(L3Frame(SAPI, prim)); }
...
public:
	// Over-ridden in L2LogicalChannel and SACCHLogicalChannel to handle uplink messages, setting the timers on the
	// main channel. The other channel types do not have an uplink so they over-ride this with an devassert(0). Yes
	// it might be better to have separate classes for these variations.
	...
	{
		devassert(mL1);
		return mL1->handoverPending(flag, handoverRef);
	}
	//@}
	...
	/**
		...
		(pat) LogicalChannel::lcopen() calls: L1FEC::open(), L1Encoder::open(), L1Encoder::open(), none of which
	   do much but reset the L1 layer classes. If there is an associated SACCH, that is opened too. On channels with
	   LAPDm, which are: TCHFACCH, SDCCH and SACCH: LogicalChannel::lcopen() also calls L2LAPDm::l2open() on each
	   SAP endpoint, which has a side effect of starting to send idle frames in downlink. After open, an ESTABLISH
	   primitive may be sent on the channel to indicate when SABM mode is established. In downlink: only for MT-SMS,
	   an ESTABLISH primitive is sent to establish LAPDm SABM mode, which is used only on SAP 3, which is used only
	   for SMS messages in OpenBTS. In uplink: the MS always establishes SABM mode.  After the lcopen(), when the
	   first good frame arrives, an ESTABLISH primitive is sent upstream toward L3, which will notify the
	   DCCHDispatcher to start looking for messages.

		(pat) 4-28-2014: Did a major overhaul of channel initialization.  Formerly lcopen was called directly
	   from getTCH (except for GPRS channels), then some callers initialized SACCH, which may already be
	   transmitting; old code also had a nasty side effect that getTCH blocked. The phy parameters in the SACCH must
	   be inited before it starts transmitting or some handsets reject the channel. This code is also used during
	   channel initialization when no phy parameters are known, eg in GSMConfig. So the former one step lcopen is
	   replaced with three steps:
		...
		The lcopen() method still exists for those callers who really dont care about phy parameters, which
	   should only be GSMConfig for the very initial channel creation when OpenBTS fires up, which should only be on
	   ARFCN C0, which must be constantly transmitting on all timeslots.
		...
		GSM 4.08 3.4.13.2: RR connection loss is communicated directly from layer1 to layer3, which is why there
	   is no "PHY-ERROR" primitive in LAPDm specs.  Layer3 is supposed to release the channel, which means stop
	   transmitting on SACCH and then wait for T3109 to elapse before marking the channel available for reuse.
	   OpenBTS totally botched this previously. Note that it is common for the handset to be able to hear BTS even
	   though the BTS cannot hear the handset. So note that when we detect loss of SACCH measurement reports we must
	   first wait by SACCH loss for a network-determined period of time, but probably 30 secs like T3109, then
	   layer3 is supposed to send a channel release, deactivate SACCH and wait another T3109 period before reusing
	   the channel.
	*/
...
public:
	...
	void l2sendm(const L3Message &msg, GSM::Primitive prim = L3_DATA, SAPI_t SAPI = SAPI0)
	{
		L2LogicalChannelBase::l2sendm(msg, prim, SAPI);
	}
	bool multiframeMode(SAPI_t SAPI) const;
...
// (pat) The operator<< for L2LogicalChannel should not be necessary but g++ prints warnings in other directories if
// missing.  Compiler bug?
...
public:
	SDCCHLogicalChannel(unsigned wCN, unsigned wTN, const CompleteMapping &wMapping);
...
public:
	...
	void l2sendm(const L3RRMessage &msg) { L2LogicalChannelBase::l2sendm((const L3Message &)msg, L3_UNIT_DATA); }
...
class SACCHLogicalChannel : public L2SAPMux, public ChannelHistory {
	...
	InterthreadQueue<L3Message> mTxQueue; // FIXME: not currently used. Queue of outbound messages from Layer3 for
					      // this SACCH.  SAPI is determined from message PD.
...
protected:
	...
	// The short-term variation can last up to 3 consecutive reports, so we want to average over a long enough
	// period to smooth that out.  Reports come every 1/2 second so we can make the averaging period pretty large.
	// Since this value is used only for handover, we dont have to worry about making the value correct during the
	// first few, in fact, we dont want a handover to happen too soon after the channel is opened anyway, so we will
	// just init it to 0 when the channel is opened and let it drift down. (pat 1-2014) GSM 5.08 A3.1 says how we
	// are supposed to average this; we are supposed to throw out the best and worst measurements and average over a
	// programmable period. Note that this averaging puts a constraint on the maximum speed of the handset through
	// the overlap area between cells for a successful handover.  To improve handover for quickly moving handsets we
	// should also watch delta(RXLEV) and delta(TA) and if they together indicate quickly moving out of the cell, do
	// the handover faster.
	// static const int cAveragePeriodRXLEV_SUB_SERVING_CELL = 8; // How many we measurement reports we average
	// over.  float mAverageRXLEV_SUB_SERVICING_CELL;		// Must be signed!
...
public:
	...
	void RSSIBumpDown(int dB)
	{
		devassert(mL1);
		mSACCHL1->RSSIBumpDown(dB);
	}
...
public:
	TCHFACCHLogicalChannel(unsigned wCN, unsigned wTN, const CompleteMapping &wMapping);
	...
	{
		devassert(mTCHL1);
		mTCHL1->sendTCH(frame);
	}

	SIP::AudioFrame *recvTCH()
	{
		devassert(mTCHL1);
		return mTCHL1->recvTCH();
	}
...
public:
	...
	L3Frame *l2recv(unsigned, unsigned)
	{
		devassert(0);
		return NULL;
	}
...
public:
	...
	void l2sendf(const L3Frame &frame) { mL3Q[frame.getSAPI()].write(new L3Frame(frame)); }

	/** L3 Loopback */
	L3Frame *l2recv(unsigned timeout_ms = 15000, unsigned SAPI = 0) { return mL3Q[SAPI].read(timeout_ms); }
};

#if WHATISTHIS
class SDCCHLogicalChannel_LB : public SDCCHLogicalChannel {
public:
	SDCCHLogicalChannel_LB(unsigned wCN, unsigned wTN, const CompleteMapping &wMapping);
};

class TCHFACCHLogicalChannel_UPLINK : public TCHFACCHLogicalChannel {
public:
	/** Custom constructor, L2 is Uplink instead of downlink. */
	TCHFACCHLogicalChannel_UPLINK(unsigned wCN, unsigned wTN, const CompleteMapping &wMapping);
};
#endif
