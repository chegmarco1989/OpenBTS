...
inline void sleepFrames(unsigned frames) { usleep(frames * gFrameMicroseconds); }

/** Sleep for 1 GSM frame period. */
inline void sleepFrame() { usleep(gFrameMicroseconds); }
...
enum GSMAlphabet { ALPHABET_7BIT, ALPHABET_8BIT, ALPHABET_UCS2 };
...
*/
static const unsigned char gGSMAlphabet[] =
	"@\243$\245\350\351\371\354\362\347\n\330\370\r\305\345D_FGLOPCSTZ \306\346\337\311 "
	"!\"#\244%&\'()*+,-./"
	"0123456789:;<=>?"
	"\241ABCDEFGHIJKLMNOPQRSTUVWXYZ\304\326\321\334\247\277abcdefghijklmnopqrstuvwxyz\344\366\361\374\341";
...
enum MobileIDType { NoIDType = 0, IMSIType = 1, IMEIType = 2, IMEISVType = 3, TMSIType = 4 };
...
public:
	...
	Time operator-(int step) const { return operator+(-step); }
	...
	int operator-(const Time &other) const { return FNDelta(mFN, other.mFN); }
	...
	bool operator==(const Time &other) const { return (mFN == other.mFN) && (mTN == other.mTN); }
...
public:
	Clock(const Time &when = Time(0)) : mBaseFN(when.FN()) {}
...
public:
	...
	Z100Timer(long wLimitTime) : mLimitTime(wLimitTime), mActive(false) {}
