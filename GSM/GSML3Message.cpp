...
string GSM::mti2string(L3PD pd, unsigned mti)
{
	...
	case L3CallControlPD:
		operator<<(result, (L3CCMessage::MessageType)mti);
		return result.str();
	// case L3PDSS2PD: break;
	case L3MobilityManagementPD:
		operator<<(result, (L3MMMessage::MessageType)mti);
		return result.str();
	case L3RadioResourcePD:
		operator<<(result, (L3RRMessage::MessageType)mti);
		return result.str();
	case L3GPRSMobilityManagementPD:
		operator<<(result, (SGSN::L3GmmMsg::MessageType)mti);
		return result.str();
	case L3SMSPD:
		operator<<(result, (SMS::CPMessage::MessageType)mti);
		return result.str();
	case L3GPRSSessionManagementPD:
		operator<<(result, (SGSN::L3SmMsg::MessageType)mti);
		return result.str();
	case L3NonCallSSPD:
		operator<<(result, (L3SupServMessage::MessageType)mti);
		return result.str();
...
GSM::L3Message *GSM::parseL3(const GSM::L3Frame &source)
{
	...
	try {
		switch (PD) {
		case L3RadioResourcePD:
			retVal = parseL3RR(source);
			break;
		case L3MobilityManagementPD:
			retVal = parseL3MM(source);
			break;
		case L3CallControlPD:
			retVal = parseL3CC(source);
			break;
		case L3SMSPD:
			retVal = SMS::parseSMS(source);
			break;
		case L3NonCallSSPD:
			retVal = parseL3SupServ(source);
			break;
		default:
			LOG(NOTICE) << "L3 parsing failed for unsupported protocol " << PD;
			return NULL;
		}
	} catch (...) { // (pat) Used to just catch L3ReadError, but lets be safer.  Specifically, need to catch
			// SMSReadError also.
