...
// Semaphore works now and reduces cpu% on idle SACCH from 0.3 to negligible, but the thread start/stop logic is pretty
// weird and I am not sure all the cases are handled, so dont enable this.
...
void L2SAPMux::flushL3In()
{
	while (L3Frame *l3f = mL3In.readNoBlock()) {
		if (l3f->primitive() != L3_RELEASE_REQUEST && l3f->primitive() != L3_HARDRELEASE_REQUEST) {
			LOG(ERR) << "channel closure caused message to be discarded:" << l3f << " "
				 << descriptiveString();
		}
		delete l3f;
	}
...
void L2SAPMux::sapStart()
{
	...
	for (int s = 0; s < 4; s++) {
		...
		if (mL2[s])
			mL2[s]->l2dlOpen(descriptiveString());
	}
}
...
void L2SAPMux::sapWriteFromL1(const L2Frame &frame)
{
	...
	case PH_CONNECT:
		...
		for (int s = 0; s <= 3; s++) {
			if (mL2[s])
				mL2[s]->l2dlWriteLowSide(
					frame); // Note: the frame may have the wrong SAP in it, but LAPDm doesnt care.
		}
		return;
	default:
...
LAPDState L2SAPMux::getLapdmState(SAPI_t sap) const
{
	return mL2[SAP2SAPI(sap)] ? mL2[SAP2SAPI(sap)]->getLapdmState() : LAPDStateUnused;
}
...
L3Frame *L2LogicalChannel::l2recv(unsigned timeout_ms)
{
	...
	if (result)
		WATCHINFO("l2recv " << this << LOGVAR2("sap", result->getSAPI()) << LOGVAR(result));
	return result;
}
...
void SACCHLogicalChannel::writeToL3(L3Frame *frame)
{
	...
	case L3_DATA:
	case L3_UNIT_DATA:
		if (processMeasurementReport(frame)) {
			return;
		}
		// Fall Through...
	case L3_ESTABLISH_CONFIRM:
	case L3_ESTABLISH_INDICATION:
		...
		mHost->writeToL3(frame); // TODO: frame should include channel indicator, but l3 doesnt care.  Would be
					 // nice for debugging.
		return;
	case L3_RELEASE_INDICATION:
	case MDL_ERROR_INDICATION:
	...
	case L3_DATA_CONFIRM:    // Sent from LAPDm when data delivered, but we dont care.
	case L3_RELEASE_CONFIRM: // Sent from LAPDm when link release is confirmed, but we dont care.
		delete frame; // We dont do anything with this; we are releasing regardless of whether we get a confirm
			      // or not.
		return;
	default:
		assert(0);
...
// There is no close in L2 - the L1 encoder/decoder are closed individually when L2 sends a RELEASE/HARDRELEASE
// primitive.
...
bool L2LogicalChannel::recyclable()
{
	...
	return (mT3101.expired() || mT3109.expired() || mT3111.expired()) && mL1->encoder()->l1IsIdle() &&
	       getSACCH()->mL1->encoder()->l1IsIdle();
}
...
void *L2LogicalChannel::ControlServiceLoop(L2LogicalChannel *hostchan)
{
	while (!gBTS.btsShutdown()) {
		...
		// The SACCH is deactivated before the host channel, so we check l1active to see if SACCH is still
		// running.
...
bool L2LogicalChannel::radioFailure() const { return !l1active() || getSACCH()->sacchRadioFailure(); }
...
//		but there is no guarantee that l3 message got through yet, so we have to wait for the L2 LAPDm DISC
// indication 		before we can switch to T3111.
//		GSM 4.08 3.4.13.1.1 NOTE 1: Layer3 Channel Release on the main signaling link causes all other signaling
// links 		to be terminated by "local end release" which 4.06 5.4.4 defines as immediately entering idle
// state, wherein 		LAPDm can respond to handshakes but send nothing new.
//		Also 24.011 (SMS) 2.3: SAP3 may be released explicitly (LAPDm DISC) or implicitly by channel release.
//		I infer, therefore, that is the job of Layer3 to make that sure that the other links (main-SAP3 and
// SACCH-SAP0,SAP3) are
...
//		I would think so, because otherwise the handset could initiate a new multi-frame-mode command while the
// ChannelRelease 		from the BTS is in flight on the main link.
...
L3ChannelDescription L2LogicalChannelBase::channelDescription() const
{
	...
	if (mL1 == NULL)
		return L3ChannelDescription(TDMA_MISC, 0, 0, 0);

	// In normal cases, we get this information from L1.
	return L3ChannelDescription(mL1->typeAndOffset(), mL1->TN(), mL1->TSC(), mL1->ARFCN());
}

ChannelHistory *L2LogicalChannel::getChannelHistory() { return mSACCH ? mSACCH->getChannelHistory() : NULL; }
...
string L2LogicalChannel::displayTimers() const
{
	...
	if (mSACCH) {
		// The only thing we want to display from L1 is the bad-frame-tracker variable, which is only useful in
		// SACCH.
...
SDCCHLogicalChannel::SDCCHLogicalChannel(unsigned wCN, unsigned wTN, const CompleteMapping &wMapping)
{
...
void SACCHLogicalChannel::sacchInit()
{
	...
	while (L3Message *straymsg = mTxQueue.readNoBlock()) {
		delete straymsg;
	}
...
// Routine relies on l2sendf passing directly through LAPDm and going all the way to L1Encoder::transmit, which calls
// waitToSend.
void SACCHLogicalChannel::serviceSACCH(unsigned &count)
{
	...
	if (L3Frame *l3fp = mL3In.readNoBlock()) {
		// We are writing on SAPI0 (instead of SAPI0Sacch), which is ok.  At the SAP layer it is just SAPI0 or
		// SAPI3
		sapWriteFromL3(*l3fp);
		delete l3fp;
	} else {
...
void *SACCHLogicalChannel::SACCHServiceLoop(SACCHLogicalChannel *sacch)
{
	...
	while (!gBTS.btsShutdown()) {
		...
		// sleepFrames(51);
			...
			int sval, semstat = sem_getvalue(&mOpenSignal, &sval);
			// cout << descriptiveString() << " WAIT " <<sem_getvalue(&mOpenSignal,&sval) <<LOGVAR(sval)
			// <<endl;
			...
			LOG(DEBUG) << descriptiveString() << " SEM_AFTER " << sem_getvalue(&mOpenSignal, &sval)
				   << LOGVAR(sval);
#else
			...
			// TODO: Can use fewer CPU cycles by using something like waitToSend to sleep until the next
			// SACCH transmission time.
			sleepFrames(1);
#endif
		...
		// until the first L3 message arrives.  If the channel fails without sending an L3 message we would
		// never notice unless we are watching these RR level timers. Close the host channel, which will also
		// close this SACCH.
...
{
	assert(mSACCH);
	mSACCH->l1InitPhy(wRSSI, wTimingError, wTimestamp);
}
void L2LogicalChannel::setPhy(const L2LogicalChannel &other)
{
	assert(mSACCH);
	mSACCH->setPhy(*other.mSACCH);
}
MSPhysReportInfo *L2LogicalChannel::getPhysInfo() const
{
	assert(mSACCH);
	return mSACCH->getPhysInfo();
}
const L3MeasurementResults &L2LogicalChannel::measurementResults() const
{
	assert(mSACCH);
	return mSACCH->measurementResults();
}

TCHFACCHLogicalChannel::TCHFACCHLogicalChannel(unsigned wCN, unsigned wTN, const CompleteMapping &wMapping)
{
