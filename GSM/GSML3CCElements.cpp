...
// Using 24.008 5.4.5 table 10.5.103 interpolated via meanings in 48.008 3.2.2.11 into 26.103 6.2.  Gotta love this
// stuff.
CodecType L3BearerCapability::getCodecType(unsigned n) const
{
	...
	case 11:
		return OHR_AMR;
	case 15:
		return CodecTypeUndefined; // "No speech version supported for GERAN."
	default:
		return CodecTypeUndefined; // undefined
	}
}
...
void L3SupportedCodecList::parseV(const L3Frame &src, size_t &rp, size_t expectedLength)
{
	...
	while (expectedLength >= 3) {
		...
		if (fixedLength >= 2)
			codeclist |= (src.readField(rp, 8) << 8);
		switch (sysid) {
		case SysIdGSM:
			mGsmPresent = true;
...
/*
 * If digit string starts with a plus strip off the plus. I suspect that this get encoded as an international type
 * somewhere else The write function send digits/information and the parse function decodes and store digits/incomming
 * information. SVG
 */
void L3BCDDigits::write(L3Frame &dest, size_t &wp) const
{
	...
	while (index < numDigits) {
		if ((index + 1) < numDigits)
			dest.writeField(wp, encode(mDigits[index + 1], &invalid), 4);
		else
	...
	if (invalid) {
		LOG(ERR) << "Invalid BCD string: '" << mDigits << "'";
	}
}
...
{
	dest.writeField(wp, 0x01, 1); // dest.writeField(wp, *digits() == '+' ? InternationalNumber : mType, 3); //
...
size_t L3CallingPartyBCDNumber::lengthV() const { return 1 + mHaveOctet3a + mDigits.lengthV(); }
...
void L3CauseElement::parseV(const L3Frame &src, size_t &rp, size_t expectedLength)
{
	...
	if (src.readField(pos, 4) != 0x0e)
		L3_READ_ERROR;
	mLocation = (Location)src.readField(pos, 4);

	// Octet 4
	if (src.readField(pos, 1) != 1)
		L3_READ_ERROR;
...
void L3CallState::text(ostream &os) const { os << mCallState; }
...
void L3KeypadFacility::parseV(const L3Frame &src, size_t &rp) { mIA5 = src.readField(rp, 8); }

void L3KeypadFacility::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mIA5, 8); }

void L3KeypadFacility::text(ostream &os) const { os << hex << "0x" << mIA5 << dec; }

// We shouldnt ever read in a Signal IE, but here it is anyway.
void L3Signal::parseV(const L3Frame &src, size_t &rp) { mSignalValue = src.readField(rp, 8); }

void L3Signal::writeV(L3Frame &dest, size_t &wp) const { dest.writeField(wp, mSignalValue, 8); }

void L3Signal::text(ostream &os) const { os << format("Signal(0x%x)", mSignalValue); }
