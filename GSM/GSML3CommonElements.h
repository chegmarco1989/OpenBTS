...
public:
	L3CellIdentity(unsigned wID = gConfig.getNum("GSM.Identity.CI")) : mID(wID) {}
...
public:
	/** Empty ID */
	L3MobileIdentity() : L3ProtocolElement(), mType(NoIDType) { mDigits[0] = '\0'; }

	/** TMSI initializer. */
	L3MobileIdentity(unsigned int wTMSI) : L3ProtocolElement(), mType(TMSIType), mTMSI(wTMSI) { mDigits[0] = '\0'; }

	/** IMSI initializer. */
	L3MobileIdentity(const char *wDigits) : L3ProtocolElement(), mType(IMSIType)
	{
	...
	{
		assert(mType != TMSIType);
		return mDigits;
	}
	unsigned int TMSI() const
	{
		assert(mType == TMSIType);
		return mTMSI;
	}
...
struct EncryptionAlgorithm {
	...
	enum { Bit5_1 = 1, Bit5_2 = 2, Bit5_3 = 4 };
};
/**
...
public:
	...
	int getA5Bits() const
	{
		int result = 0;
		if (A5_1()) {
			result |= EncryptionAlgorithm::Bit5_1;
		}
		if (A5_2()) {
			result |= EncryptionAlgorithm::Bit5_2;
		}
		if (A5_3()) {
			result |= EncryptionAlgorithm::Bit5_3;
		}
		return result;
	}
...
public:
	L3MobileStationClassmark3() : mA5_4(0), mA5_5(0), mA5_6(0), mA5_7(0) {}
...
public:
	L3CipheringKeySequenceNumber(unsigned wCIValue) : mCIValue(wCIValue) {}
