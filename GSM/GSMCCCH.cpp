...
public:
	int *miqLoadPointer; // If the descendent class sets this, it will be maintained with the number of elements in
			     // the queue.
	...
	ETP itRemove(ETP thisone)
	{ // The argument is redundant but helps check compliance with list semantics.
		LOG(DEBUG);
		ScopedLock lock(miqLock);
...
// Global linkage:
int getPCHLoad() { return gPagingQ.getPagingLoad(); }
...
struct RachCompareAdapter {
	...
	// meaning that rach2 will be serviced before rach1, since the stupid C++ priority_queue pops from the END of
	// the queue.
...
static L2LogicalChannel *preallocateChForRach(RachInfo *rach, bool *deleteMe)
{
	*deleteMe = false;
	...
	if (diff < 26) {		   // very conservative.
		...
		// channel. (pat) Not sure this is necessary.
		rach->mReadyTime += 104;
	}
	...
	LOG(DEBUG)
	"RACH " << LOGVAR(sacchStart) << LOGVAR(now) << LOGVAR(diff) << LOGVAR2("readyTime", rach->mReadyTime);
	return LCH;
}
...
bool CCCHLogicalChannel::processRaches()
{
	while (RachInfo *rach = gRachq.readNoBlock()) {
		...
		if (age > sMaxAge) {
			LOG(WARNING) << "ignoring RACH burst with age " << age;
			if (rach->mChan) {
				LOG(INFO) << "BTS congestion: unable to process RACH for pre-allocated channel within "
					     "expiration time";
		...
		if (chtype == PSingleBlock1PhaseType || chtype == PSingleBlock2PhaseType) {
			...
			L3ImmediateAssignment *iap =
				GPRS::makeSingleBlockImmediateAssign(rach, mCcchNextWriteTime.FN() + 4);
			if (iap) {
...
//					LOG(CRIT) << "LUR [Location Update Request] congestion, insufficient
//"<<LOGVAR(SDCCHAvailable) << " <= " <<LOGVAR(SDCCHReserve); 					goto failure;
//				}
//			}
//
//			LCH = gBTS.getSDCCH();
...
//			return false;
		...
		if (0) { // This was for debugging.  Adding this delay made the layer1 connection reliable.
			...
			LOG(INFO) << "sending L3ImmediateAssignment " << LCH->descriptiveString() << LOGVAR(msecsDelay)
				  << " " << assign;
		}
...
bool CCCHLogicalChannel::processPages()
{
	while (NewPagingEntry *npe1 = gPagingQ.mPageQ.readNoBlock()) {
		...
		if (npe1->mGprsClient) { // Is it a GPRS page?
			// Add 51 to the frame time because the message because the MS may be on the other
			// 51-multiframe.
...
bool CCCHLogicalChannel::ccchServiceQueue()
{
	...
	for (NewPagingEntry *gprsIt = gGprsCcchMessageQ.itBegin(); gprsIt;) {
		NewPagingEntry *gprsMsg =
			gprsIt; // This is redundant; the items are not currently modified by iteration.
		...
		if (sendGprsCcchMessage(gprsMsg, mCcchNextWriteTime)) {
			...
			// While unlikely, it is possible that a reservation on one channel failed while reservations on
			// other channels would succeed, so dont block the entire queue due to this one failure, keep
			// going.
			gprsIt = gGprsCcchMessageQ.itSkip(gprsIt);
		}
	}

	if (foundSomeNewPages)
		; // shut up gcc
...
void CCCHLogicalChannel::ccchServiceLoop()
{
	...
	assert(txInteger <= 15);
	// RACH slot definition: we see in GSM 5.02 clause 7 table 3 of 9 that a RACH can appear on timeslots 0,2,4, or
	// 6, and in table 5 of 9 we see that for a non-combined CCCH-CONF any of the 51 frames can be used, and for
	// combined CCCH-CONF the available (51*4=204 slots/51-multiframe) frames are: B4, B5, B14, B15 ... B36, B45,
	// B46 (27*4=108 slots/51-multiframe.) GSM 4.08 11.1.1, And I quote: "The minimum value of this timer is equal
	// to the time taken by T+2S slots of the mobile station's RACH. S and T are defined in sub-clause 3.3.1.2. The
	// maximum value of this timer is 5 seconds."
	static const int stval =
		GSM::RACHSpreadSlots[txInteger] +
		2 * (isCCCHCombined ? GSM::RACHWaitSParamCombined[txInteger] : GSM::RACHWaitSParam[txInteger]);
	// Subtract some frames from the maxAge to make sure we still have time left to send it; the amount should be 4
	// frames plus some time for the MS to receive and decode it plus the slack induced by the OpenBTS tranceiver
	// interface, which we do not know.
	sMaxAge = min(stval, (int)(5 * 51 * 4.2)) - 6;

	for (int i = 0; i < 51; i++) {
		mRevPCH[i] = -1;
	} // -1 means not used for paging.
...
void NewPager::serviceLoop()
{
	...
	while (!gBTS.btsShutdown()) {
		...
		if (unsigned load = gPagingQ.getPagingLoad()) {
			...
			sleepFrames(51); // There could be multiple paging channels, in which case this is longer than
