...
#define BLATHER \
	DEBUG // (pat 4-2014) These were formerly INFO but there is one message for each frame, which is too much.
...
static const unsigned cAveragePeriodTiming =
	8; // How many we measurement reports over which we average timing, minus 1.
...
TypeAndOffset L1Encoder::typeAndOffset() const { return mMapping.typeAndOffset(); }
...
void L1Encoder::resync(bool force)
{
	...
	if (force || (delta < 0) || (delta > (51 * 26))) {
		...
		OBJLOG(DEBUG) << "L1Encoder RESYNC "
			      << " next=" << mNextWriteTime << " now=" << now;
	}
}
...
bool L1Encoder::l1IsIdle() const { return !mEncActive && mFillerSendTime.passed(); }
...
TypeAndOffset L1Decoder::typeAndOffset() const { return mMapping.typeAndOffset(); }
...
void L1Decoder::close() { mDecActive = false; }

bool L1Decoder::decActive() const { return mDecActive; }
...
void L1Encoder::handoverPending(bool flag)
{
	if (flag) {
		...
		if (!ok)
			OBJLOG(ALERT) << "handover setup failed";
	} else {
		bool ok = mDownstream->clearHandover(mTN);
		if (!ok)
			OBJLOG(ALERT) << "handover clear failed";
	}
}
...
{
	if (mEncoder)
		mEncoder->downstream(radio);
	if (mDecoder)
		radio->installDecoder(mDecoder);
}
...
void L1FEC::l1close()
{
	...
	if (mEncoder)
		mEncoder->close(); // Does the sendDummyFill.
	if (mDecoder)
		mDecoder->close();
}
...
XCCHL1Decoder::XCCHL1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
	: L1Decoder(wCN, wTN, wMapping, wParent)
{
}

SharedL1Decoder::SharedL1Decoder()
	: mBlockCoder(0x10004820009ULL, 40, 224), mC(456), mU(228), mP(mU.segment(184, 40)), mDP(mU.head(224)),
	  mD(mU.head(184)), mHParity(0x06f, 6, 8), mHU(18), mHD(mHU.head(8))
{
	for (int i = 0; i < 4; i++) {
		mE[i] = SoftVector(114);
		mI[i] = SoftVector(114);
...
void MSPhysReportInfo::processPhysInfo(const RxBurst &inBurst)
{
	...
	// (pat) It is the timing error of the received bursts which means it is relative to the Timing Advance
	// currently in use.
...
XCCHL1Encoder::XCCHL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
	: SharedL1Encoder(), L1Encoder(wCN, wTN, wMapping, wParent)
{
...
SharedL1Encoder::SharedL1Encoder()
	: mBlockCoder(0x10004820009ULL, 40, 224), mC(456), mU(228), mD(mU.head(184)), mP(mU.segment(184, 40))
{
	initInterleave(4);
...
void XCCHL1Encoder::writeHighSide(const L2Frame &frame)
{
	...
	if (!encActive()) {
		OBJLOG(INFO) << "XCCHL1Encoder::writeHighSide sending on non-active channel ";
	}
	resync();
	sendFrame(frame);
}
...
SCHL1Encoder::SCHL1Encoder(L1FEC *wParent, unsigned wTN)
	: GeneratorL1Encoder(0, wTN, gSCHMapping, wParent), mBlockCoder(0x0575, 10, 25), mU(25 + 10 + 4), mE(78),
	  mD(mU.head(25)), mP(mU.segment(25, 10)), mE1(mE.segment(0, 39)), mE2(mE.segment(39, 39))
{
...
FCCHL1Encoder::FCCHL1Encoder(L1FEC *wParent, unsigned wTN) : GeneratorL1Encoder(0, wTN, gFCCHMapping, wParent)
{
	mBurst.zero();
	mFillerBurst.zero();
}
...
// TCH_FS
TCHFACCHL1Decoder::TCHFACCHL1Decoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
...
ViterbiBase *newViterbi(AMRMode mode)
{
	switch (mode) {
	case TCH_AFS12_2:
		return new ViterbiTCH_AFS12_2();
	case TCH_AFS10_2:
		return new ViterbiTCH_AFS10_2();
	case TCH_AFS7_95:
		return new ViterbiTCH_AFS7_95();
	case TCH_AFS7_4:
		return new ViterbiTCH_AFS7_4();
	case TCH_AFS6_7:
		return new ViterbiTCH_AFS6_7();
	case TCH_AFS5_9:
		return new ViterbiTCH_AFS5_9();
	case TCH_AFS5_15:
		return new ViterbiTCH_AFS5_15();
	case TCH_AFS4_75:
		return new ViterbiTCH_AFS4_75();
	case TCH_FS:
		return new ViterbiR2O4();
	default:
		assert(0);
	}
};
...
bool TCHFACCHL1Decoder::processBurst(const RxBurst &inBurst)
{
	...
	// if they are ambiguous, we will try decoding the frame as FACCH to check parity, which is a much stronger
	// condition.
	// bool stolen = inBurst.Hl();
	unsigned stolenbits = 0; // Number of stolen bit markers.  In the range 0 .. 8
	if (B == 3) {
		stolenbits = stealBitsU[4] + stealBitsU[5] + stealBitsU[6] + stealBitsU[7] + stealBitsL[0] +
			     stealBitsL[1] + stealBitsL[2] + stealBitsL[3];
	} else {
		stolenbits = stealBitsU[0] + stealBitsU[1] + stealBitsU[2] + stealBitsU[3] + stealBitsL[4] +
			     stealBitsL[5] + stealBitsL[6] + stealBitsL[7];
	}
	...
	// Now keep statistics...
	if (okFACCH) {
		countStolenFrame(1); // was 4, why?  We are counting frames, which occur every 4 bursts (even though
				     // they are spread over 8 bursts.)
		...
		// ScopedLock lock(mDecLock,__FILE__,__LINE__);
		// (pat 4-2014) There are only 3 parity bits on the speech frame so the false-positive detection
		// probability is high, resulting in setting T3109 and preventing the channel from being recycled.  Not
		// sure what to do, because there may not be any FACCH frames to set T3109.
		// mT3109.set();
	} else
		countBadFrame(4);
...
static BitPos GSMFRS_LAR_Positions[8] = { // Log Area Ratio bit positions within full rate speech frame.
	{0, 6}, {6, 6}, {12, 5}, {17, 5}, {22, 4}, {26, 4}, {30, 3}, {33, 3}};
...
bool TCHFRL1Decoder::decodeTCH_GSM(bool stolen, const SoftVector *wC)
{
	...
	if (!good) {
		...
		// (pat) The first 36 bits are LAR filter reflection coefficient parameters applicable to the entire
		// 20ms frame. See GSM 06.11 These are followed by 4 sets of 56 bits of parameters for each 5ms
		// sub-frame, consiting of:
		//	7 bits: N1, LTP lag
		//	2 bits: b1, LTP gain
		...
		// 1-2014 We dont get SID frames because we dont yet support DTX as specified in the beacon in
		// L3CellOptionsBCCH, so this code is zeroed out until we do...
		if (1 || !isSIDFrame(mPrevGoodFrame)) {
			...
			if (mNumBadFrames >= 32) {
				...
				for (unsigned f = 0; f < 4; f++) {
					// First bad frame is an extrapolation of previous good frame, which we do by
					// copying the last xmax into all locations.  Subsequent bad frames are muted.
					...
					if (xmax == 0) {
						// Dont kill the LTP gain until xmax is 0, or it sounds cruddy.  And it
						// still sounds cruddy anyway. mPrevGoodFrame.fillField(36 + 7 +
						// f*56,0,2);
						createSilenceFrame(mPrevGoodFrame);
...
bool TCHFRL1Decoder::decodeTCH_AFS(bool stolen, const SoftVector *wC)
{
	...
	if (!stolen) {
		...
		OBJLOG(DEBUG) << "TCHFACCHL1Decoder u[]=" << mTCHU;
		OBJLOG(DEBUG) << "TCHFACCHL1Decoder d[]=" << mTCHD;
		OBJLOG(DEBUG) << "TCHFACCHL1Decoder sentParity=" << sentParity << " calcParity=" << calcParity;
...
// TCH_FS
TCHFACCHL1Encoder::TCHFACCHL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, L1FEC *wParent)
	: XCCHL1Encoder(wCN, wTN, wMapping, wParent), mPreviousFACCH(true), mOffset(0)
{
	for (int k = 0; k < 8; k++) {
		mI[k].resize(114);
		mE[k].resize(114);
...
void TCHFRL1Encoder::encodeTCH(const AudioFrame *aFrame)
{
	...
	if (mAMRMode == TCH_FS) {
		encodeTCH_GSM(aFrame);
	} else {
		encodeTCH_AFS(aFrame);
	}
}
...
void TCHFACCHL1Encoder::dispatch()
{
	...
	while ((int)mSpeechQ.size() > maxQ)
		delete mSpeechQ.read();
	...
	if (L2Frame *fFrame = mL2Q.readNoBlock()) {
		...
		if (!mPreviousFACCH) {
			...
			static const BitVector2 fillerC(
				"11010000100011110000000011100111110101110011110100111100000000000011011110111111111010"
				"01101010101010101010101010101010101010101010100100001100000000000000000000000000000000"
				"00000000001101001111000000000000000000000000000000000000000000000000111010011010101010"
				"10101010101010101010101010101010100100001100000000000000000011010011110000000011100111"
				"11011010000011000011010011110000000000000000000110011000000000000000000000000000000000"
...
void MSPhysReportInfo::sacchInit1()
{
	...
	// The RACH was sent at full power, but full power probably depends on the MS power class.  We just use a
	// constant.
...
//	// FIXME: We may want to set the initial power based on the RACH power instead of just using a constant
// cInitialPower. 	mTimestamp=wTimestamp; 	mTimingError = wTimingError; 	mRSSI = wRSSI; 	OBJLOG(BLATHER) <<
//"SACCHL1Encoder init" <<LOGVARM(wRSSI) <<LOGVARM(wTimingError) <<LOGVARM(wTimestamp);
//}
...
void MSPhysReportInfo::setPhy(const SACCHL1Decoder &other)
{
	...
	OBJLOG(INFO) << "SACCHL1Decoder actuals RSSI=" << mRSSI << " timingError=" << mTimingError
		     << " timestamp=" << mTimestamp << " MSPower=" << mActualMSPower << " MSTiming=" << mActualMSTiming;
}
...
static float boundMSPower(float orderedMSPower)
{
	...
	if (orderedMSPower > maxPower)
		orderedMSPower = maxPower;
	else if (orderedMSPower < minPower)
		orderedMSPower = minPower;
	return orderedMSPower;
}

void SACCHL1Encoder::setMSPower(float orderedPower) { mOrderedMSPower = boundMSPower(orderedPower); }
...
void SACCHL1Encoder::setMSTiming(float orderedTiming)
{
	...
	if (mOrderedMSTiming < 0.0F)
		mOrderedMSTiming = 0.0F;
	else if (mOrderedMSTiming > maxTiming)
		mOrderedMSTiming = maxTiming;
}
...
void SACCHL1Encoder::initPhy(float wRSSI, float wTimingError)
{
	...
	// Arbitrarily goose the initial power up a little (10) by adjusting RSSITarget, just to make sure we get an ok
	// initialization, in case the RSSI measured power was inaccurate, or there is noise in the channel.
	...
	// OBJLOG(INFO) << "SACCHL1Encoder init timingError=" << sib.timingError()  << " actual=" <<
	// sib.actualMSTiming() << " ordered=" << mOrderedMSTiming;
	OBJLOG(INFO) << "SACCHL1Encoder init" << LOGVARM(wTimingError) << LOGVARM(wRSSI) << LOGVAR(deltaP)
		     << LOGVARM(mOrderedMSPower);
}
...
SACCHL1Encoder::SACCHL1Encoder(unsigned wCN, unsigned wTN, const TDMAMapping &wMapping, SACCHL1FEC *wParent)
	: XCCHL1Encoder(wCN, wTN, wMapping, (L1FEC *)wParent), mSACCHParent(wParent), mOrderedMSPower(cInitialPower),
	  mOrderedMSTiming(0)
{
}
...
void SACCHL1Encoder::sendFrame(const L2Frame &frame)
{
	...
	if (sib.isValid()) {
		...
		if (float SNRTarget = gConfig.getNum("GSM.Radio.SNRTarget")) {
			...
			if (deltaP > 0 && SNR < SNRTarget) { // If RSSITarget is met but SNR looks bad...
				// How do we decide what the target power should be from SNR?  And I dont want to call
				// log(). We only change upward based on SNR - we rely on RSSITarget to keep the power
				// down.
				deltaP = SNR - SNRTarget; // So how about something simple like this?  eg: SNR == 10 is
							  // ok, SNR==6 would be bad, so add 4dB?
			}
		}
		...
		if (configPowerDamping < 90 && deltaP < 4) {
			...
			// instead RSSIBumpDown will be used, although that could indirectly induce this greater jump
			// here.
			powerDamping /= 2; // This should probably be log response.
		}
		// (pat) Now, how fast does RSSI as seen in OpenBTS respond to changes ordered in the MS by
		// mOrderedPower? Do we need the damping factor to take that into account, or should we instead wait a
		// bit after ordering a power change before ordering another?  I guess we rely on the powerDamping
		// factor to handle it.
		setMSPower(powerDamping * mOrderedMSPower + (1.0F - powerDamping) * targetMSPower);
		OBJLOG(DEBUG) << "SACCHL1Encoder RSSI=" << RSSI << " target=" << RSSITarget << " deltaP=" << deltaP
			      << " actual=" << actualPower << " order=" << mOrderedMSPower;
		...
		OBJLOG(DEBUG) << "SACCHL1Encoder timingError=" << timingError << " actualTA=" << actualTiming
			      << " orderedTA=" << mOrderedMSTiming << " targetTA=" << targetMSTiming;
	}
	...
	OBJLOG(INFO) << "SACCHL1Encoder orders pow=" << mOrderedMSPower << " TA=" << mOrderedMSTiming << " with header "
		     << mU.head(16);
...
void TestTCHL1FEC()
{
	...
	for (unsigned modei = 0; modei <= TCH_FS; modei++) {
		...
		for (unsigned trial = 0; trial < 10; trial++) {
			...
			if (ok1)
				for (unsigned i = 0; i < pay1.size() && ok; i++) {
					if (pay1.bit(i) == pay2.bit(i))
						continue;
