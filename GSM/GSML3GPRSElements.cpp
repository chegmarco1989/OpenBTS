...
const char *L3IAPacketAssignment::IAPacketAssignmentTypeText(enum IAPacketAssignmentType type) const
{
	...
	case PacketUplinkAssignFixed:
		return "Fixed Packet Uplink Assignment";
	case PacketUplinkAssignDynamic:
		return "Dynamic Packet Uplink Assignment";
	case PacketUplinkAssignSingleBlock:
		return "Single Block Packet Uplink Assignment";
	case PacketDownlinkAssign:
		return "Packet Downlink Assignment";
	default:
...
static void writeStartTime(MsgCommon &dest, unsigned startframe)
{
	...
	if (os && recomputed != (int)startframemod) {
		*os << " TBF Start Time miscalculation: " << LOGVAR(startframemod) << "!=" << LOGVAR(recomputed);
	}
...
void L3IAPacketAssignment::writePacketUplinkAssignment(MsgCommon &dest) const
{
	...
	dest.write0();
	dest.write0();
	if (mPacketAssignmentType == PacketUplinkAssignFixed || mPacketAssignmentType == PacketUplinkAssignDynamic) {
		dest.write1();
		...
		if (dest.write01(mAlphaPresent)) {
			dest.WRITE_FIELD(mAlpha, 4);
		}
		...
		dest.WRITE_FIELD(mGamma, 5);
		dest.write0();
		dest.write1();			 // As per 10.5.2.16 Note 1.
...
L3GPRSSI13PowerControlParameters::L3GPRSSI13PowerControlParameters()
	: mAlpha(GPRS::GetPowerAlpha()), mTAvgW(gConfig.getNum("GPRS.MS.Power.T_AVG_W")),
	  mTAvgT(gConfig.getNum("GPRS.MS.Power.T_AVG_T")), mPCMeasChan(0),
...
{
	os << "(" << LOGVAR(mAlpha) << LOGVAR(mTAvgW) << LOGVAR(mTAvgT) << LOGVAR(mPCMeasChan) << LOGVAR(mNAvgI) << ")";
}
