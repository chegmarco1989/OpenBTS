...
public:
	...
	// Return true if there was data available and compute and return the averaged RXLEV, where 'averaged' is
	// computed by one of several algorithms.
...
// (pat) At my house, using the Blackberry, I see a regular 9.5 second heart-beat, where the measurements drop about
// 8db. The serving cell RSSI drops first, then in the next measurement report the serving RSSI is back to normal and
// the neighbor RSSI drops.  If it were just 2db more, it would be causing a spurious handover back and forth every 9.5
// seconds.  This cache alleviates that problem.
class ChannelHistory {
	// The unsigned key is the frequency index combined with the BSIC.
	// The Peering code creates the frequency index; it is sent in one of several sysinfo messages, for us probably
	// type2.
...
public:
	...
	void crackKey(unsigned key, unsigned *arfcn, unsigned *BSIC)
	{
		*BSIC = key & 0x3f;
		*arfcn = key >> 6;
	}
