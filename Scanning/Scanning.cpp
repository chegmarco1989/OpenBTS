...
void SpectrumMap::clear() { sqlite3_command(mDB, "DELETE FROM SPECTRUM_MAP WHERE 1"); }

void SpectrumMap::power(GSM::GSMBand band, unsigned ARFCN, float freq, LinkDirection &linkDir, float dBm)
{
	char q1[200];
	sprintf(q1,
		"INSERT OR REPLACE INTO SPECTRUM_MAP (BAND,TIMESTAMP,ARFCN,RSSI,FREQ,LINK) "
		"VALUES (%u,%u,%u,%f,%f,'%s')",
		(int)band, (unsigned)time(NULL), ARFCN, dBm, freq, linkDir.string());
	bool s = sqlite3_command(mDB, q1);
	if (!s)
		LOG(ALERT) << "write to spectrum map failed";
}
