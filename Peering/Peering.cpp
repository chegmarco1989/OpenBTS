...
//                                                                                       f <-------- OK ----------
//                                                                                       g --------- ACK -------->
//                                             E <------- IND HANDOVER_COMPLETE -------- h  (Note: These were reversed
//                                             in
//                                                          (BS2tranid)
...
// B = build string for handover determination                                           ah = transaction entry
// definition BH = SIPEngine.h A = processHandoverRequest in BS2
...
// d when we receive the handover complete the InboundHandoverMachine calls newSipDialogHandover, which sends the
// re-INVITE.
...
static const char *getPeeringMsgArg1(const char *message)
{
	...
	if (arg) {
		...
		return message +
		       strlen(message); // If no argument, return pointer to end of string, not a NULL pointer.
	}
}
...
static void logMessage(const char *sendOrRecv, const struct sockaddr_in *peer, const char *message)
{
	...
	if (0 == strncmp(arg1, "HANDOVER", 8)) {
		LOG(INFO) << "Peering " << sendOrRecv << LOGVAR2("peer", sockaddr2string(peer, true))
			  << LOGVAR(message);
		...
		WATCHLEVEL(INFO, " Peering " << sendOrRecv << LOGVAR2("peer", sockaddr2string(peer, true))
					     << LOGVAR2("message", string(message, eol ? eol - message
										       : strlen(message)))); // first
													     // line of
													     // message;
													     // they
													     // used to
		...
		LOG(DEBUG) << "Peering " << sendOrRecv << LOGVAR2("peer", sockaddr2string(peer, true))
			   << LOGVAR(message);
	}
}
...
{
	if (!remove(transactionID)) {
		LOG(DEBUG) << "attempt to remove non-existent FIFO " << transactionID;
	}
}
...
PeerInterface::PeerInterface() : mSocket(gConfig.getNum("Peering.Port")), mReferenceCounter(0)
{
	mSocket.nonblocking();
}
...
void PeerInterface::process(const struct sockaddr_in *peer, const char *message)
{
	...
	// (pat) It is an ACK message, and we need to queue it because the 'senduntilack' is running in a different
	// thread.
...
// pats TODO: We should check for ARFCN+BSIC conflicts among the neighbors.  That implies sucking the whole neighbor
// table in, but dont worry about now it because the whole thing should move to SR imho.
void PeerInterface::processNeighborParams(const struct sockaddr_in *peer, const char *message)
{
	try {
		...
		// static const char rspFormat[] = "RSP NEIGHBOR_PARAMS %u %u %u %d %u %u %u";  // C0 BSIC uniqueID
		// noise numArfcns tchavail tchtotal
		...
		if (0 == strncmp(message, "RSP ", 4)) {
			...
			if (!strchr(message, '=')) {
				...
				SimpleKeyValue keys;
				keys.addItems(message + sizeof("RSP NEIGHBOR_PARAMS")); // sizeof is +1 which is ok - we
											// are skipping the initial
											// space.
			...
			if (newentry.mIPAddress.size() == 0) {
				LOG(ERR) << "cannot parse peer socket address for" << LOGVAR2("C0", newentry.mC0)
					 << LOGVAR2("BSIC", newentry.mBSIC);
				return;
			}
			...
			if (overlap) {
				LOG(WARNING) << format(
					"neighbor IP=%s BSIC=%d ARFCNs=%u to %d overlaps with this BTS ARFCNs=%d to %d",
					newentry.mIPAddress.c_str(), newentry.mBSIC, newentry.mC0, neighborCEnd, myC0,
					myCEnd);
			}

			// Is there an NCC conflict?
			// (pat) ARFCN-C0 (the one carrying BCCH) and BSIC consisting of NCC (Network Color Code) and
...
void PeerInterface::processHandoverRequest(const struct sockaddr_in *peer, const char *message)
{
	...
	if (!transaction) {
		...
		// If getTCH took so long that there's too little time left in T3101, ignore this REQ and get the next
		// one.
...
void PeerInterface::processHandoverResponse(const struct sockaddr_in *peer, const char *message)
{
	...
	char handoverCommandBuffer[82];
	handoverCommandBuffer[0] = 0;
