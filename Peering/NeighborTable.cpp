...
// The C0 ARFCN is not sufficient because there could be multiple BTS on the same ARFCN, so we need to match the BSIC
// too.
bool NeighborTable::ntFindByArfcn(int arfcn, int bsic, NeighborEntry *pentry)
{
	if (arfcn < 0 || bsic < 0) {
		// These were uninitialized values.  It would not have hurt anything to just let this fall through and
		// search for them.
		return false;
	}
...
void NeighborTable::NeighborTableInit(const char *wPath)
{
	...
	if (!sqlite3_command(mDB, enableWAL)) {
		LOG(ALERT) << "Cannot enable WAL mode on database at " << wPath
...
static void makeNeighborSet(set<string> &neighborSet)
{
	...
	for (unsigned int i = 0; i < neighbors.size(); i++) {
		...
		// (pat) The measurement report allows only 32 ARFCN slots, the last of which we reserve for 3G to avoid
		// confusion. However, we cannot check that here, because some of the neighbors may share ARFCNs, and we
		// cannot tell that until they report in. Do not check this here: if (i == 31) ...
...
bool NeighborTable::ntAddInfo(NeighborEntry &newentry)
{
	...
	LOG(DEBUG) << "updating " << addrString << ":" << ntohs(address->sin_port) << " in neighbor table" << LOGVAR(C0)
		   << LOGVAR(BSIC) << LOGVAR(oldC0) << LOGVAR(oldBSIC);
	...
	// (pat) Added test to see if C0 or BCC changed.  That would not change the beacon but we need to recheck for
	// conflicts with the current BTS.
...
std::vector<unsigned> NeighborTable::getARFCNs() const
{
	...
	for (NeighborTableMap::const_iterator mit = mNeighborMap.begin(); mit != mNeighborMap.end(); mit++) {
		...
		if ((int)bcchChannelList.size() >= limit)
			break;
	}
	...
	if (IS_LOG_LEVEL(DEBUG)) {
		...
		for (vector<unsigned>::iterator it = bcchChannelList.begin(); it != bcchChannelList.end(); it++) {
			ss << " " << *it;
		}
		LOG(DEBUG) << ss.str();
	}
...
string NeighborEntry::neC0PlusBSIC() const { return format("%d:%d", (int)mC0, (int)mBSIC); }
...
bool NeighborTable::neighborCongestion(unsigned arfcn, unsigned bsic) { return false; }
