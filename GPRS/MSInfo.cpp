...
MSInfo::MSInfo(uint32_t tlli)
	: msDebugId(++Stats.countMSInfo), msTlli(tlli), msPacch(0),
	  // msMode(RROperatingMode::PacketIdle),
	  msT3191(gL2MAC.macT3191Value), msT3193(gL2MAC.macT3193Value), msT3168(gL2MAC.macT3168Value)
...
bool MSInfo::msCanUseExtendedUplink()
{
	...
	return gL2MAC.macUplinkPersist > 0 && this->msIsRegistered() &&
	       (this->sgsnGetGeranFeaturePackI(this->msTlli) || this->sgsnGetMultislotClass(this->msTlli) > 10);
}

bool MSInfo::msIsRegistered()
{
	SGSN::GmmState::state state = sgsnGetRegistrationState(this->msTlli);
	return state == SGSN::GmmState::GmmRegisteredNormal || state == SGSN::GmmState::GmmRegisteredSuspsended;
}
...
void MSInfo::msCleanUSFs()
{
	...
	if (!anyactiveuplinks) {
		...
		msAckNackUSFGrant = -1;
		for (int u = 0; u < USFMAX; u++) {
			msUSFs[u] = 0;
		} // Not used, but be tidy.
	}
...
{
	PDCHL1Uplink *up;
	RN_FOR_ALL(PDCHL1UplinkList_t, msPCHUps, up) { up->mchParent->freeUSF(this, true); }
}
...
static unsigned addTn(unsigned tn, int offset) { return (unsigned)(tn + offset) % cTnPerArfcn; }
static bool tnavail(unsigned chmask, unsigned tn) { return chmask & (1 << (tn % cTnPerArfcn)); }
...
bool MSInfo::msAddCh(unsigned chmask, const char *tnlist)
{
	...
	for (tn = tnfirst, cp = tnlist; *cp; cp++, tn = addTn(tn, 1)) {
		if (*cp == 'P') {
			continue;
		} // PACCH already allocated, so it wont look avail.
		if (!tnavail(chmask, tn)) {
			return false;
		}
	}
...
bool MSInfo::msTrySlots(unsigned chmask, int down, int up)
{
	...
	switch ((down << 4) + up) {
	case 0x41:
		return msAddCh(chmask, "DDPD");
	case 0x14:
		return msAddCh(chmask, "PUUU");
	case 0x32:
		return msAddCh(chmask, "DPB") || msAddCh(chmask, "DBP");
	case 0x23:
		return msAddCh(chmask, "PBU") || msAddCh(chmask, "BPU");
	case 0x22:
		return msAddCh(chmask, "PB") || msAddCh(chmask, "BP");
	// These are oddballs that would only be used for testing:
	case 0x31:
		return msAddCh(chmask, "DPD") || msAddCh(chmask, "DDP");
	case 0x13:
		return msAddCh(chmask, "PUU");
	case 0x21:
		return msAddCh(chmask, "PD") || msAddCh(chmask, "DP");
	case 0x12:
		return msAddCh(chmask, "PU");
	default:
		devassert(0);
		return false;
	}
...
bool MSInfo::msAssignChannels2(int maxdown, int maxup, int sum)
{
	...
	GPRSLOG(2) << format("AssignChannels3, down/up=%d/%d sum=%d mask=0x%x", maxdown, maxup, sum, mask);
	...
	// for these cases the additional bidir channel can be on either side of the pacch.  (Which fact is encapsulated
	// in the msTrySlots code.)
...
bool MSInfo::msAssignChannels()
{
	...
	if (msPCHDowns.size() == 0) {
		...
		if (maxdown > 1 || maxup > 1) {
			...
			if (maxdown > (int)slots.mMultislotRx) {
				maxdown = slots.mMultislotRx;
			}
			if (maxup > (int)slots.mMultislotTx) {
...
int MSInfo::msCountTBF1(RLCDir::type dir, TbfMacroState tbfmstate, TBF **ptbf) const
{
	...
	RN_MS_FOR_ALL_TBF(this, tbf)
	{
		if (dir == RLCDir::Either || tbf->mtDir == dir) {
			if (tbfmstate == TbfMActive && !tbf->isActive())
				continue;
			if (tbfmstate == TbfMTransmitting && !tbf->isTransmitting())
				continue;
			// We always ignore tbfs that are in the process of being deleted.
			if (tbf->mtGetState() == TBFState::Deleting)
				continue;
			if (tbf->mtGetState() == TBFState::Unused)
...
int MSInfo::msCountActiveTBF(RLCDir::type dir, TBF **ptbf) { return msCountTBF2(dir, TbfMActive, ptbf); }

int MSInfo::msCountTransmittingTBF(RLCDir::type dir, TBF **ptbf) { return msCountTBF2(dir, TbfMTransmitting, ptbf); }
...
unsigned char MSInfo::msGetDownlinkTimeslots(MultislotSymmetry sym)
{
	...
	//} else {
	PDCHL1Downlink *down;
	RN_FOR_ALL(PDCHL1DownlinkList_t, msPCHDowns, down) { result |= (1 << (7 - down->TN())); }
	//}
	return result;
}
...
ChannelCodingType MSInfo::msGetChannelCoding(RLCDirType wdir) const
{
	...
	// BEGINCONFIG
	// 'GPRS.ChannelCodingControl.RSSI',-40,0,0,'If the initial signal strength is less than this amount in DB GPRS
	// uses a lower bandwidth but more robust encoding CS-1' ENDCONFIG
...
void MSInfo::msChangeTlli(uint32_t newTlli)
{
	if (!tlliEq(newTlli, msTlli)) {
		...
		if (ms2 != this) {
			...
			LOG(ERR) << "Changing TLLI of" << this << " from" << LOGHEX(msTlli) << " to" << LOGHEX(newTlli)
				 << " MS already exists:" << ms2;
			ms2->msDelete(false);
		}
...
void StatHits::getStats(float *pER, int *pTotal, float *pWorstER, int *pWorstTotal)
{
	...
	*pWorstTotal = 0;
	...
	for (int i = 0; i < cNumHist; i++, hp++) {
		...
		if (age < 0) {
			age += RLCBSN_t::BSNPeriodicity / 48;
		} // Account for BSN wrap.
		if (age >= cNumHist) {
			hp->clear();
			continue;
		} // Data too old.
...
void StatHits::textRecent(std::ostream &os)
{
	...
	if (worstER) {
		putER(os, " low:", worstER, worstTotal);
	}
}
...
void MSStat::msStatDump(const char *indent, std::ostream &os)
{
	os << indent;
	os << " dataER:";
	msCountBlocks.textTotal(os);
	os << " recent:";
	msCountBlocks.textRecent(os);
	{
	...
	os << "\n" << indent;
	os << " rrbpER:";
	msCountRbbpReservations.textTotal(os);
	os << " recent:";
	msCountRbbpReservations.textRecent(os);
	os << " ccchER:";
	msCountCcchReservations.textTotal(os);
	os << " recent:";
	msCountCcchReservations.textRecent(os);
	os << "\n";
}
...
void MSInfo::msDump(std::ostream &os, SGSN::PrintOptions options)
{
	...
	os << this // Dumps the operator<< value, which is sprintf(MS#%d,msDebugId)
	   ...
	   << LOGVAR(rrmode) << " Bytes:" << msBytesUp << "up/" << msBytesDown
	   << "down"
	   ...
	   << "\n";
	os << "\t";
	sgsnPrint(msTlli, options | SGSN::printNoMsId, os);
	dumpSignalQuality(os);
	msStatDump("\t", os);

	if (!(options & SGSN::printVerbose)) {
		return;
	}
	...
	if (msDownlinkQueue.size()) {
		...
		if (curage > msDownlinkQDelay.getCurrent()) {
			msDownlinkQDelay.addPoint(curage);
		}
	}
	os << "\t" << LOGVAR2("DownlinkQ:bytes", msDownlinkQStat) << LOGVAR2("delay", msDownlinkQDelay) << "\n";

	os << "\t TBFs:" << LOGVAR2("total", msCountTbfs);
	if (msCountTbfFail) {
		os << LOGVAR2("failed", msCountTbfFail);
	}
	if (msCountTbfNoConnect) {
		os << LOGVAR2("NC", msCountTbfNoConnect);
	}
	os << "\n";
...
string MSInfo::id() const
{
	...
	if (msOldTlli) {
		sprintf(buf + strlen(buf), ",%x", (uint32_t)msOldTlli);
	}
	return string(buf);
}
