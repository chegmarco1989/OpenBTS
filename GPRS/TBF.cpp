...
static int configTbfRetry() { return gConfig.getNum("GPRS.TBF.Retry"); }

bool MsgTransaction::mtMsgPending(MsgTransactionType mttype)
{
	ScopedLock lock(mtLock);
	// The multiple tests here are overkill.
	return mtMsgExpectedBits.isSet(mttype) && mtExpectedAckBSN[mttype].valid() &&
	...
	bool onRach) // If true, request on RACH, else request on uplink ack-nack.
{
	...
	if (ms->msCountActiveTBF(RLCDir::Up, &activeTbf) >= 1) {
		...
		if (activeTbf->mtGetState() == TBFState::DataFinal) {
			...
			// which implies that we expect the MS to comply by setting the uplink MAC header Countdown
			// Value to 0 within the T3168 grace period.
...
std::string TBF::tbfDump(bool verbose) const
{
	...
	os << "\t" << LOGVAR2("mtState=", TBFState::name(mtState)) << LOGVAR(mtAttached) << LOGVAR(mtTFI)
	   << LOGHEX(mtTlli);
	if (mtDir == RLCDir::Down) {
		os << " size=" << engineDownPDUSize();
	}
	if (!verbose)
		return os.str();
	os << "\n\t";
	...
	if (mtAssignCounter)
		os << LOGVAR(mtAssignCounter);
	if (mtReassignCounter)
		os << LOGVAR(mtReassignCounter);
	if (mtPerformReassign)
		os << LOGVAR(mtPerformReassign);
	if (mtMS->msN3101)
		os << LOGVAR2("N3101", mtMS->msN3101);
	if (mtN3103)
		os << LOGVAR2("N3103", mtN3103);
	if (mtN3105)
		os << LOGVAR2("N3105", mtN3105);
	if (mtDeadTime.valid())
		os << LOGVAR2("deadTime", -mtDeadTime.elapsed());
	if (mtMS->msT3193.active()) {
		os << LOGVAR2("msT3193", mtMS->msT3193.remaining());
	}
	if (mtMS->msT3168.active()) {
		os << LOGVAR2("msT3168", mtMS->msT3168.remaining());
	}
	if (mtDescription.size())
		os << " descr=" << mtDescription;
	os << "\n";
...
RLCDownEngine *TBF::getDownEngine() { return (mtDir == RLCDir::Down) ? dynamic_cast<RLCDownEngine *>(this) : NULL; }
	...
	std::ostream *os) // for testing - if set, print out the message instead of sending it.
{
	...
	if (tbf->mtDir == RLCDir::Up) {
		...
		RLCMsgPacketDownlinkAssignment *dlmsg = new RLCMsgPacketDownlinkAssignment(tbf, isNewAssignment);
		msg = dlmsg;
	}
	...
	TBF *tbf)
{
	...
	pa->setPacketDownlinkAssign(tbf->mtGetTlli(), tbf->mtTFI, tbf->mtChannelCoding(), tbf->mtUnAckMode, 1);
	...
	std::ostream *os)
{
	...
	if (imsi.empty()) {
		...
		return; // This is a disaster.  I dont know what is going to happen to this TBF; will it ever timeout?
	...
	// We allocate a NewPagingEntry for convenience of queuing this request, but since this is not a real page the
	// first argument (ChannelType) is unused.
...
bool gprsPageCcchSetTime(TBF *tbf, L3ImmediateAssignment *iap, unsigned afterFrame)
{
	...
	RLCBSN_t resBSN = chan->makeReservationInt(
		RLCBlockReservation::ForPoll, // We are requesting a confirming poll from the MS.
	...
	std::ostream *os) // for testing - if set, print out the message instead of sending it.
{
	...
	if (GPRSDebug & 1) {
		GPRSLOG(1) << ms << " OnCCCH=" << tbf->mtAssignmentOnCCCH << " RRMode=" << ms->getRROperatingMode()
...
bool TBF::mtAllocateUSF()
{
	...
	RN_FOR_ALL(PDCHL1UplinkList_t, mtMS->msPCHUps, up)
	{
		...
		if (!mtMS->msCanUseDownlinkTn(up->TN())) {
			continue;
		}
...
static void propagateTFI(TBF *tbf, int tfi, TBF *newvalue)
{
	if (tbf->mtDir == RLCDir::Up) {
		PDCHL1Uplink *up;
		RN_FOR_ALL(PDCHL1UplinkList_t, tbf->mtMS->msPCHUps, up) { up->setTFITBF(tfi, RLCDir::Up, newvalue); }
	} else {
		PDCHL1Downlink *down;
...
bool TBF::mtAttach()
{
	...
	if (!ms->msAssignChannels())
		return false;
	if (!mtAllocateTFI())
		return false;
	if (mtDir == RLCDir::Up && !mtAllocateUSF())
		return false;
...
void MSInfo::msService()
{
	...
	while (msDownlinkQueue.size()) { // We have something to send
		...
		if (!msCountTBF2(RLCDir::Down, stallOnlyForActiveTBF ? TbfMActive : TbfMAny,
			...
			if (msDownlinkAttempts >= 3) {
				...
				msDelete();
				// LOG(DEBUG) << "Greater than 3 attempts sending TBF count: " << msDownlinkAttempts;
				// SVGDBG SVGDBG should msDownlinkAttempts be reset here
				return;
			}
		...
		break; // SVGDBG msDownlinkAttempts may not work because msDownlinkAttempts can get reset in
		       // macServiceLoop
	}
	...
	if (((int)gBSNNext % 24) == 0)
		msTrafficMetric = msTrafficMetric / 2;
}
...
void TBF::mtFinishSuccess()
{
	...
	Timeval now;
	mtMS->msAddConnectTime(
		now.delta(mtStartTime)); // This includes the time it took the TBF to get its assignment through.
...
void TBF::mtRetry()
{
	...
	if (retry) {
		...
		ChannelCodingType chCoding =
			(ChannelCodingType)RN_BOUND((retrycoding - 1), ChannelCodingCS1, ChannelCodingCS4);
	...
	TbfCancelMode release) // When to release and whether to retry.
{
	...
	case TBFState::Finished: {
		Timeval now;
		mtMS->msAddConnectTime(now.delta(
			mtStartTime)); // This includes the time it took the TBF to get its assignment through.
		mtMS->msCountTbfFail++;
		break;
	}
	}
...
bool TBF::isPrimary(PDCHL1Downlink *down) { return (down->parent() == mtMS->msPacch); }
...
bool TBF::mtServiceDownlink(PDCHL1Downlink *down)
{
	...
	while (1) {
		...
		case TBFState::DataWaiting2:
			...
			if (mtAssignmentOnCCCH) {
				if (mtDir == RLCDir::Down) {
					...
					if (SendExtraTA && !sendTA(down, this)) {
						return false;
					}
		...
		case TBFState::DataTransmit:
			...
			if (mtPerformReassign) {
				...
				if (mtGotAck(MsgTransReassign, true)) {
					...
					if (!mtMsgPending()) {
						if ((int)mtReassignCounter > gConfig.getNum("GPRS.Counters.Reassign")) {
							// The iphone is not answering these.  It may be because we are
							// only allowed to have 3 RRBPs out at a time, but whatever,
							// dont kill the TBF for this, just stop sending the messages.
							// If the MS wants
							// mtCancel(MSStopCause::ReassignCounter,TbfRetryAfterWait);
							// return false;
							mtPerformReassign = false;
		...
		case TBFState::DataFinal:
			if (mtAssignmentOnCCCH && !isPrimary(down)) {
				return false;
			}
