...
char *ip_sockaddr2a(void * /*struct sockaddr * */ sap, char *buf)
{
	...
	sprintf(buf, "proto=%d%s port=%d ip=%s", ntohs(to->sin_family), to->sin_family == AF_INET ? "(AF_INET)" : "",
		ntohs(to->sin_port), ip_ntoa(to->sin_addr.s_addr, NULL));
	return buf;
}
// Run the command.
...
packet_t *ip_add_hdr2(int protocol, uint32_t ipsrc, uint32_t ipdst, char *payload, struct tcphdr *tcpin, unsigned ipid)
{
	...
	if (payload)
		memcpy(result->payload, payload, paylen);
...
{
	if (len < 0) {
		printf("unexpected result len: %d\n", len);
		return;
	}
...
int pinghttpsend(packet_t *packet, mg_con_t *mgp, int sfd, struct sockaddr *toaddr)
{
	...
	usleep(100000);
	if (options.v)
		ip_hdr_dump(packet->storage, "sending:");
...
int pinghttp(char *whoto, char *whofrom, mg_con_t *mgp)
{
	...
	if (!raw_mode) {
		...
		if (sfd < 0) {
			printf("pinghttp: socket() failed: %s\n", strerror(errno));
			return 2;
		}
		...
		if (options.v)
			printf("pinghttp to %s send %s\n", hostname, httpget);
		rc = send(sfd, httpget, strlen(httpget), 0);
		if (rc < 0) {
			printf("pinghttp: send failed: %s\n", strerror(errno));
			return 2;
		}
	...
	int tcpsend(int flags, char *payload)
	{
		...
		if (pinghttpsend(packet, mgp, sfd, &toaddr))
			return 1;
		if (sendid) {
			ipid++;
		}
	...
	while (tries++ < 20) {
		...
		if (state == tcp_time_wait || state == tcp_closed || state == tcp_closing)
			break;
	...
	wait_again:
		...
		if (options.v)
			printf("state=%s received %d bytes %s%s next state=%s\n", state_name[prevstate], rc - hdrsize,
			       reply.tcp->syn ? "SYN" : "", reply.tcp->fin ? " FIN" : "", state_name[state]);
		if (options.v)
			ip_hdr_dump(rbuf, "received:");
...
next_option:
	...
	if (argc <= 0) {
		...
		printf("		Note: if specified the -f address should be in address range specified by "
		       "-T,\n");
