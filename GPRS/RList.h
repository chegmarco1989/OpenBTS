...
public:
	...
	// Yes you can use this in a const method function and yes we are changing the Mutex and yes it is ok so use a
	// const_cast. ScopedIterator(ListType const &wPList) : mPList(const_cast<ListType&>(wPList)),
	// mPMutex(mPList.mListLock) { siInit(); }
...
public:
	...
	bool next(T &var, typename ListType::iterator &itr)
	{ // Return a regular old iterator to the user.
...
public:
	...
	RListIteratorThreadSafe(ListType const &wPList)
		: ScopedIterator<T>(const_cast<ListType &>(wPList), const_cast<ListType &>(wPList).mListLock)
	{
	}
};

// Assumes the list is an RList which has an internal mutex.
#define RN_RLIST_FOR_ALL_THREAD_SAFE(type, list, var) for (RListIteratorThreadSafe<type> itr(list); itr.next(var);)
...
#define RN_FOR_ALL_CONST(type, list, var) \
	for (type::const_iterator itr = (list).begin(); itr == (list).end() ? 0 : ((var = *itr++), 1);)
