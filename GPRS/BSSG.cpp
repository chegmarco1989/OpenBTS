...
#if _UNUSED_
static int BSTLVParse(ByteType *data, int &rp, IEIType::type expected_ieitype, int expected_length)
{
	...
	if (received_ieitype != expected_ieitype || received_length != expected_length) {
		...
		LOG(ERR) << "Error in BSSG msg type=" << bstype << LOGVAR(expected_ieitype) << LOGVAR(expected_length)
	...
	case 1:
		...
		break;
	case 2:
		result = getntohs(&data[rp]);
		rp += 2;
		break;
	case 4:
		result = getntohl(&data[rp]);
		rp += 4;
		break;
	default:
		assert(0);
...
void NsRecvMsg(unsigned char *data, int nsize)
{
	...
	if (nstype != NSPDUType::NS_UNITDATA) {
		GPRSLOG(4) << "BSSG NsRecvMsg " << nstype << LOGVAR(nsize);
	}
	switch (nstype) {
...
void *recvServiceLoop(void *arg)
{
	...
	if (buf == NULL) {
		buf = (unsigned char *)malloc(rbufSize);
	}
...
void *sendServiceLoop(void *arg)
{
	...
	do {
		...
		if (GPRS::GPRSDebug & debug_level) {
			GPRSLOG(debug_level) << "BSSG ===> sendServiceLoop sent " << nstype << LOGVAR(msgsize)
					     << ulmsg->str() << timestr();
		}
		if (result != msgsize) {
...
static int opensock(uint32_t sgsnIp, int sgsnPort /*,int bssgPort*/)
{
	...
	****/
	...
	if (0 != connect(sockfd, (sockaddr *)&sgsnAddr, sizeof(sgsnAddr))) {
		LOG(ERR) << "Could not connect NS socket to" << LOGVAR(sgsnIp) << LOGVAR(sgsnPort) << LOGVAR(errno);
		close(sockfd);
		return -1;
	} else {
...
// BEGINCONFIG
// 'GPRS.SGSN.Host','127.0.0.1',0,0, 'IP address of the SGSN required for GPRS service.  The default value is the
// localhost, ie, SGSN runs on same machine as OpenBTS.' 'GPRS.SGSN.port',1920,0,0,'Port number of the SGSN required for
// GPRS service.  This must match the port specified in the SGSN config file, currently osmo_sgsn.cfg' ENDCONFIG
...
bool BSSGMain::BSSGReset()
{
	...
	for (int i = 0; 1; i++) {
		if (mbsResetAckReceived && !mbsBlocked) {
			break;
		}
		Utils::sleepf(0.1);

		if (i >= 40) { // wait 4 seconds
			GPRSLOG(1) << LOGVAR(mbsResetReceived) << LOGVAR(mbsResetAckReceived) << LOGVAR(mbsBlocked);
...
BSSGDownlinkMsg *BSSGMain::BSSGReadLowSide() { return mbsRxQ.readNoBlock(); }
