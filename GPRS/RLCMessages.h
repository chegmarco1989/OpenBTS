...
class RLCDownlinkMessage : public MACDownlinkHeader, public RLCMessage {
public:
	...
	RLCDownlinkMessage(MessageType mtype, bool wRequiresAck, TBF *wtbf)
		: mMessageType(mtype), mPageMode(0), mTBF(wtbf)
	{
...
void RLCDownlinkMessage::write(BitVector &vec) const
{
	...
	while (dst.wp < RLCBlockSizeInBits[0]) {
		dst.writeField(0x2b, 8);
	}
}
#endif
...
void MSRACap_s::writeBody(MsgCommon &dst) const
{
	...
	if (dst.write01(mA5BitsPresent)) {
		dst.WRITE_ITEM(mA5Bits);
	}
	dst.WRITE_ITEM(mESInd);
	dst.WRITE_ITEM(mVGCS);
	dst.WRITE_ITEM(mVBS);
	if (mMultiSlotCapPresent) {
		if (dst.write01(mHSCDMultiSlotClassPresent))
			dst.WRITE_ITEM(mHSCDMultiSlotClass);
...
void RLCMsgPacketDownlinkAckNack::parseBody(const BitVector &src, size_t &rp)
{
	...
	if (mHaveChannelRequest) {
		mCRD.parseElement(src, rp);
	}
...
void RLCMsgPacketDownlinkAckNack::writeBody(MsgCommon &dst) const
{
	dst.WRITE_ITEM(mTFI);
	mAND.writeBody(dst);
	if (dst.write01(mHaveChannelRequest)) {
		mCRD.writeBody(dst);
	}
	mCQR.writeBody(dst);
}
...
{
	if ((mAccessTypePresent = src.readField(rp, 1))) {
		mAccessType = src.readField(rp, 2);
	}
	...
	if ((mChangeMarkPresent = src.readField(rp, 1))) {
		mChangeMark = src.readField(rp, 2);
	}
	mCValue = src.readField(rp, 6);
	if ((mSignVarPresent = src.readField(rp, 1))) {
		mSignVar = src.readField(rp, 2);
	}
	for (int i = 0; i < 8; i++) {
...
{
	if (dst.write01(mAccessTypePresent)) {
		dst.WRITE_ITEM(mAccessType);
	}
	...
	mCRD.writeBody(dst);
	if (dst.write01(mChangeMarkPresent)) {
		dst.WRITE_ITEM(mChangeMark);
	}
	dst.WRITE_ITEM(mCValue);
	if (dst.write01(mSignVarPresent)) {
		dst.WRITE_ITEM(mSignVar);
	}
	for (int i = 0; i < 8; i++) {
		if (dst.write01(mILevelPresent[i]))
			dst.WRITE_ITEM(mILevelTN[i]);
	}
...
#if RLCMESSAGES_IMPLEMENTATION
void RLCMsgPacketUplinkDummyControlBlock::parseBody(const BitVector &src, size_t &rp) { mTLLI = src.readField(rp, 32); }
void RLCMsgPacketUplinkDummyControlBlock::writeBody(MsgCommon &dst) const { dst.writeField(mTLLI, 32, "TLLI", tohex); }
...
struct RLCMsgPacketTimingAdvanceIE : public RLCMsgDownlinkIE {
	...
	{
		mHaveTimingAdvanceValue = 1;
		mTimingAdvanceValue = ta;
	}
...
struct RLCMsgPowerControlParametersIE : public RLCMsgDownlinkIE {
	...
	void setGamma(int timeslot, int gamma)
	{
		mGammaTN[timeslot] = gamma;
		mHaveTN[timeslot] = true;
	}
...
struct RLCMsgGlobalTFIorTLLIElt : public RLCMsgDownlinkIE {
	...
	{
		mHaveGlobalTFI = 1;
		mGlobalTFI.setTFI(wIsDownlinkTFI, wTFI);
	}
...
public:
	...
	void setUplinkTFI(int tfi)
	{
		mUplinkTFIAssignmentPresent = 1;
		mUplinkTFIAssignment = tfi;
	}
	void setUSF(int timeslot, int usf)
	{
		mUSFTN[timeslot] = usf;
		mHaveTN[timeslot] = true;
	}
...
public:
	...
	RLCMsgPacketAccessReject(TBF *tbf)
		: RLCDownlinkMessage(RLCDownlinkMessage::PacketAccessReject, false, NULL), mTLLI(tbf->mtMS->msTlli)
	{
	}
...
#if RLCMESSAGES_IMPLEMENTATION
RLCMsgPacketUplinkAckNack::RLCMsgPacketUplinkAckNack(TBF *wtbf, const RLCMsgPacketAckNackDescriptionIE &wAND)
	: RLCDownlinkMessage(RLCDownlinkMessage::PacketUplinkAckNack, false, wtbf), mUplinkTFI(wtbf->mtTFI),
	  mChannelCodingCommand(wtbf->mtChannelCoding()), mAND(wAND),
	  // 6-26-2012: set TBF_EST true by default.
	  mTBFEst(gConfig.getBool("GPRS.TBF.EST")) // Allow MS to request another uplink assignment instead of Packet
						   // Control Acknowledgment in RRBP response.
{
...
class RLCMsgPacketMeasurementOrder : public RLCDownlinkMessage {
	RLCMsgGlobalTFIorTLLIElt mMSID;
	void setTLLI(int tlli) { mMSID.setTLLI(tlli); }
	void setGlobalTFI(bool wIsDownlinkTFI, int wTFI) { mMSID.setGlobalTFI(wIsDownlinkTFI, wTFI); }
	...
	RLCMsgPacketMeasurementOrder(TBF *wtbf)
		: RLCDownlinkMessage(RLCDownlinkMessage::PacketMeasurementOrder, false, wtbf), mNetworkControlOrder(2),
		  mNCNonDrxPeriod(0), mNCReportingPeriodI(7), // Max these out
