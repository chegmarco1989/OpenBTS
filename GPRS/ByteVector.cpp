...
void ByteVector::setSegment(size_t start, ByteVector &other)
{
	...
	if (mSizeBits / 8 < start + othersize) {
		mSizeBits = (start + othersize) * 8;
	}
}
...
void ByteVector::copyToSegment(ByteVector &other, size_t start, size_t span) const
{
	...
	if (other.size() < start + span) {
		other.mSizeBits = (start + span) * 8;
	}
}

/** Copy all of this Vector to a segment of another Vector. */
void ByteVector::copyToSegment(ByteVector &other, size_t start /*=0*/) const { copyToSegment(other, start, size()); }

void ByteVector::append(const ByteType *bytes, unsigned len) { memcpy(&mStart[grow(len)], bytes, len); }

// Does change size().
void ByteVector::appendFill(ByteType byte, size_t span) { memset(&mStart[grow(span)], byte, span); }
...
void ByteVector::setUInt16(size_t writeIndex, unsigned value)
{ // 2 byte value
	BVASSERT(writeIndex <= size() - 2);
	sethtons(&mStart[writeIndex], value);
}

void ByteVector::setUInt32(size_t writeIndex, unsigned value)
{ // 4 byte value
...
unsigned ByteVector::getUInt16(size_t readIndex) const
{ // 2 byte value
...
unsigned ByteVector::getUInt32(size_t readIndex) const
{ // 4 byte value
...
int ByteVector::compare(const ByteVector &bv1, const ByteVector &bv2)
{
	...
	if (bytes) {
		if ((result = memcmp(bv1.begin(), bv2.begin(), bytes))) {
			return result;
		}
	}
	...
	if (rem) {
		if ((result = (int)bv1.getField2(bytes, 0, rem) - (int)bv2.getField2(bytes, 0, rem))) {
			return result;
		}
	}
...
{
	if (sizeBits() != other.sizeBits()) {
		return false;
	} // Quick check to avoid full compare.
	...
	for (byten = 0; byten <= 1; byten++) {
		for (bitn = 0; bitn <= 7; bitn++) {
			for (l = 1; l <= 33; l++) {
				...
				if (printall) {
					std::cout << "ok:" << LOGVAR(byten) << LOGVAR(bitn) << LOGVAR(l) << LOGVAR(val)
						  << " result=" << bv << "\n";
				}
				...
				if (byten == 1) {
					assert(bv.getByte(0) == 3);
				}
				if (bitn) {
					assert(bv.getField2(byten, 0, bitn) == pat.getField2(byten, 0, bitn));
				}
				...
				for (int b1 = 0; b1 < 3; b1++) {
					for (int k = 0; k <= 7; k++) {
						...
						if (bv.getBit2(b1, k) != bv.getField2(b1, k, 1)) {
							std::cout << "getBit fail:" << LOGVAR(byten) << LOGVAR(bitn)
								  << LOGVAR(l) << "\n";
						}
					}
				}
				...
				if (bv != bv2) {
					std::cout << "ByteVector BitVector mismatch:" << LOGVAR(byten) << LOGVAR(bitn)
						  << LOGVAR(l) << LOGVAR(val) << "\n";
			...
			for (l = 1; l <= 33; l++) {
				...
				for (int start = 0; start <= 17; start++) { // start bit for append test
					...
					if ((result = bv.sizeBits()) != (expected = (byten * 8) + start)) {
						std::cout << "appendField length1 error" << LOGVAR(expected)
							  << LOGVAR(result) << "\n";
						std::cout << "at:" << LOGVAR(byten) << LOGVAR(bitn) << LOGVAR(l)
							  << LOGVAR(val) << LOGVAR(start) << "\n";
					}

					bv.appendField(val, bitn); // This is the test.

					if ((result = bv.sizeBits()) != (expected = (byten * 8) + start + bitn)) {
						std::cout << "appendField length2 error" << LOGVAR(expected)
							  << LOGVAR(result) << "\n";
						std::cout << "at:" << LOGVAR(byten) << LOGVAR(bitn) << LOGVAR(l)
							  << LOGVAR(val) << LOGVAR(start) << "\n";
					}
					...
					if ((result = bv.getField(byten * 8 + start, bitn)) !=
					    (expected = (val & ((1ull << bitn) - 1)))) {
						bv.setAppendP(10); // Needed to read beyond end of our test.
						std::cout << "appendField value error" << LOGVAR(expected)
							  << LOGVAR(result) << "\n";
						std::cout << "at:" << LOGVAR(byten) << LOGVAR(bitn) << LOGVAR(l)
							  << LOGVAR(val) << LOGVAR(start) << "\n";
