...
void RLCEngineBase::incSN(unsigned &psn) { psn = addSN(psn, 1); }
...
bool RLCEngineBase::deltaEQ(unsigned sn1, unsigned sn2) const { return sn1 == sn2; }
...
bool RLCDownEngine::dataAvail() { return mDownPDU.size() || queueFrontExistsAndIsNotTlliChangeCommand(mtMS); }
...
bool RLCDownEngine::resendNeeded(int bsn)
{
	if (mSt.VB[bsn]) {
		return false;
	} // block positively acknowledged.
	if (mDownStalled) {
		return true;
	} // in this case, resend every block ever sent.
...
void RLCDownEngine::advanceVS()
{
	...
	if (!mtUnAckMode) { // if it is ack mode
		...
		for (; !deltaEQ(mSt.VS, mSt.TxQNum) && !resendNeeded(mSt.VS); incSN(mSt.VS))
			continue;
...
finished:
	// GPRSLOG(1) << getTBF() <<msg->str() <<LOGVAR(mSt.TxQNum) <<LOGVAR(mSt.VA) <<LOGVAR(mSt.VS) <<
	// GPRSLOG(mResendSsn);
	GPRSLOG(1) << getTBF() << msg->str() << engineDumpStr();
}
	...
	while (mSt.RxQ[mSt.VQ /*% mSNS*/]) {
		...
		if (block->mE) {
			...
			for (int i = 0; i < n; i++) {
				...
				GPRSLOG(64) << "RLCUpEngine seg:" << LOGVAR(i) << LOGVAR(n) << LOGVAR(lenbytes)
					    << LOGVAR(sizebytes) << LOGVAR(payload.size())
					    << LOGVAR2("o", payload.isOwner());
			...
			if (payload.size() && segs[n - 1].M) {
				GPRSLOG(64) << "RLCUpEngine M=1:" << LOGVAR(payload.size())
					    << LOGVAR2("o", payload.isOwner());
				addUpPDU(payload);
			}
		}
		...
		if (mtUpState == RlcUpPersistFinal) {
...
bool RLCUpEngine::sendNonFinalAckNack(PDCHL1Downlink *down)
{
	...
	if (ms->msAckNackUSFGrant == ms->msNumDataUSFGrants) {
		return false;
	}
...
bool RLCUpEngine::engineService(PDCHL1Downlink *down)
{
	...
	if (mUpStalled || mNumUpBlocksSinceAckNack >= mNumUpPerAckNack) {
		...
		if (!mtMsgPending(MsgTransTransmit))
			return sendNonFinalAckNack(down);
	}

#if UPLINK_PERSIST
	// Persistent uplink uses Extended Uplink TBF defined in 44.060 9.3.1b and 9.5
	if (mUpPersistentMode) {
		// LOGWATCHF("X%s UpState=%d keepalive=%d
		...
		if (mtUpState == RlcUpQuiescent) {
			...
			if (gBSNNext <= mDataPersistFinalEndBSN) {
				return false;
			}
...
void RLCUpEngine::engineRecvDataBlock(RLCUplinkDataBlock *block, int tn)
{
	...
	if (mSt.RxQ[BSN]) {
		...
		if (mSt.VN[BSN] == false) {
			GLOG(ERR) << getTBF() << " VN out of sync" << LOGVAR(BSN) << LOGVAR(mSt.VN);
		}
		delete mSt.RxQ[BSN];
	...
	int tn) // Timeslot Number, for debugging only.
{
	...
	if (vs == mSt.TxQNum) {
		...
		if (mSt.TxQ[mSt.TxQNum]) {
			delete mSt.TxQ[mSt.TxQNum];
			mSt.TxQ[mSt.TxQNum] = 0;
		}
	...
	int tn) // Timeslot Number, for debugging only.
{
	...
	while (payloadavail > 0 && pducnt < maxPdus && licnt < maxPdus) {
		...
		if (mDownPDU.size() == 0) {
			...
			if (queueFrontExistsAndIsNotTlliChangeCommand(mtMS)) {
				...
				LOGWATCHF("pdu %zu\n", mDownPDU.size());
				GPRSLOG(1) << getTBF() << " <=== engineWriteHighSidePull: size=" << mDownPDU.size()
					   << " " << dlmsg->mDescr << timestr();
				GPRSLOG(2048) << " <=== engineWriteHighSidePull size=" << mDownPDU.size()
					      << " pdu:" << mDownPDU.hexstr();
			}
	...
	char report[300];
	if (GPRSDebug || configGetNumQ("GPRS.WATCH", 0))
		sprintf(report, "T%s tn=%d block=%d cc=%d qn=%d fbi=%d", getTBF()->tbfid(1), tn, bsn,
			(int)block->mChannelCoding, mSt.TxQNum, fbi);
	if (licnt == 0) {
		...
		for (int i = 0; i < licnt; i++) {
			...
			if (GPRSDebug)
				sprintf(report + strlen(report), " li=%d:%d:%d", li[i], mbit[i], i == licnt - 1);
		...
		for (int j = 0; j < pducnt; j++) {
			...
			if (GPRSDebug)
				sprintf(report + strlen(report), " seg=%zu", pdus[j].size());
		}
		...
		if (GPRSDebug)
			sprintf(report + strlen(report), " fill=%d", fillsize);
		if (fillsize)
			block->mPayload.appendFill(0x2b, fillsize);
	}
	LOGWATCHF("%s\n", report);
...
bool RLCDownEngine::engineService(PDCHL1Downlink *down)
{
	...
	if (block->mIdle) {
		...
		if (gL2MAC.macDownlinkKeepAlive)
			mtDownKeepAliveTimer.setNow();
		if (gL2MAC.macDownlinkPersist)
			mtDownPersistTimer.setNow();
	}
	...
	if (block->mFBI) {
		...
		// mtMsgReset();
		if (!down->send1DataFrame(this, block, 2, MsgTransDataFinal, &mtN3105)) {
			return false;
		}
		// This is not needed:
...
RLCUpEngine::RLCUpEngine(MSInfo *wms, int wOctetCount) : TBF(wms, RLCDir::Up), mUpPDU(0), mBytesPending(wOctetCount)
{
	mtUpState = RlcUpTransmit;
...
{
	os << LOGVAR2("VS", mSt.VS) << LOGVAR2("VA", mSt.VA) << LOGVAR2("TxQNum", mSt.TxQNum)
	   << LOGVAR2("stalled", stalled()) << LOGVAR(mPrevAckSsn) << LOGVAR(mResendSsn);
}

void RLCUpEngine::engineDump(std::ostream &os) const
{
	os << LOGVAR2("VR", mSt.VR) << LOGVAR2("VQ", mSt.VQ) << LOGVAR2("stalled", stalled())
	   << LOGVAR(mNumUpBlocksSinceAckNack) << LOGVAR(mtUpState);
}
