...
#define GPRSLOG(level) \
	if (GPRS::GPRSDebug & (level)) \
	_LOG(DEBUG) << "GPRS" << (level) << "," << GPRS::gBSNNext << ":"
#define LOGWATCHF(...) \
	if (GPRS::gGprsWatch & 1) \
		printf(__VA_ARGS__); \
	GPRSLOG(1) << "watch:" << format(__VA_ARGS__);

// If gprs debugging is on, print these messages regardless of Log.Level.
#define GLOG(wLevel) \
	if (GPRSDebug || IS_LOG_LEVEL(wLevel)) \
	_LOG(wLevel) << " " << timestr() << "," << GPRS::gBSNNext << ":"

// Like assert() but dont core dump unless we are testing.
#undef devassert
#define devassert(code) \
	{ \
		if (GPRS::GPRSDebug || IS_LOG_LEVEL(DEBUG)) { \
			assert(code); \
		} else if (!(code)) { \
