...
#include <GSM/GSMTransfer.h> // For RadData
...
#if RLCHDR_IMPLEMENTATION
RLCRawBlock::RLCRawBlock(int wbsn, const BitVector &wData, float wRSSI, float wTimingError, ChannelCodingType cc)
{
	...
	: public MACDownlinkHeader // 1 byte MAC header
{
	...
	RLCDownlinkDataBlockHeader() { MACDownlinkHeader::init(MACPayloadType::RLCData); }
...
void RLCUplinkDataBlockHeader::write(BitVector &dst) const
...
class RLCUplinkDataBlock : public RLCUplinkDataBlockHeader {
	BitVector mData;
...
public:
	...
	BitVector getPayload()
	{
		return mData.tail(mHeaderSizeBits);
	}
...
/** GSM 04.60 10.2.1 */
class RLCDownlinkDataBlock : public RLCDownlinkDataBlockHeader, public Text2Str {
public:
