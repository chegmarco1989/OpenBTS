...
const char *PDCHCommon::getAnsweringUsfText(char *buf, RLCBSN_t bsn)
{
	...
	sprintf(buf, " AnsweringUsf=%d %d [%d] %d %d", pdch->getUsf(bsn - 2), pdch->getUsf(bsn - 1), pdch->getUsf(bsn),
		pdch->getUsf(bsn + 1), pdch->getUsf(bsn + 2));
	return buf;
}
...
{
	os << " PDCH" << LOGVAR2("ARFCN", ARFCN()) << LOGVAR2("TN", TN()) << LOGVAR2("FER", fmtfloat2(100.0 * FER()))
	   << "%"
	   << "\n";
	if (verbose) {
		os << "\t";
	...
	unsigned *pcounter)
{
	...
	if (!setMACFields(block, mchParent, tbf, makeres, mttype, pcounter)) {
		return false;
	}
	...
	if (block->mChannelCoding == 0) {
		devassert(tobits.size() == 184);
	}
	if (GPRSDebug & 1) {
		...
		GPRSLOG(1) << "send1DataFrame " << parent() << " " << tbf << LOGVAR(tbf->mtExpectedAckBSN) << " "
			   << sshdr.str() << " " << (res ? res->str() : "") << LOGVAR2("content", content);
		//<< " enc="<<tbf->mtChannelCoding <<" "<<os.str() << "\nbits:" <<bits.str();
		//<<" " <<block->str() <<"\nbits:" <<tobits.hexstr();
	}
#if FEC_DEBUG
	BitVector copybits;
	copybits.clone(tobits);
#endif
	...
	if (result && !(*result == tobits)) {
		...
		thing[result->size()] = 0;
		GPRSLOG(1) << "encoding error" << LOGVAR2("cs", (int)debugDecoder.getCS()) << LOGVAR(diffbit)
	...
	unsigned *pcounter)		     // If non-null, incremented if a reservation is made.
{
	...
	if (GPRSDebug) {
		if ((!dummy && (GPRSDebug & 2)) || (!idle && (GPRSDebug & 32)) || (GPRSDebug & 1024)) {
			ByteVector content(tobits);
			GPRSLOG(2 | 32 | 1024) << "send1MsgFrame " << parent() << " " << msg->mTBF << " " << msg->str()
...
BitVector *GprsDecoder::getResult()
{
	...
	case ChannelCodingCS1:
		return &mD;
	default:
		devassert(0); // Others not supported yet.
		return NULL;
	}
}
...
void GprsEncoder::encodeCS1(const BitVector &src) { encodeFrame41(src, 0); }
...
void PDCHL1Uplink::writeLowSideRx(const RxBurst &inBurst)
{
	...
	if (avg > 0.5) {
		GPRSLOG(256) << "FEC:" << LOGVAR(B) << " " << inBurst << LOGVAR(avg);
	}
	...
	if (B == 3) {
		...
		if (GPRSDebug) {
			...
			if (missedRes || (GPRSDebug & (result ? 4 : 256))) {
				...
				char buf[30];
				ss << "writeLowSideRx " << parent() << (result ? " === good" : "=== bad")
				   << (res ? " res:" : "")
				   << (res ? res->str() : "")
				   //<<LOGVAR(cshead)
				   //<<LOGVAR2("cs",(int)mchCS14Dec.getCS())
				   << LOGVAR(cc) << LOGVAR2("revusf", decodeUSF(mchCS14Dec.mC)) << LOGVAR(burst_fn)
				   << LOGVAR(bsn) << LOGVAR2("RSSI", inBurst.RSSI())
				   << LOGVAR2("TE", inBurst.timingError())
				   ...
				   << " qbits=" << qbits[0] << qbits[1] << qbits[2] << qbits[3] << qbits[4] << qbits[5]
				   << qbits[6] << qbits[7] << LOGVAR(low) << LOGVAR(avg);
				if (missedRes) {
					for (int i = 0; i < 4; i++) {
...
void GPRSWriteLowSideRx(const GSM::RxBurst &inBurst, PDCHL1FEC *pdch) { pdch->uplink()->writeLowSideRx(inBurst); }
...
void PDCHL1Downlink::dlService()
{
	...
	if ((GPRSDebug & 512) || debugCntTotal % 1024 == 0) {
		GPRSLOG(2) << "dlService sent total=" << debugCntTotal << " dummy=" << debugCntDummy << " "
			   << this->parent();
	}
	...
	for (RListIterator<TBF *> itrl(gL2MAC.macTBFs); itrl.next(tbf, itr);) {
		if (!tbf->canUseDownlink(this)) {
			GPRSLOG(4) << "dlService" << tbf << " state " << tbf->mtGetState()
				   << " reqch:" << tbf->mtMS->msPacch << " can not use downlink:" << this->parent();
			continue;
		}
		...
		if (tbf->mtServiceDownlink(this)) {
			GPRSLOG(2) << "dlService" << tbf << LOGVAR(oldstate) << " state=" << tbf->mtGetState()
				   << " reqch:" << tbf->mtMS->msPacch << " using ch:" << this->parent();
...
void PDCHCommon::setTFITBF(int tfi, RLCDir::type dir, TBF *tbf) { mchParent->mchTFIs->setTFITBF(tfi, dir, tbf); }
