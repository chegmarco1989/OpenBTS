...
public:
	...
	void addGood()
	{ // increment only good count, not total.
		mTotal.mGood++;
		mHistory[histind()].mGood++;
	}
	void addMiss() { addTotal(); }
	void addHit()
	{ // increment good and total.
		unsigned i = histind();
		mTotal.mTotal++;
		mTotal.mGood++;
		mHistory[i].mTotal++;
...
struct MSStat {
	...
	void talkedUp(bool doubleCount = false)
	{
		msDownlinkAttempts = 0;
		msTalkUpTime.setNow();
		if (!doubleCount) {
			msTrafficMetric++;
		}
...
#define RN_MS_FOR_ALL_TBF(ms, tbf) for (RListIterator<TBF *> itr(ms->msTBFs); itr.next(tbf);)
	...
	// Bool_z msUplinkRequest;		// Request from phone to establish uplink was delayed due to existing
	// uplink tbf.  RLCMsgChannelRequestDescriptionIE msUplinkRequestCRD;  // The CRD for delayed request above.
	...
	bool canUseUplink(PDCHL1Uplink *up) { return msPCHUps.find(up); }
	// Can this TBF use the specified downlink?
	bool canUseDownlink(PDCHL1Downlink *down) { return msPCHDowns.find(down); }
