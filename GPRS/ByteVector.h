...
class Dorky {
}; // Used to force use of a particular constructor.
...
class ByteVector //: public Vector<ByteType>
	...
	void initstr(const char *wdata, unsigned wlen)
	{
		init(wlen);
		setAppendP(0);
		append(wdata, wlen);
...
public:
	...
	{ // Remaining full bytes for appends
	...
	ByteVector(size_t wSize = 0)
	{
		RN_MEMCHKNEW(ByteVector);
		init(wSize);
	}
	...
	ByteVector(const BitVector &other)
	{
		RN_MEMCHKNEW(ByteVector) init((other.size() + 7) / 8);
		setAppendP(0);
		append(other);
	}
	...
	{
		if (bitind())
			appendField(0, 8 - bitind()); // 0 fill partial byte.
	...
	void setByte(size_t ind, ByteType byte)
	{
		BVASSERT(ind < size());
		mStart[ind] = byte;
	}
	...
	void appendByte(unsigned value)
	{
		BVASSERT(bitind() == 0);
		setByte(grow(1), value);
	}
	void appendUInt16(unsigned value)
	{
		BVASSERT(bitind() == 0);
		setUInt16(grow(2), value);
	}
	void appendUInt32(unsigned value)
	{
		BVASSERT(bitind() == 0);
		setUInt32(grow(4), value);
	}
	ByteType getByte(size_t ind) const
	{
		BVASSERT(ind < size());
		return mStart[ind];
	}
	...
	bool getBitR1(size_t byteIndex, unsigned bitIndex) const { return getBit2(byteIndex, 8 - bitIndex); }
	...
	uint64_t getField(size_t bitPos, unsigned lengthBits) const
	{ // aka peekField
	...
	void appendField(ItemWithValueAndWidth &item) { appendField(item.getValue(), item.getWidth()); }
