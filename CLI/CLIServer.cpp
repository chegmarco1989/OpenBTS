...
{
	printf("%s> ", this->mCommandName.c_str());
	fflush(stdout);
}
...
// (pat) 4-2014: Moved this code written by Dave G. from OpenBTS.cpp to this directory so that it can be shared by
// multiple apps. I did not bother to rename all the gReports variables.
void Parser::cliServer()
{
	...
	string neInterface = gConfig.defines(CLIInterfaceOpt) ? gConfig.getStr(CLIInterfaceOpt)
							      : string(""); // TODO: Future implementation
	bool netTcp = false;

	if (netPort.size()) {
		in_addr_t iInterface =
			neInterface.size() == 0 ? inet_addr("127.0.0.1") : inet_addr(neInterface.c_str());
			...
			for (int i = 0; i < FD_SETSIZE; i++) {
				...
				} else if (i == netSockFd) // accept (tcp)
				{
					if (netTcp) {
						...
						int fd = accept(i, &peer, &len); // (pat) On the other hand, might be
						...
						// as a security measure, since the peer is allowed complete access to
						// OpenBTS.
						if (1) {
				...
				{
					// step 1 - read the command.  Format is <len>data, because it may take several
					// reads to get the full read
					//			along a stream.
					int len;
					...
					if (len < (int)sizeof(len)) // should never get here
					{
						char buf[BUFSIZ];
						sprintf(buf,
							"Unable to read complete length, s.b. %zu bytes, got %d "
							"bytes\n",
							sizeof(len), len);
						LOG(ERR) << buf;
						gReports.incr("OpenBTS.CLI.Command.ResponseFailure");
					...
					// (pat 3-2014) I dont think the above helped.  But I had to fix up the exit
					// routines in order to run gprof, so it is ok to exit normally now.
					if (res < 0) {
						isRunning = false;
						break;
					}
