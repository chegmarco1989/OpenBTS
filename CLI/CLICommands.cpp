...
{
	if (0 == strncmp(imsi.c_str(), "IMSI", 4))
		return imsi.substr(4);
	return imsi;
}

static char *translateIMSI(char *imsi)
{
	if (0 == strncmp(imsi, "IMSI", 4))
		return imsi + 4;
	return imsi;
}
...
static CLIStatus nop(int argc, char **argv, ostream &os) { return SUCCESS; }

/** Print or clear the TMSI table. */
static const char *tmsisHelp =
	"[-a | -l | -ll | -r | -tab | clear | dump [-l] <filename> | delete -tmsi <tmsi> | delete -imsi <imsi> | query "
	"<query> set name=value] -- print the TMSI table\n"
	...
	"   set name=value -- set TMSI database field name to value. If value is a string, use apostrophes, e.g.: set "
	"IMSI='12345678901234'\n"
	"   query -- run sql query, which may be quoted as shown: tmsis query \"UPDATE TMSI_TABLE SET AUTH=0 WHERE "
	"IMSI=='123456789012'\". This option may be removed in the future.";
static CLIStatus tmsis(int argc, char **argv, ostream &os)
{
	...
	map<string, string> options =
		cliParse(argc, argv, os, "-a -l -ll -r -tab dump: clear delete -imsi: -tmsi: query: set:");
...
static const char *cbsHelp =
	"-- List or add Cell Broadcast Service messages.\n"
	...
	// (pat) We dont support lanugages yet.  There are three different ways to specify them so this simple idea is
	// not good enough; There is an interface to the language below, but it is not hooked up in the CBS code.
	//"	-lang <language> -- 2 character USC2 language specification as per from GSM 3.38 section 5, or - to
	// reset.\n"
...
static CLIStatus cbscmd(int argc, char **argv, ostream &os)
{
	...
	CBMessage temp; // Message template.
	{
		...
		if (code.size()) {
			temp.setMessageCode(atoi(code.c_str()));
		}
		if (un.size()) {
			temp.setUpdateNumber(atoi(un.c_str()));
		}
		if (id.size()) {
			temp.setMessageId(atoi(id.c_str()));
		}
		if (row.size()) {
			temp.setRow(atoi(row.c_str()));
		}
		if (lang.size()) {
	...
	if (subcmd == "clear") {
		...
		vector<string> vh;
		if (verbose) {
			tab.push_back(stringSplit(vh, "row gs msg  update msg  send  send text"));
			vh.clear();
			tab.push_back(stringSplit(vh, "_   _  code number id   count age      "));
			vh.clear();
			tab.push_back(stringSplit(vh, "--- -- ---- ------ ---- ----- ---- ----"));
			vh.clear();
		} else {
			tab.push_back(stringSplit(vh, "row text"));
			vh.clear();
			tab.push_back(stringSplit(vh, "--- ----"));
			vh.clear();
		}
...
static CLIStatus printStats(int argc, char **argv, ostream &os)
{
	...
	//	<< '/' << gConfig.getNum("GPRS.Multislot.Max.Downlink") << '/' <<
	// gConfig.getNum("GPRS.Multislot.Max.Uplink") << endl;
	os << "current PDCHs: " << GPRS::gL2MAC.macActiveChannels() << endl;
	os << "utilization: " << 100 * GPRS::gL2MAC.macComputeUtilization() << "%" << endl;
	// os << "== Resources ==" << endl;
	// os << format("timeslots: %d Combination-1 (TCH/F) and %d Combination-7 (SDCCH) timeslots",gNumC1s,gNumC7s)
	// <<endl;
	return SUCCESS;
}
...
{
	if (argc == 1) {
		os << "MCC=" << gConfig.getStr("GSM.Identity.MCC") << " MNC=" << gConfig.getStr("GSM.Identity.MNC")
		   << " LAC=" << gConfig.getNum("GSM.Identity.LAC") << " CI=" << gConfig.getNum("GSM.Identity.CI")
		   << endl;
		return SUCCESS;
	}
...
const char *handoverHelp =
	"[-cause <causestring>] imsi neighbor -- attempt handover to neighbor specified by ip address.";
...
static const char *callsHelp =
	...
	"Note this includes both CS (voice call) and SMS transactions.  If -a specified with -t, show all "
	"transactions, "
	"else only active.";
...
static const char *endcallHelp =
	"IMSI or trans# -- terminate the given transaction by IMSI or by transaction number (from the chans command "
	"output.)";
			...
			const vector<string> &fields)      // Fields to put in the result row.
{
	...
	for (unsigned i = 0; i < fields.size(); i++) {
		...
		if (chan->getSACCH()) {
			meas = chan->getSACCH()->measurementResults();
		}
		...
		if (field == "CN") {
			...
			if (chan->inUseByGPRS()) {
				...
				// (pat) Thats all we can say about this channel, although we could show the aggregate
				// FER of all MS using the channel.
				return;
			}
		} else if (field == "LAPDm") {
...
static const char *chansHelp =
	"[-a -l -tab] -- report PHY status for active channels\n"
	...
	"  Signal - Uplink signal strength if MS were at full transmit power, relative to configured "
...
CLIStatus printChansV4(std::ostream &os, bool showAll, int verbosity, bool tabSeparated)
{
	...
	if (verbosity) {
		header1 =
			"CN TN chan transaction LAPDm recyc Signal RSSI RSSP SNR FER BER TA  TXPWR TA_DL RXLEV_DL "
			"BER_DL "
			"Time IMSI Neighbor    Handover";
		header2 =
			"_  _  type id          state _     dB     dB   dB   _   pct pct sym dBm   sym   dBm      pct  "
			"  "
			"M:S  _    ARFCN(dBm)  C0:BSIC";
		if (verbosity == 2) {
		...
		// header1 = "CN TN chan transaction LAPDm recyc FER RSSI TXPWR TA_DL RXLEV_DL BER_DL Neighbor Neighbor
		// IMSI";  header2 = "_  _  type id          state _     pct dB   dBm   sym   dBm      pct    ARFCN
		// dBm";
	}
	...
	for (L2ChanList::iterator it = chans.begin(); it != chans.end(); it++) {
		...
		LOG(DEBUG);
		// It would be a bug to have a non-empty tids on an inactive channel, but in that case we really want to
		// show it.
...
{
	using namespace GSM;
	os << "CN TN chan      transaction active recyc UPFER RSSI TXPWR TA_DL RXLEV_DL BER_DL Neighbor Neighbor"
	   << endl;
	...
	for (L2ChanList::iterator it = chans.begin(); it != chans.end(); it++) {
		...
		// It would be a bug to have a non-empty tids on an active channel, but in that case we really want to
		// show it.
...
static CLIStatus powerCommand(int argc, char **argv, ostream &os)
{
	...
	if (argc > 1) {
		...
		if (!powerCheckAttenValidity("GSM.Radio.PowerManager.MinAttenDB", min, os)) {
			return BAD_VALUE;
		}
		if (!powerCheckAttenValidity("GSM.Radio.PowerManager.MaxAttenDB", max, os)) {
			return BAD_VALUE;
		}
...
static CLIStatus noise(int argc, char **argv, ostream &os)
{
	...
	if (diff <= 0) {
		os << "WARNING: the current noise level exceeds the MS RSSI target, uplink connectivity will be "
		      "impossible."
		   << endl;
	} else if (diff <= 10) {
		os << "WARNING: the current noise level is approaching the MS RSSI target, uplink connectivity will be "
		      "extremely limited."
		   << endl;
	} else {
...
static const char *neighborsHelp =
	...
	"  -tab -- for tab-separated output format\n"
	"With no -imsi, dump the neighbor table, which specifies the ARFCN and BSCI of OpenBTS neighbors that "
	"responded.\n";
static CLIStatus neighbors(int argc, char **argv, ostream &os)
{
	...
	prettyTable_t tab;
	vector<string> vh;
	// We only report the C0 of the neighbor, which is the ARFCN of the beacon; we dont know how many ARFCNs total
	// it uses.
	tab.push_back(
		stringSplit(vh, "host               C0  BSIC FreqIndex Noise ARFCNs TCH-Avail Updated      Holdoff"));
	vh.clear();
	tab.push_back(
		stringSplit(vh, "------------------ --- ---- --------- ----- ------ --------- ------------ -------"));
	vh.clear();
	...
	if (result) {
		...
		while (fgets(line, 200, result)) {
			vh.clear();
			stringSplit(vh, line);
			// (pat) The line includes the host, CO, and BSIC.  Now compute the freq index and append it to
			// the data.
			if (vh.size() >= 2) {
