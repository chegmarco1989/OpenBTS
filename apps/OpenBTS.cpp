...
struct TimeSlots {
	...
	{
		// If CCCH-CONF is not 1 we need at least 1 x C7.  Well, not really, if VEA is set and user is willing
		// to forego SMS.
	...
	static int defaultC1s() { return maxC1plusC7() - getNumC7s(); }

	// (pat) If config NumC1s or NumC7s is undefined or "auto", we will set ok values, but the caller still
	// must check that getNumC1s() + getNumC7s() <= maxC1plusC7() to catch the case where the user set specific and
	// invalid values.
...
int main(int argc, char **argv)
{
	...
	try {
		...
		C0radio->setPower(gConfig.getNum(
			"GSM.Radio.PowerManager.MaxAttenDB")); // Previously: "GSM.Radio.PowerManager.MinAttenDB"
		...
		// the clamp here could be improved to take the customer's current ratio of C1:C7 and scale it back to
		// fit in the window
		if (TimeSlots::maxC1plusC7() < TimeSlots::getNumC1s() + TimeSlots::getNumC7s()) {
			LOG(CRIT) << "scaling back GSM.Channels.NumC1s and GSM.Channels.NumC7s to fit inside number of "
				     "available timeslots.";
		...
		LOG(NOTICE) << format(
			"Creating %d Combination-1 (TCH/F) timeslots and %d Combination-7 (SDCCH) timeslots", gNumC1s,
			gNumC7s);
		...
		if (sCount < (int)(numARFCNs * 8)) {
			LOG(CRIT) << "Only " << sCount << " timeslots configured in an " << numARFCNs
				  << "-ARFCN system.";
		}
		...
		// dont put any useful information in the e.what(0 field. man zmq_cpp for more info.
...
vector<string> configurationCrossCheck(const string &key)
{
	...
	if (key.compare("GSM.Timer.T3113") == 0 || key.compare("SIP.Timer.B") == 0) {
		...
		if (gsm.compare(sip) != 0) {
			warning << "GSM.Timer.T3113 (" << gsm << ") and SIP.Timer.B (" << sip
				<< ") should usually have the same value";
		...
		if (gConfig.getBool("Control.VEA") && gConfig.getStr("GSM.CellSelection.NECI").compare("1") != 0) {
			warning << "Control.VEA is enabled but will not be functional until GSM.CellSelection.NECI is "
				   "set to \"1\"";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}
		...
		if (gsm >= sip) {
			warning << "GSM.Timer.T3212 (" << gsm << ") should be shorter than SIP.RegistrationPeriod ("
				<< sip << ")";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}
		...
		if ((gprs - gsm) != 10) {
			warning << "GPRS.ChannelCodingControl.RSSI (" << gprs
				<< ") should normally be 10db greater than GSM.Radio.RSSITarget (" << gsm << ")";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}

		// TODO : This NEEDS to be an error not a warning. OpenBTS will fail to start because of an assert if an
		// invalid value is used. GSM.Radio.C0 needs to be inside the valid range of ARFCNs for GSM.Radio.Band
		...
		if (range.length()) {
			warning << "GSM.Radio.C0 (" << c0 << ") falls outside the valid range of ARFCNs " << range
				<< " for GSM.Radio.Band (" << band << ")";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}
		...
		if ((detach - update) < 240) {
			warning << "SGSN.Timer.ImplicitDetach (" << detach
				<< ") should be at least 240 seconds greater than SGSN.Timer.RAUpdate (" << update
				<< ")";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}
		...
		// SIP.SMSC should normally be NULL if SMS.MIMIEType is "text/plain" and "smsc" if SMS.MIMEType is
		...
		if (sms.compare("application/vnd.3gpp.sms") == 0 && sip.compare("smsc") != 0) {
			warning << "SMS.MIMEType is set to \"application/vnc.3gpp.sms\", SIP.SMSC should usually be "
				   "set to \"smsc\"";
			warnings.push_back(warning.str());
			warning.str(std::string());
		} else if (sms.compare("text/plain") == 0 && sip.compare("") != 0) {
			warning << "SMS.MIMEType is set to \"text/plain\", SIP.SMSC should usually be empty (use "
				   "unconfig to clear)";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}
	...
	} else if (key.compare("SIP.Local.IP") == 0 || key.compare("SIP.Proxy.Registration") == 0 ||
		   key.compare("SIP.Proxy.SMS") == 0 || key.compare("SIP.Proxy.Speech") == 0 ||
		   key.compare("SIP.Proxy.USSD") == 0) {
		...
		if (gConfig.getNum("GSM.MS.Power.Min") > gConfig.getNum("GSM.MS.Power.Max")) {
			warning << "GSM.MS.Power.Min is set higher than GSM.MS.Power.Max. Swap the values or set a new "
				   "minimum.";
			warnings.push_back(warning.str());
			warning.str(std::string());
		}
	...
	} else if (key.compare("GSM.Radio.ARFCNs") == 0 || key.compare("GSM.Channels.NumC1s") == 0 ||
		   key.compare("GSM.Channels.NumC7s") == 0) {
		int max = TimeSlots::maxC1plusC7();
		int current = TimeSlots::getNumC1s() + TimeSlots::getNumC7s();
		if (max < current) {
			warning << "There are only " << max << " channels available but " << current
				<< " are configured. ";
			...
			if (avail == 1) {
				...
				warning << "There are still " << avail
					<< " channels available for additional capacity. ";
			}
