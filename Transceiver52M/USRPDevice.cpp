...
enum dboardConfigType { TXA_RXB, TXB_RXA, TXA_RXA, TXB_RXB };
...
USRPDevice::USRPDevice(int sps, bool skipRx) : skipRx(skipRx)
{
...
int USRPDevice::open(const std::string &, ReferenceType)
{
	...
	if (!skipRx) {
		try {
			m_uRx = usrp_standard_rx_sptr(usrp_standard_rx::make(
				0, decimRate * sps, 1, -1, usrp_standard_rx::FPGA_MODE_NORMAL, 1024, 16 * 8, rbf));
	...
	try {
		m_uTx = usrp_standard_tx_sptr(usrp_standard_tx::make(0, decimRate * 2, 1, -1, 1024, 16 * 8, rbf));
...
double USRPDevice::maxTxGain() { return m_dbTx->gain_max(); }

double USRPDevice::minTxGain() { return m_dbTx->gain_min(); }

double USRPDevice::maxRxGain() { return m_dbRx->gain_max(); }

double USRPDevice::minRxGain() { return m_dbRx->gain_min(); }
...
int USRPDevice::writeSamples(short *buf, int len, bool *underrun, unsigned long long timestamp, bool isControl)
{
	writeLock.lock();
...
RadioDevice *RadioDevice::make(int sps, bool skipRx) { return new USRPDevice(sps, skipRx); }
