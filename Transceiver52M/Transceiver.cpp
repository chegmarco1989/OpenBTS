	  ...
	  mClockSocket(wBasePort, TRXAddress, wBasePort + 100), mNoises(NOISE_CNT), mSPSTx(wSPS), mSPSRx(1)
{
	...
	mTransmitPriorityQueueServiceLoopThread =
		new Thread(32768); ///< thread to process transmit bursts from GSM core
...
radioVector *Transceiver::fixRadioVector(BitVector &burst, int RSSI, GSM::Time &wTime)
{

	// modulate and stick into queue
	signalVector *modBurst = modulateBurst(burst, 8 + (wTime.TN() % 4 == 0), mSPSTx);
...
void Transceiver::pushRadioVector(GSM::Time &nowTime)
{
	...
	if (addFiller) {
		...
		if (IGPRS == mChanType[TN]) {
			LOG(DEBUG) << (sendVec ? "adding" : "setting") << " GPRS filler burst on T" << TN << " FN "
				   << nowTime.FN();
		}
		if (!sendVec) {
			sendVec = tmpVec;
...
void Transceiver::driveControl()
{
	...
	if (strcmp(command, "POWEROFF") == 0) {
		...
		if (mOn) {
			sprintf(response, "RSP NOISELEV 0 %d", (int)round(20.0 * log10(rxFullScale / mNoiseLev)));
		} else {
...
void Transceiver::driveReceiveFIFO()
{
	...
	if (rxBurst) {
		...
		LOG(DEBUG) << "burst parameters: "
			   << " time: " << burstTime << " RSSI: " << RSSI << " TOA: " << TOA << " bits: " << *rxBurst;
