...
struct PulseSequence {
	PulseSequence() : c0(NULL), c1(NULL), empty(NULL), c0_buffer(NULL), c1_buffer(NULL) {}
...
static bool generateC1Pulse(int sps, PulseSequence *pulse)
{
	...
	pulse->c1 = new signalVector((complex *)pulse->c1_buffer, 0, len);
	...
	if (sps == 4) {
		...
		*xP++ = 4.46348606e-03;
		...
		for (int i = 0; i < len; i++) {
			...
			*xP++ = 0.96 * exp(-1.1380 * arg * arg - 0.527 * arg * arg * arg * arg);
		}
...
signalVector *frequencyShift(signalVector *y, signalVector *x, float freq, float startPhase, float *finalPhase)
{
...
bool generateMidamble(int sps, int tsc)
{
	...
	memcpy(_midMidamble->begin(), midMidamble->begin(), midMidamble->size() * sizeof(complex));
...
static float computePeakRatio(signalVector *corr, int sps, float toa, complex amp)
{
	int num = 0;
	complex *peak;
	float rms, avg = 0.0;
 ...
 */
int detectRACHBurst(signalVector &rxBurst, float thresh, int sps, complex *amp, float *toa)
{
...
SoftVector *demodulateBurst(signalVector &rxBurst, int sps, complex channel, float TOA)
{
	...
	signalVector &b) // feedback filter
{
	...
	postForwardFull = convolve(&rxBurst, &w, NULL, CUSTOM, 0, rxBurst.size() + w.size() - 1);
	if (!postForwardFull)
		return NULL;
