...
public:
	/** Constructors */
	signalVector(int dSize = 0, Symmetry wSymmetry = NONE) : Vector<complex>(dSize), realOnly(false), aligned(false)
	{
		symmetry = wSymmetry;
	};

	signalVector(complex *wData, size_t start, size_t span, Symmetry wSymmetry = NONE)
		: Vector<complex>(NULL, wData + start, wData + start + span), realOnly(false), aligned(false)
	{
		symmetry = wSymmetry;
	};

	signalVector(const signalVector &vec1, const signalVector &vec2)
		: Vector<complex>(vec1, vec2), realOnly(false), aligned(false)
	{
		symmetry = vec1.symmetry;
	};

	signalVector(const signalVector &wVector) : Vector<complex>(wVector.size()), realOnly(false), aligned(false)
	{
		wVector.copyTo(*this);
		symmetry = wVector.getSymmetry();
	};

	signalVector(size_t size, size_t start) : Vector<complex>(size + start), realOnly(false), aligned(false)
	{
		mStart = mData + start;
		symmetry = NONE;
	};
	...
	signalVector(const signalVector &wVector, size_t start, size_t tail = 0)
		: Vector<complex>(start + wVector.size() + tail), realOnly(false), aligned(false)
	{
		mStart = mData + start;
		wVector.copyTo(*this);
...
*/
int detectRACHBurst(signalVector &rxBurst, float detectThreshold, int sps, complex *amplitude, float *TOA);
